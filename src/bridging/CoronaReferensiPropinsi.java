...
public class CoronaReferensiPropinsi extends javax.swing.JDialog {
    ...
    public void tampil() {
        try {
            ...
            root = mapper.readTree(api.getRest()
                    .exchange(link + "/Referensi/propinsi", HttpMethod.GET, requestEntity, String.class)
                    .getBody());
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    if (list.path("propinsi_name").asText().toLowerCase().contains(TCari.getText().toLowerCase())) {
                        tabMode.addRow(
                                new Object[]{list.path("prop_kode").asText(), list.path("propinsi_name").asText()});
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(CoronaReferensiPropinsi.class.getName());
