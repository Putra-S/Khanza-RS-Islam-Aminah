...
public class KendaliMutuKendaliBiayaJKN extends javax.swing.JDialog {
    ...
    public KendaliMutuKendaliBiayaJKN(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No. SEP", "No. RM", "Nama Pasien", "Poliklinik/Inst.", "Nama Dokter", "Biaya Real Obat",
                    "Status Klaim", "Deskripsi CBG", "Tarif CBG", "Deskripsi TopUp", "TopUp Tarif",
                    "Tot. Trf. Grouping", "Pemakaian Obat (%)", "stts_lanjut"}) {
            @Override
        ...
        tabMode1 = new DefaultTableModel(null,
                new Object[]{"No. SEP", "No. RM", "Nama Pasien", "Rg. Perawatan Inap", "Nama DPJP", "Biaya Real Obat",
                    "Status Klaim", "Deskripsi CBG", "Tarif CBG", "Deskripsi TopUp", "TopUp Tarif",
                    "Tot. Trf. Grouping", "Pemakaian Obat (%)", "stts_lanjut"}) {
            @Override
            public boolean isCellEditable(int rowIndex, int colIndex) {
...
//GEN-FIRST:event_TKdKeyPressed
    private void btnDokterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        kdDokter.setText("");//GEN-FIRST:event_btnDokterActionPerformed
        TDokter.setText("");
...
//                    + "group by b.nm_gedung ORDER BY b.nm_gedung", dialog_simpan);
    ...
    public void tampilRanap() {
        ...
        try {
            ...
            ps1 = koneksi
                    .prepareStatement("SELECT enc.no_sep,enc.no_rm,enc.nm_pasien, b.nm_gedung unit, d.nm_dokter dpjp, "
                            ...
                            + "left join dpjp_ranap dr on dr.no_rawat=ki.no_rawat "
                            + "left join dokter d on d.kd_dokter=dr.kd_dokter WHERE "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and enc.no_sep like ? or "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and enc.no_rm like ? or "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and enc.nm_pasien like ? or "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and enc.klaim_final like ? or "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and eg.cbg_desc like ? or "
                            + "ki.stts_pulang<>'Pindah Kamar' and b.nm_gedung<>'igd' and b.nm_gedung<>'-' and b.status='1' and "
                            + unitnya + " and rp.status_lanjut='ranap' and " + "d.nm_dokter like '%" + TDokter.getText()
                            + "%' and enc.tglsep BETWEEN ? AND ? and CONCAT(FORMAT((esc.tarif_obat/ IFNULL(eg.cbg_tarif+egsc.tarif,eg.cbg_tarif))*100,2),' ','%') like ? "
                            + "ORDER BY enc.tglsep");
            try {
                ...
                while (rs1.next()) {
                    tabMode1.addRow(new Object[]{rs1.getString("no_sep"), rs1.getString("no_rm"),
                        rs1.getString("nm_pasien"), rs1.getString("unit"), rs1.getString("dpjp"),
                        rs1.getDouble("by_obat_real"), rs1.getString("klaim_final"), rs1.getString("cbg_desc"),
                        rs1.getDouble("cbg_tarif"), rs1.getString("topup_desc"), rs1.getDouble("topup_tarif"),
                        rs1.getDouble("total_trf_grp"), rs1.getString("perc_pakai_obat"),
    ...
    public void tampilRalan() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "SELECT enc.no_sep,enc.no_rm,enc.nm_pasien, p.nm_poli, d.nm_dokter, ROUND(esc.tarif_obat * 0.8) by_obat_real, "
                    ...
                    + "INNER JOIN dokter d ON d.kd_dokter = rp.kd_dokter "
                    + "LEFT JOIN eklaim_grouping_spc_cmg egsc ON egsc.no_sep = enc.no_sep WHERE "
                    + "rp.status_lanjut='ralan' and " + unitnya + " and d.nm_dokter like '%" + TDokter.getText()
                    + "%' and enc.tglsep BETWEEN ? AND ? and enc.no_sep like ? or "
                    + "rp.status_lanjut='ralan' and " + unitnya + " and d.nm_dokter like '%" + TDokter.getText()
                    + "%' and enc.tglsep BETWEEN ? AND ? and enc.no_rm like ? or "
                    + "rp.status_lanjut='ralan' and " + unitnya + " and d.nm_dokter like '%" + TDokter.getText()
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode
                            .addRow(new Object[]{rs.getString("no_sep"), rs.getString("no_rm"), rs.getString("nm_pasien"),
                        rs.getString("nm_poli"), rs.getString("nm_dokter"), rs.getDouble("by_obat_real"),
                        rs.getString("klaim_final"), rs.getString("cbg_desc"), rs.getDouble("cbg_tarif"),
                        rs.getString("topup_desc"), rs.getDouble("topup_tarif"), rs.getDouble("total_trf_grp"),
                        rs.getString("perc_pakai_obat"), rs.getString("status_lanjut")});
                }
                this.setCursor(Cursor.getDefaultCursor());
            } catch (Exception e) {
