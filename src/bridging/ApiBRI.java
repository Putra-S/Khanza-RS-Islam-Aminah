...
public class ApiBRI {
  ...
  public String Token() {
    ...
    try {
      ...
      token =
          getRest()
              .postForEntity(
                  urlapi + "/oauth/client_credential/accesstoken?grant_type=client_credentials",
                  request,
                  String.class)
              .getBody();
    ...
    try {
      ...
      json =
          mapper
              .readTree(
                  getRest()
                      .exchange(urlapi + "/v1/briva", HttpMethod.POST, requestEntity, String.class)
                      .getBody())
              .toString();
      ...
      if (status == true) {
        JOptionPane.showMessageDialog(
            ...
            "Tagihan BRIVA "
                + norawat
                + " sebesar "
                + Valid.SetAngka(Double.parseDouble(bayar))
                + " berhasil dibuat..!");
        Valid.panggilUrl(
            "billing/LaporanBilling12.php?norawat="
                + norawat.replaceAll(" ", "_")
                + "&pasien="
                + nama.replaceAll(" ", "_")
                + "&bayar="
                + bayar
                + "&petugas="
                + akses.getkode().replaceAll(" ", "_")
                + "&keterangan="
                + keterangan.replaceAll(" ", "_")
                + "&usere="
                + koneksiDB.USERHYBRIDWEB()
                + "&passwordte="
                + koneksiDB.PASHYBRIDWEB());
      } else {
        JOptionPane.showMessageDialog(
            null,
            "Tagihan BRIVA "
  ...
  public void bodyWithDeleteRequest(String norawat) throws Exception {
    ...
    try {
      ...
      System.out.println("Authorization : " + "Bearer " + token);
      json =
          "institutionCode=" + institution_code + "&brivaNo=" + briva_no + "&custCode=" + norawat;
      System.out.println("JSON : " + json);
      requestEntity = new HttpEntity(json, headers);
      json =
          mapper
              .readTree(
                  restTemplate
                      .exchange(
                          urlapi + "/v1/briva", HttpMethod.DELETE, requestEntity, String.class)
                      .getBody())
              .toString();
      System.out.println("Respon : " + json);
    } catch (Exception ex) {
      ...
      signature =
          Signature(
              "/v1/briva/report/"
                  ...
                  + briva_no
                  + "/"
                  + tanggalawal.replaceAll("-", "")
                  + "/"
                  + tanggalakhir.replaceAll("-", ""),
              "GET",
              token,
              timestamp,
              json);
  ...
  public boolean statusVA(String nomorcustomer) {
    ...
    try {
      ...
      signature =
          Signature(
              "/v1/briva/status/" + institution_code + "/" + briva_no + "/" + nomorcustomer,
              "GET",
              token,
              timestamp,
              json);
