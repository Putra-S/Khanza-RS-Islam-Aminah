...
public class SatuSehatKirimCondition extends javax.swing.JDialog {
    ...
    private String link = "", json = "", idpasien = "", cekbok = "";
    ...
    public SatuSehatKirimCondition(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"P", "Tanggal Registrasi", "No.Rawat", "No.RM", "Nama Pasien", "No.KTP Pasien",
                    "Stts Rawat", "Stts Lanjut", "Tanggal Pulang", "ID Encounter", "ICD 10", "Nama Penyakit",
                    "ID Condition"}) {
                    ...
                    try {
                        ...
                        json = "{"
                                    ...
                                    + "\"encounter\": {"
                                        ...
                                        + "\"display\": \"Diagnosa " + tbObat.getValueAt(i, 4).toString()
                                        + " selama kunjungan/dirawat dari tanggal " + tbObat.getValueAt(i, 1).toString()
                                        + " sampai " + tbObat.getValueAt(i, 8).toString() + "\""
                                    + "}"
                                + "}";
                        System.out.println("URL : " + link + "/Condition");
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(link + "/Condition",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_condition",
                ...
                try {
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "\"display\": \"Diagnosa " + tbObat.
                                        getValueAt(i, 4).toString() + " selama kunjungan/dirawat dari tanggal " + tbObat.
                                getValueAt(i, 1).toString() + " sampai " + tbObat.
                                getValueAt(i, 8).toString() + "\""
                                + "}"
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Condition/" + tbObat.
                                        getValueAt(i, 12).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Condition/" + tbObat.getValueAt(i, 12).
    ...
    private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = " and satu_sehat_condition.id_condition is null and pasien.no_ktp <> '-' and penyakit.kd_penyakit <> '-' ";
        } else {
            cekbok = "";
        }
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("tgl_registrasi") + " " + rs.getString("jam_reg"),
                        ...
                        rs.getString("pulang"), rs.getString("id_encounter"), rs.getString("kd_penyakit"),
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatKirimCondition.class.getName());
