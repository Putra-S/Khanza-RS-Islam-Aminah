...
public class CoronaReferensiStatusKeluar extends javax.swing.JDialog {
    ...
    public void tampil() {
        try {
            ...
            root = mapper.readTree(api.getRest()
                    .exchange(link + "/Referensi/status_keluar", HttpMethod.GET, requestEntity, String.class)
                    .getBody());
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    if (list.path("status").asText().toLowerCase().contains(TCari.getText().toLowerCase())) {
                        tabMode.addRow(new Object[]{list.path("id_status").asText(), list.path("status").asText()});
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(CoronaReferensiStatusKeluar.class.getName());
