...
public class BPJSCekKlaimJasaRaharja extends javax.swing.JDialog {
    ...
    public BPJSCekKlaimJasaRaharja(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No", "No.SEP", "Tgl.SEP", "Tgl.Pulang", "No.R.M.",
            "Jenis Pelayanan", "Diagnosa", "No.Kartu",
            "Nama Pasien", "Tgl.Kejadian", "No.Register", "Status Dijamin",
            "Status Dikirim", "Biaya Dijamin",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            BPJSCekKlaimJasaRaharja dialog = new BPJSCekKlaimJasaRaharja(
        ...
        try {
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                response = mapper.readTree(api.Decrypt(root.path("response").
                        asText(), utc)).path("jaminan");
                // response = root.path("response").path("rujukan");
                if (response.isArray()) {
                    i = 1;
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{i + "", list.path("sep").
                            path("noSEP").asText(),
                            list.path("sep").path("tglSEP").asText(), list.path(
                            "sep").path("tglPlgSEP").asText(),
                            ...
                            .replaceAll("1", "Ranap")
                            .replaceAll("2", "Ralan"),
                            list.path("sep").path("poli").asText(), list.path(
                            "sep").path("diagnosa").asText(),
                            list.path("sep").path("peserta").path("noKartu").
                            asText(),
                            list.path("sep").path("peserta").path("nama").
                            asText(),
                            list.path("jasaRaharja").path("tglKejadian").
                            asText(),
                            list.path("jasaRaharja").path("noRegister").asText(),
                            list.path("jasaRaharja").path("ketStatusDijamin").
                            asText(),
                            list.path("jasaRaharja").path("ketStatusDikirim").
                            asText(),
                            Valid.SetAngka(list.path("jasaRaharja").path(
                            "biayaDijamin").asDouble()),
                            Valid.SetAngka(list.path("jasaRaharja").path(
                            "plafon").asDouble()),
                            Valid.SetAngka(list.path("jasaRaharja").path(
                            "jmlDibayar").asDouble()),
                            list.path("jasaRaharja").path("resultsJasaRaharja").
                            asText(), "Ralan"});
                        i++;
                    }
                }
    ...
    private static final Logger LOG = Logger.getLogger(
            BPJSCekKlaimJasaRaharja.class.getName());
