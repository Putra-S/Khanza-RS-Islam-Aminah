 ...
 * @author dosen
 */
public class MobileJKNFKTPReferensiDokter extends javax.swing.JDialog {
    ...
    public MobileJKNFKTPReferensiDokter(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No.",
            "Kode Unit/Poli", "Nama Unit/Poli", "Kode Sub",
            "Nama Sub Spesialis", "Kode Dokter", "Nama Dokter", "Jam Praktek",
            "Kapasitas"}) {
            @Override
        ...
        try {
            otorisasi = koneksiDB.USERMOBILEJKNFKTP() + ":" + koneksiDB.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptCariMobileJKNFKTPReferensiDokter.jasper",
                    "report", "[ Pencarian Referensi Dokter ]",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            MobileJKNFKTPReferensiDokter dialog = new MobileJKNFKTPReferensiDokter(
    ...
    public void tampil() {
        try {
            ...
            System.out.println("URL : " + link + "/ref/poli/tanggal/" + Valid.
            ...
            root = mapper.readTree(api.getRest()
                    .exchange(link + "/ref/poli/tanggal/" + Valid.SetTgl(
                            Tanggal.getSelectedItem() + ""), HttpMethod.GET,
            ...
            if (nameNode.path("message").asText().equals("OK")) {
                ...
                if (response.isArray()) {
                    ...
                    for (JsonNode list : response) {
                        ...
                        headers2.add("X-cons-id", koneksiDB.
                                CONSIDMOBILEJKNFKTP());
                        utc = String.valueOf(api.GetUTCdatetimeAsString());
                        ...
                        headers2.add("user_key", koneksiDB.
                                USERKEYMOBILEJKNFKTP());
                        requestEntity2 = new HttpEntity(headers2);
                        System.out.println(
                                "URL : " + link + "/ref/dokter/kodepoli/" + list.
                                        path("kodepoli").asText()
                                + "/tanggal/" + Valid.SetTgl(Tanggal.
                                        getSelectedItem() + ""));
                        // System.out.println(rest.exchange(URL, HttpMethod.GET,
                        // requestEntity, String.class).getBody());
                        root2 = mapper.readTree(api.getRest()
                                .exchange(
                                        link + "/ref/dokter/kodepoli/" + list.
                                                path("kodepoli").asText() + "/tanggal/"
                                        + Valid.SetTgl(
                                                Tanggal.getSelectedItem() + ""),
                                        HttpMethod.GET, requestEntity2,
                                        String.class)
                                .getBody());
                        nameNode2 = root2.path("metadata");
                        if (nameNode2.path("message").asText().equals("OK")) {
                            response2 = mapper.readTree(api.Decrypt(root2.path(
                                    "response").asText(), utc));
                            if (response2.isArray()) {
                                for (JsonNode list2 : response2) {
                                    if (list2.path("kodedokter")
                                            .asText()
                                            .toLowerCase()
                                            .contains(TCari.getText().
                                                    toLowerCase())
                                            || list2.path("namadokter")
                                                    .asText()
                                                    .toLowerCase()
                                                    .contains(TCari.getText().
                                                            toLowerCase())) {
                                        tabMode.addRow(new Object[]{i + ".",
                                            ...
                                            list2.path("kodedokter").asText(),
                                            list2.path("namadokter").asText(),
                                            list2.path("jampraktek").asText(),
                                            list2.path("kapasitas").asText()});
                                        i++;
                                    }
                                }
                            }
    ...
    private static final Logger LOG = Logger.getLogger(
            MobileJKNFKTPReferensiDokter.class.getName());
