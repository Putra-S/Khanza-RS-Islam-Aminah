 ...
 * @author dosen
 */
public class SatuSehatKirimSpecimenLabPK extends javax.swing.JDialog {
    ...
    public SatuSehatKirimSpecimenLabPK(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"P", "No.Rawat", "No.RM", "Nama Pasien", "No.KTP Pasien", "No.Permintaan",
                    "Tgl & Jam Sampel", "Detail Pemeriksaan", "Lab Code", "Lab System", "Lab Display",
                    "ID Service Request", "ID Detail", "ID Specimen", "Kode Pemeriksaan"}) {
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    getValueAt(i, 4).toString().isEmpty()) && tbObat.getValueAt(
                    i, 13).toString().isEmpty()) {
                try {
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "\"identifier\": ["
                                + "{"
                                + "\"system\": \"http://sys-ids.kemkes.go.id/specimen/" + koneksiDB.
                                        IDSATUSEHAT() + "\","
                                ...
                                + "}"
                                + "],"
                                + "\"receivedTime\": \"" + tbObat.getValueAt(i,
                                        6).toString().replaceAll(" ", "T") + "+07:00\""
                                + "}";
                        System.out.println("URL : " + link + "/Specimen");
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(link + "/Specimen",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_specimen_lab",
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUpdateActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    ...
                    getValueAt(i, 13).toString().isEmpty())) {
                try {
                    idpasien = cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(
                            i, 4).toString());
                    try {
                        ...
                        json = "{"
                                ...
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Specimen/" + tbObat.
                                        getValueAt(i, 13).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Specimen/" + tbObat.getValueAt(i, 13).
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("tgl_sampel") + " " + rs.getString("jam_sampel"), rs.getString("Pemeriksaan"),
                        rs.getString("sampel_code"), rs.getString("sampel_system"), rs.getString("sampel_display"),
                        rs.getString("id_servicerequest"), rs.getString("id_template"), rs.getString("id_specimen"),
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatKirimSpecimenLabPK.class.getName());
