...
public class SatuSehatKirimEncounter extends javax.swing.JDialog {
    ...
    private String link = "", json = "", iddokter = "", idpasien = "", cekbok = "", condition = "", penyakit = "";
    ...
    private void initComponents() {
        ...
        BtnUpdateEn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/upload24.png"))); // NOI18N
        BtnUpdateEn.setMnemonic('K');
        BtnUpdateEn.setText("Update Encounter");
        BtnUpdateEn.setToolTipText("Alt+K");
        BtnUpdateEn.setName("BtnUpdateEn"); // NOI18N
        BtnUpdateEn.setPreferredSize(new java.awt.Dimension(160, 30));
        BtnUpdateEn.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnUpdateEnActionPerformed(evt);
            }
        });
        panelGlass8.add(BtnUpdateEn);
            ...
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                ...
                try {
                    ...
                    try {
                        ...
                        json = api.getRest().exchange(link + "/Encounter",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_encounter",
                                    "?,?", "No.Rawat", 2, new String[]{
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "\"code\": \"" + (tbObat.getValueAt(i, 11).
                                        toString().equals("Ralan") ? "AMB" : "IMP") + "\","
                                + "\"display\": \"" + (tbObat.getValueAt(i, 11).
                                        toString().equals("Ralan") ? "ambulatory" : "inpatient encounter") + "\""
                                + "},"
                                + "\"subject\": {"
                                + "\"reference\": \"Patient/" + idpasien + "\","
                                ...
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Encounter/" + tbObat.
                                        getValueAt(i, 15).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Encounter/" + tbObat.getValueAt(i, 15).
    ...
    private void BtnUpdateEnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUpdateEnActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    getValueAt(i, 5).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 8).toString().isEmpty()) && (!tbObat.
                ...
                try {
                    ...
                    idpasien = cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(
                            i, 5).toString());
                    Thread.sleep(1500);
                    condition = Sequel.cariIsi(
                            "select satu_sehat_condition.id_condition from satu_sehat_condition where satu_sehat_condition.no_rawat=?",
                            tbObat.getValueAt(i, 2).toString());
                    penyakit = Sequel.cariIsi(
                            "select penyakit.nm_penyakit from satu_sehat_condition left join penyakit on satu_sehat_condition.kd_penyakit=penyakit.kd_penyakit where satu_sehat_condition.no_rawat=?",
                            tbObat.getValueAt(i, 2).toString());
                    try {
                        headers = new HttpHeaders();
                        headers.setContentType(MediaType.APPLICATION_JSON);
                        ...
                        root = mapper.readTree(json);
                        response = root.path("id");
                        System.err.println("Notifikasi :" + response.asText());
    ...
    private widget.Button BtnUpdateEn;
	...
	private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = " and satu_sehat_encounter.id_encounter is null and pasien.no_ktp <> '-' ";
        } else {
            cekbok = "";
        }
