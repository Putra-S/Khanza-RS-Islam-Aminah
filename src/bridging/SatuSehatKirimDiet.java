...
public class SatuSehatKirimDiet extends javax.swing.JDialog {
    ...
    private String link = "", json = "", idpasien = "", idpraktisi = "", cekbok = "";
    ...
    public SatuSehatKirimDiet(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"P", "Tanggal Registrasi", "No.Rawat", "No.RM", "Nama Pasien", "No.KTP Pasien",
                    "ID Encounter", "Instruksi Diet/Gizi", "Petugas/Praktisi", "No.KTP Praktisi", "Tanggal",
                    "ID Diet"}) {
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    ...
                    getValueAt(i, 6).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 9).toString().isEmpty()) && tbObat.getValueAt(
                    i, 11).toString().isEmpty()) {
                try {
                    idpraktisi = cekViaSatuSehat.tampilIDParktisi(tbObat.
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "\"identifier\" : {"
                                + "\"system\" : \"http://sys-ids.kemkes.go.id/composition/" + koneksiDB.
                                        IDSATUSEHAT() + "\","
                        ...
                        json = api.getRest().exchange(link + "/Composition",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_diet", "?,?,?",
                                    "Diet/Gizi", 3, new String[]{
                                        tbObat.getValueAt(i, 2).toString(),
                                        tbObat.getValueAt(i, 10).toString().
                                                substring(0, 19), response.
                                        asText()
                                    }) == true) {
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUpdateActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    getValueAt(i, 5).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 6).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 9).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 11).toString().isEmpty())) {
                try {
                    idpraktisi = cekViaSatuSehat.tampilIDParktisi(tbObat.
                            getValueAt(i, 9).toString());
                    idpasien = cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Composition/" + tbObat.
                                        getValueAt(i, 11).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Composition/" + tbObat.
                                        getValueAt(i, 11).toString(),
    ...
    private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = " and satu_sehat_diet.id_diet is null ";
        } else {
            cekbok = "";
        }
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + "where catatan_adime_gizi.instruksi<>'' and nota_jalan.tanggal between ? and ? " + cekbok
                    + " "
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("tgl_registrasi") + " " + rs.getString("jam_reg"),
                        ...
                        rs.getString("no_ktp"), rs.getString("id_encounter"), rs.getString("instruksi"),
                        rs.getString("nama"), rs.getString("ktppraktisi"), rs.getString("tanggal"),
                        rs.getString("satu_sehat_diet")});
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
            } finally {
            ...
            ps = koneksi.prepareStatement(
                    "select reg_periksa.tgl_registrasi,reg_periksa.jam_reg,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,"
                    + "pasien.nm_pasien,pasien.no_ktp,satu_sehat_encounter.id_encounter,asuhan_gizi.intervensi_gizi,"
                    + "pegawai.nama,pegawai.no_ktp as ktppraktisi,asuhan_gizi.tanggal,"
                    + "ifnull(satu_sehat_diet.id_diet,'') as satu_sehat_diet "
                    + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join nota_inap on nota_inap.no_rawat=reg_periksa.no_rawat "
                    + "inner join satu_sehat_encounter on satu_sehat_encounter.no_rawat=reg_periksa.no_rawat "
                    + "inner join asuhan_gizi on asuhan_gizi.no_rawat=reg_periksa.no_rawat "
                    + "inner join pegawai on asuhan_gizi.nip=pegawai.nik "
                    + "left join satu_sehat_diet on satu_sehat_diet.no_rawat=asuhan_gizi.no_rawat "
                    + "and satu_sehat_diet.tanggal=asuhan_gizi.tanggal "
                    + "where asuhan_gizi.intervensi_gizi<>'' and nota_inap.tanggal between ? and ? " + cekbok
                    + " "
                    + (TCari.getText().isEmpty() ? ""
                    : "and (reg_periksa.no_rawat like ? or reg_periksa.no_rkm_medis like ? or "
                    + "pasien.nm_pasien like ? or pasien.no_ktp like ? or pegawai.no_ktp like ? or pegawai.nama like ?) "));
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("tgl_registrasi") + " " + rs.getString("jam_reg"),
                        rs.getString("no_rawat"), rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                        rs.getString("no_ktp"), rs.getString("id_encounter"), rs.getString("intervensi_gizi"),
                        rs.getString("nama"), rs.getString("ktppraktisi"), rs.getString("tanggal"),
                        rs.getString("satu_sehat_diet")});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatKirimDiet.class.getName());
