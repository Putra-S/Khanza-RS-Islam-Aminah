...
public class ApiSOFTMEDIX {
    ...
    private static final Logger LOG = Logger.getLogger(ApiSOFTMEDIX.class.getName());

    private Connection koneksi = koneksiDB.condb();

    private PreparedStatement ps, ps2;
    private ResultSet rs, rs2;
    private String URLAPISOFTMEDIX = "",
            PRODUCTSOFTMEDIX = "",
            VERSIONSOFTMEDIX = "",
            USERIDSOFTMEDIX = "",
            KEYSOFTMEDIX = "",
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    stringbalik
                            = getRest()
                                    .exchange(
                                            URLAPISOFTMEDIX + "/order", HttpMethod.POST, requestEntity, String.class)
                                    .getBody();
            ...
            if (response.isArray()) {
                ...
                for (JsonNode list : response) {
                    System.out.println(
                            i
                            + " "
                            + root.path("result").path("obx").path("order_lab").asText()
                            + " | "
