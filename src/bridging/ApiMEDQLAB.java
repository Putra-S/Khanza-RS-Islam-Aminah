...
public class ApiMEDQLAB {
      ...
      try {
        ...
        if (rs.next()) {
          ...
          requestJson =
              "{"
                  + "\"no_pendaftaran\": \""
                  + rs.getString("noorder").substring(4, 14)
                  + "\","
                  + "\"no_rm\": \""
                  ...
                  + "\","
                  + "\"cito\": \""
                  + (rs.getString("informasi_tambahan").toLowerCase().contains("cito")
                      ? "true"
                      : "false")
                  + "\","
                  + "\"diagnose\": \"\","
          ...
          root =
              mapper.readTree(
                  getRest()
                      .exchange(
                          URL + "/api/v1/saveOrder", HttpMethod.POST, requestEntity, String.class)
                      .getBody());
        ...
        if (e.toString().contains("UnknownHostException") || e.toString().contains("404")) {
          ...
          JOptionPane.showMessageDialog(
              null, "Pengiriman gagal, silahkan hubungi Administrator...!");
        }
      } finally {
        if (rs != null) {
          rs.close();
  ...
  public void ambil(String nopermintaan) {
    try {
      ...
      requestJson =
          getRest()
              .exchange(
                  URL + "/api/v1/getResult/json?no_laboratorium=" + nopermintaan,
                  HttpMethod.GET,
                  requestEntity,
                  String.class)
              .getBody();
      ...
      if (root.path("metaData").path("code").asText().equals("200")) {
        Sequel.queryu(
            "delete from temporary_permintaan_lab where temp37='" + akses.getalamatip() + "'");
        response = root.path("response").path("data").path("pemeriksaan");
        if (response.isArray()) {
          i = 0;
          for (JsonNode list : response) {
            hasil = "";
            if (!list.path("value").asText().isEmpty()) {
              hasil = list.path("value").asText();
            }
            if (!list.path("value_string").asText().isEmpty()) {
              hasil = list.path("value_string").asText();
            }
            if (!list.path("value_memo").asText().isEmpty()) {
              hasil = list.path("value_memo").asText();
            }

            System.out.println(
                " id : "
                    + list.path("testid_simrs").asText()
                    + ", name : "
                    + list.path("name").asText()
                    + ", value : "
                    + list.path("value").asText()
                    + ", value_string : "
                    + list.path("value_string").asText()
                    + ", value_memo : "
                    + list.path("value_memo").asText()
                    + ", keterangan : "
                    + list.path("flag").asText()
                    + ", nilai_normal : "
            ...
            if (response2.isArray()) {
              for (JsonNode list2 : response2) {
                hasil = "";
                if (!list2.path("value").asText().isEmpty()) {
                  hasil = list2.path("value").asText();
                }
                if (!list2.path("value_string").asText().isEmpty()) {
                  hasil = list2.path("value_string").asText();
                }
                if (!list2.path("value_memo").asText().isEmpty()) {
                  hasil = list2.path("value_memo").asText();
                }

                System.out.println(
                    " id : "
                        + list2.path("testid_simrs").asText()
                        + ", name : "
                        + list2.path("name").asText()
                        + ", value : "
                        + list2.path("value").asText()
                        + ", value_string : "
                        + list2.path("value_string").asText()
                        + ", value_memo : "
                        + list2.path("value_memo").asText()
                        + ", keterangan : "
                        + list2.path("flag").asText()
                        + ", nilai_normal : "
                        + list2.path("nilai_normal").asText());
                Sequel.menyimpan(
                    "temporary_permintaan_lab",
                    "'"
                        + i
                ...
                if (response3.isArray()) {
                  for (JsonNode list3 : response3) {
                    hasil = "";
                    if (!list3.path("value").asText().isEmpty()) {
                      hasil = list3.path("value").asText();
                    }
                    if (!list3.path("value_string").asText().isEmpty()) {
                      hasil = list3.path("value_string").asText();
                    }
                    if (!list3.path("value_memo").asText().isEmpty()) {
                      hasil = list3.path("value_memo").asText();
                    }

                    System.out.println(
                        " id : "
                            + list3.path("testid_simrs").asText()
                            + ", name : "
                            + list3.path("name").asText()
                            + ", value : "
                            + list3.path("value").asText()
                            + ", value_string : "
                            + list3.path("value_string").asText()
                            + ", value_memo : "
                            + list3.path("value_memo").asText()
                            + ", keterangan : "
                            + list3.path("flag").asText()
                            + ", nilai_normal : "
                            + list3.path("nilai_normal").asText());
                    Sequel.menyimpan(
                        "temporary_permintaan_lab",
                        "'"
                            + i
                    ...
                    if (response4.isArray()) {
                      for (JsonNode list4 : response4) {
                        hasil = "";
                        if (!list4.path("value").asText().isEmpty()) {
                          hasil = list4.path("value").asText();
                        }
                        if (!list4.path("value_string").asText().isEmpty()) {
                          hasil = list4.path("value_string").asText();
                        }
                        if (!list4.path("value_memo").asText().isEmpty()) {
                          hasil = list4.path("value_memo").asText();
                        }

                        System.out.println(
                            " id : "
                                + list4.path("testid_simrs").asText()
                                + ", name : "
                                + list4.path("name").asText()
                                + ", value : "
                                + list4.path("value").asText()
                                + ", value_string : "
                                + list4.path("value_string").asText()
                                + ", value_memo : "
                                + list4.path("value_memo").asText()
                                + ", keterangan : "
                                + list4.path("flag").asText()
                                + ", nilai_normal : "
                                + list4.path("nilai_normal").asText());
                        Sequel.menyimpan(
                            "temporary_permintaan_lab",
                            "'"
                                + i
  ...
  private static final Logger LOG = Logger.getLogger(ApiMEDQLAB.class.getName());
}
