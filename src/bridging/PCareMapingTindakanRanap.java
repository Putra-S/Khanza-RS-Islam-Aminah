...
public class PCareMapingTindakanRanap extends javax.swing.JDialog {
    ...
    public PCareMapingTindakanRanap(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Kode RS", "Nama Tindakan RS", "Kode PCare", "Nama Tindakan PCare"}) {
            @Override
            ...
            public void windowClosed(WindowEvent e) {
                if (barangpcare.getTable().getSelectedRow() != -1) {
                    KdTindakanPCare.setText(
                            barangpcare.getTable().getValueAt(barangpcare.getTable().getSelectedRow(), 1).toString());
                    NmTindakanPCare.setText(
                            barangpcare.getTable().getValueAt(barangpcare.getTable().getSelectedRow(), 2).toString());
...
private void btnPoliBPJSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPoliBPJSActionPerformed
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new Object[]{rs.getString("kd_jenis_prw"), rs.getString("nm_perawatan"),
                        rs.getString("kd_tindakan_pcare"), rs.getString("nm_tindakan_pcare")});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(PCareMapingTindakanRanap.class.getName());
