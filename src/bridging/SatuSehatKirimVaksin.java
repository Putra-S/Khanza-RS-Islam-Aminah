 ...
 * @author dosen
 */
public class SatuSehatKirimVaksin extends javax.swing.JDialog {
    ...
    public SatuSehatKirimVaksin(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"P", "Tanggal Registrasi", "No.Rawat", "No.RM", "Nama Pasien", "No.KTP Pasien",
                    "Stts Rawat", "Stts Lanjut", "ID Encounter", "Vaksin Code", "Vaksin System", "Kode Vaksin",
                    "Vaksin Display", "Route Code", "Route System", "Route Display", "Dose Code", "Dose System",
                    "Dose Unit", "No.Batch", "Tanggal & Jam Beri", "Jml/Kps", "Dosis/No", "ID Lokasi Satu Sehat",
                    "Nama Unit/Poli", "Dokter Penanggung Jawab", "No.KTP Praktisi", "ID Imunisasi Satu Sehat",
                    "No.Faktur"}) {
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    ...
                    getValueAt(i, 8).toString().isEmpty()) && tbObat.getValueAt(
                    i, 27).toString().isEmpty()) {
                try {
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "},"
                                + "\"occurrenceDateTime\": \"" + tbObat.
                                        getValueAt(i, 20).toString() + "\","
                                + "\"expirationDate\": \"" + Sequel.cariIsi(
                                        "SELECT data_batch.tgl_kadaluarsa FROM data_batch WHERE data_batch.no_batch='" + tbObat.
                                                getValueAt(i, 19).toString() + "' and data_batch.kode_brng='" + tbObat.
                                        getValueAt(i, 11).toString() + "' and data_batch.no_faktur='" + tbObat.
                                        getValueAt(i, 28).toString() + "'") + "\","
                                + "\"recorded\": \"" + tbObat.getValueAt(i, 20).
                                        toString() + "\","
                                + "\"primarySource\": true,"
                                + "\"location\": {"
                                ...
                                + "\"display\": \"" + tbObat.getValueAt(i, 24).
                                        toString() + "\""
                                + "},"
                                + "\"lotNumber\": \"" + tbObat.getValueAt(i, 19).
                                        toString() + "\","
                                + "\"route\": {"
                                + "\"coding\": ["
                                + "{"
                                ...
                                + "},"
                                + "\"doseQuantity\": {"
                                + "\"value\": " + tbObat.getValueAt(i, 21).
                                        toString() + ","
                                + "\"unit\": \"" + tbObat.getValueAt(i, 18).
                                        toString() + "\","
                                ...
                                + "\"protocolApplied\" : ["
                                + "{"
                                + "\"doseNumberPositiveInt\" : " + tbObat.
                                        getValueAt(i, 22).toString().replaceAll(
                                        "[^0-9.]", "")
                                + "}"
                                + "]"
                                + "}";
                        ...
                        json = api.getRest().exchange(link + "/Immunization",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_immunization",
                                    "?,?,?,?,?,?,?", "Imunisasi/Vaksin", 7,
                                    new String[]{
                                        tbObat.getValueAt(i, 2).toString(),
                                        tbObat.getValueAt(i, 20).toString().
                                                substring(0, 10), tbObat.
                                        getValueAt(i, 20).toString().substring(
                                        11, 19),
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnUpdateActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                    getValueAt(i, 5).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 8).toString().isEmpty()) && (!tbObat.
                    getValueAt(i, 27).toString().isEmpty())) {
                try {
                    idpasien = cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(
                            i, 5).toString());
                    iddokter = cekViaSatuSehat.tampilIDParktisi(tbObat.
                    ...
                    try {
                        ...
                        json = "{"
                                ...
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Immunization/" + tbObat.
                                        getValueAt(i, 27).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Immunization/" + tbObat.getValueAt(i,
                                        27).toString(), HttpMethod.PUT,
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false,
                        ...
                        rs.getString("no_ktp"), rs.getString("stts"), rs.getString("status_lanjut"),
                        rs.getString("id_encounter"), rs.getString("vaksin_code"), rs.getString("vaksin_system"),
                        rs.getString("kode_brng"), rs.getString("vaksin_display"), rs.getString("route_code"),
                        rs.getString("route_system"), rs.getString("route_display"),
                        rs.getString("dose_quantity_code"), rs.getString("dose_quantity_system"),
                        rs.getString("dose_quantity_unit"), rs.getString("no_batch"),
                        rs.getString("tgl_perawatan") + "T" + rs.getString("jam") + "+07:00", rs.getString("jml"),
                        rs.getString("aturan"), rs.getString("id_lokasi_satusehat"), rs.getString("nm_poli"),
                        rs.getString("nama"), rs.getString("ktppraktisi"), rs.getString("id_immunization"),
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatKirimVaksin.class.getName());
