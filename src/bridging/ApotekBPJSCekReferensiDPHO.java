...
public class ApotekBPJSCekReferensiDPHO extends javax.swing.JDialog {
    ...
    public ApotekBPJSCekReferensiDPHO(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"Kode Obat",
            "Nama Obat", "PRB", "Kronis", "Kemo", "Harga",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            ApotekBPJSCekReferensiDPHO dialog = new ApotekBPJSCekReferensiDPHO(
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                if (response.path("list").isArray()) {
                    for (JsonNode list : response.path("list")) {
                        if (list.path("namaobat").asText().toLowerCase().
                                contains(keyword.toLowerCase())
                                || list.path("kodeobat").asText().toLowerCase().
                                        contains(keyword.toLowerCase())
                                || list.path("restriksi").asText().toLowerCase().
                                        contains(keyword.toLowerCase())
                                || list.path("generik").asText().toLowerCase().
                                        contains(keyword.toLowerCase())) {
                            tabMode.addRow(new Object[]{list.path("kodeobat").
                                asText(),
                                list.path("namaobat").asText(),
                                list.path("prb").asText(),
                                list.path("kronis").asText(), list.path("kemo").
                                asText(),
                                Valid.SetAngka(list.path("harga").asDouble()),
                                list.path("restriksi").asText(),
                                list.path("generik").asText(), list.
                                path("aktif").asText()});
                        }
                    }
                }
    ...
    private static final Logger LOG = Logger.getLogger(
            ApotekBPJSCekReferensiDPHO.class.getName());
