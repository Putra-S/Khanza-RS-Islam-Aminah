...
public class BPJSTaskIDMobileJKN extends javax.swing.JDialog {
    ...
    public BPJSTaskIDMobileJKN(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "No.HP", "No.Kartu",
            "NIK", "Tanggal", "Poliklinik", "Dokter", "Waktu RS", "Waktu", "Task Name", "Task ID"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    try {
                        ...
                        requestJson = "{" + "\"kodebooking\": \"" + rs.getString("no_rawat") + "\"" + "}";
                        requestEntity = new HttpEntity(requestJson, headers);
                        URL = link + "/antrean/getlisttask";
                        System.out.println("URL : " + URL);
                        System.out.println("JSON : " + requestJson);
                        root = mapper.readTree(
                                api.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                            ...
                            if (response.isArray()) {
                                for (JsonNode list : response) {
                                    tabMode.addRow(new Object[]{rs.getString("no_rawat"),
                                        ...
                                        rs.getString("nm_dokter"), list.path("wakturs").asText(),
                                        list.path("waktu").asText(), list.path("taskname").asText(),
            ...
            try {
                ...
                while (rs.next()) {
                    try {
                        ...
                        headers.add("x-signature", api.getHmac(utc));
                        headers.add("user_key", koneksiDB.USERKEYAPIMOBILEJKN());
                        requestJson = "{" + "\"kodebooking\": \"" + rs.getString("nobooking") + "\"" + "}";
                        requestEntity = new HttpEntity(requestJson, headers);
                        URL = link + "/antrean/getlisttask";
                        System.out.println("URL : " + URL);
                        System.out.println("JSON : " + requestJson);
                        ...
                        if (nameNode.path("code").asText().equals("200")) {
                            ...
                            if (response.isArray()) {
                                for (JsonNode list : response) {
                                    tabMode.addRow(new Object[]{rs.getString("nobooking"),
                                        rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                                        rs.getString("nohp"), rs.getString("nomorkartu"), rs.getString("nik"),
                                        rs.getString("tanggalperiksa"), rs.getString("nm_poli"),
                                        rs.getString("nm_dokter"), list.path("wakturs").asText(),
                                        list.path("waktu").asText(), list.path("taskname").asText(),
                                        list.path("taskid").asText()});
                                }
    ...
    private static final Logger LOG = Logger.getLogger(BPJSTaskIDMobileJKN.class.getName());
