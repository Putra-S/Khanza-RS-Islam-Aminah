...
public class CoronaDiagnosa extends javax.swing.JDialog {
    ...
    public CoronaDiagnosa(java.awt.Frame parent, boolean modal) {
        ...
        tabMode2 = new DefaultTableModel(null, new Object[]{"No.KTP/Paspor", "No.RM", "Nama Pasien", "Tgl.Masuk",
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    private void tampil() {
        ...
        try {
            ...
            root = mapper.readTree(api.getRest()
                    .exchange(link + "/Referensi/icd", HttpMethod.POST, requestEntity, String.class)
                    .getBody());
            response = root.path("icd - " + TCari.getText());
            if (response.isArray()) {
                for (JsonNode list : response) {
                    tabMode
                            .addRow(new Object[]{false, list.path("icd").asText(), list.path("description").asText()});
                }
            }
    ...
    public void tampil2() {
        ...
        try {
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode2.addRow(new Object[]{rs.getString("no_pengenal"), rs.getString("no_rkm_medis"),
                        rs.getString("nama_lengkap"), rs.getString("tgl_masuk"), rs.getString("kode_icd"),
    ...
    private static final Logger LOG = Logger.getLogger(CoronaDiagnosa.class.getName());
