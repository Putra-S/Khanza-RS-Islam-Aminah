...
public class BPJSRujukanKhusus extends javax.swing.JDialog {
    ...
    public BPJSRujukanKhusus(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rujukan", "No.Peserta", "Nama Pasien", "Tgl.Ruj.Awal", "Tgl.Ruj.Akhir"}) {
            @Override
        ...
        tabMode2 = new DefaultTableModel(null, new Object[]{"P", "Kode ICD X", "Nama Penyakit"}) {
        ...
        tabMode3 = new DefaultTableModel(null, new Object[]{"P", "Kode Prosedur", "Nama Prosedur"}) {
            Class[] types = new Class[]{java.lang.Boolean.class, java.lang.Object.class, java.lang.Object.class,
                java.lang.Object.class, java.lang.Object.class};
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi
                    .prepareStatement("SELECT * FROM bridging_rujukan_bpjs_khusus where tglrujukan_awal between ? and ? "
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(
                            new Object[]{rs.getString("no_rujukan"), rs.getString("nokapst"), rs.getString("nmpst"),
                                rs.getString("tglrujukan_awal"), rs.getString("tglrujukan_berakhir")});
                }
            } catch (Exception e) {
        ...
        if (tbProgramPRB.getSelectedRow() != -1) {
            ...
            try {
                ...
                try {
                    ...
                    while (rs.next()) {
                        tabMode2.addRow(
                                new Object[]{true, rs.getString("kode_diagnosa"), rs.getString("nama_diagnosa")});
                    }
                } catch (Exception e) {
                    System.out.println("Notif : " + e);
                } finally {
                ...
                try {
                    ...
                    while (rs.next()) {
                        tabMode3.addRow(
                                new Object[]{true, rs.getString("kode_prosedur"), rs.getString("nama_prosedur")});
                    }
                } catch (Exception e) {
                    System.out.println("Notif : " + e);
                } finally {
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                if (response.path("diagnosa").isArray()) {
                    for (JsonNode list : response.path("diagnosa")) {
                        tabMode2.addRow(new Object[]{false, list.path("kode").asText(), list.path("nama").asText()});
                    }
                }
            } else {
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                if (response.path("procedure").isArray()) {
                    for (JsonNode list : response.path("procedure")) {
                        tabMode3.addRow(new Object[]{false, list.path("kode").asText(), list.path("nama").asText()});
                    }
                }
            } else {
                JOptionPane.showMessageDialog(null, nameNode.path("message").asText());
    ...
    public void bodyWithDeleteRequest() throws Exception {
        ...
        try {
            ...
            root = mapper.readTree(restTemplate
                    .exchange(URL, HttpMethod.DELETE, new HttpEntity<String>(requestJson, headers), String.class)
                    .getBody());
            nameNode = root.path("metaData");
            System.out.println("code : " + nameNode.path("code").asText());
            System.out.println("message : " + nameNode.path("message").asText());
            if (nameNode.path("code").asText().equals("200")) {
                Sequel.meghapus("bridging_rujukan_bpjs_khusus_diagnosa", "no_rujukan",
                        tbProgramPRB.getValueAt(tbProgramPRB.getSelectedRow(), 0).toString());
                Sequel.meghapus("bridging_rujukan_bpjs_khusus_prosedur", "no_rujukan",
                        tbProgramPRB.getValueAt(tbProgramPRB.getSelectedRow(), 0).toString());
                tampil();
            } else {
                JOptionPane.showMessageDialog(null, nameNode.path("message").asText());
    ...
    private static final Logger LOG = Logger.getLogger(BPJSRujukanKhusus.class.getName());
