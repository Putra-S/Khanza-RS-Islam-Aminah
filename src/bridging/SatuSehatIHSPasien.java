 ...
 * @author dosen
 */
public class SatuSehatIHSPasien extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public SatuSehatIHSPasien(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode = new DefaultTableModel(null, new String[]{"P", "No Rkm Medis", "Nama Pasien", "NIK Pasien",
            "IHS Pasien", "Tgl. Lahir", "Jenis Kelamin"}) {
    ...
    private void initComponents() {
        ...
        BtnGetName.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/download24.png"))); // NOI18N
        BtnGetName.setMnemonic('T');
        BtnGetName.setText("Get IHS Nama");
        BtnGetName.setToolTipText("Alt+T");
        BtnGetName.setName("BtnGetName"); // NOI18N
        BtnGetName.setPreferredSize(new java.awt.Dimension(130, 30));
        BtnGetName.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnGetNameActionPerformed(evt);
            }
        });
        panelGlass6.add(BtnGetName);
            ...
            if (tbKamar.getValueAt(i, 0).toString().equals("true") && (tbKamar.
                ...
                try {
                    ...
                    try {
                        ...
                        System.out.println(
                                "URL : " + link + "/Patient?identifier=https://fhir.kemkes.go.id/id/nik|" + tbKamar.
                                        getValueAt(i, 3).toString());
                        json = api.getRest().exchange(
                                link + "/Patient?identifier=https://fhir.kemkes.go.id/id/nik|" + tbKamar.
                                        getValueAt(i, 3).toString(),
                        ...
                        if (!ihsnumber.isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_ihs_pasien",
                                    "?,?,?", "No.Rkm.Medis", 3, new String[]{
            ...
            if (tbKamar.getValueAt(i, 0).toString().equals("true") && (tbKamar.
                ...
                try {
                    ...
                    try {
                        ...
                        requestEntity = new HttpEntity(headers);
                        System.out.println(
                                "URL : " + link + "/Patient?name=" + tbKamar.getValueAt(i, 2).toString() + "&birthdate=" + tbKamar.getValueAt(i, 5).toString() + "&gender=" + tbKamar.getValueAt(i, 6).toString().replace("L", "male").replace("P", "female"));
                        json = api.getRest().exchange(
                                link + "/Patient?name=" + tbKamar.getValueAt(i, 2).toString() + "&birthdate=" + tbKamar.getValueAt(i, 5).toString() + "&gender=" + tbKamar.getValueAt(i, 6).toString().replace("L", "male").replace("P", "female"),
                                HttpMethod.GET, requestEntity, String.class).
                                getBody();
                        System.out.println("JSON : " + json);
                        root = mapper.readTree(json);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SatuSehatIHSPasien dialog = new SatuSehatIHSPasien(new javax.swing.JFrame(), true);
    ...
    private widget.Button BtnGetName;
	...
	private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = " and satu_sehat_ihs_pasien.ihsnumber is null or satu_sehat_ihs_pasien.ihsnumber = '' ";
        } else {
            cekbok = "";
        }
        ...
        try {
            ps = koneksi.prepareStatement(
                    "SELECT pasien.no_rkm_medis,pasien.nm_pasien,satu_sehat_ihs_pasien.ihsnumber,pasien.no_ktp,pasien.tgl_lahir,pasien.jk "
                    + "FROM pasien LEFT JOIN satu_sehat_ihs_pasien on pasien.no_rkm_medis=satu_sehat_ihs_pasien.no_rkm_medis "
                    + "WHERE LENGTH(pasien.no_ktp)=16 and pasien.no_ktp not like '%0000'" + cekbok + "");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                        rs.getString("no_ktp"), rs.getString("ihsnumber"), rs.getString("tgl_lahir"),
                        rs.getString("jk")});
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatIHSPasien.class.getName());
