 ...
 * @author dosen
 */
public class BPJSCekReferensiDokterDPJP extends javax.swing.JDialog {
    ...
    private BPJSCekReferensiPoli spesialis = new BPJSCekReferensiPoli(null,
            false);
    ...
    public BPJSCekReferensiDokterDPJP(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No.", "Kode Dokter",
            "Nama Dokter"}) {
            @Override
        ...
        spesialis.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if (spesialis.getTable().getSelectedRow() != -1) {
                    KdSep.setText(spesialis.getTable().getValueAt(spesialis.
                            getTable().getSelectedRow(), 1).toString());
                    NmSep.setText(spesialis.getTable().getValueAt(spesialis.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptCariBPJSReferensiDokter.jasper", "report",
                    "[ Pencarian Referensi Dokter DPJP ]",
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        if (KdSep.getText().trim().isEmpty() || NmSep.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null,
                    "Silahkan pilih spesialis dulu..!!");
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            BPJSCekReferensiDokterDPJP dialog = new BPJSCekReferensiDokterDPJP(
    ...
    public void tampil(String poli) {
        try {
            ...
            URL = link + "/referensi/dokter/pelayanan/1/tglPelayanan/" + Valid.
                    SetTgl(DTPCari1.getSelectedItem() + "")
                    + "/Spesialis/" + KdSep.getText();
                ...
                if (response.path("list").isArray()) {
                    ...
                    for (JsonNode list : response.path("list")) {
                        if (list.path("kode").asText().toLowerCase().contains(
                                poli.toLowerCase())
                                || list.path("nama").asText().toLowerCase().
                                        contains(poli.toLowerCase())) {
                            tabMode.addRow(
                                    new Object[]{i + ".", list.path("kode").
                                                asText(), list.path("nama").
                                                asText()});
                        }
                        i++;
                    }
    ...
    public void tampil2(String poli) {
        try {
            URL = link + "/referensi/dokter/pelayanan/2/tglPelayanan/" + Valid.
                    SetTgl(DTPCari1.getSelectedItem() + "")
                    + "/Spesialis/" + KdSep.getText();
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                tabMode.addRow(new Object[]{"B", "Rawat Jalan", ""});
                response = mapper.readTree(api.Decrypt(root.path("response").
                        asText(), utc));
                // response = root.path("response");
                if (response.path("list").isArray()) {
    ...
    private static final Logger LOG = Logger.getLogger(
            BPJSCekReferensiDokterDPJP.class.getName());
