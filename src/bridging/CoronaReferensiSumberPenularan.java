...
public class CoronaReferensiSumberPenularan extends javax.swing.JDialog {
    ...
    public void tampil() {
        try {
            ...
            root = mapper.readTree(api.getRest()
                    .exchange(link + "/Referensi/sebab_penularan", HttpMethod.GET, requestEntity, String.class)
                    .getBody());
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    if (list.path("sources").asText().toLowerCase().contains(TCari.getText().toLowerCase())) {
                        tabMode.addRow(new Object[]{list.path("id_source").asText(), list.path("sources").asText()});
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(CoronaReferensiSumberPenularan.class.getName());
