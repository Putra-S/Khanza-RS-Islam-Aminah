...
public class BPJSAntreanPerTanggal extends javax.swing.JDialog {
    ...
    public BPJSAntreanPerTanggal(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Kode Booking", "Tanggal", "Kode Poli", "Kode Dokter", "Jam Praktek", "NIK", "Noka",
                    "No. HP", "RM", "Jenis Kunjungan", "No. Ref", "Sumber Data", "Peserta", "No. Antrean",
...
//            Valid.pindah(evt, TCari, BtnAll);
    ...
    private void tampil() {
                        ...
                        if (nameNode.path("code").asText().equals("200")) {
                            ...
                            if (response.isArray()) {
                                for (JsonNode list : response) {
                                    tabMode.addRow(new Object[]{list.path("kodebooking").asText(),
                                        list.path("tanggal").asText(), list.path("kodepoli").asText(),
                                        list.path("kodedokter").asText(), list.path("jampraktek").asText(),
                                        list.path("nik").asText(), list.path("nokapst").asText(),
                                        list.path("nohp").asText(), list.path("norekammedis").asText(),
                                    ...
                                    if (list.path("status").asText().equals("Batal")) {
                                        tot_batal += 1;
                                    }
                                    if (list.path("status").asText().equals("Belum dilayani")
        ...
        sep = Sequel
                .cariInteger("select count(bridging_sep.no_rawat) from bridging_sep where bridging_sep.tglsep between '"
    ...
    private static final Logger LOG = Logger.getLogger(BPJSAntreanPerTanggal.class.getName());
