...
public class BPJSCekReferensiJadwalHFIS extends javax.swing.JDialog {
    ...
    public BPJSCekReferensiJadwalHFIS(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"No.", "Kode Subspesialis", "Nama Subspesialis", "Kode Poli", "Nama Poli", "Kode Dokter",
                    "Nama Dokter", "Hari", "Nama Hari", "Libur", "Jadwal", "Kapasitas"}) {
            @Override
            ...
            public void windowClosed(WindowEvent e) {
                if (poli.getTable().getSelectedRow() != -1) {
                    if (i == 1) {
                        ...
                        KodeSubspesialis
                                .setText(poli.getTable().getValueAt(poli.getTable().getSelectedRow(), 3).toString());
                        NmSubspesialis
                                .setText(poli.getTable().getValueAt(poli.getTable().getSelectedRow(), 4).toString());
                ...
                if (response.isArray()) {
                    ...
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{i + ".", list.path("kodesubspesialis").asText(),
                            ...
                            list.path("namapoli").asText(), list.path("kodedokter").asText(),
                            list.path("namadokter").asText(), list.path("hari").asText(),
                            list.path("namahari").asText(), list.path("libur").asText(),
                            list.path("jadwal").asText(), list.path("kapasitaspasien").asText()});
                        i++;
                    }
                }
            } else {
    ...
    private static final Logger LOG = Logger.getLogger(BPJSCekReferensiJadwalHFIS.class.getName());
