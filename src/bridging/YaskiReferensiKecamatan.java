...
public class YaskiReferensiKecamatan extends javax.swing.JDialog {
        ...
        try {
            ...
            if (response.isArray()) {
                ...
                for (JsonNode list : response) {
                    if (list.path("nama").asText().toLowerCase().contains(poli.toLowerCase())
                            && list.path("id_kabupaten").asText().equals(KdKab.getText())) {
                        tabMode.addRow(new Object[]{i + ".", list.path("id").asText(), list.path("nama").asText()});
                        i++;
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(YaskiReferensiKecamatan.class.getName());
