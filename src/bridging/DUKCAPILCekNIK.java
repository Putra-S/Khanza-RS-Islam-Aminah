...
public class DUKCAPILCekNIK {
    ...
    try {
      ...
      requestJson =
          "{"
              ...
              + "\","
              + "\"user_id\" : \""
              + koneksiDB.USERDUKCAPIL()
              + "\","
              + "\"password\": \""
              + koneksiDB.PASSDUKCAPIL()
              + "\","
              + "\"IP_USER\":\""
              + prop.getProperty("IPUSERDUKCAPIL")
              + "\""
              + "}";
        ...
        for (JsonNode list : nameNode) {
          try {
            if (list.path("RESPON").asText().equals("Data Tidak Ditemukan")) {
              ...
              Sequel.queryu2(
                  "insert into log_dukcapil_aceh values('"
                      + NIK
                      + "',now(),'"
                      + akses.getkode()
                      + "')");
  ...
  private static final Logger LOG = Logger.getLogger(DUKCAPILCekNIK.class.getName());
}
