...
public class SatuSehatKirimMedicationRequest extends javax.swing.JDialog {
        ...
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.
                ...
                try {
                    ...
                    if (tbObat.getValueAt(i, 27).toString().isEmpty()) {
                        try {
                            ...
                            json = "{"
                                    ...
                                    + "\"code\": \"" + tbObat.getValueAt(i, 28).
                                            toString().replaceAll("Ralan",
                                                    "outpatient").replaceAll(
                                                    "Ranap", "inpatient") + "\","
                                    + "\"display\": \"" + tbObat.getValueAt(i,
                                            28).toString().replaceAll("Ralan",
                                                    "Outpatient").replaceAll(
                                                    "Ranap", "Inpatient") + "\""
                                    + "}"
                                    + "]"
                                    ...
                                    + "}";
                            System.out.println(
                                    "URL : " + link + "/MedicationRequest");
                            System.out.println("Request JSON : " + json);
                            requestEntity = new HttpEntity(json, headers);
                            json = api.getRest().exchange(
                                    link + "/MedicationRequest", HttpMethod.POST,
                    ...
                    if (tbObat.getValueAt(i, 27).toString().isEmpty()) {
                        try {
                            ...
                            json = "{"
                                    ...
                                    + "}";
                            System.out.println(
                                    "URL : " + link + "/MedicationRequest/" + tbObat.
                                            getValueAt(i, 26).toString());
                            System.out.println("Request JSON : " + json);
                            requestEntity = new HttpEntity(json, headers);
                            json = api.getRest().exchange(
                                    link + "/MedicationRequest/" + tbObat.
                                            getValueAt(i, 26).toString(),
    ...
    private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = " and satu_sehat_medicationrequest.id_medicationrequest is null and pasien.no_ktp <> '-' and resep_dokter.aturan_pakai like '%X%' ";
        } else {
            cekbok = "";
        }
