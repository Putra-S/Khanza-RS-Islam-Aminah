 ...
 * @author dosen
 */
public class PCareCekFaskesThalasemia extends javax.swing.JDialog {
    ...
    public PCareCekFaskesThalasemia(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No.", "Kode PPK", "Nama PPK", "Alamat", "No.Telp",
        ...
        spesialis.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if (spesialis.getTable().getSelectedRow() != -1) {
                    KdSpesialis
                            .setText(spesialis.getTable().getValueAt(spesialis.getTable().getSelectedRow(), 1).toString());
                    NmSpesialis
                            .setText(spesialis.getTable().getValueAt(spesialis.getTable().getSelectedRow(), 2).toString());
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptCariPCAREFaskesThalasemia.jasper", "report",
                    "[ Pencarian Referensi Rujukan Thalasemia ]",
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        if (KdSpesialis.getText().trim().isEmpty() || NmSpesialis.getText().
            ...
            JOptionPane.showMessageDialog(null,
                    "Silahkan pilih/masukkan no kartu..!!");
            NoKartu.requestFocus();
        } else {
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            tampil(Khusus.getSelectedItem().toString().substring(0, 3),
                    KdSpesialis.getText(), NoKartu.getText(), Tanggal.
    ...
    public void tampil(String spesialistik, String kode, String nokartu, String tanggal) {
        try {
            URL = link + "/spesialis/rujuk/khusus/" + spesialistik + "/subspesialis/" + kode + "/noKartu/" + nokartu
                    + "/tglEstRujuk/" + tanggal;
                ...
                if (response.path("list").isArray()) {
                    ...
                    for (JsonNode list : response.path("list")) {
                        tabMode.addRow(new Object[]{i + ".", list.path("kdppk").asText(), list.path("nmppk").asText(),
                            ...
                            list.path("kelas").asText(), list.path("nmkc").asText(), list.path("distance").asText(),
                            list.path("jadwal").asText(), list.path("jmlRujuk").asText(),
                            list.path("kapasitas").asText(), list.path("persentase").asText()});
                        i++;
                    }
                }
            } else {
    ...
    private static final Logger LOG = Logger.getLogger(PCareCekFaskesThalasemia.class.getName());
