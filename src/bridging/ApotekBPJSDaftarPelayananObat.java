...
public class ApotekBPJSDaftarPelayananObat extends javax.swing.JDialog {
    ...
    public ApotekBPJSDaftarPelayananObat(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"No.SEP Apotek", "No.SEP Asal", "No.Resep",
                    "No.Kartu", "Nama Peserta", "Kode Jenis",
                    "Jenis Obat", "Tgl.Pelayanan", "Kode Obat", "Nama Obat",
                    "Tipe Obat", "Signa 1", "Signa 2",
                    "Hari", "Permintaan", "Jumlah", "Harga"}) {
            @Override
        ...
        if (koneksiDB.CARICEPAT().equals("aktif")) {
            NomorSEP.getDocument().addDocumentListener(
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            for (int i = 0; i < tabMode.getRowCount(); i++) {
                Sequel.menyimpan("temporary", "'" + i + "','"
                        ...
                        + tabMode.getValueAt(i, 16).toString() + "','','','','','','','','','','','','','','','','','','','','','" + akses.
                        getalamatip() + "'", "Daftar Pelayanan Obat Apotek BPJS");
            }
            ...
            Valid.MyReportqry("rptApotekBPJSDaftarPelayananKlaim.jasper",
                    "report", "[ Daftar Pelayanan Apotek BPJS ]",
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        if (NomorSEP.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(null,
                    "Silahkan masukkan nomor SEP terlebih dahulu..!!!");
        } else {
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            tampil(NomorSEP.getText());
            this.setCursor(Cursor.getDefaultCursor());
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            ApotekBPJSDaftarPelayananObat dialog = new ApotekBPJSDaftarPelayananObat(
        ...
        try {
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                if (response.path("detailsep").path("listobat").isArray()) {
                    for (JsonNode list : response.path("detailsep").path(
                            "listobat")) {
                        tabMode.addRow(new Object[]{list.path("kodeobat").
                            asText(), list.path("namaobat").asText(),
                            list.path("prb").asText(), list.path("kronis").
                            asText(), list.path("kemo").asText(),
                            Valid.SetAngka(list.path("harga").asDouble()), list.
                            path("restriksi").asText(),
                            list.path("generik").asText(), list.path("aktif").
                            asText()});
                    }
                }
            } else {
    ...
    public void bodyWithDeleteRequest() throws Exception {
        ...
        try {
            ...
            requestJson = "{\"nosepapotek\":\"" + tbKamar.getValueAt(tbKamar.
                    getSelectedRow(), 0).toString()
                    + "\",\"noresep\":\"" + tbKamar.getValueAt(tbKamar.
                            getSelectedRow(), 2).toString()
                    + "\",\"kodeobat\":\"" + tbKamar.getValueAt(tbKamar.
                            getSelectedRow(), 8).toString()
                    + "\",\"tipeobat\":\"" + tbKamar.getValueAt(tbKamar.
    ...
    private static final Logger LOG = Logger.getLogger(
            ApotekBPJSDaftarPelayananObat.class.getName());
