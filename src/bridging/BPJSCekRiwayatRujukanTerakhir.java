...
public class BPJSCekRiwayatRujukanTerakhir extends javax.swing.JDialog {
    ...
    public BPJSCekRiwayatRujukanTerakhir(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"ICD 10", "Nama Diagnosa", "No.Rujukan", "Kode Tujuan", "Nama Tujuan", "Tgl.Rujukan",
            ...
            if (nameNode.path("code").asText().equals("200")) {
                ...
                if (response.isArray()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("diagnosa").path("kode").asText(),
                            list.path("diagnosa").path("nama").asText(), list.path("noKunjungan").asText(),
                            list.path("poliRujukan").path("kode").asText(),
                            list.path("poliRujukan").path("nama").asText(), list.path("tglKunjungan").asText(),
                            list.path("provPerujuk").path("kode").asText(),
                            list.path("provPerujuk").path("nama").asText(), "FKTP"});
                    }
                }
            }
                ...
                if (response.isArray()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("diagnosa").path("kode").asText(),
                            ...
                            list.path("poliRujukan").path("nama").asText(), list.path("tglKunjungan").asText(),
                            list.path("provPerujuk").path("kode").asText(),
                            list.path("provPerujuk").path("nama").asText(), "FKTL"});
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(BPJSCekRiwayatRujukanTerakhir.class.getName());
