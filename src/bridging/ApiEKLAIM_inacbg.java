...
public class ApiEKLAIM_inacbg {
    ...
    public boolean ngirimJKN(String noRawat) {
        try {
            ps = koneksi.prepareStatement(
                            "select bs.no_kartu, bs.no_sep, bs.nomr, bs.nama_pasien, "
                            + "concat(bs.tanggal_lahir,' ','00:00:00') tgl_lhr, if(p.jk='L','1','2') jk, bs.tglsep, "
                            + "bs.jnspelayanan from bridging_sep bs INNER JOIN pasien p ON p.no_rkm_medis=bs.nomr where no_rawat='"
                            + noRawat
                            + "'");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    ...
                    stringbalik
                            = getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody();
        ...
        try {
            ...
            if (root.path("metadata").path("code").asText().equals("200")) {
                ...
                requestJson11
                        ...
                        + "\","
                        + "\"nomor_sep\": \""
                        + root.path("metadata").path("claim_number").asText()
                        + "\","
                        + "\"tglsep\": \""
                        + tglReg
                        + "\","
            ...
            if (erornya.toString().contains("UnknownHostException")
                    || erornya.toString().contains("false")) {
    ...
    public boolean menggrouper(
            ...
            String prosedur_inadrg,
            String diagnosa_inadrg,
            String caraMsk,
            String classPayor,
            String sistol,
            String diastol,
            ...
            if (root.path("metadata").path("code").asText().equals("200")) {
                i
                        = JOptionPane.showConfirmDialog(
                                null,
                                ""
                                + root.path("metadata").path("message").asText()
                                + ". Apakah proses ini tetap akan dilanjutkan..?",
                                "Konfirmasi",
    ...
    public boolean ngecekDiagnosa(String kodeICD) {
        try {
            ...
            // if (root.path("metadata").path("code").asText().equals("200") &&
            // root.path("response").path("count").asText().equals("1")) {
            // x = true;
            // } else {
            // x = false;
            // }
    ...
    public boolean ngecekDiagnosaINADRG(String kodeICD) {
        try {
            ...
            if (root.path("response").path("data").isArray()) {
                for (JsonNode list : root.path("response").path("data")) {
                    if (list.path("code").asText().toLowerCase().equals(kodeICD.toLowerCase())
                            && list.path("validcode").asText().toLowerCase().equals("1")) {
                        x = true;
                        break;
                    } else {
                        x = false;
    ...
    public void mengunggahFile(
            String noPengajuan, String jnsDokumen, String filenya, String enkodePDF) {
        try {
            headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.add("Content-Type", "application/json;charset=UTF-8");
