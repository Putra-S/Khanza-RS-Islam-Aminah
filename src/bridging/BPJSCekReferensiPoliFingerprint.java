...
public class BPJSCekReferensiPoliFingerprint extends javax.swing.JDialog {
    ...
    public BPJSCekReferensiPoliFingerprint(java.awt.Frame parent, boolean modal) {
        ...
        if (koneksiDB.CARICEPAT().equals("aktif")) {
            Poli.getDocument().addDocumentListener(
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            BPJSCekReferensiPoliFingerprint dialog = new BPJSCekReferensiPoliFingerprint(
            ...
            if (nameNode.path("code").asText().equals("1")) {
                ...
                if (response.isArray()) {
                    ...
                    for (JsonNode list : response) {
                        if (list.path("kodesubspesialis").asText().toLowerCase().
                                contains(poli.toLowerCase())
                                || list.path("namasubspesialis").asText().
                                        toLowerCase().contains(poli.
                                                toLowerCase())
                                || list.path("kodepoli").asText().toLowerCase().
                                        contains(poli.toLowerCase())
                                || list.path("namapoli").asText().toLowerCase().
                                        contains(poli.toLowerCase())) {
                            tabMode.addRow(new Object[]{i + ".", list.path(
                                "kodesubspesialis").asText(),
                                list.path("namasubspesialis").asText(), list.
    ...
    private static final Logger LOG = Logger.getLogger(
            BPJSCekReferensiPoliFingerprint.class.getName());
