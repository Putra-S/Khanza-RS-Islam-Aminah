...
public class DUKCAPILJakartaCekNik {
    ...
    private static final Logger LOG = Logger.getLogger(DUKCAPILJakartaCekNik.class.getName());
    ...
    public void tampil(String nik) {
        try {
            URL
                    = prop.getProperty("URLDUKCAPILJAKARTA")
                    + "?usernm="
                    + koneksiDB.USERDUKCAPILJAKARTA()
                    + "&pass="
                    + koneksiDB.PASSDUKCAPILJAKARTA()
                    + "&app=SILaporLahir&pget=Kelahiran&pusr=admin&proc=GETNIK&nik="
                    + nik
                    + "&pkey="
                    + Sequel.cariIsi(
                            "select md5(concat('"
                            + prop.getProperty("VAR1DUKCAPILJAKARTA")
                            + "',md5(date_format(current_date(),'%d%m%Y')),'"
                            + prop.getProperty("VAR2DUKCAPILJAKARTA")
                            + "'))");
            ...
            try {
                ...
                TGL_LHR
                        = nameNode.path("TGL_LHR").asText().substring(6, 10)
                        + "-"
                        + nameNode.path("TGL_LHR").asText().substring(3, 5)
                        + "-"
                        + nameNode.path("TGL_LHR").asText().substring(0, 2);
                ...
                JENIS_KLMIN
                        = nameNode
                                .path("JENIS_KLMIN")
                                .asText()
                                .replaceAll("1", "LAKI-LAKI")
                                .replaceAll("2", "PEREMPUAN");
                ...
                DSC_STAT_KWN
                        = nameNode
                                .path("DSC_STAT_KWN")
                                .asText()
                                .replaceAll("Belum Kawin", "BELUM MENIKAH")
                                .replaceAll("Sudah Kawin", "MENIKAH");
