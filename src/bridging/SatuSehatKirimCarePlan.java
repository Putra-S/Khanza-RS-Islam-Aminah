...
public final class SatuSehatKirimCarePlan extends javax.swing.JDialog {
    ...
    public SatuSehatKirimCarePlan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new String[]{"P", "Tanggal Registrasi", "No.Rawat", "No.RM", "Nama Pasien", "No.KTP Pasien",
                    "ID Encounter", "Rencana Perawatan/Care Plan", "Petugas/Praktisi", "No.KTP Praktisi", "Tanggal",
                    "ID Care Plan", "Status"}) {
            @Override
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && (!tbObat.getValueAt(i, 5).toString().isEmpty()) && (!tbObat.getValueAt(i, 6).toString().isEmpty()) && (!tbObat.getValueAt(i, 9).toString().isEmpty()) && tbObat.getValueAt(i, 11).toString().isEmpty()) {
                try {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("tgl_registrasi") + " " + rs.getString("jam_reg"),
                        ...
                        rs.getString("no_ktp"), rs.getString("id_encounter"), rs.getString("rtl"),
                        rs.getString("nama"), rs.getString("ktppraktisi"),
                        rs.getString("tgl_perawatan") + " " + rs.getString("jam_rawat"),
                        rs.getString("satu_sehat_careplan"), "Ralan"});
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
            } finally {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("tgl_registrasi") + " " + rs.getString("jam_reg"),
                        ...
                        rs.getString("nama"), rs.getString("ktppraktisi"),
                        rs.getString("tgl_perawatan") + " " + rs.getString("jam_rawat"),
                        rs.getString("satu_sehat_careplan"), "Ranap"});
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
            } finally {
