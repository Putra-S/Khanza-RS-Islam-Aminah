...
public class BRIApiIntegrasi extends javax.swing.JDialog {
    ...
    public BRIApiIntegrasi(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"Kode Akun", "Akun Rekening", "Consumer Key",
            "Consumer Secret", "Institution Code",
            "BRIVA No", "URL API"};
        ...
        ConsumerSecret.setDocument(new batasInput((byte) 35).getKata(
                ConsumerSecret));
        InstitutionCode.setDocument(new batasInput((byte) 15).getKata(
                InstitutionCode));
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        ...
        } else if (ConsumerKey.getText().trim().isEmpty()) {
            Valid.textKosong(ConsumerKey, "Consumer Key");
        } else if (ConsumerSecret.getText().trim().isEmpty()) {
            Valid.textKosong(ConsumerSecret, "Consumer Secret");
        } else if (InstitutionCode.getText().trim().isEmpty()) {
            Valid.textKosong(InstitutionCode, "Institution Code");
        } else if (BrivaNo.getText().trim().isEmpty()) {
            Valid.textKosong(BrivaNo, "BIVA No");
        } else if (UrlApi.getText().trim().isEmpty()) {
            Valid.textKosong(UrlApi, "URL API");
        } else {
            if (tbSpesialis.getSelectedRow() > -1) {
                Sequel.queryu("delete from set_akun_bankbri");
                if (Sequel.menyimpantf("set_akun_bankbri",
                        "?,aes_encrypt(?,'nur'),aes_encrypt(?,'windi'),aes_encrypt(?,'nur'),aes_encrypt(?,'windi'),aes_encrypt(?,'dewi')",
                        "Akun Rekening", 6, new String[]{
                            kdrek.getText(), ConsumerKey.getText(),
                            ConsumerSecret.getText(), InstitutionCode.getText(),
                            BrivaNo.getText(), UrlApi.getText()
                        }) == true) {
                    tampil();
                    emptTeks();
                }
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (kdrek.getText().trim().isEmpty() || nmrek.getText().trim().isEmpty()) {
            ...
            if (Sequel.menyimpantf("set_akun_bankbri",
                    "?,aes_encrypt(?,'nur'),aes_encrypt(?,'windi'),aes_encrypt(?,'nur'),aes_encrypt(?,'windi'),aes_encrypt(?,'dewi')",
                    "Akun Rekening", 6, new String[]{
                        kdrek.getText(), ConsumerKey.getText(), ConsumerSecret.
                        getText(), InstitutionCode.getText(), BrivaNo.getText(),
                        UrlApi.getText()
                    }) == true) {
                tampil();
                emptTeks();
            ...
            JOptionPane.showMessageDialog(null,
                    "Maaf, Hanya diijinkan satu akun pengaturan ...!!!!");
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            BRIApiIntegrasi dialog = new BRIApiIntegrasi(
    ...
    private static final Logger LOG = Logger.getLogger(BRIApiIntegrasi.class.
            getName());
