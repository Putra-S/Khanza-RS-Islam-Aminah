...
public class BPJSCekDataSEPInternal extends javax.swing.JDialog {
    ...
    public BPJSCekDataSEPInternal(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No", "Kode P.R.", "Nama Poli Rujukan", "Poli Asal",
            "Tgl.Rujuk", "No.SEP", "No.SEP Referensi",
            "PPK Pelayanan", "No.Kartu", "Tanggal SEP", "No.Surat",
            "Flag Internal", "Kode Poli Asal",
            "Kode Poli Tujuan", "Kode Penunjang", "Nama Penunjang",
            "Diagnosa PPK", "Kode Dokter", "Nama Dokter",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            BPJSCekDataSEPInternal dialog = new BPJSCekDataSEPInternal(
                ...
                if (response.isArray()) {
                    ...
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{i + "", list.path(
                            "tujuanrujuk").asText(),
                            list.path("nmtujuanrujuk").asText(), list.path(
                            "nmpoliasal").asText(),
                            list.path("tglrujukinternal").asText(), list.path(
                            "nosep").asText(),
                            list.path("nosepref").asText(), list.path(
                            "ppkpelsep").asText(),
                            list.path("nokapst").asText(), list.path("tglsep").
                            asText(),
                            list.path("nosurat").asText(), list.path(
                            "flaginternal").asText(),
                            list.path("kdpoliasal").asText(), list.path(
                            "kdpolituj").asText(),
                            list.path("kdpenunjang").asText(), list.path(
                            "nmpenunjang").asText(),
                            list.path("diagppk").asText(),
                            list.path("kddokter").asText(),
                            list.path("nmdokter").asText(), list.path(
                            "flagprosedur").asText(),
                            list.path("opsikonsul").asText(), list.path(
                            "flagsep").asText(),
                            list.path("fuser").asText(), list.path("fdate").
                            asText(),
        ...
        try {
            ...
            requestJson = "{" + "\"request\": {" + "\"t_sep\": {" + "\"noSep\": \""
                    ...
                    + tbKamar.getValueAt(tbKamar.getSelectedRow(), 10).
                            toString() + "\"," + "\"tglRujukanInternal\": \""
                    + tbKamar.getValueAt(tbKamar.getSelectedRow(), 4).toString() + "\"," + "\"kdPoliTuj\": \""
                    + tbKamar.getValueAt(tbKamar.getSelectedRow(), 13).
                            toString() + "\"," + "\"user\": \"" + user + "\""
                    + "}" + "}" + "}";
    ...
    private static final Logger LOG = Logger.getLogger(
            BPJSCekDataSEPInternal.class.getName());
