...
public class DlgCariTemplateLaborat extends javax.swing.JDialog {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4),
                        rs.getString(5)});
                    iyem = iyem + "{\"IDPeriksa\":\"" + rs.getString(1) + "\",\"NamaPemeriksaan\":\"" + rs.getString(2)
                            + "\",\"IDTemplate\":\"" + rs.getString(3) + "\",\"DetailPemeriksaan\":\"" + rs.getString(4)
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("IDPeriksa").asText(),
                            list.path("NamaPemeriksaan").asText(), list.path("IDTemplate").asText(),
                            list.path("DetailPemeriksaan").asText(), list.path("Satuan").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("IDPeriksa").asText().toLowerCase().contains(TCari.getText().toLowerCase())
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariTemplateLaborat.class.getName());
