 ...
 * @author dosen
 */
public class SatuSehatKirimMedication extends javax.swing.JDialog {
    ...
    private String link = "", json = "", cekbok = "";
    ...
    public SatuSehatKirimMedication(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"P", "KFA Code", "KFA System", "Kode Barang",
            "KFA Display", "Form Code", "Form System", "Form Display", "Status", "ID Medication Satu Sehat"}) {
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimActionPerformed
        for (i = 0; i < tbObat.getRowCount(); i++) {
            if (tbObat.getValueAt(i, 0).toString().equals("true") && tbObat.
                    getValueAt(i, 9).toString().isEmpty()) {
                try {
                    try {
                        ...
                        json = "{"
                                ...
                                + "\"identifier\": ["
                                + "{"
                                + "\"system\" : \"http://sys-ids.kemkes.go.id/medication/" + koneksiDB.
                                        IDSATUSEHAT() + "\","
                                + "\"use\": \"official\","
                                + "\"value\" : \"" + tbObat.getValueAt(i, 3).
                                        toString() + "\""
                                + "}"
                                + "],"
                                + "\"code\": {"
                                + "\"coding\": ["
                                + "{"
                                + "\"system\": \"" + tbObat.getValueAt(i, 2).
                                        toString() + "\","
                                + "\"code\": \"" + tbObat.getValueAt(i, 1).
                                        toString() + "\","
                                ...
                                + "]"
                                + "},"
                                + "\"status\": \"" + tbObat.getValueAt(i, 8).
                                        toString() + "\","
                                + "\"form\": {"
                                + "\"coding\": ["
                                + "{"
                                + "\"system\": \"" + tbObat.getValueAt(i, 6).
                                        toString() + "\","
                                + "\"code\": \"" + tbObat.getValueAt(i, 5).
                                        toString() + "\","
                        ...
                        json = api.getRest().exchange(link + "/Medication",
                        ...
                        if (!response.asText().isEmpty()) {
                            if (Sequel.menyimpantf2("satu_sehat_medication",
                                    "?,?", "Obat/Alkes", 2, new String[]{
                ...
                try {
                    try {
                        ...
                        json = "{"
                                ...
                                + "}";
                        System.out.println(
                                "URL : " + link + "/Medication/" + tbObat.
                                        getValueAt(i, 9).toString());
                        System.out.println("Request JSON : " + json);
                        requestEntity = new HttpEntity(json, headers);
                        json = api.getRest().exchange(
                                link + "/Medication/" + tbObat.getValueAt(i, 9).
    ...
    private void tampil() {
        if (cekBoxBelumKirim.isSelected() == true) {
            cekbok = "where satu_sehat_medication.id_medication is null ";
        } else {
            cekbok = "where satu_sehat_mapping_obat.obat_code like '%%' ";
        }
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    : "and (satu_sehat_mapping_obat.obat_code like ? or satu_sehat_mapping_obat.kode_brng like ? or "
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("obat_code"), rs.getString("obat_system"),
                        rs.getString("kode_brng"), rs.getString("obat_display"), rs.getString("form_code"),
                        rs.getString("form_system"), rs.getString("form_display"),
                        rs.getString("status").replaceAll("0", "inactive").replaceAll("1", "active"),
    ...
    private static final Logger LOG = Logger.getLogger(SatuSehatKirimMedication.class.getName());
