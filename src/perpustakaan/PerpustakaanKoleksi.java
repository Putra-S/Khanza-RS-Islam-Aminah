...
public class PerpustakaanKoleksi extends javax.swing.JDialog {
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (Judul.getText().trim().isEmpty()) {
            Valid.textKosong(Judul, "Nama Koleksi");
        } else if (Halaman.getText().trim().isEmpty()) {
            Valid.textKosong(Halaman, "Jumlah Halaman");
        } else if (KodePenerbit.getText().trim().isEmpty() || NamaPenerbit.
                getText().trim().isEmpty()) {
            Valid.textKosong(KodePenerbit, "Penerbit");
        } else if (KodePengarang.getText().trim().isEmpty() || NamaPengarang.
                getText().trim().isEmpty()) {
            Valid.textKosong(KodePengarang, "Pengarang");
        } else if (IdJenis.getText().trim().isEmpty() || nm_jenis.getText().
                trim().isEmpty()) {
            Valid.textKosong(IdJenis, "Jenis");
        } else if (KodeKategori.getText().trim().isEmpty() || NamaKategori.
                getText().trim().isEmpty()) {
            Valid.textKosong(KodeKategori, "Kategori");
        } else {
            //menyimpan-------------------------------------------------
            Sequel.menyimpan("perpustakaan_buku",
                    "'" + KodeBuku.getText() + "','" + Judul.getText() + "','" + Halaman.
                    getText() + "','"
                    + KodePenerbit.getText() + "','" + KodePengarang.getText() + "','" + ThnTerbit.
                    getSelectedItem() + "','" + ISBN.getText() + "','"
                    + KodeKategori.getText() + "','" + IdJenis.getText() + "'",
                    "Kode Koleksi");
            //----------------------------------------------------------
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (KodeBuku.getText().trim().isEmpty()) {
            ...
            if (tbJnsPerawatan.getSelectedRow() > -1) {
                //menyimpan-------------------------------------------------
                Sequel.mengedit("perpustakaan_buku",
                        "kode_buku='" + tbJnsPerawatan.getValueAt(
                                tbJnsPerawatan.getSelectedRow(), 0) + "'",
                        "kode_buku='" + KodeBuku.getText() + "',judul_buku='" + Judul.
                        getText() + "',jml_halaman='" + Halaman.getText() + "',kode_penerbit='"
                        + KodePenerbit.getText() + "',kode_pengarang='" + KodePengarang.
                        getText() + "',thn_terbit='" + ThnTerbit.
                                getSelectedItem() + "',isbn='" + ISBN.getText() + "',id_kategori='"
                        + KodeKategori.getText() + "',id_jenis='" + IdJenis.
                        getText() + "'");
                //----------------------------------------------------------
                KodeBuku.requestFocus();
                tampil();
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptKoleksiPerpustakaan.jasper", "report",
                    ...
                    + "or perpustakaan_jenis_buku.nama_jenis like '%" + TCari.
                            getText().trim() + "%' order by perpustakaan_buku.kode_buku",
                    param);
        }
...
private void KodePenerbitKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KodePenerbitKeyPressed
    if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
        Sequel.cariIsi(
                "select perpustakaan_penerbit.nama_penerbit from perpustakaan_penerbit where perpustakaan_penerbit.kode_penerbit=?",
                NamaPenerbit, KodePenerbit.getText());
    } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
        Sequel.cariIsi(
                "select perpustakaan_penerbit.nama_penerbit from perpustakaan_penerbit where perpustakaan_penerbit.kode_penerbit=?",
                NamaPenerbit, KodePenerbit.getText());
...
private void btnPenerbitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPenerbitActionPerformed
    penerbit.isCek();
    penerbit.setSize(internalFrame1.getWidth() - 20,
...
private void KodePengarangKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KodePengarangKeyPressed
    if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
        Sequel.cariIsi(
                "select perpustakaan_pengarang.nama_pengarang from perpustakaan_pengarang where perpustakaan_pengarang.kode_pengarang=?",
                NamaPengarang, KodePengarang.getText());
    } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
        Sequel.cariIsi(
                "select perpustakaan_pengarang.nama_pengarang from perpustakaan_pengarang where perpustakaan_pengarang.kode_pengarang=?",
                NamaPengarang, KodePengarang.getText());
...
private void btnPengarangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPengarangActionPerformed
    pengarang.isCek();
    pengarang.setSize(internalFrame1.getWidth() - 20,
            internalFrame1.getHeight() - 20);
...
private void KodeKategoriKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KodeKategoriKeyPressed
    if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
        Sequel.cariIsi(
                "select perpustakaan_kategori.nama_kategori from perpustakaan_kategori where perpustakaan_kategori.id_kategori='" + KodeKategori.
                        getText() + "'", NamaKategori);
    } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
        Sequel.cariIsi(
                "select perpustakaan_kategori.nama_kategori from perpustakaan_kategori where perpustakaan_kategori.id_kategori='" + KodeKategori.
                        getText() + "'", NamaKategori);
...
private void IdJenisKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_IdJenisKeyPressed
    if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
        Sequel.cariIsi(
                "select perpustakaan_jenis_buku.nama_jenis from perpustakaan_jenis_buku where perpustakaan_jenis_buku.id_jenis=?",
                nm_jenis, IdJenis.getText());
    } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
        Sequel.cariIsi(
                "select perpustakaan_jenis_buku.nama_jenis from perpustakaan_jenis_buku where perpustakaan_jenis_buku.id_jenis=?",
                nm_jenis, IdJenis.getText());
