...
public class DlgRestoreObat extends javax.swing.JDialog {
    ...
    public DlgRestoreObat(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"P", "Kode Barang", "Nama Barang", "Kode Satuan", "Nama Satuan", "Kandungan", "Hrg.Beli(Rp)",
            "Ralan(Rp)", "Ranap K1(Rp)", "Ranap K2(Rp)", "Ranap K3(Rp)", "Kelas Utama/BPJS(Rp)", "Ranap VIP(Rp)",
            "Ranap VVIP(Rp)", "Beli Luar(Rp)", "Jual Bebas(Rp)", "Karyawan(Rp)", "Stok Minimal", "Kode Jenis",
            "Nama Jenis", "Kapasitas", "Kadaluwarsa", "Kode I.F.", "Industri Farmasi"};
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("kode_brng"), rs.getString("nama_brng"),
                        rs.getString("kode_sat"), rs.getString("satuan"), rs.getString("letak_barang"),
                        rs.getDouble("h_beli"), rs.getDouble("ralan"), rs.getDouble("kelas1"),
                        rs.getDouble("kelas2"), rs.getDouble("kelas3"), rs.getDouble("utama"), rs.getDouble("vip"),
                        rs.getDouble("vvip"), rs.getDouble("beliluar"), rs.getDouble("jualbebas"),
                        rs.getDouble("karyawan"), rs.getString("stokminimal"), rs.getString("kdjns"),
                        rs.getString("nama"), rs.getDouble("kapasitas"), rs.getString("expire"),
                        rs.getString("kode_industri"), rs.getString("nama_industri")});
                }
    ...
    private static final Logger LOG = Logger.getLogger(DlgRestoreObat.class.getName());
