...
public class SuratBebasNarkoba extends javax.swing.JDialog {
    ...
    public SuratBebasNarkoba(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Surat Sakit", "No.Rawat", "No.R.M.", "Nama Pasien", "Tanggal Surat", "Kategori",
                    "KD Dokter", "Dokter", "Keperluan", "Opiat/Morphin", "Ganja/Canabis", "Amphetamin",
                    "Methampetamin", "Benzodiazepin", "Cocain"
    ...
    public void tampil() {
        ...
        try {
            tgl = " surat_skbn.tanggalsurat between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + "' and '"
            ...
            if (TCari.getText().trim().isEmpty()) {
                ps = koneksi.prepareStatement(
                        ...
                        + "from surat_skbn inner join reg_periksa on surat_skbn.no_rawat=reg_periksa.no_rawat "
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join dokter on surat_skbn.kd_dokter=dokter.kd_dokter " + "where " + tgl
                ...
                ps = koneksi.prepareStatement(
                        ...
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join dokter on surat_skbn.kd_dokter=dokter.kd_dokter " + "where " + tgl
                        + "and no_surat like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and surat_skbn.no_rawat like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and reg_periksa.no_rkm_medis like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and surat_skbn.tanggalsurat like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and surat_skbn.kategori like '%" + TCari.getText().trim() + "%' "
                        + "order by surat_skbn.no_surat");
            }
     ...
     * @param tgl2
     */
    public void setNoRm(String norwt, String norm, String namapasien, String kodedokter, String namadokter, Date tgl1,
            Date tgl2) {
        TNoRw.setText(norwt);
        TCari.setText(norwt);
    ...
    private void autoSKBN() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_skbn.no_surat,3),signed)),0) from surat_skbn where surat_skbn.tanggalsurat='"
                + Valid.SetTgl(TanggalSurat.getSelectedItem() + "") + "' ",
                "SKBN" + TanggalSurat.getSelectedItem().toString().substring(6, 10)
    ...
    private static final Logger LOG = Logger.getLogger(SuratBebasNarkoba.class.getName());
