...
public class SuratPersetujuanUmum extends javax.swing.JDialog {
    ...
    public SuratPersetujuanUmum(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "J.K.", "Tgl.Lahir", "Tanggal", "Pengobatan Kepada",
            "Nilai Kepercayaan",
            "Nama Penanggung Jawab", "Umur P.J.", "Nomor KTP P.J.", "J.K. P.J.",
            "Nomor Telp/HP", "Bertindak Untuk", "NIP", "Nama Petugas"
        }) {
            @Override
        ...
        if (TNoRw.getText().trim().isEmpty() || TPasien.getText().trim().
            ...
            if (Sequel.menyimpantf("surat_persetujuan_umum",
                    "?,?,?,?,?,?,?,?,?,?,?,?", "Data", 12, new String[]{
                        ...
                        Tanggal.getSelectedItem() + ""), "-", "", NamaPJ.
                        getText(), UmurPJ.getText(), NoKTP.getText(),
                        JKPJ.getSelectedItem().toString().substring(0, 1),
                        BertindakAtas.getSelectedItem().toString(), NoTelp.
                        getText(), NIP.getText()
                    }) == true) {
                tabMode.addRow(new String[]{
                    ...
                    NamaPJ.getText(), UmurPJ.getText(), NoKTP.getText(), JKPJ.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataPersetujuanUmum.jasper", "report",
                        "::[ Data Persetujuan Umum ]::",
                        ...
                        + "surat_persetujuan_umum.tanggal between '" + Valid.
                                SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + "' order by surat_persetujuan_umum.tanggal",
                        param);
            } else {
                Valid.MyReportqry("rptDataPersetujuanUmum.jasper", "report",
                        "::[ Data Persetujuan Umum ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "surat_persetujuan_umum.no_telp like '%" + TCari.
                                getText().trim() + "%' or surat_persetujuan_umum.nama_pj like '%" + TCari.
                                getText().trim() + "%' or "
                        + "surat_persetujuan_umum.nip like '%" + TCari.getText().
                                trim() + "%' or petugas.nama like '%" + TCari.
                                getText().trim() + "%') "
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            if (tbObat.getSelectedRow() > -1) {
                ...
                Sequel.queryu(
                        "insert into antripersetujuanumum values('" + tbObat.
                                getValueAt(tbObat.getSelectedRow(), 0).
                                toString() + "','" + tbObat.getValueAt(tbObat.
                                getSelectedRow(), 1).toString() + "')");
                Sequel.queryu(
                        "delete from surat_persetujuan_umum_pembuat_pernyataan where no_surat='" + tbObat.
                                getValueAt(tbObat.getSelectedRow(), 0).
                                toString() + "'");
            } else {
                JOptionPane.showMessageDialog(rootPane,
        ...
        if (tbObat.getSelectedRow() > -1) {
            if (lokasifile.isEmpty()) {
                ...
                param.put("photo",
                        ...
                        PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/persetujuanumum/" + lokasifile);
                ...
                param.put("finger",
                        ...
                        SetTgl3(tbObat.getValueAt(tbObat.getSelectedRow(), 7).
                                toString()));
                Valid.MyReportqry("rptSuratPersetujuanUmum.jasper", "report",
                        ...
                        + "where surat_persetujuan_umum.no_surat='" + tbObat.
                                getValueAt(tbObat.getSelectedRow(), 0).
                                toString() + "'", param);
            }
        } else {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SuratPersetujuanUmum dialog = new SuratPersetujuanUmum(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        "pengobatan_kepada"), rs.getString("nilai_kepercayaan"),
                        rs.getString("nama_pj"),
                        rs.getString("umur_pj"), rs.getString("no_ktppj"), rs.
                        getString("jkpj"), rs.getString("no_telp"), rs.
                        getString("bertindak_atas"),
    ...
    public void emptTeks() {
        ...
        NoKTP.setText("");
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_persetujuan_umum.no_surat,3),signed)),0) from surat_persetujuan_umum where surat_persetujuan_umum.tanggal='" + Valid.
                        SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PSU" + Tanggal.getSelectedItem().toString().substring(6, 10) + Tanggal.
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            LahirPasien.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 6).
                    toString());
            NamaPJ.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                    toString());
            UmurPJ.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).
                    toString());
            NoKTP.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 12).
                    toString());
            JKPJ.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(), 13).
                    toString().replaceAll("L", "Laki-laki").replaceAll("P",
                    "Perempuan"));
            NoTelp.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 14).
                    toString());
            BertindakAtas.setSelectedItem(tbObat.getValueAt(tbObat.
        ...
        try {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    UmurPJ.setText(rs.getString("umurdaftar") + " " + rs.
                            getString("sttsumur"));
                }
            } catch (SQLException e) {
                System.out.println("Notif : " + e);
    ...
    private void ganti() {
        if (Sequel.mengedittf("surat_persetujuan_umum", "no_surat=?",
                "no_surat=?,no_rawat=?,tanggal=?,nama_pj=?,umur_pj=?,no_ktppj=?,jkpj=?,bertindak_atas=?,no_telp=?,nip=?",
                11, new String[]{
                    NoSurat.getText(), TNoRw.getText(), Valid.SetTgl(Tanggal.
                    getSelectedItem() + ""), NamaPJ.getText(), UmurPJ.getText(),
                    NoKTP.getText(),
                    JKPJ.getSelectedItem().toString().substring(0, 1),
                    BertindakAtas.getSelectedItem().toString(), NoTelp.getText(),
                    NIP.getText(),
            ...
            tbObat.setValueAt(BertindakAtas.getSelectedItem().toString(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from surat_persetujuan_umum where no_surat=?", 1,
                new String[]{
                    tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
                }) == true) {
    ...
    private void panggilPhoto() {
        if (FormPhoto.isVisible() == true) {
            ...
            try {
                ps = koneksi.prepareStatement(
                        "select surat_persetujuan_umum_pembuat_pernyataan.photo from surat_persetujuan_umum_pembuat_pernyataan where surat_persetujuan_umum_pembuat_pernyataan.no_surat=?");
                try {
    ...
    private static final Logger LOG = Logger.getLogger(
            SuratPersetujuanUmum.class.getName());
}
