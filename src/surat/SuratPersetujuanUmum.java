...
public class SuratPersetujuanUmum extends javax.swing.JDialog {
    ...
    public SuratPersetujuanUmum(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Persetujuan", "No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "J.K.", "Tgl.Lahir",
                    "Tanggal", "Pengobatan Kepada", "Nilai Kepercayaan", "Nama Penanggung Jawab", "Umur P.J.",
                    "Nomor KTP P.J.", "J.K. P.J.", "Nomor Telp/HP", "Bertindak Untuk", "NIP", "Nama Petugas"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_surat"), rs.getString("no_rawat"),
                        ...
                        rs.getString("tgl_lahir"), rs.getString("tanggal"), rs.getString("pengobatan_kepada"),
                        rs.getString("nilai_kepercayaan"), rs.getString("nama_pj"), rs.getString("umur_pj"),
                        rs.getString("no_ktppj"), rs.getString("jkpj"), rs.getString("no_telp"),
                        rs.getString("bertindak_atas"), rs.getString("nip"), rs.getString("nama")});
                }
    ...
    public void emptTeks() {
        ...
        NoKTP.setText("");
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_persetujuan_umum.no_surat,3),signed)),0) from surat_persetujuan_umum where surat_persetujuan_umum.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PSU" + Tanggal.getSelectedItem().toString().substring(6, 10)
    ...
    private void ganti() {
        if (Sequel.mengedittf("surat_persetujuan_umum", "no_surat=?",
                ...
                new String[]{NoSurat.getText(), TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    ...
                    JKPJ.getSelectedItem().toString().substring(0, 1), BertindakAtas.getSelectedItem().toString(),
                    NoTelp.getText(), NIP.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from surat_persetujuan_umum where no_surat=?", 1,
                    ...
                    if (rs.next()) {
                        if (rs.getString("photo").isEmpty() || rs.getString("photo").equals("-")) {
                            ...
                            LoadHTML2.setText("<html><body><center><img src='http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                    + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/persetujuanumum/"
                                    + rs.getString("photo")
    ...
    private static final Logger LOG = Logger.getLogger(SuratPersetujuanUmum.class.getName());
