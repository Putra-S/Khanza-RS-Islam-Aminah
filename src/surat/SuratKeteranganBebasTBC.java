...
public class SuratKeteranganBebasTBC extends javax.swing.JDialog {
    ...
    public SuratKeteranganBebasTBC(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Surat", "No.Rawat", "No.R.M.", "Nama Pasien",
            "Tanggal", "Kode Dokter", "Dokter", "Keperluan"}) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoSurat.getText().trim().isEmpty()) {
            ...
            if (Sequel.
                    menyimpantf("surat_bebas_tbc", "?,?,?,?,?", "No.Surat", 5,
                            new String[]{
                                NoSurat.getText(), TNoRw.getText(), Valid.
                                SetTgl(TanggalSurat.getSelectedItem() + ""),
                                KdDok.getText(), Keperluan.getText()
                            }) == true) {
                tabMode.addRow(new String[]{
                    NoSurat.getText(), TNoRw.getText(), TNoRM.getText(),
                    TPasien.getText(), Valid.SetTgl(TanggalSurat.
                    getSelectedItem() + ""), KdDok.getText(), TDokter.getText(),
                    Keperluan.getText()
                });
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (NoSurat.getText().trim().isEmpty()) {
            ...
            if (tbObat.getSelectedRow() != -1) {
                if (Sequel.mengedittf("surat_bebas_tbc", "no_surat=?",
                        "no_surat=?,no_rawat=?,tanggalsurat=?,kd_dokter=?,keperluan=?",
                        6, new String[]{
                            NoSurat.getText(), TNoRw.getText(), Valid.SetTgl(
                            TanggalSurat.getSelectedItem() + ""), KdDok.
                            getText(), Keperluan.getText(), tbObat.getValueAt(
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            tgl = " surat_bebas_tbc.tanggalsurat between '" + Valid.SetTgl(
                    DTPCari1.getSelectedItem() + "") + "' and '" + Valid.SetTgl(
                    DTPCari2.getSelectedItem() + "") + "' ";
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataSuratBebasTBC.jasper", "report",
                        "::[ Data Surat Bebas TBC ]::",
                ...
                Valid.MyReportqry("rptDataSuratBebasTBC.jasper", "report",
                        ...
                        + "where " + tgl + "and no_surat like '%" + TCari.
                                getText().trim() + "%' or "
                        + tgl + "and surat_bebas_tbc.no_rawat like '%" + TCari.
                                getText().trim() + "%' or "
                        + tgl + "and reg_periksa.no_rkm_medis like '%" + TCari.
                                getText().trim() + "%' or "
                        + tgl + "and pasien.nm_pasien like '%" + TCari.getText().
                                trim() + "%' or "
                        + tgl + "and surat_bebas_tbc.tanggalsurat like '%" + TCari.
                                getText().trim() + "%' "
    ...
    private void MnCetakSuratBebasTBCActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratBebasTBCActionPerformed
        if (TPasien.getText().trim().isEmpty()) {
            ...
            Valid.MyReportqry("rptSuratKeteranganBebasTBC.jasper", "report",
    ...
    public void tampil() {
        ...
        try {
            tgl = " surat_bebas_tbc.tanggalsurat between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + "' and '"
            ...
            if (TCari.getText().trim().isEmpty()) {
                ps = koneksi.prepareStatement(
                        ...
                        + "from surat_bebas_tbc inner join reg_periksa on surat_bebas_tbc.no_rawat=reg_periksa.no_rawat "
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join dokter on surat_bebas_tbc.kd_dokter=dokter.kd_dokter " + "where " + tgl
                ...
                ps = koneksi.prepareStatement(
                        ...
                        + "and surat_bebas_tbc.no_rawat like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and reg_periksa.no_rkm_medis like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or " + tgl
                        + "and surat_bebas_tbc.tanggalsurat like '%" + TCari.getText().trim() + "%' "
                        + "order by surat_bebas_tbc.no_surat");
            }
    ...
    public void emptTeks() {
        ...
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_bebas_tbc.no_surat,3),signed)),0) from surat_bebas_tbc where surat_bebas_tbc.tanggalsurat='"
                + Valid.SetTgl(TanggalSurat.getSelectedItem() + "") + "' ",
                "STBC" + TanggalSurat.getSelectedItem().toString().substring(6, 10)
    ...
    private static final Logger LOG = Logger.getLogger(SuratKeteranganBebasTBC.class.getName());
