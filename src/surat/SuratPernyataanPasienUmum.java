...
public class SuratPernyataanPasienUmum extends javax.swing.JDialog {
    ...
    public SuratPernyataanPasienUmum(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Pernyataan", "No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "J.K.", "Tgl.Lahir",
                    "Tanggal", "Nama Penanggung Jawab", "Tempat Lahir P.J.", "Tgl.Lahir P.J.", "Hubungan",
                    "J.K. P.J.", "Nomor Telp/HP", "Nomor KTP", "Alamat P.J.", "NIP", "Nama Petugas"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_surat"), rs.getString("no_rawat"),
                        ...
                        rs.getString("tempat_lahirpj"), rs.getString("lahirpj"), rs.getString("hubungan"),
                        rs.getString("jkpj"), rs.getString("no_telp"), rs.getString("no_ktppj"),
    ...
    public void emptTeks() {
        ...
        AlamatPj.setText("");
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_pernyataan_pasien_umum.no_surat,3),signed)),0) from surat_pernyataan_pasien_umum where surat_pernyataan_pasien_umum.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PPU" + Tanggal.getSelectedItem().toString().substring(6, 10)
    ...
    private void ganti() {
        if (Sequel.mengedittf("surat_pernyataan_pasien_umum", "no_surat=?",
                ...
                new String[]{NoSurat.getText(), TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    NamaPJ.getText(), NoKTP.getText(), TempatLahir.getText(),
                    Valid.SetTgl(TglLahir.getSelectedItem() + ""),
                    JKPJ.getSelectedItem().toString().substring(0, 1), AlamatPj.getText(),
                    Hubungan.getSelectedItem().toString(), NoTelp.getText(), NIP.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from surat_pernyataan_pasien_umum where no_surat=?", 1,
                    ...
                    if (rs.next()) {
                        if (rs.getString("photo").isEmpty() || rs.getString("photo").equals("-")) {
                            ...
                            LoadHTML2.setText("<html><body><center><img src='http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                    + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/pernyataanumum/"
                                    + rs.getString("photo")
    ...
    private static final Logger LOG = Logger.getLogger(SuratPernyataanPasienUmum.class.getName());
