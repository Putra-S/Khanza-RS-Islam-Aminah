...
public class TokoSirkulasi2 extends javax.swing.JDialog {
    ...
    private double ttltotalbeli = 0, totalbeli = 0, stok = 0, totalstok = 0, jumlahbeli = 0, jumlahpiutang = 0,
            totalpiutang = 0, ttltotalpiutang = 0, ttltotalpesan = 0, totalpesan = 0, jumlahpesan = 0,
            jumlahpenjualan = 0, totalpenjualan = 0, ttltotalpenjualan = 0, jumlahretursup = 0, totalretursup = 0,
            ttltotalretursup = 0, jumlahreturjual = 0, totalreturjual = 0, ttltotalreturjual = 0,
            jumlahreturpiutang = 0, totalreturpiutang = 0, ttltotalreturpiutang = 0, stokakhir, totalstokakhir = 0,
            ttltotalstokakhir = 0, stokawal, totalstokawal = 0, ttltotalstokawal = 0;
    ...
    public TokoSirkulasi2(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Kode Barang", "Nama Barang", "Satuan", "Tgl.Opname", "Stok Awal", "Stok Awal(Rp)",
                    ...
                    "Piutang", "Piutang(Rp)", "Retur Sup", "Retur Sup(Rp)", "Retur Jual", "Retur Jual(Rp)",
                    "Retur Piutang", "Retur Piutang(Rp)", "Stok Akhir", "Stok Akhir(Rp)"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if (tabMode.getRowCount() == 0) {//GEN-FIRST:event_BtnPrintActionPerformed
            ...
            Valid.MyReport("rptSirkulasiToko2.jasper", "report",
                    "::[ Sirkulasi Barang Toko ]::", param);
    ...
    private void prosesCari() {
        ...
        try {
            ps = koneksi.prepareStatement("select tokobarang.kode_brng,tokobarang.nama_brng, "
                    ...
                    + "tokobarang.nama_brng like ? and kodesatuan.satuan like ? " + " order by tokobarang.kode_brng");
            try {
                ttltotalbeli = 0;
                ttltotalpesan = 0;
                ttltotalpenjualan = 0;
                ...
                while (rs.next()) {
                    ...
                    tglopname = Sequel.cariIsi("select tanggal from tokoopname where kode_brng='" + rs.getString(1)
                            + "' and tanggal between '" + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                            + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' order by tanggal asc limit 1");
                    ...
                    ps2 = koneksi.prepareStatement("select sum(toko_detail_beli.jumlah), sum(toko_detail_beli.total) "
                            ...
                            + " where toko_detail_beli.kode_brng=? and tokopembelian.tgl_beli " + " between ? and ? ");
                    try {
                    ...
                    ps2 = koneksi.prepareStatement("select sum(toko_detail_jual.jumlah), sum(toko_detail_jual.total) "
                            ...
                            + " where toko_detail_jual.kode_brng=? and " + " tokopenjualan.tgl_jual between ? and ?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi
                            .prepareStatement("select sum(toko_detail_piutang.jumlah), sum(toko_detail_piutang.total) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(toko_detail_returbeli.jml_retur), sum(toko_detail_returbeli.total) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(toko_detail_returjual.jml_retur), sum(toko_detail_returjual.total) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(toko_detail_returpiutang.jml_retur), sum(toko_detail_returpiutang.total) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(tokoopname.real),(tokoopname.real*tokoopname.dasar) from tokoopname where kode_brng=? and tanggal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        rs2 = ps2.executeQuery();
                    ...
                    if ((stok > 0) || (jumlahbeli > 0) || (jumlahpesan > 0) || (jumlahpenjualan > 0)
                            || (jumlahpiutang > 0) || (jumlahretursup > 0) || (jumlahreturjual > 0)
                            || (jumlahreturpiutang > 0)) {
                        if (stokawal <= 0) {
                            stokawal = stok - jumlahbeli - jumlahpesan + jumlahpenjualan + jumlahpiutang
                                    + jumlahretursup - jumlahreturjual - jumlahreturpiutang;
                            stokakhir = stok;
                            totalstokawal = totalstok - totalbeli - totalpesan + totalpenjualan + totalpiutang
                                    + totalretursup - totalreturjual - totalreturpiutang;
                            totalstokakhir = totalstok;
                        } else {
                            stokakhir = stokawal + jumlahbeli + jumlahpesan - jumlahpenjualan - jumlahpiutang
                                    - jumlahretursup + jumlahreturjual + jumlahreturpiutang;
                            totalstokakhir = totalstokawal + totalbeli + totalpesan - totalpenjualan - totalpiutang
                                    - totalretursup + totalreturjual + totalreturpiutang;
                        }
                ...
                tabMode.addRow(new Object[]{"<>>", "Total :", "", "", "", Valid.SetAngka(ttltotalstokawal), "",
                    ...
                    Valid.SetAngka(ttltotalpenjualan), "", Valid.SetAngka(ttltotalpiutang), "",
                    Valid.SetAngka(ttltotalretursup), "", Valid.SetAngka(ttltotalreturjual), "",
                    Valid.SetAngka(ttltotalreturpiutang), "", Valid.SetAngka(ttltotalstokakhir)});
            } catch (SQLException e) {
    ...
    private static final Logger LOG = Logger.getLogger(TokoSirkulasi2.class.getName());
