...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReport("rptPengajuanBarangToko.jasper", "report",
                    "::[ Data Pengajuan Permintaan/Pengadaan Barang Toko / Minimarket / Koperasi ]::",
                    param);
        }
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (tbDokter.getSelectedRow() > -1) {
        if (Sequel.cariInteger(
                "select count(no_pengajuan) from toko_pengajuan_barang where no_pengajuan=?",
                tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                        trim()) == 0) {
            Valid.textKosong(TCari, "pilihan data");
        } else {
            Sequel.queryu(
                    "delete from toko_pengajuan_barang where no_pengajuan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim());
            tampil();
        }
    ...
    private void ppProsesPengajuanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppProsesPengajuanActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (Sequel.cariInteger(
                ...
                Sequel.queryu(
                        "update toko_pengajuan_barang set status='Proses Pengajuan' where no_pengajuan=?",
        ...
        if (tbDokter.getSelectedRow() > -1) {
            if (Sequel.cariInteger(
                ...
                if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 2).toString().
                    ...
                    Sequel.queryu(
                            "update toko_pengajuan_barang set status='Disetujui' where no_pengajuan=?",
                            tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).
                                    toString().trim());
                    this.setCursor(Cursor.
                            getPredefinedCursor(Cursor.WAIT_CURSOR));
                    TokoSuratPemesanan aplikasi = new TokoSuratPemesanan(null,
                            false);
    ...
    private void ppDitolakActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDitolakActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (Sequel.cariInteger(
                ...
                Sequel.queryu(
                        "update toko_pengajuan_barang set status='Ditolak' where no_pengajuan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).
                                toString().trim());
                tampil();
            }
