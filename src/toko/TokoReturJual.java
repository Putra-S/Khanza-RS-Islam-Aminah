...
public class TokoReturJual extends javax.swing.JDialog {
    ...
    private String hpptoko = "", Retur_Jual_Toko = Sequel.cariIsi("select Retur_Jual_Toko from set_akun"),
            Kontra_Retur_Jual_Toko = Sequel.cariIsi("select Kontra_Retur_Jual_Toko from set_akun");
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(no_retur_jual,3),signed)),0) from tokoreturjual where tgl_retur='"
                + Valid.SetTgl(TglRetur.getSelectedItem() + "") + "'",
                "TRJ" + TglRetur.getSelectedItem().toString().substring(6, 10)
                + TglRetur.getSelectedItem().toString().substring(3, 5)
                + TglRetur.getSelectedItem().toString().substring(0, 2),
                3, NoRetur);
    }

    private static final Logger LOG = Logger.getLogger(TokoReturJual.class.getName());
