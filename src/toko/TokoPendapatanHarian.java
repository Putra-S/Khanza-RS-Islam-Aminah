 ...
 * @author perpustakaan
 */
public class TokoPendapatanHarian extends javax.swing.JDialog {
    ...
    public TokoPendapatanHarian(java.awt.Frame parent, boolean modal) {
        ...
        setSize(885, 674);
        tabMode = new DefaultTableModel(null, new String[]{"Tanggal", "NIP", "Petugas", "Ongkir", "PPN", "Total"}) {
        ...
        tabMode2 = new DefaultTableModel(null,
                new String[]{"Tanggal", "NIP", "Petugas", "Ongkir", "Uang Muka", "Total", "Sisa Piutang"}) {
            Class[] types = new Class[]{java.lang.String.class, java.lang.String.class, java.lang.String.class,
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (TabRawat.getSelectedIndex() == 0) {
            if (tabMode.getRowCount() == 0) {
                ...
                Valid.MyReportqry("rptPendapatanHarian1Toko.jasper", "report",
                        "::[ Penjualan Harian Toko ]::",
                        ...
                        + "from tokopenjualan inner join petugas on tokopenjualan.nip=petugas.nip where tokopenjualan.tgl_jual between '" + Valid.
                                SetTgl(Tgl1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(Tgl2.getSelectedItem() + "") + "' "
                        + (TCari.getText().isEmpty() ? "" : "and (tokopenjualan.nip like '%" + TCari.
            ...
            if (tabMode2.getRowCount() == 0) {
                ...
                Valid.MyReportqry("rptPendapatanHarian2Toko.jasper", "report",
                        "::[ Piutang Harian Toko ]::",
                        ...
                        + "where tokopiutang.tgl_piutang between '" + Valid.
                                SetTgl(Tgl1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(Tgl2.getSelectedItem() + "") + "' "
                        + (TCari.getText().isEmpty() ? "" : "and (tokopiutang.nip like '%" + TCari.
                        getText() + "%' or petugas.nama like '%" + TCari.
                                getText() + "%')")
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    public void tampil() {
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + (TCari.getText().isEmpty() ? "" : "and (tokopenjualan.nip like ? or petugas.nama like ?)")
                    + "group by tokopenjualan.tgl_jual,tokopenjualan.nip order by tokopenjualan.tgl_jual,tokopenjualan.nip");
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode.addRow(new Object[]{rs.getString("tgl_jual"), rs.getString("nip"), rs.getString("nama"),
                        rs.getDouble("ongkir"), rs.getDouble("ppn"), rs.getDouble("total")});
                }
                if (total > 0) {
                    tabMode.addRow(new Object[]{"", "", "Total :", total, totalppn, totalpenjualan});
                }
    ...
    public void tampil2() {
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + (TCari.getText().isEmpty() ? "" : "and (tokopiutang.nip like ? or petugas.nama like ?)")
                    + "group by tokopiutang.tgl_piutang,tokopiutang.nip order by tokopiutang.tgl_piutang,tokopiutang.nip");
            try {
                ps.setString(1, Valid.SetTgl(Tgl1.getSelectedItem() + ""));
                ps.setString(2, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                ...
                while (rs.next()) {
                    ...
                    tabMode2.addRow(new Object[]{rs.getString("tgl_piutang"), rs.getString("nip"),
                        rs.getString("nama"), rs.getDouble("ongkir"), rs.getDouble("uangmuka"),
                        rs.getDouble("total"), rs.getDouble("sisapiutang")});
                }
                if (total > 0) {
                    tabMode2.addRow(
                            new Object[]{"", "", "Total :", totalongkir, totaluangmuka, totalpiutang, totalsisa});
                }
            } catch (SQLException e) {
                System.out.println("Notifikasi : " + e);
            } finally {
    ...
    private void isHitung() {
        Total.setText("Total Pendapatan = Pendapatan Penjualan + Pendapatan Uang Muka Piutang = "
                + Valid.SetAngka(totalpenjualan) + " + " + Valid.SetAngka(totaluangmuka) + " = "
                + Valid.SetAngka(totalpenjualan + totaluangmuka));
    }

    private static final Logger LOG = Logger.getLogger(TokoPendapatanHarian.class.getName());
