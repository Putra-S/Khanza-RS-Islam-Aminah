...
public class DlgCariPeminjamPiutang extends javax.swing.JDialog {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    iyem = iyem + "{\"Kode\":\"" + rs.getString(1) + "\",\"Peminjam\":\"" + rs.getString(2)
                            + "\",\"Alamat\":\"" + rs.getString(3) + "\",\"NoTelp\":\"" + rs.getString(4)
                            + "\",\"KodeRekening\":\"" + rs.getString(5) + "\",\"NamaRekening\":\"" + rs.getString(6)
                            + "\"},";
                }
            } catch (Exception e) {
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    if (list.path("Kode").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                            || list.path("Peminjam").asText().toLowerCase().contains(TCari.getText().toLowerCase())) {
                        tabMode.addRow(new Object[]{list.path("Kode").asText(), list.path("Peminjam").asText(),
                            list.path("Alamat").asText(), list.path("NoTelp").asText(),
                            list.path("KodeRekening").asText(), list.path("NamaRekening").asText()});
                    }
                }
            }
            myObj.close();
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariPeminjamPiutang.class.getName());
