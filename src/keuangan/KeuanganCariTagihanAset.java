...
public class KeuanganCariTagihanAset extends javax.swing.JDialog {
    ...
    public KeuanganCariTagihanAset(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No.Tagihan", "Tanggal", "Diajukan Oleh", "Keteragan",
            "Status Tagihan"};
...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptTagihanAset.jasper", "report",
                    "::[ Data Titip Faktur/Tagihan Aset/Inventaris ]::",
            ...
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                try {
                    ps = koneksi.prepareStatement(
                            "select inventaris_detail_titip_faktur.no_faktur from inventaris_detail_titip_faktur where inventaris_detail_titip_faktur.no_tagihan=?");
                    try {
                        ...
                        while (rs.next()) {
                            Sequel.queryu(
                                    "update inventaris_pemesanan set status='Belum Lunas' where no_faktur=?",
                    ...
                    Sequel.queryu(
                            "delete from inventaris_titip_faktur where no_tagihan=?",
    ...
    private void MnDitagihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDitagihkanActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                Sequel.queryu(
                        "update inventaris_titip_faktur set status='Ditagihkan' where no_tagihan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString().trim());
                tampil();
            }
    ...
    private void MnDibayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDibayarActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                if (Sequel.cariIsi(
                        "select status from inventaris_titip_faktur where no_tagihan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString()).equals("Ditagihkan")) {
                    this.setCursor(Cursor.
                            getPredefinedCursor(Cursor.WAIT_CURSOR));
                    KeuanganHutangAsetIventarisBelumLunas form = new KeuanganHutangAsetIventarisBelumLunas(
                            null, false);
                    form.isCek();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganCariTagihanAset dialog = new KeuanganCariTagihanAset(
    ...
    private void tampil() {
        ...
        try {
            ...
            if (!NoTagihan.getText().trim().isEmpty()) {
                notagihan = " and inventaris_titip_faktur.no_tagihan like '%" + NoTagihan.
                        getText() + "%' ";
            }

            tanggal = " inventaris_titip_faktur.tanggal between '" + Valid.
                    SetTgl(Tanggal1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(Tanggal2.getSelectedItem() + "") + "' ";

            if (!Status.getSelectedItem().toString().equals("Semua")) {
                status = " and inventaris_titip_faktur.status like '%" + Status.
                        getSelectedItem().toString() + "%' ";
            }

            if (!NmPeg.getText().trim().isEmpty()) {
                petugas = " and concat(inventaris_titip_faktur.nip,petugas.nama) like '%" + KdPeg.
                        getText() + NmPeg.getText() + "%' ";
            }

            if (!TCari.getText().trim().isEmpty()) {
                cari = " and (inventaris_titip_faktur.no_tagihan like '%" + TCari.
                        getText() + "%' or petugas.nama like '%" + TCari.
                                getText() + "%' or inventaris_titip_faktur.keterangan like '%" + TCari.
                                getText() + "%' or inventaris_detail_titip_faktur.no_faktur like '%" + TCari.
                                getText() + "%' or inventaris_suplier.nama_suplier like '%" + TCari.
                                getText() + "%' ) ";
            }
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{
                        rs.getString("no_tagihan"), rs.getString("tanggal"), rs.
                        getString("nip") + " " + rs.getString("petugas"), rs.
                        getString("keterangan"), rs.getString("status")
                    });
                    tabMode.addRow(new Object[]{
                        "", "Tgl.Tempo", "No.Faktur", "Nama Suplier",
                        "Sisa Hutang"
                    });
                    ps2 = koneksi.prepareStatement(
                            ...
                            + "inner join inventaris_pemesanan on inventaris_detail_titip_faktur.no_faktur=inventaris_pemesanan.no_faktur "
                            + "inner join inventaris_suplier on inventaris_suplier.kode_suplier=inventaris_pemesanan.kode_suplier "
                            + "where inventaris_detail_titip_faktur.no_tagihan='" + rs.
                                    getString("no_tagihan") + "' and "
                            + tanggal + status + notagihan + petugas + cari + " order by inventaris_titip_faktur.tanggal");
                    try {
                        nilaitagihan = 0;
                        rs2 = ps2.executeQuery();
                        while (rs2.next()) {
                            nilaitagihan += (rs2.getDouble("tagihan") - rs2.
                                    getDouble("bayar"));
                            totaltagihan += (rs2.getDouble("tagihan") - rs2.
                                    getDouble("bayar"));
                            tabMode.addRow(new Object[]{
                                "", rs2.getString("tgl_tempo"), rs2.getString(
                                "no_faktur"),
                                rs2.getString("kode_suplier") + " " + rs2.
                                getString("nama_suplier"), Valid.SetAngka((rs2.
                                getDouble("tagihan") - rs2.getDouble("bayar")))
                            });
                        }
                        if (nilaitagihan > 0) {
                            tabMode.addRow(new Object[]{
                                "", "", "Jumlah Hutang : ", "", Valid.SetAngka(
                                nilaitagihan)
                            });
                        }
                    } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(
            KeuanganCariTagihanAset.class.getName());
}
