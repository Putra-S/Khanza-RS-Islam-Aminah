...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptCashFlow.jasper", "report",
    ...
    private void prosesCari() {
        ...
        try {
            ...
            rs = koneksi
                    .prepareStatement("select rekening.kd_rek, rekening.nm_rek, sum(rekeningtahun.saldo_awal) "
                            + "from rekening inner join rekeningtahun " + "on rekening.kd_rek=rekeningtahun.kd_rek "
                            + "where rekening.tipe='N' and rekening.balance='D' " + "and rekeningtahun.thn between '"
            ...
            while (rs.next()) {
                debkred = rs
                        .getDouble(3)/*
									 ...
									 * + "detailjurnal.kd_rek='"+rs.getString(1)
									 * +"' and jurnal.tgl_jurnal between '"+Valid.SetTgl(
									 * Tgl1.getSelectedItem()+"")+
									 * "' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+
									 * "'"); if(debkred<0){ debkred=debkred*(-1); }
                         */;
			 ...
			 * "' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"'"); if(debkred<0){
			 * debkred=debkred*(-1); } kasawal=kasawal+debkred; tabMode.addRow(new
			 * Object[]{"",i+". "+rs.getString(2),df2.format(debkred)}); i++; }
             */
            ...
            rs = koneksi.prepareStatement(
                    "select detailjurnal.kd_rek, rekening.nm_rek,(sum(detailjurnal.kredit)-sum(detailjurnal.debet)) as ttlkredit "
                    ...
                    + " where rekening.tipe='R' and rekening.balance='K' " + " and jurnal.tgl_jurnal between '"
                    ...
                    .executeQuery();
            while (rs.next()) {
                debkred = rs.getDouble(3) + Sequel
                        .cariIsiAngka("select sum(rekeningtahun.saldo_awal) from rekeningtahun where rekeningtahun.kd_rek='"
                ...
                penerimaan += debkred;
                tabMode.addRow(
                        new Object[]{"", i + ". " + rs.getString(1) + " " + rs.getString(2), df2.format(debkred)});
                i++;
            }
            /*
			 * rs=koneksi.
			 * prepareStatement("select detailjurnal.kd_rek, rekening.nm_rek,(sum(detailjurnal.kredit)-sum(detailjurnal.debet)) as ttlkredit "
			 * + " from jurnal inner join detailjurnal inner join rekening "+
			 * " on jurnal.no_jurnal=detailjurnal.no_jurnal "+
			 * " and detailjurnal.kd_rek=rekening.kd_rek "+
			 * " where rekening.tipe='N' and rekening.balance='K' "+
			 ...
			 * +rs.getString(1)+"' and rekeningtahun.thn "+
			 * "between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"").substring(0,4)
			 * +"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"").substring(0,4)+"'");
            ...
            pengeluaran = 0;
            rs = koneksi.prepareStatement(
                    "select detailjurnal.kd_rek, rekening.nm_rek,(sum(detailjurnal.debet)-sum(detailjurnal.kredit)) as ttldebet "
                    + " from jurnal inner join detailjurnal inner join rekening "
                    + " on jurnal.no_jurnal=detailjurnal.no_jurnal "
                    + " and detailjurnal.kd_rek=rekening.kd_rek "
                    + " where rekening.tipe='R' and rekening.balance='D' " + " and jurnal.tgl_jurnal between '"
                    + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' " + " and '"
                    + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                    + " group by detailjurnal.kd_rek order by detailjurnal.kd_rek")
                    .executeQuery();
            ...
            /*
			 * rs=koneksi.
			 * prepareStatement("select detailjurnal.kd_rek, rekening.nm_rek,(sum(detailjurnal.debet)-sum(detailjurnal.kredit)) as ttldebet "
			 * + " from jurnal inner join detailjurnal inner join rekening "+
			 * " on jurnal.no_jurnal=detailjurnal.no_jurnal "+
			 * " and detailjurnal.kd_rek=rekening.kd_rek "+
			 ...
			 * );
			 * 
			 * while(rs.next()){ debkred=rs.getDouble(3)+Sequel.cariIsiAngka(
			 * "select sum(rekeningtahun.saldo_awal) from rekeningtahun where rekeningtahun.kd_rek='"
			 * +rs.getString(1)+"' and rekeningtahun.thn "+
			 * "between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"").substring(0,4)
			 * +"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"").substring(0,4)+"'");
			 * /*if(debkred<0){ debkred=debkred*(-1); }
			 * 
			 * pengeluaran=pengeluaran+debkred; tabMode.addRow(new
			 * Object[]{"",i+". "+rs.getString(2),df2.format(debkred)}); i++; }
             */
            ...
            tabMode.addRow(new Object[]{"",
                df2.format(kasawal) + " + ( " + df2.format(penerimaan) + " - " + df2.format(pengeluaran) + " )  : ",
                df2.format(kasawal + penerimaan - pengeluaran)});
            tabMode.addRow(new Object[]{"", "", ""});
        } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgCashflow.class.getName());
