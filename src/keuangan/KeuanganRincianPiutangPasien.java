 ...
 * @author perpustakaan
 */
public class KeuanganRincianPiutangPasien extends javax.swing.JDialog {
    ...
    private double sisapiutang = 0, cicilan = 0, all = 0, Laborat = 0, Radiologi = 0, Operasi = 0, Obat = 0,
            ...
            Retur_Obat = 0, Resep_Pulang = 0, Service = 0, ttlLaborat = 0, ttlRadiologi = 0, ttlOperasi = 0,
            ...
            ttlResep_Pulang = 0, ttlService = 0, ttlUangMuka = 0, ttlCicilan = 0, diskon = 0, ttldiskon = 0,
    ...
    public KeuanganRincianPiutangPasien(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat/Tagihan", "Tgl.Piutang", "No.RM", "Pasien", "Status", "Jatuh Tempo",
                    "Cara Bayar", "Registrasi", "Tindakan", "Obt+Emb+Tsl", "Retur Obat", "Resep Pulang", "Laborat",
                    "Radiologi", "Potongan", "Tambahan", "Kamar+Service", "Operasi", "Harian", "Total Piutang",
                    "Uang Muka", "Cicilan", "Diskon Bayar", "Tidak Terbayar", "Sisa Piutang"}) {
                    ...
                    case "Laporan 1 (HTML)":
                        ...
                        for (i = 0; i < tabMode.getRowCount(); i++) {
                            htmlContent.append(
                                    ...
                                    append(Valid.SetAngka(Double.parseDouble(
                    ...
                    case "Laporan 4 (Jasper)":
                        ...
                        for (int i = 0; i < row; i++) {
                            Sequel.menyimpan("temporary", "'" + i + "','"
                                    ...
                                    + Valid.SetAngka(Double.parseDouble(tabMode.
                                            getValueAt(i, 24).toString())) + "','','','','','','','','','','','','" + akses.
                        ...
                        Valid.MyReportqry("rptRincianPiutangPasien.jasper",
                                "report", "::[ Rekap Piutang Masuk ]::",
            ...
            if (evt.getClickCount() == 2) {
                ...
                if (kolom == 1) {
                    ...
                    if (akses.getbayar_piutang() == true) {
                        DlgBayarPiutang bayarpiutang = new DlgBayarPiutang(null,
                                false);
                        bayarpiutang.emptTeks();
                        String norm = Sequel.cariIsi(
                                "select piutang_pasien.no_rkm_medis from piutang_pasien where piutang_pasien.no_rawat='" + tbBangsal.
                                        getValueAt(tbBangsal.getSelectedRow(), 0).
                                        toString() + "'");
                        String nama = Sequel.cariIsi(
                                "select pasien.nm_pasien from pasien where pasien.no_rkm_medis='" + norm + "'");
                        bayarpiutang.setData(tbBangsal.getValueAt(tbBangsal.
                                getSelectedRow(), 0).toString(), norm, nama);
                        bayarpiutang.tampil();
                        bayarpiutang.setSize(this.getWidth() - 20, this.
                                getHeight() - 20);
                    ...
                    if (status.equals("Ralan")) {
                        ...
                        billing.setSize(internalFrame1.getWidth() - 20,
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    public void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select piutang_pasien.no_rawat, piutang_pasien.tgl_piutang,piutang_pasien.no_rkm_medis,pasien.nm_pasien, "
                    ...
                    + "where penjab.png_jawab like ? and piutang_pasien.tgl_piutang between ? and ? "
                    + (TCari.getText().trim().isEmpty() ? ""
                    : " and (piutang_pasien.no_rawat like ? or piutang_pasien.no_rkm_medis like ? or "
                    + "pasien.nm_pasien like ? or piutang_pasien.status like ? )")
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    diskon = Sequel.cariIsiAngka(
                            "SELECT ifnull(SUM(bayar_piutang.diskon_piutang),0) FROM bayar_piutang where bayar_piutang.no_rawat=?",
                            rs.getString(1));
                    tidakdibayar = Sequel.cariIsiAngka(
                            "SELECT ifnull(SUM(bayar_piutang.tidak_terbayar),0) FROM bayar_piutang where bayar_piutang.no_rawat=?",
                            rs.getString(1));
                    tabMode.addRow(new Object[]{rs.getString("no_rawat"), rs.getString("tgl_piutang"),
                        rs.getString("no_rkm_medis"), rs.getString("nm_pasien"), rs.getString("status"),
                        rs.getString("tgltempo"), rs.getString("png_jawab"), Registrasi,
                        (Ranap_Dokter + Ranap_Dokter_Paramedis + Ranap_Paramedis + Ralan_Dokter
                        + Ralan_Dokter_Paramedis + Ralan_Paramedis),
                        Obat, Retur_Obat, Resep_Pulang, Laborat, Radiologi, Potongan, Tambahan, (Kamar + Service),
                        Operasi, Harian, rs.getDouble("totalpiutang"), rs.getDouble("uangmuka"), cicilan, diskon,
                        tidakdibayar, (rs.getDouble("sisapiutang") - cicilan - diskon - tidakdibayar)});
                    all += rs.getDouble("totalpiutang");
                ...
                tabMode.addRow(new Object[]{">>Total", ":", "", "", "", "", "", ttlRegistrasi,
                    (ttlRanap_Dokter + ttlRanap_Paramedis + ttlRalan_Dokter + ttlRalan_Paramedis), ttlObat,
                    ttlRetur_Obat, ttlResep_Pulang, ttlLaborat, ttlRadiologi, ttlPotongan, ttlTambahan,
                    (ttlKamar + ttlService), ttlOperasi, ttlHarian, all, ttlUangMuka, ttlCicilan, ttldiskon,
                    ttltidakdibayar, sisapiutang});
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganRincianPiutangPasien.class.getName());
