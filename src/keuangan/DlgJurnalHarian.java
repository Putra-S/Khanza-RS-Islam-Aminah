...
//GEN-FIRST:event_TKdKeyPressed
    private void TglJurnal2KeyPressed(java.awt.event.KeyEvent evt) {//GEN-LAST:event_TKdKeyPressed
        Valid.pindah(evt, TglJurnal1, TCari);//GEN-FIRST:event_TglJurnal2KeyPressed
    ...
    private void tampil() {
        try {
            ...
            ps = koneksi
                    .prepareStatement("select jurnal.no_jurnal, jurnal.no_bukti, jurnal.tgl_jurnal,jurnal.keterangan,"
                ...
                while (rs.next()) {
                    ...
                    if (rs.getDouble("kredit") > 0) {
                        tabMode.addRow(new Object[]{
                            rs.getString("tgl_jurnal") + " " + rs.getString("jam_jurnal"), rs.getString("kd_rek"),
                            "     " + rs.getString("nm_rek"), "No.Jur " + rs.getString("no_jurnal") + ", No.Buk "
                            + rs.getString("no_bukti") + ", " + rs.getString("keterangan"),
                            rs.getDouble("debet"), rs.getDouble("kredit")});
                    } else {
                        tabMode.addRow(new Object[]{rs.getString("tgl_jurnal") + " " + rs.getString("jam_jurnal"),
                            rs.getString("kd_rek"), rs.getString("nm_rek"), "No.Jur " + rs.getString("no_jurnal")
                            + ", No.Buk " + rs.getString("no_bukti") + ", " + rs.getString("keterangan"),
                            rs.getDouble("debet"), rs.getDouble("kredit")});
                    }
                }
    ...
    private static final Logger LOG = Logger.getLogger(DlgJurnalHarian.class.getName());
