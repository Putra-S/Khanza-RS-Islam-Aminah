...
public class DlgAkunBayar extends javax.swing.JDialog {
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (nama.getText().trim().isEmpty()) {
            ...
            Sequel.menyimpan("akun_bayar", "?,?,?", "Nama Akun", 3,
                    new String[]{
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        for (int i = 0; i < tbJadwal.getRowCount(); i++) {
            if (tbJadwal.getValueAt(i, 0).toString().equals("true")) {
                Sequel.meghapus("akun_bayar", "nama_bayar", tbJadwal.getValueAt(
                        i, 1).toString());
            }
        }
        tampil();
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (nama.getText().trim().isEmpty()) {
            ...
            if (tbJadwal.getSelectedRow() > -1) {
                Sequel.mengedit("akun_bayar", "nama_bayar=?",
                        "nama_bayar=?,kd_rek=?,ppn=?", 4, new String[]{
                            nama.getText(), kdrek.getText(), ppn.getText(),
                            tbJadwal.getValueAt(tbJadwal.getSelectedRow(), 1).
                                    toString()
                        });
                tampil();
                emptTeks();
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptAkunBayar.jasper", "report",
                    "::[ Akun Bayar ]::",
                    ...
                    + "where akun_bayar.nama_bayar like '%" + TCari.getText().
                            trim() + "%' or "
                    + "rekening.nm_rek like '%" + TCari.getText().trim() + "%' order by akun_bayar.nama_bayar",
                    param);
        }
...
private void BtnPoliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPoliActionPerformed
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi
                    .prepareStatement("select akun_bayar.nama_bayar,akun_bayar.kd_rek,rekening.nm_rek,akun_bayar.ppn "
                            + "from akun_bayar inner join rekening on akun_bayar.kd_rek=rekening.kd_rek "
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(
                            new Object[]{false, rs.getString(1), rs.getString(2), rs.getString(3), rs.getDouble(4)});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgAkunBayar.class.getName());
