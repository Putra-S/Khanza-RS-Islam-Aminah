 ...
 * @author dosen
 */
public class DlgKategoriPemasukan extends javax.swing.JDialog {
    ...
    public DlgKategoriPemasukan(java.awt.Frame parent, boolean modal) {
        ...
        try {
            ps = koneksi.prepareStatement("select * from kategori_pemasukan_lain " + "where kode_kategori like ? or "
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (NmAkun.getText().trim().isEmpty() || KdAkun.getText().trim().
                isEmpty()) {
            Valid.textKosong(Kd, "Akun Rekening");
        } else if (NmKontraAKun.getText().trim().isEmpty() || KdKontraAkun.
                getText().trim().isEmpty()) {
            Valid.textKosong(KdKontraAkun, "Kontra Akun Rekening");
        } else {
            if (Sequel.menyimpantf("kategori_pemasukan_lain", "?,?,?,?",
                    "Kode Kategori", 4, new String[]{
                        Kd.getText(), Nm.getText(), KdAkun.getText(),
                        KdKontraAkun.getText()
                    }) == true) {
                tampil();
                emptTeks();
            }
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        Valid.hapusTable(tabMode, Kd, "kategori_pemasukan_lain ",
                "kode_kategori");
        tampil();
        emptTeks();
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (Kd.getText().trim().isEmpty()) {
            ...
            if (tbKamar.getSelectedRow() > -1) {
                Sequel.mengedit("kategori_pemasukan_lain", "kode_kategori=?",
                        "kode_kategori=?,nama_kategori=?,kd_rek=?,kd_rek2=?", 5,
                        new String[]{
                            Kd.getText(), Nm.getText(), KdAkun.getText(),
                            KdKontraAkun.getText(), tbKamar.getValueAt(tbKamar.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptKategoriPemasukanLain.jasper", "report",
...
//   Valid.pindah(evt,Kd,Tahun);
    ...
    private void KdKontraAkunKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KdKontraAkunKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi(
                    "select rekening.nm_rek from rekening where rekening.kd_rek=?",
                    NmKontraAKun, KdKontraAkun.getText());
        } else if (evt.getKeyCode() == KeyEvent.VK_UP) {
    ...
    private void getData() {
        ...
        if (row != -1) {
            ...
            Sequel.cariIsi("select kd_rek from kategori_pemasukan_lain where kode_kategori=?", KdAkun,
                    tbKamar.getValueAt(row, 0).toString());
            Sequel.cariIsi("select kd_rek2 from kategori_pemasukan_lain where kode_kategori=?", KdKontraAkun,
                    tbKamar.getValueAt(row, 0).toString());
        }
    }
    ...
    private static final Logger LOG = Logger.getLogger(DlgKategoriPemasukan.class.getName());
