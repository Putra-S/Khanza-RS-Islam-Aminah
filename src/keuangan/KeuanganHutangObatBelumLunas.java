 ...
 * @author perpustakaan
 */
public class KeuanganHutangObatBelumLunas extends javax.swing.JDialog {
    ...
    private String koderekening = "", tanggaldatang = "", tanggaltempo = "",
            Bayar_Pemesanan_Obat = Sequel.cariIsi("select Bayar_Pemesanan_Obat from set_akun");
    ...
    public KeuanganHutangObatBelumLunas(java.awt.Frame parent, boolean modal) {
        ...
        Object[] rowRwJlDr = {"P", "No.Faktur", "No.Order", "Supplier", "Petugas Penerima", "Tgl.Faktur", "Tgl.Datang",
            "Tgl.Tempo", "Posisi Barang", "Tagihan", "Sisa Hutang", "Pembayaran", "Sisa", "Bank Suplier",
            "No.Rekening"};
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptHutangBelumLunas.jasper", "report",
                    "::[ Data Hutang Obat, Alkes & BHP Medis ]::",
            ...
            if (tbBangsal.getSelectedRow() != -1) {
                if (evt.getClickCount() == 1) {
                    if (tbBangsal.getSelectedColumn() == 0) {
                        if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 11).
                                toString().equals("0")) {
                            tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                                    getValueAt(tbBangsal.getSelectedRow(), 12).
                                    toString()), tbBangsal.getSelectedRow(), 11);
                        }
                        ...
                        } else if (tbBangsal.getValueAt(tbBangsal.
                                getSelectedRow(), 0).toString().equals("false")) {
                            tbBangsal.setValueAt(0, tbBangsal.getSelectedRow(),
                                    11);
                            tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                                    getValueAt(tbBangsal.getSelectedRow(), 12).
                                    toString()), tbBangsal.getSelectedRow(), 10);
                        }
                        getData();
                    }
                    panggilPhoto();
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnBayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnBayarActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        if (no_bukti.getText().trim().isEmpty()) {
            ...
            JOptionPane.showMessageDialog(null,
                    "Maaf, data sudah habis. Tidak ada data yang bisa anda bayar...!!!!");
            ...
            for (int i = 0; i < row; i++) {
                if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                    if (Double.parseDouble(tbBangsal.getValueAt(i, 11).
                            toString()) > 0) {
                        if (Sequel.menyimpantf("bayar_pemesanan",
                                "?,?,?,?,?,?,?", "Data", 7, new String[]{
                                    Valid.SetTgl(
                                            tgl_bayar.getSelectedItem() + ""),
                                    tabMode.getValueAt(i, 1).toString(), nip.
                                    getText(),
                                    tabMode.getValueAt(i, 11).toString(),
                                    keterangan.getText(), AkunBayar.
                                    getSelectedItem().toString(),
                                    no_bukti.getText()
                                }) == true) {
                            if ((Double.parseDouble(tabMode.getValueAt(i, 10).
                                    toString()) <= 0) || (Double.parseDouble(
                                            tabMode.getValueAt(i, 10).toString()) <= -0)) {
                                Sequel.mengedit("pemesanan",
                                        "no_faktur='" + tabMode.getValueAt(i, 1).
                                                toString() + "'",
                            ...
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                            "Rekening", 4, new String[]{
                                        Bayar_Pemesanan_Obat, "HUTANG USAHA",
                                        tabMode.getValueAt(i, 11).toString(),
                                        "0"
                                    });
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                        ...
                                        koderekening, AkunBayar.
                                                getSelectedItem().toString(),
                                        "0", tabMode.getValueAt(i, 11).
                                                toString()
                                    });
                            if (jur.simpanJurnal(no_bukti.getText(), "U",
                                    "BAYAR PELUNASAN HUTANG OBAT/BHP/ALKES NO.FAKTUR " + tabMode.
                                            getValueAt(i, 1).toString() + ", OLEH " + akses.
                                    getkode()) == false) {
                                sukses = false;
                            }
                        } else {
                    ...
                    if (!notagihan.isEmpty()) {
                        Sequel.mengedit("pemesanan", "no_faktur='" + tabMode.
    ...
    private void ppSemuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppSemuaActionPerformed
        for (row = 0; row < tbBangsal.getRowCount(); row++) {
            ...
            if (tbBangsal.getValueAt(row, 11).toString().equals("0")) {
                tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                        getValueAt(row, 12).toString()), row, 11);
            }
            if (tbBangsal.getValueAt(row, 0).toString().equals("true")) {
                tbBangsal.setValueAt(
                        (Double.parseDouble(tbBangsal.getValueAt(row, 12).
                                toString())
                        - Double.parseDouble(tbBangsal.getValueAt(row, 11).
                                toString())),
                        row, 10);
            } else if (tbBangsal.getValueAt(row, 0).toString().equals("false")) {
                tbBangsal.setValueAt(0, row, 11);
                tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                        getValueAt(row, 12).toString()), row, 10);
            }
        }
        getData();
    ...
    private void tbBangsalPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_tbBangsalPropertyChange
        if (this.isVisible() == true) {
            if (tbBangsal.getSelectedRow() != -1) {
                if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 11).
                        toString().equals("0")) {
                    tbBangsal.setValueAt(Double.valueOf(tbBangsal.getValueAt(
                            tbBangsal.getSelectedRow(), 12).toString()),
                            tbBangsal.getSelectedRow(), 11);
                }
                if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 0).
                        toString().equals("true")) {
    ...
    private void tampil() {
        ...
        try {
            ...
            if (ChkTanggalDatang.isSelected() == true) {
                tanggaldatang = " pemesanan.tgl_pesan between '" + Valid.SetTgl(TglDatang1.getSelectedItem() + "")
                        + "' and '" + Valid.SetTgl(TglDatang2.getSelectedItem() + "") + "' and ";
            }
            if (ChkTanggalTempo.isSelected() == true) {
                tanggaltempo = " pemesanan.tgl_tempo between '" + Valid.SetTgl(TglTempo1.getSelectedItem() + "")
                        + "' and '" + Valid.SetTgl(TglTempo2.getSelectedItem() + "") + "' and ";
            }
            ps = koneksi.prepareStatement("select pemesanan.no_faktur,pemesanan.no_order,datasuplier.nama_suplier, "
                    ...
                    + "and pemesanan.nip=petugas.nip and pemesanan.kd_bangsal=bangsal.kd_bangsal where " + tanggaldatang
                    + tanggaltempo
                    + "(pemesanan.status='Belum Dibayar' or pemesanan.status='Belum Lunas') and datasuplier.nama_suplier like ? "
                    + (TCari.getText().trim().isEmpty() ? ""
                    : "and (pemesanan.no_faktur like ? or pemesanan.no_order like ? or pemesanan.tgl_tempo like ? or "
                    + "datasuplier.nama_suplier like ? or bangsal.nm_bangsal like ? or petugas.nama like ?) ")
                    + "order by pemesanan.tgl_tempo ");
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("no_faktur"), rs.getString("no_order"),
                        ...
                        rs.getDouble("tagihan"), (rs.getDouble("tagihan") - rs.getDouble("bayar")), 0,
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganHutangObatBelumLunas.class.getName());
