 ...
 * @author perpustakaan
 */
public class KeuanganHutangNonMedisBelumLunas extends javax.swing.JDialog {
    ...
    private String koderekening = "", tanggaldatang = "", tanggaltempo = "",
            Bayar_Pemesanan_Non_Medis = Sequel.cariIsi("select Bayar_Pemesanan_Non_Medis from set_akun");
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptHutangNonMedisBelumLunas.jasper", "report",
                    "::[ Data Hutang Barang Non Medis ]::",
        ...
        if (tabMode.getRowCount() != 0) {
            if (evt.getClickCount() == 1) {
                if (tbBangsal.getSelectedColumn() == 0) {
                    if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 10).
                            toString().equals("0")) {
                        tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                                getValueAt(tbBangsal.getSelectedRow(), 11).
                    ...
                    if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 0).
                        ...
                        tbBangsal.setValueAt(Double.valueOf(tbBangsal.
                                getValueAt(tbBangsal.getSelectedRow(), 11).
                                toString()), tbBangsal.getSelectedRow(), 9);
                    }
                }
                panggilPhoto();
            }
            ...
            if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 0).
                    ...
                    tbBangsal.setValueAt(Double.valueOf(tbBangsal.getValueAt(
                            tbBangsal.getSelectedRow(), 11).toString()),
                            tbBangsal.getSelectedRow(), 9);
                }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
        ...
        if (no_bukti.getText().trim().isEmpty()) {
            ...
            for (int i = 0; i < row; i++) {
                if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                    if (Double.parseDouble(tbBangsal.getValueAt(i, 10).
                            toString()) > 0) {
                        if (Sequel.menyimpantf("bayar_pemesanan_non_medis",
                                "?,?,?,?,?,?,?", "Data", 7, new String[]{
                                    Valid.SetTgl(
                            ...
                            if ((Double.parseDouble(tabMode.getValueAt(i, 9).
                                    toString()) <= 0) || (Double.parseDouble(
                                            tabMode.getValueAt(i, 9).toString()) <= -0)) {
                                Sequel.mengedit("ipsrspemesanan",
                            ...
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                        ...
                                        "HUTANG BARANG NON MEDIS", tabMode.
                            ...
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                        ...
                                        koderekening, AkunBayar.
                                                getSelectedItem().toString(),
                                        "0", tabMode.getValueAt(i, 10).
                                                toString()
                                    });
                            if (jur.simpanJurnal(no_bukti.getText(), "U",
                                    "BAYAR PELUNASAN HUTANG BARANG NON MEDIS NO.FAKTUR " + tabMode.
            ...
            if (sukses == true) {
                if (!notagihan.isEmpty()) {
                    Sequel.queryu(
                            "update ipsrs_titip_faktur set status='Dibayar' where no_tagihan=?",
                            notagihan);
                    notagihan = "";
                }
                Sequel.Commit();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganHutangNonMedisBelumLunas dialog = new KeuanganHutangNonMedisBelumLunas(new javax.swing.JFrame(),
                    true);
    ...
    public void tampil() {
        ...
        try {
            ...
            if (ChkTanggalDatang.isSelected() == true) {
                tanggaldatang = " ipsrspemesanan.tgl_pesan between '" + Valid.SetTgl(TglDatang1.getSelectedItem() + "")
                        + "' and '" + Valid.SetTgl(TglDatang2.getSelectedItem() + "") + "' and ";
            }
            if (ChkTanggalTempo.isSelected() == true) {
                tanggaltempo = " ipsrspemesanan.tgl_tempo between '" + Valid.SetTgl(TglTempo1.getSelectedItem() + "")
                        + "' and '" + Valid.SetTgl(TglTempo2.getSelectedItem() + "") + "' and ";
            }
            ps = koneksi
                    .prepareStatement("select ipsrspemesanan.no_faktur,ipsrspemesanan.no_order,ipsrssuplier.nama_suplier, "
                            ...
                            + "and ipsrspemesanan.nip=petugas.nip where " + tanggaldatang + tanggaltempo
                            + "(ipsrspemesanan.status='Belum Dibayar' or ipsrspemesanan.status='Belum Lunas') and ipsrssuplier.nama_suplier like ? "
                            + (TCari.getText().trim().isEmpty() ? ""
                            : "and (ipsrspemesanan.no_faktur like ? or ipsrspemesanan.no_order like ? or ipsrspemesanan.tgl_tempo like ? or "
                            + "ipsrssuplier.nama_suplier like ? or petugas.nama like ?)")
                            + " order by ipsrspemesanan.tgl_tempo ");
            try {
                    ...
                    if (rs.next()) {
                        if (rs.getString("photo").isEmpty() || rs.getString("photo").equals("-")) {
                            ...
                            LoadHTML.setText("<html><body><center><img src='http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                    + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/penerimaanlogistik/"
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganHutangNonMedisBelumLunas.class.getName());
