...
public class DlgBayarPiutang extends javax.swing.JDialog {
    ...
    public DlgBayarPiutang(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Tgl.Bayar", "No.RM", "Pasien", "Cicilan(Rp)", "Keterangan", "No.Tagihan", "Kode Akun",
                    "Kontra AKun", "Diskon(Rp)", "Kode Akun Diskon", "Tidak Terbayar(Rp)",
                    "Kode Akun Tidak Terbayar"}) {
            @Override
        ...
        Cicilan.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            ...
            public void insertUpdate(DocumentEvent e) {
                ...
                if ((!Cicilan.getText().isEmpty()) && (!DiskonBayar.getText().isEmpty())
                        && (!TidakTerbayar.getText().isEmpty())) {
                    Sisa.setText(Valid.SetAngka(sisapiutang - Double.parseDouble(Cicilan.getText())
                            - Double.parseDouble(DiskonBayar.getText()) - Double.parseDouble(TidakTerbayar.getText())));
                }
            }
            ...
            public void windowClosed(WindowEvent e) {
                if (pasien.getTable().getSelectedRow() != -1) {
                    ...
                    sisapiutang = Sequel.cariIsiAngka(
                            "SELECT ifnull(SUM(piutang_pasien.sisapiutang),0) FROM piutang_pasien where piutang_pasien.no_rkm_medis=?",
                            Kdmem.getText())
                            + Sequel.cariIsiAngka(
                                    "SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.no_rkm_medis=?",
                                    Kdmem.getText())
                            - Sequel.cariIsiAngka(
                                    "SELECT ifnull(SUM(bayar_piutang.besar_cicilan)+SUM(bayar_piutang.diskon_piutang)+SUM(bayar_piutang.tidak_terbayar),0) FROM bayar_piutang where bayar_piutang.no_rkm_medis=?",
                                    Kdmem.getText());
                    Sisa.setText(Valid.SetAngka(sisapiutang));
                    if ((!Cicilan.getText().isEmpty()) && (!DiskonBayar.getText().isEmpty())
                            && (!TidakTerbayar.getText().isEmpty())) {
                        Sisa.setText(Valid.SetAngka(sisapiutang - Double.parseDouble(Cicilan.getText())
                                - Double.parseDouble(DiskonBayar.getText())
                                - Double.parseDouble(TidakTerbayar.getText())));
                    }
                    Sequel.cariIsi(
                            "select reg_periksa.no_rawat from reg_periksa where reg_periksa.no_rkm_medis=? order by reg_periksa.tgl_registrasi desc limit 1",
                            NoRawat, Kdmem.getText());
                }
            }
        ...
        try {
            ps = koneksi
                    .prepareStatement("select set_akun.Diskon_Piutang,set_akun.Piutang_Tidak_Terbayar from set_akun");
            try {
                rs = ps.executeQuery();
                if (rs.next()) {
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoRawat.getText().trim().isEmpty()) {
            Valid.textKosong(NoRawat, "No.Tagihan/No.Rawat");
        } else if (Cicilan.getText().trim().isEmpty() || Cicilan.getText().
                trim().equals("0")) {
            Valid.textKosong(Cicilan, "Besar Cicilan");
        } else if (Nmmem.getText().trim().isEmpty()) {
            ...
            sisapiutang = (Sequel.cariIsiAngka(
                    "SELECT ifnull(SUM(piutang_pasien.sisapiutang),0) FROM piutang_pasien where piutang_pasien.no_rawat=?",
                    NoRawat.getText())
                    + Sequel.cariIsiAngka(
                            "SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.nota_piutang=?",
                            NoRawat.getText())
                    - Sequel.cariIsiAngka(
                            "SELECT ifnull(SUM(bayar_piutang.besar_cicilan)+SUM(bayar_piutang.diskon_piutang)+SUM(bayar_piutang.tidak_terbayar),0) FROM bayar_piutang where bayar_piutang.no_rawat=?",
                            NoRawat.getText())
                    - Double.parseDouble(Cicilan.getText()) - Double.
                    parseDouble(DiskonBayar.getText()) - Double.parseDouble(
                    TidakTerbayar.getText()));
            koderekening = "";
            try {
            ...
            if (Sequel.menyimpantf("bayar_piutang", "?,?,?,?,?,?,?,?,?,?,?",
                    "Pembayaran", 11, new String[]{
                        Valid.SetTgl(Tanggal.getSelectedItem() + ""), Kdmem.
                        getText(), Cicilan.getText(),
                        Keterangan.getText(), NoRawat.getText(), koderekening,
                        kontraakun, DiskonBayar.getText(),
                        Diskon_Piutang, TidakTerbayar.getText(),
                        Piutang_Tidak_Terbayar
                    }) == true) {
                if (sisapiutang <= 1) {
                    Sequel.mengedit("piutang_pasien", "no_rawat='" + NoRawat.
                            getText() + "'", "status='Lunas'");
                }
                Sequel.mengedit("detail_piutang_pasien", "no_rawat='" + NoRawat.
                        getText() + "' and nama_bayar='" + AkunPiutang.
                                getSelectedItem().toString() + "'",
                        "sisapiutang=sisapiutang-" + (Double.parseDouble(
                                Cicilan.getText()) + Double.parseDouble(
                        DiskonBayar.getText()) + Double.parseDouble(
                        TidakTerbayar.getText())));
                Sequel.queryu("delete from tampjurnal");
                Sequel.menyimpan("tampjurnal",
                        "'" + kontraakun + "','BAYAR PIUTANG','0','" + (Double.
                                parseDouble(Cicilan.getText()) + Double.
                        parseDouble(DiskonBayar.getText()) + Double.parseDouble(
                        TidakTerbayar.getText())) + "'", "Rekening");
                if (Double.parseDouble(Cicilan.getText()) > 0) {
                    Sequel.menyimpan("tampjurnal",
                            "'" + koderekening + "','" + AkunBayar.
                                    getSelectedItem() + "','" + Cicilan.
                                    getText() + "','0'", "Rekening");
                }
                if (Double.parseDouble(DiskonBayar.getText()) > 0) {
                    Sequel.menyimpan("tampjurnal",
                            "'" + Diskon_Piutang + "','Diskon Piutang Belum Lunas','" + DiskonBayar.
                                    getText() + "','0'", "Rekening");
                }
                if (Double.parseDouble(TidakTerbayar.getText()) > 0) {
                    Sequel.menyimpan("tampjurnal",
                            "'" + Piutang_Tidak_Terbayar + "','Kerugian/Piutang Tidak Terbayar','" + TidakTerbayar.
                                    getText() + "','0'", "Rekening");
                }
                sukses = jur.simpanJurnal(NoRawat.getText(), "U",
                        "BAYAR PIUTANG" + ", OLEH " + akses.getkode());
            } else {
                sukses = false;
            }
            ...
            if (sukses == true) {
                tabMode.addRow(new Object[]{
                    Valid.SetTgl(Tanggal.getSelectedItem() + ""), Kdmem.
                    getText(), Nmmem.getText(), Valid.
                    SetAngka(Cicilan.getText()),
                    Keterangan.getText(), NoRawat.getText(), koderekening,
                    kontraakun, Valid.SetAngka(DiskonBayar.getText()),
                    Diskon_Piutang, Valid.SetAngka(TidakTerbayar.getText()),
                    Piutang_Tidak_Terbayar
                });
                totalpiutang();
                emptTeks();
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (NoRawat.getText().trim().isEmpty()) {
            ...
            if (tbKamar.getSelectedRow() > -1) {
                ...
                if (Sequel.queryu2tf(
                        "delete from bayar_piutang where tgl_bayar=? and no_rkm_medis=? and no_rawat=? and kd_rek=? and kd_rek_kontra=?",
                        5, new String[]{
                            tbKamar.getValueAt(tbKamar.getSelectedRow(), 0).
                                    toString(), tbKamar.getValueAt(tbKamar.
                                    getSelectedRow(), 1).toString(), tbKamar.
                    ...
                    Sequel.mengedit("piutang_pasien", "no_rawat='" + tbKamar.
                            getValueAt(tbKamar.getSelectedRow(), 5).toString() + "'",
                            "status='Belum Lunas'");
                    Sequel.mengedit("detail_piutang_pasien",
                            "no_rawat='" + tbKamar.getValueAt(tbKamar.
                                    getSelectedRow(), 5).toString() + "' and nama_bayar='" + Sequel.
                            ...
                            "sisapiutang=sisapiutang+" + (Double.parseDouble(
                                    tbKamar.getValueAt(tbKamar.getSelectedRow(),
                                            3).toString()) + Double.parseDouble(
                                    tbKamar.getValueAt(tbKamar.getSelectedRow(),
                                            8).toString()) + Double.parseDouble(
                                    tbKamar.getValueAt(tbKamar.getSelectedRow(),
                                            10).toString())));
                    Sequel.queryu("delete from tampjurnal");
                    Sequel.menyimpan("tampjurnal", "'" + tbKamar.getValueAt(
                            tbKamar.getSelectedRow(), 7).toString() + "','BAYAR PIUTANG','" + (Double.
                            parseDouble(tbKamar.getValueAt(tbKamar.
                                    getSelectedRow(), 3).toString()) + Double.
                                    parseDouble(tbKamar.getValueAt(tbKamar.
                                            getSelectedRow(), 8).toString()) + Double.
                                    parseDouble(tbKamar.getValueAt(tbKamar.
                                            getSelectedRow(), 10).toString())) + "','0'",
                            "Rekening");
                    if (Double.parseDouble(tbKamar.getValueAt(tbKamar.
                            getSelectedRow(), 8).toString()) > 0) {
                        Sequel.menyimpan("tampjurnal", "'" + tbKamar.getValueAt(
                                tbKamar.getSelectedRow(), 9).toString() + "','DISKON BAYAR','0','" + tbKamar.
                    ...
                    if (Double.parseDouble(tbKamar.getValueAt(tbKamar.
                            getSelectedRow(), 10).toString()) > 0) {
                        Sequel.menyimpan("tampjurnal", "'" + tbKamar.getValueAt(
                                tbKamar.getSelectedRow(), 11).toString() + "','TIDAK TERBAYAR','0','" + tbKamar.
                                getValueAt(tbKamar.getSelectedRow(), 10).
                                toString() + "'", "Rekening");
                    }
                    if (Double.parseDouble(tbKamar.getValueAt(tbKamar.
                            getSelectedRow(), 3).toString()) > 0) {
                        Sequel.menyimpan("tampjurnal", "'" + tbKamar.getValueAt(
                                tbKamar.getSelectedRow(), 6).toString() + "','Kontra Akun','0','" + tbKamar.
                                getValueAt(tbKamar.getSelectedRow(), 3).
                                toString() + "'", "Rekening");
                    }
                    sukses = jur.simpanJurnal(NoRawat.getText(), "U",
                            "PEMBATALAN BAYAR PIUTANG" + ", OLEH " + akses.
                                    getkode());
                } else {
                    sukses = false;
                }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptBayar.jasper", "report",
                    "::[ Bayar Piutang ]::",
                    ...
                    + "bayar_piutang.tgl_bayar between '" + Valid.SetTgl(Tgl1.
                            getSelectedItem() + "") + "' and '" + Valid.SetTgl(
                            Tgl2.getSelectedItem() + "") + "' and "
                    + "(bayar_piutang.no_rawat like '%" + TCari.getText() + "%' or bayar_piutang.no_rkm_medis like '%" + TCari.
...
private void KeteranganKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KeteranganKeyPressed
    ...
    private void CicilanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_CicilanKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sisa.setText(Valid.SetAngka(sisapiutang));
            if ((!Cicilan.getText().isEmpty()) && (!DiskonBayar.getText().
                    isEmpty()) && (!TidakTerbayar.getText().isEmpty())) {
                Sisa.setText(Valid.SetAngka(sisapiutang - Double.parseDouble(
                        Cicilan.getText()) - Double.parseDouble(DiskonBayar.
                        getText()) - Double.parseDouble(TidakTerbayar.getText())));
            }
        } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            Sisa.setText(Valid.SetAngka(sisapiutang));
            if ((!Cicilan.getText().isEmpty()) && (!DiskonBayar.getText().
...
private void ppNotaPiutangBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppNotaPiutangBtnPrintActionPerformed
    ...
    if (tabMode.getRowCount() == 0) {
        JOptionPane.showMessageDialog(null,
                "Maaf, data sudah habis. Tidak ada data yang bisa anda print...!!!!");
        BtnKeluar.requestFocus();
    } else if (Cicilan.getText().isEmpty() || Cicilan.getText().equals("0")) {
        JOptionPane.showMessageDialog(null,
                "Maaf, pilih dulu data yang mau dicetak notanya...!!!!");
        tbKamar.requestFocus();
    } else if (tabMode.getRowCount() != 0) {
        if (tbKamar.getSelectedRow() > -1) {
            Sequel.queryu("delete from temporary where temp37='" + akses.
                    getalamatip() + "'");
            Sequel.menyimpan("temporary", "'0','No.Rawat',': " + tabMode.
                    getValueAt(tbKamar.getSelectedRow(), 5).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                    getalamatip() + "'", "Rekap Harian Tindakan Dokter");
            Sequel.menyimpan("temporary", "'1','Tgl.Bayar',': " + tabMode.
                    getValueAt(tbKamar.getSelectedRow(), 0).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                    getalamatip() + "'", "Rekap Harian Tindakan Dokter");
            Sequel.menyimpan("temporary", "'2','No.Rekam Medik',': " + tabMode.
                    getValueAt(tbKamar.getSelectedRow(), 1).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                    getalamatip() + "'", "Rekap Harian Tindakan Dokter");
            Sequel.menyimpan("temporary", "'3','Nama Pasien',': " + tabMode.
                    getValueAt(tbKamar.getSelectedRow(), 2).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                    getalamatip() + "'", "Rekap Harian Tindakan Dokter");
            Sequel.menyimpan("temporary", "'4','Keterangan',': " + tabMode.
                    getValueAt(tbKamar.getSelectedRow(), 4).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                    getalamatip() + "'", "Rekap Harian Tindakan Dokter");
            Sequel.menyimpan("temporary", "'5','Besar Cicilan',': " + Valid.
                    SetAngka(Double.parseDouble(tabMode.getValueAt(tbKamar.
                            getSelectedRow(), 3).toString())) + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                            getalamatip() + "'", "Rekap Harian Tindakan Dokter");

            Valid.panggilUrl("billing/LaporanPiutang.php?petugas=" + akses.
                    getkode().replaceAll(" ", "_") + "&tanggal=" + Tanggal.
                    getSelectedItem().toString().replaceAll(" ", "_") + "&alamatip=" + akses.
...
private void BtnSeekActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSeekActionPerformed
    pasien.emptTeks();
    pasien.isCek();
    pasien.setSize(internalFrame1.getWidth() - 20,
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        if (NoRawat.getText().trim().isEmpty()) {
            ...
            if (Sequel.cariInteger(
                    "select count(piutang.nota_piutang) from piutang where piutang.nota_piutang=?",
                    NoRawat.getText()) > 0) {
                kontraakun = Sequel.cariIsi(
                        "select set_akun.Piutang_Obat from set_akun");
                AkunPiutang.removeAllItems();
                AkunPiutang.addItem(Sequel.cariIsi(
                        "select rekening.nm_rek from rekening where rekening.kd_rek=?",
                        kontraakun));
                Kdmem.setText(Sequel.cariIsi(
                        "select piutang.no_rkm_medis from piutang where piutang.nota_piutang=?",
                        NoRawat.getText()));
                Nmmem.setText(Sequel.cariIsi(
                        "select piutang.nm_pasien from piutang where piutang.nota_piutang=?",
                        NoRawat.getText()));
                sisapiutang = Sequel.cariIsiAngka(
                        "SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.nota_piutang=?",
                        NoRawat.getText())
                ...
                Valid.loadCombo(AkunPiutang,
                        "select detail_piutang_pasien.nama_bayar from detail_piutang_pasien where detail_piutang_pasien.no_rawat='" + NoRawat.
                                getText() + "'");
                if (AkunPiutang.getSelectedItem() == null) {
                    JOptionPane.showMessageDialog(null,
                            "Nomor tagihan tidak ditemukan...!!");
                    NoRawat.requestFocus();
                } else {
                    Kdmem.setText(Sequel.cariIsi(
                    ...
                    Nmmem.setText(Sequel.cariIsi(
                            ...
                            Kdmem.getText()));
                    kontraakun = Sequel.cariIsi(
                            "select akun_piutang.kd_rek from akun_piutang where akun_piutang.nama_bayar=?",
                            AkunPiutang.getSelectedItem().toString());
                    sisapiutang = Sequel.cariIsiAngka(
                            "select detail_piutang_pasien.sisapiutang from detail_piutang_pasien where detail_piutang_pasien.no_rawat='" + NoRawat.
                                    getText() + "' and detail_piutang_pasien.nama_bayar='" + AkunPiutang.
                                    getSelectedItem().toString() + "'");
                    Sisa.setText(Valid.SetAngka(sisapiutang));
                    status = "pasien";
                }
    ...
    public void setData(String norawat, String norm, String nama) {
        ...
        isForm();
        kontraakun = "";
        if (Sequel.cariInteger("select count(piutang.nota_piutang) from piutang where piutang.nota_piutang=?",
                NoRawat.getText()) == 1) {
            kontraakun = Sequel.cariIsi("select set_akun.Piutang_Obat from set_akun");
            AkunPiutang.removeAllItems();
            AkunPiutang
                    .addItem(Sequel.cariIsi("select rekening.nm_rek from rekening where rekening.kd_rek=?", kontraakun));
            sisapiutang = Sequel.cariIsiAngka(
                    "SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.nota_piutang=?",
                    NoRawat.getText())
                    - Sequel.cariIsiAngka(
            ...
            Valid.loadCombo(AkunPiutang,
                    "select detail_piutang_pasien.nama_bayar from detail_piutang_pasien where detail_piutang_pasien.no_rawat='"
                    + NoRawat.getText() + "'");
            kontraakun = Sequel.cariIsi("select akun_piutang.kd_rek from akun_piutang where akun_piutang.nama_bayar=?",
                    AkunPiutang.getSelectedItem().toString());
            sisapiutang = Sequel.cariIsiAngka(
                    "select detail_piutang_pasien.sisapiutang from detail_piutang_pasien where detail_piutang_pasien.no_rawat='"
                    + NoRawat.getText() + "' and nama_bayar='" + AkunPiutang.getSelectedItem().toString()
                    + "'");
            Sisa.setText(Valid.SetAngka(sisapiutang));
            status = "pasien";
        }
    ...
    private static final Logger LOG = Logger.getLogger(DlgBayarPiutang.class.getName());
