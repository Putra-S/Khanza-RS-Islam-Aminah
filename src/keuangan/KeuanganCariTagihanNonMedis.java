...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptTagihanNonMedis.jasper", "report",
                    "::[ Data Titip Faktur/Tagihan Barang Non Medis dan Penunjang ( Lab & RO ) ]::",
            ...
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                try {
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            Sequel.queryu(
                                    "update ipsrspemesanan set status='Belum Lunas' where no_faktur=?",
                    ...
                    Sequel.queryu(
                            "delete from ipsrs_titip_faktur where no_tagihan=?",
    ...
    private void MnDitagihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDitagihkanActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                Sequel.queryu(
                        "update ipsrs_titip_faktur set status='Ditagihkan' where no_tagihan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString().trim());
                tampil();
            }
    ...
    private void MnDibayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDibayarActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                if (Sequel.cariIsi(
                        "select status from ipsrs_titip_faktur where no_tagihan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString()).equals("Ditagihkan")) {
                    this.setCursor(Cursor.
                            getPredefinedCursor(Cursor.WAIT_CURSOR));
                    KeuanganHutangNonMedisBelumLunas form = new KeuanganHutangNonMedisBelumLunas(
                            null, false);
                    form.isCek();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganCariTagihanNonMedis dialog = new KeuanganCariTagihanNonMedis(
    ...
    private void tampil() {
        ...
        try {
            ...
            if (!NoTagihan.getText().trim().isEmpty()) {
                notagihan = " and ipsrs_titip_faktur.no_tagihan like '%" + NoTagihan.
                        getText() + "%' ";
            }

            tanggal = " ipsrs_titip_faktur.tanggal between '" + Valid.SetTgl(
                    Tanggal1.getSelectedItem() + "") + "' and '" + Valid.SetTgl(
                    Tanggal2.getSelectedItem() + "") + "' ";

            if (!Status.getSelectedItem().toString().equals("Semua")) {
                status = " and ipsrs_titip_faktur.status like '%" + Status.
                        getSelectedItem().toString() + "%' ";
            }

            if (!NmPeg.getText().trim().isEmpty()) {
                petugas = " and concat(ipsrs_titip_faktur.nip,petugas.nama) like '%" + KdPeg.
                        getText() + NmPeg.getText() + "%' ";
            }

            if (!TCari.getText().trim().isEmpty()) {
                cari = " and (ipsrs_titip_faktur.no_tagihan like '%" + TCari.
                        getText() + "%' or petugas.nama like '%" + TCari.
                                getText() + "%' or ipsrs_titip_faktur.keterangan like '%" + TCari.
                                getText() + "%' or ipsrs_detail_titip_faktur.no_faktur like '%" + TCari.
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    ps2 = koneksi.prepareStatement(
                            ...
                            + "inner join ipsrspemesanan on ipsrs_detail_titip_faktur.no_faktur=ipsrspemesanan.no_faktur "
                            + "inner join ipsrssuplier on ipsrssuplier.kode_suplier=ipsrspemesanan.kode_suplier "
                            + "where ipsrs_detail_titip_faktur.no_tagihan='" + rs.
    ...
    private static final Logger LOG = Logger.getLogger(
            KeuanganCariTagihanNonMedis.class.getName());
}
