 ...
 * @author dosen
 */
public class KeuanganBayarPemesananFarmasi extends javax.swing.JDialog {
            ...
            try {
                if (sisahutang > 0) {
                    ...
                    sukses = jur.simpanJurnal(no_bukti.getText(), "U",
                            "BAYAR PELUNASAN HUTANG OBAT/BHP/ALKES NO.FAKTUR " + no_faktur.
                    ...
                    if (sukses == true) {
                        if ((sisahutang <= Double.parseDouble(besar_bayar.
                            ...
                            Sequel.mengedit("pemesanan", "no_faktur=?",
                                    "status='Sudah Dibayar'", 1, new String[]{
                                        no_faktur.getText()});
                        } else {
                            Sequel.mengedit("pemesanan", "no_faktur=?",
                                    "status='Belum Lunas'", 1, new String[]{
                                        no_faktur.getText()});
                        }
                        if (Sequel.menyimpantf2("bayar_pemesanan",
                                "?,?,?,?,?,?,?", "Data", 7, new String[]{
                                    Valid.SetTgl(
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        try {
            ...
            sukses = true;
            if (Sequel.queryu2tf(
                    "delete from bayar_pemesanan where tgl_bayar=? and no_faktur=? and "
                ...
                Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                        new String[]{
                            Bayar_Pemesanan_Obat, "HUTANG USAHA", "0",
                            besar_bayar.getText()
                        });
                sukses = jur.simpanJurnal(no_bukti.getText(), "U",
                        "BATAL BAYAR PELUNASAN HUTANG OBAT/BHP/ALKES NO.FAKTUR " + no_faktur.
                                getText() + ", OLEH " + akses.getkode());
            } else {
                sukses = false;
            }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptBayarPemesanan.jasper", "report",
                    "::[ Bayar Pemesanan ]::",
                    ...
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
                    getText().trim() + "%' and bayar_pemesanan.no_faktur like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
                    getText().trim() + "%' and datasuplier.nama_suplier like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
                    getText().trim() + "%' and bayar_pemesanan.nama_bayar like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
                    getText().trim() + "%' and bayar_pemesanan.no_bukti like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
                    getText().trim() + "%' and bayar_pemesanan.keterangan like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bayar_pemesanan.tgl_bayar between '" + Valid.SetTgl(
                            TglCari1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglCari2.getSelectedItem() + "") + "' and datasuplier.nama_suplier like '%" + nmsup.
...
private void BtnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPetugasActionPerformed
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("select bayar_pemesanan.tgl_bayar,pemesanan.tgl_faktur,pemesanan.tgl_pesan,"
                    + "pemesanan.tgl_tempo, bayar_pemesanan.no_faktur,"
                    + "datasuplier.nama_suplier,bayar_pemesanan.nama_bayar,bayar_pemesanan.no_bukti,"
                    + "bayar_pemesanan.besar_bayar,bayar_pemesanan.keterangan,"
                    + "bayar_pemesanan.nip,petugas.nama from bayar_pemesanan inner join petugas "
    ...
    public void setData(String nofaktur) {
        ...
        sisahutang = Math.round(Sequel
                .cariIsiAngka("SELECT pemesanan.tagihan FROM pemesanan where pemesanan.no_faktur=?", nofaktur)
                - Sequel.cariIsiAngka(
                        "SELECT ifnull(SUM(bayar_pemesanan.besar_bayar),0) FROM bayar_pemesanan where bayar_pemesanan.no_faktur=?",
                        nofaktur));
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganBayarPemesananFarmasi.class.getName());
