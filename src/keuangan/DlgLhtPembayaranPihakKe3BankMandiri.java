 ...
 * @author perpustakaan
 */
public class DlgLhtPembayaranPihakKe3BankMandiri extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public DlgLhtPembayaranPihakKe3BankMandiri(java.awt.Frame parent,
            boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        setSize(885, 674);
        Object[] rowRwJlDr = {
            "No.Pembayaran", "Tgl.Pembayaran", "No.Rekening Sumber",
            "No.Rekening Tujuan", "Nama Rekening Tujuan", "Kota/Kabupaten",
            "Nominal", "Nomor Tagihan", "Metode Pembayaran", "Bank Tujuan",
            "Kode Transaksi", "Asal Transaksi", "Status Transaksi"
        };
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPembayaranPihakke3BankMandiri.jasper",
                    "report", "::[ Data Pembayaran Pihak Ke 3 Bank Mandiri ]::",
                    ...
                    + "pembayaran_pihak_ke3_bankmandiri.tgl_pembayaran between '" + Valid.
                            SetTgl(Tgl1.getSelectedItem() + "") + " 00:00:01' and '" + Valid.
                    SetTgl(Tgl2.getSelectedItem() + "") + " 23:59:59' "
                    + (TCari.getText().isEmpty() ? "" : "and (pembayaran_pihak_ke3_bankmandiri.nomor_pembayaran like '%" + TCari.
                    getText().trim() + "%' or pembayaran_pihak_ke3_bankmandiri.no_rekening_tujuan like '%" + TCari.
                            getText().trim() + "%' "
                    + "or pembayaran_pihak_ke3_bankmandiri.atas_nama_rekening_tujuan like '%" + TCari.
                            getText().trim() + "%' or pembayaran_pihak_ke3_bankmandiri.status_transaksi like '%" + TCari.
                            getText().trim() + "%' "
                    + "or pembayaran_pihak_ke3_bankmandiri.nomor_tagihan like '%" + TCari.
                            getText().trim() + "%' "
                    + "or metode_pembayaran_bankmandiri.nama_metode like '%" + TCari.
                            getText().trim() + "%' or pembayaran_pihak_ke3_bankmandiri.kode_transaksi like '%" + TCari.
                            getText().trim() + "%' or "
                    + "bank_tujuan_transfer_bankmandiri.nama_bank like '%" + TCari.
                            getText().trim() + "%' or pembayaran_pihak_ke3_bankmandiri.asal_transaksi like '%" + TCari.
                            getText().trim() + "%') ")
                    + "order by pembayaran_pihak_ke3_bankmandiri.tgl_pembayaran",
                    param);
        }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void ppMT940ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppMT940ActionPerformed
        ...
        if (jfc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                json = "{" + data.toString().replaceAll(";:86:",
                        "\",\"referensi\":\"").replaceAll(";:61:",
                                "\"},{\"transaksi\":\"").replaceAll(":61:",
                                "\"transaksi\":\"") + "}";
                ...
                if (!json.isEmpty()) {
                    ...
                    if (root.path("data").isArray()) {
                        for (JsonNode list : root.path("data")) {
                            if (list.path("transaksi").asText().substring(6, 7).
                                    equals("D")) {
                                System.out.println(
                                        "Melakukan pemrosesan \"Terkonfirmasi\" transaksi " + list.
                                                path("referensi").asText());
                                Sequel.mengedit3(
                                        "pembayaran_pihak_ke3_bankmandiri",
                                        "nomor_pembayaran=?",
                                        "status_transaksi='Terkonfirmasi'", 1,
                                        new String[]{
                                            list.path("referensi").asText()
                                        });
                            } else if (list.path("transaksi").asText().
                                    substring(6, 7).equals("R")) {
                                if (list.path("transaksi").asText().substring(7,
                                        8).equals("D")) {
                                    System.out.println(
                                            "Melakukan pemrosesan \"Pembayaran Gagal\" transaksi " + list.
                                                    path("referensi").asText());
                                    Sequel.mengedit3(
                                            "pembayaran_pihak_ke3_bankmandiri",
                                            "nomor_pembayaran=?",
                                            "status_transaksi='Pembayaran Gagal'",
                                            1, new String[]{
                                                list.path("referensi").asText()
                                            });
                                }
                    ...
                    JOptionPane.showMessageDialog(null,
                            "Proses eksekusi file MT940 sudah selesai, silahkan cek kembali daftar transaksi..");
                }
            } catch (Exception e) {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgLhtPembayaranPihakKe3BankMandiri dialog = new DlgLhtPembayaranPihakKe3BankMandiri(
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + (TCari.getText().isEmpty() ? "" : "and (pembayaran_pihak_ke3_bankmandiri.nomor_pembayaran like ? or pembayaran_pihak_ke3_bankmandiri.no_rekening_tujuan like ? or "
            ...
            try {
                ...
                total = 0;
                while (rs.next()) {
                    total += rs.getDouble("nominal_pembayaran");
                    tabMode.addRow(new Object[]{
                        rs.getString("nomor_pembayaran"), rs.getString(
                        "tgl_pembayaran"), rs.getString("no_rekening_sumber"),
                        rs.getString("no_rekening_tujuan"), rs.getString(
                        "atas_nama_rekening_tujuan"), rs.getString(
                        "kota_atas_nama_rekening_tujuan"),
                        Valid.SetAngka(rs.getDouble("nominal_pembayaran")), rs.
                        getString("nomor_tagihan"), rs.getString("nama_metode"),
                        rs.getString("nama_bank"), rs.
                        getString("kode_transaksi"), rs.getString(
                        "asal_transaksi"), rs.getString("status_transaksi")
                    });
                }
                if (total > 0) {
                    tabMode.addRow(new Object[]{
    ...
    private static final Logger LOG = Logger.getLogger(
            DlgLhtPembayaranPihakKe3BankMandiri.class.getName());
}
