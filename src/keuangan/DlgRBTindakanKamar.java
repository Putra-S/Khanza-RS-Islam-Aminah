...
public class DlgRBTindakanKamar extends javax.swing.JDialog {
    ...
    private PreparedStatement psbangsal, pspasien, psobat, psobatlangsung, psperiksalab, psdetailperiksalab, psoperasi,
            psranapdr, psranapdrpr, psranappr, pstambahan, pspotongan, psregistrasi, psradiologi, pskamar;
    ...
    private double obat = 0, obatlangsung = 0, laborat = 0, operasi = 0, radiologi = 0, jm = 0, jm2 = 0, jm3 = 0,
            ttlbiaya = 0, detailobat = 0, detailobatlangsung = 0, ttlobat = 0, ttlobatlangsung = 0, ttllaborat = 0,
            ttljm = 0, detaillaborat = 0, ttldetaillaborat = 0, tambahan, potongan, detailtambahan, detailpotongan,
            registrasi = 0, detailregistrasi, ttlpotongan = 0, ttltambahan = 0, ttlregistrasi = 0, detailradiologi = 0,
            ttlradiologi = 0, kamar = 0, ttlkamar = 0;
    ...
    public DlgRBTindakanKamar(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No.", "Bangsal", "Tgl.Masuk", "Tgl.Keluar", "Cara Bayar", "Obt+Emb+Tsl", "Laborat",
            "Paket Tindakan", "Tambahan", "Potongan", "Registrasi", "Radiologi", "Kamar", "Total"};
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
        ...
        try {
            ...
            try {
                ...
                while (rsbangsal.next()) {
                    ...
                    try {
                        ...
                        while (rspasien.next()) {
                            obat = 0;
                            psobat = koneksi
                                    .prepareStatement("select sum(total) from detail_pemberian_obat where no_rawat=?");
                            try {
                            ...
                            laborat = 0;
                            psperiksalab = koneksi
                                    .prepareStatement("select sum(biaya) from periksa_lab where no_rawat=?");
                            try {
                            ...
                            detaillaborat = 0;
                            psdetailperiksalab = koneksi
                                    .prepareStatement("select sum(biaya_item) from detail_periksa_lab where no_rawat=?");
                            try {
                            ...
                            jm = 0;
                            psranapdr = koneksi
                                    .prepareStatement("select sum(biaya_rawat) from rawat_inap_dr where no_rawat=?");
                            try {
                            ...
                            jm2 = 0;
                            psranapdrpr = koneksi
                                    .prepareStatement("select sum(biaya_rawat) from rawat_inap_drpr where no_rawat=?");
                            try {
                            ...
                            jm3 = 0;
                            psranappr = koneksi
                                    .prepareStatement("select sum(biaya_rawat) from rawat_inap_pr where no_rawat=?");
                            try {
                            ...
                            operasi = 0;
                            psoperasi = koneksi
                                    .prepareStatement("select sum(biayaoperator1+biayaoperator2+biayaoperator3+"
                            ...
                            tambahan = 0;
                            pstambahan = koneksi
                                    .prepareStatement("select sum(besar_biaya) from tambahan_biaya where no_rawat=?");
                            try {
                            ...
                            registrasi = 0;
                            psregistrasi = koneksi
                                    .prepareStatement("select sum(biaya_reg) from reg_periksa where no_rawat=?");
                            try {
                            ...
                            radiologi = 0;
                            psradiologi = koneksi
                                    .prepareStatement("select sum(biaya) from periksa_radiologi where no_rawat=?");
                            try {
                            ...
                            kamar = 0;
                            pskamar = koneksi
                                    .prepareStatement("select sum(ttl_biaya) from kamar_inap where no_rawat=?");
                            try {
                            ...
                            ttlbiaya = ttlbiaya + obat + obatlangsung + laborat + detaillaborat + jm + jm2 + jm3
                                    + operasi + tambahan - potongan + registrasi + radiologi + kamar;
                            tabMode.addRow(new Object[]{"",
                                ...
                                Valid.SetAngka(obat + obatlangsung + laborat + detaillaborat + jm + jm2 + jm3
                                + operasi + tambahan - potongan + registrasi + radiologi + kamar)});
                            a++;
                        }
                    } catch (Exception e) {
                ...
                tabMode.addRow(new Object[]{">>", "Total :", "", "", "", Valid.SetAngka(ttlobat + ttlobatlangsung),
                    Valid.SetAngka(ttllaborat + ttldetaillaborat), Valid.SetAngka(ttljm),
                    Valid.SetAngka(ttltambahan), Valid.SetAngka(ttlpotongan), Valid.SetAngka(ttlregistrasi),
                    Valid.SetAngka(ttlradiologi), Valid.SetAngka(ttlkamar), Valid.SetAngka(ttlbiaya)});
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgRBTindakanKamar.class.getName());
