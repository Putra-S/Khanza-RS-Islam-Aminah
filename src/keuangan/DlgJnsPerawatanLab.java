 ...
 * @author dosen
 */
public class DlgJnsPerawatanLab extends javax.swing.JDialog {
    ...
    public DlgJnsPerawatanLab(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"P", "Kode Periksa", "Nama Pemeriksaan", "J.S. RS", "Paket BHP", "J.M. Perujuk", "J.M. Dokter",
            "J.M. Petugas", "K.S.O.", "Menejemen", "Total Tarif", "Jenis Bayar", "Kelas", "Kategori"};
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (BagianRs.getText().trim().isEmpty()) {
            Valid.textKosong(BagianRs, "J.S.Rumah Sakit");
        } else if (Bhp.getText().trim().isEmpty()) {
            Valid.textKosong(Bhp, "BHP");
        } else if (JMDokter.getText().trim().isEmpty()) {
            Valid.textKosong(JMDokter, "J.M. Dokter");
        } else if (JMLaborat.getText().trim().isEmpty()) {
            Valid.textKosong(JMLaborat, "J.M. Petugas");
        } else if (JMPerujuk.getText().trim().isEmpty()) {
            Valid.textKosong(JMPerujuk, "J.M. Perujuk");
        } else if (KSO.getText().trim().isEmpty()) {
            Valid.textKosong(KSO, "K.S.O");
        } else if (Menejemen.getText().trim().isEmpty()) {
            Valid.textKosong(Menejemen, "Menejemen");
        } else {
            if (Sequel.menyimpantf("jns_perawatan_lab",
                    "?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Kode Periksa", 14,
                    new String[]{
                        TKd.getText(), TNm.getText(), BagianRs.getText(), Bhp.
                        getText(), JMPerujuk.getText(), JMDokter.getText(),
                        JMLaborat.getText(), KSO.getText(), Menejemen.getText(),
                        TotalBiaya.getText(), kdpnj.getText(), "1",
                        Kelas.getSelectedItem().toString(), Kategori.
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        for (i = 0; i < tbJnsPerawatan.getRowCount(); i++) {
            if (tbJnsPerawatan.getValueAt(i, 0).toString().equals("true")) {
                Sequel.mengedit("jns_perawatan_lab",
                        "kd_jenis_prw='" + tbJnsPerawatan.getValueAt(i, 1).
                                toString() + "'", "status='0'");
            }
        }
        BtnCariActionPerformed(evt);
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (TKd.getText().trim().isEmpty()) {
            ...
            Sequel.mengedit("jns_perawatan_lab", "kd_jenis_prw=?",
                    "kd_jenis_prw=?,nm_perawatan=?,bagian_rs=?,tarif_tindakan_petugas=?,total_byr=?,kd_pj=?,tarif_tindakan_dokter=?,tarif_perujuk=?,bhp=?,kso=?,menejemen=?,kelas=?,kategori=?",
                    14, new String[]{
                        TKd.getText(), TNm.getText(), BagianRs.getText(),
                        JMLaborat.getText(), TotalBiaya.getText(), kdpnj.
                        getText(), JMDokter.getText(), JMPerujuk.getText(), Bhp.
                        getText(),
                        KSO.getText(), Menejemen.getText(), Kelas.
                        getSelectedItem().toString(),
                        Kategori.getSelectedItem().toString(), tbJnsPerawatan.
                        getValueAt(tbJnsPerawatan.getSelectedRow(), 1).
                        toString()
                    });
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptTarifLab.jasper", "report",
                    "::[ Data Tarif Laboratorium ]::",
                    ...
                    + " jns_perawatan_lab.status='1' and (jns_perawatan_lab.kd_jenis_prw like '%" + TCari.
                            getText().trim() + "%' or "
                    + " jns_perawatan_lab.nm_perawatan like '%" + TCari.
                            getText().trim() + "%' or "
                    + " jns_perawatan_lab.kelas like '%" + TCari.getText().
                            trim() + "%' or "
                    + " penjab.png_jawab like '%" + TCari.getText().trim() + "%' or "
                    + " jns_perawatan_lab.kategori like '%" + TCari.getText().
                            trim() + "%') "
...
private void btnPjActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPjActionPerformed
    ...
    private void ppTemplateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppTemplateActionPerformed
        if (TNm.getText().trim().isEmpty()) {
            Valid.textKosong(TNm, "Nama Pemeriksaan");
        } else {
            if (tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(), 13).
                    toString().equals("PK") || tbJnsPerawatan.getValueAt(
                    tbJnsPerawatan.getSelectedRow(), 13).toString().equals("MB")) {
                template.KdPeriksa.setText(tbJnsPerawatan.getValueAt(
                        tbJnsPerawatan.getSelectedRow(), 1).toString());
                template.NmPeriksa.setText(tbJnsPerawatan.getValueAt(
                        tbJnsPerawatan.getSelectedRow(), 2).toString());
                ...
                JOptionPane.showMessageDialog(null,
                        "Template Laboratorium hanya untuk kategori PK dan MB");
            }
        }
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString(1), rs.getString(2), rs.getDouble(3),
                        ...
                        rs.getDouble(9), rs.getDouble(10), rs.getString(11), rs.getString(12), rs.getString(13)});
                }
            } catch (Exception e) {
    ...
    public void emptTeks() {
        ...
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(kd_jenis_prw,5),signed)),0) from jns_perawatan_lab", "J", 6,
                TKd);
        TKd.requestFocus();
    }
    ...
    private void getData() {
        if (tbJnsPerawatan.getSelectedRow() != -1) {
            ...
            Sequel.cariIsi("select kd_pj from jns_perawatan_lab where kd_jenis_prw=?", kdpnj,
    ...
    private void isjml() {
        if ((!BagianRs.getText().isEmpty()) && (!JMLaborat.getText().isEmpty()) && (!JMDokter.getText().isEmpty())
                && (!JMPerujuk.getText().isEmpty()) && (!Bhp.getText().isEmpty()) && (!KSO.getText().isEmpty())
                && (!Menejemen.getText().isEmpty())) {
            TotalBiaya.setText(Valid.SetAngka2(Double.parseDouble(BagianRs.getText().trim())
                    + Double.parseDouble(JMDokter.getText().trim()) + Double.parseDouble(JMLaborat.getText().trim())
                    + Double.parseDouble(JMPerujuk.getText().trim()) + Double.parseDouble(Bhp.getText().trim())
                    + Double.parseDouble(KSO.getText().trim()) + Double.parseDouble(Menejemen.getText().trim())));
        }
    }
    ...
    private static final Logger LOG = Logger.getLogger(DlgJnsPerawatanLab.class.getName());
