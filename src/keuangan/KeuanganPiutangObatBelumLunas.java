...
public class KeuanganPiutangObatBelumLunas extends javax.swing.JDialog {
    ...
    public KeuanganPiutangObatBelumLunas(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"P", "No.Nota", "Tgl.Piutang", "Pasien", "Catatan", "Total Piutang", "Uang Muka",
                    "Ogkos Kirim", "Cicilan+Disk+T.Terbayar", "Sisa Piutang", "Jatuh Tempo", "Bayar",
...
private void MnDetailPiutangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDetailPiutangActionPerformed
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    : "where piutang.nota_piutang like ? or petugas.nama like ? or "
                    + "piutang.no_rkm_medis like ? or piutang.nm_pasien like ?")
                    + " having piutang.sisapiutang-cicilan>0 order by piutang.tgl_piutang");
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("nota_piutang"), rs.getString("tgl_piutang"),
                        rs.getString("no_rkm_medis") + " " + rs.getString("nm_pasien"), rs.getString("catatan"),
                        (rs.getDouble("uangmuka") + rs.getDouble("sisapiutang") - rs.getDouble("ongkir")),
                        rs.getDouble("uangmuka"), rs.getDouble("ongkir"), rs.getDouble("cicilan"),
                        (rs.getDouble("sisapiutang") - rs.getDouble("cicilan")), rs.getString("tgltempo"),
                        (rs.getDouble("sisapiutang") - rs.getDouble("cicilan")), 0, 0});
            ...
            if (tbBangsal.getSelectedRow() != -1) {
                if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 0).toString().equals("true")) {
                    if (Double.parseDouble(tabMode.getValueAt(tbBangsal.getSelectedRow(), 9).toString()) < 0) {
                        ...
                        tbBangsal.setValueAt((Double
                                .parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 5).toString())
                                ...
                                + Double
                                        .parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 8).toString())
                                + Double.parseDouble(
                                        tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 11).toString())
                                + Double.parseDouble(
                                        tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 12).toString())
                                + Double.parseDouble(
                                        tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 13).toString())),
                                tbBangsal.getSelectedRow(), 9);
                    }
                } else if (tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 0).toString().equals("false")) {
                    tbBangsal
                            .setValueAt((Double.parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 5).toString())
                                    + Double.parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 7).toString()))
                                    - (Double.parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 6).toString())
                                    + Double.parseDouble(
                                            tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 8).toString())),
                                    tbBangsal.getSelectedRow(), 9);
                    tbBangsal
                            .setValueAt((Double.parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 5).toString())
                                    + Double.parseDouble(tbBangsal.getValueAt(tbBangsal.getSelectedRow(), 7).toString()))
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganPiutangObatBelumLunas.class.getName());
