 ...
 * @author perpustakaan
 */
public class DlgPembayaranPerAKunBayar2 extends javax.swing.JDialog {
    ...
    private String tanggal2 = "", nopemasukanlain = "", nonota = "", petugas = "", norawatjalan = "", norawatinap = "",
            notajual = "", nodeposit = "";
    ...
    private void tampil() {
        ...
        try {
            ...
            htmlContent.append(
                    "<tr class='head'>" + "<td valign='middle' bgcolor='#FFFAFA' align='center' width='27px'>No.</td>"
            ...
            kolom = 0;
            akunbayar = new String[Sequel.cariInteger(
                    "select count(kd_rek) from rekening where kd_rek in (select kd_rek from akun_bayar group by kd_rek)")];
            psakunbayar = koneksi.prepareStatement(
                    "select kd_rek,nm_rek from rekening where kd_rek in (select kd_rek from akun_bayar group by kd_rek) order by nm_rek");
            try {
                rsakunbayar = psakunbayar.executeQuery();
                while (rsakunbayar.next()) {
                    akunbayar[kolom] = rsakunbayar.getString("kd_rek");
                    kolom++;
                    htmlContent.append("<td valign='middle' bgcolor='#FFFAFA' align='center' width='130px'>")
                            .append(rsakunbayar.getString("nm_rek"))
                            .append("</td>");
                }
            } catch (Exception e) {
            ...
            all = 0;
            ps = koneksi
                    .prepareStatement("select no_nota,tgl_bayar,nama_pasien,jumlah_bayar,petugas from tagihan_sadewa "
            ...
            try {
                ...
                no = 1;
                while (rs.next()) {
                    petugas = rs.getString("petugas") + " " + Sequel
                            .cariIsi("select pegawai.nama from pegawai where pegawai.nik=?", rs.getString("petugas"));
                    norawatinap = "";
                    norawatjalan = "";
                    notajual = "";
                    nopemasukanlain = "";
                    ...
                    } else if (nonota.isEmpty()) {
                        nonota = Sequel.cariIsi("select no_nota from nota_jalan where no_rawat=?",
                                rs.getString("no_nota"));
                        if (!nonota.isEmpty()) {
                            norawatjalan = rs.getString("no_nota");
                        } else if (nonota.isEmpty()) {
                            nonota = Sequel.cariIsi("select nota_jual from penjualan where nota_jual=?",
                                    rs.getString("no_nota"));
                            if (!nonota.isEmpty()) {
                                notajual = rs.getString("no_nota");
                            } else if (nonota.isEmpty()) {
                                nonota = Sequel.cariIsi("select no_deposit from deposit where no_deposit=?",
                                        rs.getString("no_nota"));
                                if (!nonota.isEmpty()) {
                                    nodeposit = rs.getString("no_nota");
                                } else {
                                    nonota = Sequel.cariIsi("select no_masuk from pemasukan_lain where no_masuk=?",
                                            rs.getString("no_nota"));
                                    if (!nonota.isEmpty()) {
                    ...
                    if ((petugas.toLowerCase().trim().contains(User.getText().toLowerCase().trim()))
                            ...
                            || nonota.toLowerCase().trim().contains(TCari.getText().toLowerCase().trim()))) {
                        ...
                        htmlContent.append("<tr class='isi'><td valign='middle' align='center'>")
                                ...
                                .append(rs.getString("tgl_bayar"))
                                .append("</td><td valign='middle' align='center'>")
                                .append(nonota)
                                .append("</td><td valign='middle' align='left'>")
                                .append(rs.getString("nama_pasien"))
                                .append("</td><td valign='middle' align='right'>")
                                .append(Valid.SetAngka(rs.getDouble("jumlah_bayar")))
                                .append("</td><td valign='middle' align='left'>")
                                .append(petugas)
                                .append("</td>");
                        for (i = 0; i < kolom; i++) {
                            bayar = 0;
                            if (!norawatinap.isEmpty()) {
                                bayar = Sequel.cariIsiAngka(
                                        "select detail_nota_inap.besar_bayar from detail_nota_inap inner join akun_bayar on detail_nota_inap.nama_bayar=akun_bayar.nama_bayar where detail_nota_inap.no_rawat='"
                                        + norawatinap + "' and akun_bayar.kd_rek='" + akunbayar[i] + "'");
                                htmlContent.append("<td valign='middle' align='right'>")
                                        .append(Valid.SetAngka(bayar))
                                        .append("</td>");
                            } else if (!norawatjalan.isEmpty()) {
                                bayar = Sequel.cariIsiAngka(
                                        "select detail_nota_jalan.besar_bayar from detail_nota_jalan inner join akun_bayar on detail_nota_jalan.nama_bayar=akun_bayar.nama_bayar where detail_nota_jalan.no_rawat='"
                                        + norawatjalan + "' and akun_bayar.kd_rek='" + akunbayar[i] + "'");
                                htmlContent.append("<td valign='middle' align='right'>")
                                        .append(Valid.SetAngka(bayar))
                                        .append("</td>");
                            } else if (!notajual.isEmpty()) {
                                bayar = Sequel.cariIsiAngka(
                                        "select (sum(detailjual.total)+penjualan.ongkir+penjualan.ppn) from detailjual inner join penjualan on penjualan.nota_jual=detailjual.nota_jual inner join akun_bayar on penjualan.nama_bayar=akun_bayar.nama_bayar where penjualan.nota_jual='"
                                        + notajual + "' and akun_bayar.kd_rek='" + akunbayar[i] + "'");
                                htmlContent.append("<td valign='middle' align='right'>")
                                        .append(Valid.SetAngka(bayar))
                                        .append("</td>");
                            } else if (!nodeposit.isEmpty()) {
                                bayar = Sequel.cariIsiAngka(
                                        "select sum(deposit.besar_deposit) from deposit inner join akun_bayar on deposit.nama_bayar=akun_bayar.nama_bayar where deposit.no_deposit='"
                                        + nodeposit + "' and akun_bayar.kd_rek='" + akunbayar[i] + "'");
                                htmlContent.append("<td valign='middle' align='right'>")
                                        .append(Valid.SetAngka(bayar))
                                        .append("</td>");
                            } else if (!nopemasukanlain.isEmpty()) {
                                bayar = Sequel.cariIsiAngka(
                                        "select sum(pemasukan_lain.besar) from pemasukan_lain inner join kategori_pemasukan_lain on kategori_pemasukan_lain.kode_kategori=pemasukan_lain.kode_kategori where pemasukan_lain.no_masuk='"
                                        + nopemasukanlain + "' and kategori_pemasukan_lain.kd_rek2='"
                                        + akunbayar[i] + "'");
                                htmlContent.append("<td valign='middle' align='right'>")
                                        .append(Valid.SetAngka(bayar))
                                        .append("</td>");
            ...
            htmlContent.append(
                    "<tr class='isi'><td valign='middle' align='center'></td><td valign='middle' align='right'>Total :</td><td valign='middle' align='center'></td><td valign='middle' align='left'></td><td valign='middle' align='right'>")
                    .append(Valid.SetAngka(all))
                    .append("</td><td valign='middle' align='left'></td>");
            for (i = 0; i < kolom; i++) {
                htmlContent.append("<td valign='middle' align='right'>")
                        .append(Valid.SetAngka(totalbayar[i]))
                        .append("</td>");
            }
            htmlContent.append("</tr>");
            if (kolom == 0) {
                ...
                LoadHTML.setText("<html>" + "<table width='" + Integer.toString(700 + (kolom * 90))
    ...
    private void tampil2() {
        ...
        try {
            ...
            htmlContent.append(
                    "<tr class='isi'><td valign='middle' align='center'></td><td valign='middle' align='right'>Total :</td><td valign='middle' align='center'></td><td valign='middle' align='left'></td><td valign='middle' align='right'>")
                    .append(Math.round(all))
                    .append("</td><td valign='middle' align='left'></td>");
            for (i = 0; i < kolom; i++) {
                htmlContent.append("<td valign='middle' align='right'>")
                        .append(Math.round(totalbayar[i]))
                        .append("</td>");
            }
            htmlContent.append("</tr>");
            if (kolom == 0) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgPembayaranPerAKunBayar2.class.getName());
