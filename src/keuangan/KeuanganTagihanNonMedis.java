 ...
 * @author perpustakaan
 */
public class KeuanganTagihanNonMedis extends javax.swing.JDialog {
    ...
    private void tbBangsalMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbBangsalMouseClicked
        if (tabMode.getRowCount() != 0) {
            if (kdsup.getText().trim().isEmpty() || nmsup.getText().trim().
                    isEmpty()) {
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if (NoTagihan.getText().trim().isEmpty()) {
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("ipsrs_titip_faktur",
                        "?,?,?,?,'Ditagihkan'", "No.Tagihan", 4, new String[]{
                            NoTagihan.getText(), Valid.SetTgl(Tanggal.
                            getSelectedItem() + ""), nip.getText(), Keterangan.
                            getText()
                        }) == true) {
                    row = tbBangsal.getRowCount();
                    for (i = 0; i < row; i++) {
                        if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                            if (Sequel.menyimpantf2("ipsrs_detail_titip_faktur",
                                    "?,?", "Detail Tagihan", 2, new String[]{
                                        NoTagihan.getText(), tbBangsal.
                                        getValueAt(i, 1).toString(),}) == false) {
                                sukses = false;
                            } else {
                                Sequel.mengedit("ipsrspemesanan", "no_faktur=?",
                                        "status='Titip Faktur'", 1,
                                        new String[]{tbBangsal.getValueAt(i, 1).
                                                    toString()});
                            }
                        }
                    }
                    ...
                    JOptionPane.showMessageDialog(rootPane,
                            "Gagal Menyimpan, kemungkinan No.Tagihan sudah ada sebelumnya...!!");
                }
                if (sukses == true) {
                    autoNomor();
                    Sequel.Commit();
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        KeuanganCariTagihanNonMedis form = new KeuanganCariTagihanNonMedis(null,
                false);
        form.emptTeks();
        form.isCek();
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(no_tagihan,3),signed)),0) from ipsrs_titip_faktur where tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "TN" + Tanggal.getSelectedItem().toString().substring(6, 10)
    ...
    private static final Logger LOG = Logger.getLogger(KeuanganTagihanNonMedis.class.getName());
