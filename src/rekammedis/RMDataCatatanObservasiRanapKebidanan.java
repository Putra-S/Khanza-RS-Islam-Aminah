 ...
 * @author perpustakaan
 */
public class RMDataCatatanObservasiRanapKebidanan extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public RMDataCatatanObservasiRanapKebidanan(java.awt.Frame parent,
            boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "HR(x/menit)", "RR(x/menit)", "Suhu(°C)", "SpO2(%)", "Kontraksi/HIS",
            "BJJ", "PPV", "VT", "NIP", "Nama Petugas"
        }) {
            @Override
  ...
  private void initComponents() {
    ...
    Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "27-11-2023" }));
    ...
    BtnDokter5.setToolTipText("Ambil TTV SOAP");
    ...
    BtnDokter5.setBounds(800, 70, 28, 23);
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.
                        MyReportqry(
                                "rptDataCatatanObservasiRanapKebidanan.jasper",
                                "report",
                                "::[ Data Catatan Observasi Rawat Inap Kebidanan ]::",
                                ...
                                + "catatan_observasi_ranap_kebidanan.tgl_perawatan between '" + Valid.
                                        SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                                SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_observasi_ranap_kebidanan.tgl_perawatan,catatan_observasi_ranap_kebidanan.jam_rawat",
                                param);
            } else {
                Valid.
                        MyReportqry(
                                ...
                                + "(reg_periksa.no_rawat like '%" + TCari.
                                        getText().trim() + "%' or pasien.no_rkm_medis like '%" + TCari.
                                        getText().trim() + "%' or "
                                + "pasien.nm_pasien like '%" + TCari.getText().
                                        trim() + "%' or catatan_observasi_ranap_kebidanan.nip like '%" + TCari.
    ...
    private void MnCatatanObservasiRanapKebidananActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanObservasiRanapKebidananActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            Valid.
                    MyReportqry(
                            "rptFormulirCatatanObservasiRanapKebidanan.jasper",
                            "report",
                            "::[ Formulir Catatan Observasi Rawat Inap Kebidanan ]::",
                            ...
                            + "catatan_observasi_ranap_kebidanan.nip,petugas.nama from catatan_observasi_ranap_kebidanan inner join reg_periksa on catatan_observasi_ranap_kebidanan.no_rawat=reg_periksa.no_rawat "
                            + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                            + "inner join petugas on catatan_observasi_ranap_kebidanan.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
  ...
  private void BtnDokter5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnDokter5ActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanObservasiRanapKebidanan dialog = new RMDataCatatanObservasiRanapKebidanan(
  ...
  // End of variables declaration//GEN-END:variables
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        "suhu"), rs.getString("spo2"), rs.getString("kontraksi"),
                        rs.getString("bjj"), rs.getString("ppv"), rs.getString(
                        "vt"), rs.getString("nip"), rs.getString("nama")
                    });
                }
            } catch (Exception e) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            SPO.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 13).
                    toString());
            Kontraksi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 14).
                    toString());
            BJJ.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 15).
                    toString());
            PPV.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 16).
                    toString());
            VT.
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_observasi_ranap_kebidanan",
                "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,gcs=?,td=?,"
                + "hr=?,rr=?,suhu=?,spo2=?,kontraksi=?,bjj=?,ppv=?,vt=?,nip=?",
                17, new String[]{
                    TNoRw.getText(), Valid.
                    ...
                    SPO.getText(), Kontraksi.getText(), BJJ.getText(), PPV.
                    getText(), VT.getText(), NIP.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from catatan_observasi_ranap_kebidanan where tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                3, new String[]{
                    tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString(),
                    tbObat.getValueAt(tbObat.getSelectedRow(), 7).toString(),
                    tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_observasi_ranap_kebidanan",
                "?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 14, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + ""), Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
            ...
            tabMode.addRow(new String[]{
                ...
                getText(), SPO.getText(), Kontraksi.getText(), BJJ.getText(),
                PPV.getText(),
    ...
    private static final Logger LOG = Logger.getLogger(
            RMDataCatatanObservasiRanapKebidanan.class.getName());
