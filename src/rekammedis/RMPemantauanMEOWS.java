 ...
 * @author perpustakaan
 */
public class RMPemantauanMEOWS extends javax.swing.JDialog {
    ...
    public RMPemantauanMEOWS(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Pernapasan", "Skor 1", "Saturasi", "Skor 2", "Temperatur", "Skor 3",
            "TD Sistole", "Skor 4", "TD Diastole", "Skor 5", "Denyut Jantung",
            "Skor 6", "Kesadaran", "Skor 7", "Ketuban", "Skor 8",
            "Discharge", "Skor 9", "Proteinuria", "Skor 10", "Total Skor",
            "Parameter Total", "Code Blue", "NIP", "Petugas", "Tgl.Lahir"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (Skor9.getText().trim().isEmpty()) {
            Valid.textKosong(Skor9, "Skor 9");
        } else if (Skor10.getText().trim().isEmpty()) {
    ...
    private void MnPemantauanMEOWSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPemantauanMEOWSActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("diagnosa", Sequel.cariIsi(
                    "select kamar_inap.diagnosa_awal from kamar_inap where kamar_inap.diagnosa_awal<>'' and kamar_inap.no_rawat=? ",
                    TNoRw.getText()));
            Valid.MyReportqry("rptFormulirPemantauanMEOWS.jasper", "report",
                    "::[ Pemantauan MEOWS Obstetri ]::",
                    ...
                    + "inner join petugas on pemantauan_meows_obstetri.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMPemantauanMEOWS dialog = new RMPemantauanMEOWS(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        "parameter_pernapasan"), rs.getString("skor_pernapasan"),
                        rs.getString("parameter_saturasi"), rs.getString(
                        "skor_saturasi"),
                        rs.getString("parameter_temperatur"), rs.getString(
                        "skor_temperatur"), rs.getString(
                        "parameter_tekanan_darah_sistole"), rs.getString(
                        "skor_tekanan_darah_sistole"),
                        rs.getString("parameter_tekanan_darah_diastole"), rs.
                        getString("skor_tekanan_darah_diastole"), rs.getString(
                        "parameter_denyut_jantung"), rs.getString(
                        "skor_denyut_jantung"),
                        rs.getString("parameter_kesadaran"), rs.getString(
                        "skor_kesadaran"), rs.getString("parameter_ketuban"),
                        rs.getString("skor_ketuban"), rs.getString(
                        "parameter_discharge"),
                        rs.getString("skor_discharge"), rs.getString(
                        "parameter_proteinuria"), rs.getString(
                        "skor_proteinuria"), rs.getString("skor_total"), rs.
                        getString("parameter_total"),
                        rs.getString("code_blue"), rs.getString("nip"), rs.
                        getString("nama"), rs.getString("lahir")
                    });
                }
            } catch (Exception e) {
    ...
    private void isjml() {
        if ((!Skor1.getText().isEmpty()) && (!Skor2.getText().isEmpty()) && (!Skor3.
                getText().isEmpty()) && (!Skor4.getText().isEmpty()) && (!Skor5.
                getText().isEmpty()) && (!Skor6.getText().isEmpty()) && (!Skor7.
                getText().isEmpty()) && (!Skor8.getText().isEmpty()) && (!Skor9.
                getText().isEmpty()) && (!Skor10.getText().isEmpty())) {
    ...
    private void isHitung() {
        if (Integer.parseInt(TotalSkor.getText()) >= 7) {
            ParameterSkor.setText(
                    "Monitoring secara kontinyu oleh dokter jaga & Bidan/Katim/PJ Shift, Informasikan & konsultasikan ke DPJP untuk menentukan rencana perawatan pasien selanjutnya/rawat intensive. Aktifasi Code Blue jika pasien Henti Nafas & Henti jantung.");
        } else if (Integer.parseInt(TotalSkor.getText()) >= 5) {
            ParameterSkor.setText(
                    "Assessment segera oleh Bidan/Katim/PJ Shift dan dokter jaga, evaluasi perawatan dan monitoring setiap 1-2 jam, pertimbangkan perawatan dengan monitoring yang sesuai.");
        } else if (Integer.parseInt(TotalSkor.getText()) >= 2) {
            ParameterSkor.setText(
                    "Assessment segera oleh Bidan respon segera, maksimal 5 menit, evaluasi perawatan dan frekuensi monitoring setiap 4 - 6 jam.");
        } else if (Integer.parseInt(TotalSkor.getText()) >= 0) {
            ParameterSkor.setText(
                    "Berisiko rendah, observasi setiap 7 jam atau Setiap Shift");
        }
    }
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Skor8.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 21).
                    toString());
            cmbSkor9.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),
                    22).toString());
            Skor9.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 23).
                    toString());
            cmbSkor10.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),
                    24).toString());
            Skor10.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 25).
                    toString());
            TotalSkor.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 26).
                    toString());
            ParameterSkor.setText(
                    tbObat.getValueAt(tbObat.getSelectedRow(), 27).toString());
            cmbCodeBlue.setSelectedItem(tbObat.getValueAt(tbObat.
                    getSelectedRow(), 28).toString());
            TglLahir.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 31).
                    toString());
    ...
    private void ganti() {
        ...
        isjml();
        isHitung();
        if (Sequel.mengedittf("pemantauan_meows_obstetri",
                "tanggal=? and no_rawat=?",
                "no_rawat=?,tanggal=?,parameter_pernapasan=?,skor_pernapasan=?,parameter_saturasi=?,"
                ...
                + "parameter_discharge=?,skor_discharge=?,parameter_proteinuria=?,skor_proteinuria=?,skor_total=?,parameter_total=?,code_blue=?,nip=?",
                28, new String[]{
                    TNoRw.getText(), Valid.
                    ...
                    cmbSkor9.getSelectedItem().toString(), Skor9.getText(),
                    cmbSkor10.getSelectedItem().toString(), Skor10.getText(),
                    TotalSkor.getText(), ParameterSkor.getText(), cmbCodeBlue.
                    getSelectedItem().toString(), KdPetugas.getText(),
            ...
            tbObat.setValueAt(cmbSkor9.getSelectedItem().toString(), tbObat.
                    getSelectedRow(), 22);
            tbObat.setValueAt(Skor9.getText(), tbObat.getSelectedRow(), 23);
            tbObat.setValueAt(cmbSkor10.getSelectedItem().toString(), tbObat.
                    getSelectedRow(), 24);
            ...
            tbObat.setValueAt(ParameterSkor.getText(), tbObat.getSelectedRow(),
                    27);
            tbObat.setValueAt(cmbCodeBlue.getSelectedItem().toString(), tbObat.
                    getSelectedRow(), 28);
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from pemantauan_meows_obstetri where tanggal=? and no_rawat=?",
                2, new String[]{
                    tbObat.getValueAt(tbObat.getSelectedRow(), 5).toString(),
                    tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
                }) == true) {
    ...
    private void simpan() {
        ...
        isjml();
        isHitung();
        if (Sequel.menyimpantf("pemantauan_meows_obstetri",
                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data",
                26, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.
    ...
    private static final Logger LOG = Logger.getLogger(RMPemantauanMEOWS.class.
            getName());
}
