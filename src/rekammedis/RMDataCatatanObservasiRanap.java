...
public class RMDataCatatanObservasiRanap extends javax.swing.JDialog {
  ...
  private void initComponents() {
    ...
    intext = new widget.TextBox();
    jLabel30 = new widget.Label();
    outtext = new widget.TextBox();
    ...
    intext.setFocusTraversalPolicyProvider(true);
    intext.setName("intext"); // NOI18N
    intext.addKeyListener(new java.awt.event.KeyAdapter() {
      public void keyPressed(java.awt.event.KeyEvent evt) {
        intextKeyPressed(evt);
      }
    });
    FormInput.add(intext);
    intext.setBounds(800, 70, 100, 23);

    jLabel30.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
    jLabel30.setText(":");
    jLabel30.setName("jLabel30"); // NOI18N
    FormInput.add(jLabel30);
    jLabel30.setBounds(900, 70, 10, 23);

    outtext.setFocusTraversalPolicyProvider(true);
    ...
    FormInput.add(outtext);
    outtext.setBounds(910, 70, 100, 23);
    ...
    BtnDokter5.setBounds(1010, 70, 28, 23);
    ...
    private void BtnSimpanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnSimpanKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_SPACE) {
            ...
            Valid.pindah(evt, outtext, BtnBatal);
        }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataCatatanObservasiRanap.jasper",
                        "report", "::[ Data Catatan Observasi Rawat Inap ]::",
                        ...
                        + "catatan_observasi_ranap.tgl_perawatan between '" + Valid.
                                SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_observasi_ranap.tgl_perawatan,catatan_observasi_ranap.jam_rawat",
                        param);
            } else {
                Valid.MyReportqry("rptDataCatatanObservasiRanap.jasper",
                        "report", "::[ Data Catatan Observasi Rawat Inap ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or catatan_observasi_ranap.nip like '%" + TCari.
    ...
    private void MnCatatanObservasiIGDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanObservasiIGDActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("logo", Sequel.cariGambar(
                    "select setting.logo from setting"));
            Valid.MyReportqry("rptFormulirCatatanObservasiRanap.jasper",
                    "report", "::[ Formulir Catatan Observasi Rawat Inap ]::",
                    ...
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join petugas on catatan_observasi_ranap.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
    ...
    private void SPOKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_SPOKeyPressed
        Valid.pindah(evt, Suhu, intext);
    }//GEN-LAST:event_SPOKeyPressed

    private void intextKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_intextKeyPressed
        Valid.pindah(evt, SPO, outtext);
    }//GEN-LAST:event_intextKeyPressed

    private void outtextKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_outtextKeyPressed
  ...
  private widget.TextBox outtext;
        ...
        try {
            if (TCari.getText().trim().isEmpty()) {
                ps = koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"
                        + "pasien.jk,pasien.tgl_lahir,catatan_observasi_ranap.tgl_perawatan,catatan_observasi_ranap.jam_rawat,catatan_observasi_ranap.gcs,"
                        + "catatan_observasi_ranap.td,catatan_observasi_ranap.hr,catatan_observasi_ranap.rr,catatan_observasi_ranap.suhu,catatan_observasi_ranap.spo2,catatan_observasi_ranap.intext,catatan_observasi_ranap.outtext,"
                        + "catatan_observasi_ranap.nip,petugas.nama from catatan_observasi_ranap inner join reg_periksa on catatan_observasi_ranap.no_rawat=reg_periksa.no_rawat "
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join petugas on catatan_observasi_ranap.nip=petugas.nip where "
