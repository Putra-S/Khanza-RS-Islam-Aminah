 ...
 * @author perpustakaan
 */
public class RMPemantauanPEWSD extends javax.swing.JDialog {
    ...
    public RMPemantauanPEWSD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Skor Respirasi", "Skor 1", "Skor Oksigen", "Skor 2",
            "Skor Suplemen Oksigen", "Skor 3",
            "Skor Tekanan Darah", "Skor 4", "Skor Laju Jantung", "Skor 5",
            "Skor Kesadaran", "Skor 6", "Skor Temperatur", "Skor 7", "Ttl.Skor",
            "Kesimpulan", "NIP", "Petugas", "Tgl.Lahir"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (TNoRw.getText().trim().isEmpty() || TPasien.getText().trim().
            ...
            isjml();
            isHitung();
            if (Sequel.menyimpantf("pemantauan_pews_dewasa",
                    "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 19,
                    new String[]{
                ...
                tabMode.addRow(new String[]{
                    ...
                    cmbSkor6.getSelectedItem().toString(), Skor6.getText(),
                    cmbSkor7.getSelectedItem().toString(), Skor7.getText(),
                    TotalSkor.getText(), ParameterSkor.getText(),
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.
                        MyReportqry("rptDataPemantauanPEWSDewasa.jasper",
                                ...
                                + "pemantauan_pews_dewasa.tanggal between '" + Valid.
                                        SetTgl(DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.
                                SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' order by pemantauan_pews_dewasa.tanggal ",
                                param);
            } else {
                Valid.
                        MyReportqry("rptDataPemantauanPEWSDewasa.jasper",
                                ...
                                + "where pemantauan_pews_dewasa.tanggal between '" + Valid.
                                        SetTgl(DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.
                                ...
                                + "(reg_periksa.no_rawat like '%" + TCari.
                                        ...
                                        getText() + "%' or pemantauan_pews_dewasa.nip like '%" + TCari.
    ...
    private void MnPemantauanPEWSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPemantauanPEWSActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            Valid.MyReportqry("rptFormulirPemantauanPEWSD.jasper", "report",
                    ...
                    + "inner join petugas on pemantauan_pews_dewasa.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMPemantauanPEWSD dialog = new RMPemantauanPEWSD(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        "parameter_laju_respirasi"), rs.getString(
                        "skor_laju_respirasi"),
                        rs.getString("parameter_saturasi_oksigen"), rs.
                        getString("skor_saturasi_oksigen"), rs.getString(
                        "parameter_suplemen_oksigen"),
                        rs.getString("skor_suplemen_oksigen"), rs.getString(
                        "parameter_tekanan_darah_sistolik"), rs.getString(
                        "skor_tekanan_darah_sistolik"),
                        rs.getString("parameter_laju_jantung"), rs.getString(
                        "skor_laju_jantung"), rs.
                        getString("parameter_kesadaran"),
                        rs.getString("skor_kesadaran"), rs.getString(
                        "parameter_temperatur"), rs.getString("skor_temperatur"),
                        rs.getString("skor_total"),
    ...
    private void isHitung() {
        i = 0;
        if (cmbSkor1.getSelectedItem().equals("<= 5") || cmbSkor1.
                getSelectedItem().equals(">= 35")) {
            i++;
        }
        if (cmbSkor4.getSelectedItem().equals("<= 70")) {
            i++;
        }
        if (cmbSkor5.getSelectedItem().equals(">= 140") || cmbSkor5.
                getSelectedItem().equals("<= 40")) {
            i++;
        }
        ...
        if (Integer.parseInt(TotalSkor.getText()) > 8) {
            ParameterSkor.setText(
                    "Lakukan RJP oleh petugas/tim primer, aktivasi code blue henti jantung (...), respon Tim Medis Emergency (TME) segera, maksimal 5 menit, informasikan dan konsultasikan ke DPJP");
        } else if ((Integer.parseInt(TotalSkor.getText()) >= 7) || (i > 0)) {
            ParameterSkor.setText(
                    "Resusitasi dan monitoring secara kontinyu ole dokter jaga dan perawat senior, aktivasi code blue kegawatan medis (....) respon Tim Medis Emergency (TME) segera, maksimal 10 menit, informasikr dan konsultasikan ke DPJP");
        } else if (Integer.parseInt(TotalSkor.getText()) >= 5) {
            ParameterSkor.setText(
                    "Asesment segera oleh dokter jaga (respon segera, maks 5 menit) konsultasi DPJP dan spesialis terkait, eskalasi perawatan dan monitoring tap jam, pertimbangkan perawatan dengan monitoring yang sesua (HCU)");
        } else if (Integer.parseInt(TotalSkor.getText()) > 0) {
            ParameterSkor.setText(
                    "Assement segera oleh perawat senior, respon segera, maks 5 menit, eskalasi perawat dan frekuensi monitoring per 4-6 jam, jika diperlukan assesment oleh dokter jaga bangsal");
        } else if (Integer.parseInt(TotalSkor.getText()) == 0) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Skor7.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 19).
                    toString());
            TotalSkor.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 20).
                    toString());
            ParameterSkor.setText(
                    tbObat.getValueAt(tbObat.getSelectedRow(), 21).toString());
            TglLahir.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 24).
                    toString());
    ...
    private void ganti() {
        ...
        isjml();
        isHitung();
        if (Sequel.mengedittf("pemantauan_pews_dewasa",
                "tanggal=? and no_rawat=?",
                "no_rawat=?,tanggal=?,parameter_laju_respirasi=?,skor_laju_respirasi=?,parameter_saturasi_oksigen=?,"
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from pemantauan_pews_dewasa where tanggal=? and no_rawat=?",
                2, new String[]{
                    tbObat.getValueAt(tbObat.getSelectedRow(), 5).toString(),
                    tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
                }) == true) {
    ...
    private static final Logger LOG = Logger.getLogger(RMPemantauanPEWSD.class.
            getName());
}
