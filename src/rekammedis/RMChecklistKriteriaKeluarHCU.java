...
public class RMChecklistKriteriaKeluarHCU extends javax.swing.JDialog {
    ...
    public RMChecklistKriteriaKeluarHCU(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "J.K.",
            "Tanggal", "Pasien Yang Sudah Tidak Memerlukan Perawatan Di Ruang HCU",
            "Hypertensi Emergency Teratasi / Normal",
            "HR 60 - 100 x/menit (Hasil EKG Menunjukkan Perbaikan / Sinus Rithme)", "Kesadaran Dengan GCS >= 7",
            "Gangguan Inotropic / Vasoaktif Gent Teratasi / Normal", "MAP >= 60 mmHg", "RR 12 - 20x/menit",
            "Gangguan Elektrolit (Na, Ca, CI, Mg, Cal) & Asam Basa Normal",
            "Perdarahan Saluran Pencernaan Teratasi", "Temperature 35 - 38°C", "Gagal Jantung Akut Teratasi",
            "Gangguan Sirkulasi Teratasi", "NIP/Kode Dokter", "DPJP/Dokter Jaga/HCU"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("tanggal"), rs.getString("kriteria1"), rs.getString("kriteria2"),
                        rs.getString("kriteria3"), rs.getString("kriteria4"), rs.getString("kriteria5"),
                        rs.getString("kriteria6"), rs.getString("kriteria7"), rs.getString("kriteria8"),
                        rs.getString("kriteria9"), rs.getString("kriteria10"), rs.getString("kriteria11"),
                        rs.getString("kriteria12"), rs.getString("nik"), rs.getString("nama")});
                }
            } catch (Exception e) {
    ...
    private void ganti() {
        if (Sequel.mengedittf("checklist_kriteria_keluar_hcu", "no_rawat=? and tanggal=?",
                ...
                new String[]{TNoRw.getText(),
                    ...
                    Kriteria3.getSelectedItem().toString(), Kriteria4.getSelectedItem().toString(),
                    Kriteria5.getSelectedItem().toString(), Kriteria6.getSelectedItem().toString(),
                    Kriteria7.getSelectedItem().toString(), Kriteria8.getSelectedItem().toString(),
                    Kriteria9.getSelectedItem().toString(), Kriteria10.getSelectedItem().toString(),
                    Kriteria11.getSelectedItem().toString(), Kriteria12.getSelectedItem().toString(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from checklist_kriteria_keluar_hcu where no_rawat=? and tanggal=?", 2,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("checklist_kriteria_keluar_hcu", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 15,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " "
                    + Tanggal.getSelectedItem().toString().substring(11, 19),
                    Kriteria1.getSelectedItem().toString(), Kriteria2.getSelectedItem().toString(),
    ...
    private static final Logger LOG = Logger.getLogger(RMChecklistKriteriaKeluarHCU.class.getName());
