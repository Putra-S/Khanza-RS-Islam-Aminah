...
public final class RMSkriningAdiksiNikotin extends javax.swing.JDialog {
    ...
    public RMSkriningAdiksiNikotin(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "Umur", "Kode Petugas", "Nama Petugas",
                    "Tanggal", "Rokok Dihisab", "N.R.D", "Menyalakan Rokok", "N.M.R", "Rokok Tidak Direlakan",
                    "N.R.T", "Jam Pertama", "N.J.P", "Kesulitan Menahan", "N.K.M", "Saat Sakit Berat", "N.S.B",
                    "N.Total", "Kesimpulan", "Skala Motivasi"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("nama"), rs.getString("tanggal"), rs.getString("rokok_dihisap"),
                        rs.getString("nilai_rokok_dihisap"), rs.getString("menyalakan_rokok"),
                        rs.getString("nilai_menyalakan_rokok"), rs.getString("tidak_rela"),
                        rs.getString("nilai_tidak_rela"), rs.getString("jam_pertama"),
                        rs.getString("nilai_jam_pertama"), rs.getString("rasa_ingin"),
                        rs.getString("nilai_rasa_ingin"), rs.getString("sakit_berat"),
                        rs.getString("nilai_sakit_berat"), rs.getString("nilai_total"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("skrining_adiksi_nikotin", "no_rawat=?",
                "no_rawat=?,tanggal=?,rokok_dihisap=?,nilai_rokok_dihisap=?,menyalakan_rokok=?,nilai_menyalakan_rokok=?,tidak_rela=?,nilai_tidak_rela=?,jam_pertama=?,nilai_jam_pertama=?,rasa_ingin=?,nilai_rasa_ingin=?,sakit_berat=?,nilai_sakit_berat=?,"
                + "nilai_total=?,keterangan_hasil_skrining=?,skala_motivasi=?,nip=?",
                19,
                new String[]{TNoRw.getText(),
                    ...
                    RokokDihisab.getSelectedItem().toString(), NilaiRokokDihisab.getText(),
                    MenyalakanRokok.getSelectedItem().toString(), NilaiMenyalakanRokok.getText(),
                    TidakRela.getSelectedItem().toString(), NilaiTidakRela.getText(),
                    JamPertama.getSelectedItem().toString(), NilaiJamPertama.getText(),
                    RasaIngin.getSelectedItem().toString(), NilaiRasaIngin.getText(),
                    SakitBerat.getSelectedItem().toString(), NilaiSakitBerat.getText(), TotalNilai.getText(),
                    Kesimpulan.getText(), SkalaMotivasi.getSelectedItem().toString(), KdPetugas.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from skrining_adiksi_nikotin where no_rawat=?", 1,
    ...
    private void isTotal() {
        try {
            TotalNilai.setText("" + (Integer.parseInt(NilaiRokokDihisab.getText())
                    + Integer.parseInt(NilaiMenyalakanRokok.getText()) + Integer.parseInt(NilaiTidakRela.getText())
                    + Integer.parseInt(NilaiJamPertama.getText()) + Integer.parseInt(NilaiRasaIngin.getText())
                    + Integer.parseInt(NilaiSakitBerat.getText())));
    ...
    private void simpan() {
        if (Sequel.menyimpantf("skrining_adiksi_nikotin", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 18,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    RokokDihisab.getSelectedItem().toString(), NilaiRokokDihisab.getText(),
                    ...
                    RasaIngin.getSelectedItem().toString(), NilaiRasaIngin.getText(),
                    SakitBerat.getSelectedItem().toString(), NilaiSakitBerat.getText(), TotalNilai.getText(),
                    Kesimpulan.getText(), SkalaMotivasi.getSelectedItem().toString(),
