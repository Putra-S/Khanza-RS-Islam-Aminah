 ...
 * @author perpustakaan
 */
public class RMDataCatatanCekGDS extends javax.swing.JDialog {
    ...
    public RMDataCatatanCekGDS(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK",
            "Tgl.Lahir", "Tgl.Obser", "Jam Obser", "GDS", "Insulin", "Obat Gula", "NIP", "Nama Petugas"}) {
            @Override
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataCatatanCekGDS.jasper", "report",
                        "::[ Data Catatan Cek GDS ]::",
                        ...
                        + "catatan_cek_gds.tgl_perawatan between '" + Valid.
                                SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_cek_gds.tgl_perawatan,catatan_cek_gds.jam_rawat",
                        param);
            } else {
                Valid.MyReportqry("rptDataCatatanCekGDS.jasper", "report",
                        "::[ Data Catatan Cek GDS ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or catatan_cek_gds.nip like '%" + TCari.
    ...
    private void MnCatatanCekGDSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanCekGDSActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("logo", Sequel.cariGambar(
                    "select setting.logo from setting"));
            Valid.MyReportqry("rptFormulirCatatanCekGDS.jasper", "report",
                    "::[ Formulir Catatan Cek GDS ]::",
                    ...
                    + "from catatan_cek_gds inner join reg_periksa on catatan_cek_gds.no_rawat=reg_periksa.no_rawat "
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join petugas on catatan_cek_gds.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
                            getValueAt(tbObat.getSelectedRow(), 0).toString() + "' order by catatan_cek_gds.tgl_perawatan,catatan_cek_gds.jam_rawat",
                    param);
        }
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jam_rawat"), rs.getString("gdp"), rs.getString("insulin"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_cek_gds", "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,gdp=?,insulin=?,obat_gula=?,nip=?", 10,
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from catatan_cek_gds where tgl_perawatan=? and jam_rawat=? and no_rawat=?", 3,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_cek_gds", "?,?,?,?,?,?,?", "Data", 7,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    GDP.getText(), Insulin.getText(), ObatGula.getText(), NIP.getText()}) == true) {
    ...
    private static final Logger LOG = Logger.getLogger(RMDataCatatanCekGDS.class.getName());
