...
public class RMDataCatatanObservasiCHBP extends javax.swing.JDialog {
    ...
    public RMDataCatatanObservasiCHBP(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tgl.Lahir", "Tgl.Obser",
                    "Jam Obser", "TD(mmHg)", "Nadi(x/menit)", "Suhu Rectal(°C)", "DJJ", "HIS", "PPV", "Keterangan",
                    "NIP", "Nama Petugas"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jam_rawat"), rs.getString("td"), rs.getString("hr"), rs.getString("suhu"),
                        rs.getString("djj"), rs.getString("his"), rs.getString("ppv"), rs.getString("keterangan"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_observasi_chbp", "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,td=?,hr=?,suhu=?,djj=?," + "his=?,ppv=?,keterangan=?,nip=?", 14,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    ...
                    TD.getText(), Nadi.getText(), Suhu.getText(), DJJ.getText(), HIS.getText(), PPV.getText(),
                    Keterangan.getText(), NIP.getText(), tbObat.getValueAt(tbObat.getSelectedRow(), 6).toString(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from catatan_observasi_chbp where tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                3,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_observasi_chbp", "?,?,?,?,?,?,?,?,?,?,?", "Data", 11,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    TD.getText(), Nadi.getText(), Suhu.getText(), DJJ.getText(), HIS.getText(), PPV.getText(),
                    Keterangan.getText(), NIP.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), Umur.getText(),
                ...
                Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(), TD.getText(),
                Nadi.getText(), Suhu.getText(), DJJ.getText(), HIS.getText(), PPV.getText(), Keterangan.getText(),
    ...
    private static final Logger LOG = Logger.getLogger(RMDataCatatanObservasiCHBP.class.getName());
