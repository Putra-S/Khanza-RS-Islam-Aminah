...
public class RMDataCatatanObservasiCHBP extends javax.swing.JDialog {
    ...
    public RMDataCatatanObservasiCHBP(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Tgl.Obser", "Jam Obser", "TD(mmHg)",
            "Nadi(x/menit)", "Suhu Rectal(°C)", "DJJ", "HIS", "PPV",
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.
                        MyReportqry("rptDataCatatanObservasiCHBP.jasper",
                                "report", "::[ Data Catatan Observasi CHBP ]::",
                                ...
                                + "catatan_observasi_chbp.tgl_perawatan between '" + Valid.
                                        SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                                SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_observasi_chbp.tgl_perawatan,catatan_observasi_chbp.jam_rawat",
                                param);
            } else {
                Valid.
                        MyReportqry("rptDataCatatanObservasiCHBP.jasper",
                                ...
                                + "pasien.nm_pasien like '%" + TCari.getText().
                                        trim() + "%' or catatan_observasi_chbp.nip like '%" + TCari.
    ...
    private void MnCatatanObservasiCHBPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanObservasiCHBPActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("logo", Sequel.cariGambar(
                    "select setting.logo from setting"));
            Valid.
                    MyReportqry("rptFormulirCatatanObservasiCHBP.jasper",
                            "report", "::[ Formulir Catatan Observasi CHBP ]::",
                            ...
                            + "from catatan_observasi_chbp inner join reg_periksa on catatan_observasi_chbp.no_rawat=reg_periksa.no_rawat "
                            + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                            + "inner join petugas on catatan_observasi_chbp.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanObservasiCHBP dialog = new RMDataCatatanObservasiCHBP(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("td"), rs.getString("hr"),
                        rs.getString("suhu"), rs.getString("djj"), rs.getString(
                        "his"), rs.getString("ppv"), rs.getString("keterangan"),
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Nadi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 9).
                    toString());
            Suhu.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                    toString());
            DJJ.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).
                    toString());
            HIS.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 12).
                    toString());
            PPV.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 13).
                    toString());
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_observasi_chbp",
                "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,td=?,hr=?,suhu=?,djj=?,"
                + "his=?,ppv=?,keterangan=?,nip=?", 14, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + ""), Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                    getSelectedItem(), TD.getText(), Nadi.getText(), Suhu.
                    getText(),
                    DJJ.getText(), HIS.getText(), PPV.getText(), Keterangan.
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from catatan_observasi_chbp where tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                3, new String[]{
    ...
    private void simpan() {
        if (Sequel.
                menyimpantf("catatan_observasi_chbp", "?,?,?,?,?,?,?,?,?,?,?",
                        "Data", 11, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + ""), Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                    getSelectedItem(),
                    TD.getText(), Nadi.getText(), Suhu.getText(), DJJ.getText(),
                    HIS.getText(), PPV.getText(), Keterangan.getText(), NIP.
                    getText()
                }) == true) {
            tabMode.addRow(new String[]{
    ...
    private static final Logger LOG = Logger.getLogger(
            RMDataCatatanObservasiCHBP.class.getName());
