...
public class MasterCariTemplateSOAPIE extends javax.swing.JDialog {
    ...
    private void tampil() {
        /*
		 * Valid.tabelKosong(tabMode); try{ file=new
		 * File("./cache/template_pemeriksaan_dokter.iyem"); file.createNewFile();
		 * fileWriter = new FileWriter(file); iyem=""; ps=koneksi.
		 * prepareStatement("select template_pemeriksaan_dokter.no_template,template_pemeriksaan_dokter.kd_dokter,dokter.nm_dokter,template_pemeriksaan_dokter.keluhan,"
		 * +
		 ...
		 * ); try { rs=ps.executeQuery(); while(rs.next()){ tabMode.addRow(new
		 * String[]{rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.
		 * getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9)});
		 * iyem=iyem+"{\"NoTemplate\":\""+rs.getString(1)+"\",\"KodeDokter\":\""+rs.
		 * getString(2)+"\",\"NamaDokter\":\""+rs.getString(3)+"\",\"Subyek\":\""+rs.
		 * getString(4)+"\",\"Obyek\":\""+rs.getString(5)+"\",\"Asesmen\":\""+rs.getString
		 * (6)+"\",\"Planning\":\""+rs.getString(7)+"\",\"Instruksi\":\""+rs.getString(8)+
		 * "\",\"Evaluasi\":\""+rs.getString(9)+"\"},";
		 * 
		 * } } catch (Exception e) { System.out.println("Notifikasi : "+e); } finally{
		 * if(rs!=null){ rs.close(); } if(ps!=null){ ps.close(); } }
		 * fileWriter.write("{\"template_pemeriksaan_dokter\":["+iyem.substring(0,iyem.
		 * length()-1)+"]}"); fileWriter.flush(); fileWriter.close(); iyem=null;
    ...
    /*
	 * private void tampil2() { try { myObj = new
	 * FileReader("./cache/template_pemeriksaan_dokter.iyem"); root =
	 * mapper.readTree(myObj); Valid.tabelKosong(tabMode); response =
	 * root.path("template_pemeriksaan_dokter"); if(response.isArray()){ for(JsonNode
	 * list:response){
	 * 
	 * if(list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&
	 * list.path("Subyek").asText().toLowerCase().contains(TCari.getText().toLowerCase())|
	 * | list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&list.path("Obyek").asText().toLowerCase().contains(TCari.getText().
	 * toLowerCase())||
	 * list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&list.path("Asesmen").asText().toLowerCase().contains(TCari.getText(
	 * ).toLowerCase())||
	 * list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&list.path("Planning").asText().toLowerCase().contains(TCari.getText
	 * ().toLowerCase())||
	 * list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&list.path("Instruksi").asText().toLowerCase().contains(TCari.
	 * getText().toLowerCase())||
	 * list.path("KodeDokter").asText().toLowerCase().contains(kddokter.getText().
	 * toLowerCase())&&list.path("Evaluasi").asText().toLowerCase().contains(TCari.getText
	 * ().toLowerCase())){ tabMode.addRow(new Object[]{
	 * list.path("NoTemplate").asText(),list.path("KodeDokter").asText(),list.path(
	 * "NamaDokter").asText(),
	 * list.path("Subyek").asText(),list.path("Obyek").asText(),list.path("Asesmen").
	 * asText(),
	 * list.path("Planning").asText(),list.path("Instruksi").asText(),list.path("Evaluasi"
	 * ).asText() }); } } } myObj.close(); } catch (Exception ex) {
	 * System.out.println("Notifikasi : "+ex); } LCount.setText(""+tabMode.getRowCount());
	 * }
     */
        ...
        try {
            if (kddokter.getText().isEmpty()) {
                ...
                ps = koneksi.prepareStatement(
                        ...
                        + // "template_pemeriksaan_dokter.kd_dokter like ? and
                        // template_pemeriksaan_dokter.keluhan like ? or "+
                        // "template_pemeriksaan_dokter.kd_dokter like ? and
                        // template_pemeriksaan_dokter.pemeriksaan like ? or
                        // "+
                        // "template_pemeriksaan_dokter.kd_dokter like ? and
                        // template_pemeriksaan_dokter.rencana like ? or "+
                        // "template_pemeriksaan_dokter.kd_dokter like ? and
                        // template_pemeriksaan_dokter.instruksi like ? or "+
                        // "template_pemeriksaan_dokter.kd_dokter like ? and
                        // template_pemeriksaan_dokter.evaluasi like ? "+
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_template"), rs.getString("kd_dokter"),
    ...
    private static final Logger LOG = Logger.getLogger(MasterCariTemplateSOAPIE.class.getName());
