...
public class RMHasilPemeriksaanUSGUrologi extends javax.swing.JDialog {
    ...
    public RMHasilPemeriksaanUSGUrologi(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "Kode Dokter", "Nama Dokter", "Tanggal",
                    "Kiriman Dari", "Diagnosa Klinis", "Ginjal Kanan", "Ginjal Kiri", "Vesica Urinaria",
                    "Tambahan"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("diagnosa_klinis"), rs.getString("ginjal_kanan"), rs.getString("ginjal_kiri"),
                        rs.getString("vesica_urinaria"), rs.getString("tambahan")});
                }
            } catch (Exception e) {
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from hasil_pemeriksaan_usg_urologi where no_rawat=?", 1,
    ...
    private void ganti() {
        if (Sequel.mengedittf("hasil_pemeriksaan_usg_urologi", "no_rawat=?",
                ...
                new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), TglLahir.getText(),
                    ...
                    KirimanDari.getText(), DiagnosaKlinis.getText(), GinjalKanan.getText(), GinjalKiri.getText(),
                    Vesica.getText(), Tambahan.getText(),
                    ...
                    if (rs.next()) {
                        if (rs.getString("photo").isEmpty() || rs.getString("photo").equals("-")) {
                            ...
                            LoadHTML2.setText("<html><body><center><a href='http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                    + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/hasilpemeriksaanusgurologi/"
    ...
    private void simpan() {
        if (Sequel.menyimpantf("hasil_pemeriksaan_usg_urologi", "?,?,?,?,?,?,?,?,?", "No.Rawat", 9,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " "
                    + Tanggal.getSelectedItem().toString().substring(11, 19),
                    KdDokter.getText(), DiagnosaKlinis.getText(), KirimanDari.getText(), GinjalKanan.getText(),
                    GinjalKiri.getText(), Vesica.getText(), Tambahan.getText()}) == true) {
    ...
    private static final Logger LOG = Logger.getLogger(RMHasilPemeriksaanUSGUrologi.class.getName());
