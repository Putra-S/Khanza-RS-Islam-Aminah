...
public class RMDataMonitoringReaksiTranfusi extends javax.swing.JDialog {
    ...
    public RMDataMonitoringReaksiTranfusi(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tgl.Lahir", "Tanggal", "Jam",
                    "Produk/Jenis Darah", "No.Kantong", "Lokasi Insersi", "TD(mmHg)", "HR(x/menit)", "RR(x/menit)",
                    "Suhu(°C)", "Alergi", "Keterangan", "NIP", "Nama Petugas"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jam_rawat"), rs.getString("produk_darah"), rs.getString("no_kantong"),
                        rs.getString("lokasi_insersi"), rs.getString("td"), rs.getString("hr"), rs.getString("rr"),
                        rs.getString("suhu"), rs.getString("jenis_reaksi_alergi"), rs.getString("keterangan"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("monitoring_reaksi_tranfusi", "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                ...
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    ...
                    JenisDarah.getText(), NoKantong.getText(), LokasiInsersi.getText(), TD.getText(), HR.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from monitoring_reaksi_tranfusi where tgl_perawatan=? and jam_rawat=? and no_rawat=?", 3,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("monitoring_reaksi_tranfusi", "?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 13,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    JenisDarah.getText(), NoKantong.getText(), LokasiInsersi.getText(), TD.getText(), HR.getText(),
                    RR.getText(), Suhu.getText(), Alergi.getText(), Keterangan.getText(),
                    NIP.getText()}) == true) {
    ...
    private static final Logger LOG = Logger.getLogger(RMDataMonitoringReaksiTranfusi.class.getName());
