...
public class RMDataCatatanKeseimbanganCairan extends javax.swing.JDialog {
    ...
    public RMDataCatatanKeseimbanganCairan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Tanggal", "Jam", "Infus",
            "Tranfusi", "Minum", "Urine/Feses", "Drain", "NGT/Oral", "IWL",
            "Keseimbangan Cairan",
        ...
        Keseimbangan.setDocument(new batasInput((byte) 4).getOnlyAngka(
                Keseimbangan));
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (Infus.getText().trim().isEmpty()) {
            Valid.textKosong(Infus, "Infus");
        } else if (Tranfusi.getText().trim().isEmpty()) {
            Valid.textKosong(Tranfusi, "Tranfusi");
        } else if (Minum.getText().trim().isEmpty()) {
            Valid.textKosong(Minum, "Minum");
        } else if (Urine.getText().trim().isEmpty()) {
            Valid.textKosong(Urine, "Urine/Feses");
        } else if (Drain.getText().trim().isEmpty()) {
            Valid.textKosong(Drain, "Drain");
        } else if (NGT.getText().trim().isEmpty()) {
            Valid.textKosong(NGT, "NGT/Oral");
        } else if (IWL.getText().trim().isEmpty()) {
            Valid.textKosong(IWL, "IWL");
        } else if (Keseimbangan.getText().trim().isEmpty()) {
            ...
            if (akses.getkode().equals("Admin Utama")) {
                ...
                if (NIP.getText().equals(tbObat.getValueAt(tbObat.
                    ...
                    if (Sequel.cekTanggal48jam(tbObat.getValueAt(tbObat.
                            getSelectedRow(), 6).toString() + " " + tbObat.
                                    getValueAt(tbObat.getSelectedRow(), 7).
                                    toString(), Sequel.ambiltanggalsekarang()) == true) {
                        hapus();
                    }
                } else {
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataCatatanKeseimbanganCairan.jasper",
                        "report", "::[ Data Catatan Keseimbangan Cairan ]::",
                        ...
                        + "catatan_keseimbangan_cairan.tgl_perawatan between '" + Valid.
                                SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_keseimbangan_cairan.tgl_perawatan,catatan_keseimbangan_cairan.jam_rawat",
                        param);
            } else {
                Valid.MyReportqry("rptDataCatatanKeseimbanganCairan.jasper",
                        "report", "::[ Data Catatan Keseimbangan Cairan ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or catatan_keseimbangan_cairan.nip like '%" + TCari.
    ...
    private void MnCatatanKeseimbanganCairanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanKeseimbanganCairanActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("logo", Sequel.cariGambar(
                    "select setting.logo from setting"));
            Valid.MyReportqry("rptFormulirCatatanKeseimbanganCairan.jasper",
                    "report", "::[ Formulir Catatan Keseimbangan Cairan ]::",
                    ...
                    + "from catatan_keseimbangan_cairan inner join reg_periksa on catatan_keseimbangan_cairan.no_rawat=reg_periksa.no_rawat "
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join petugas on catatan_keseimbangan_cairan.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
                            getValueAt(tbObat.getSelectedRow(), 0).toString() + "' order by catatan_keseimbangan_cairan.tgl_perawatan,catatan_keseimbangan_cairan.jam_rawat",
                    param);
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanKeseimbanganCairan dialog = new RMDataCatatanKeseimbanganCairan(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("infus"), rs.getString("tranfusi"),
                        rs.getString("minum"), rs.getString("urine"), rs.
                        getString("drain"), rs.getString("ngt"), rs.getString(
                        "iwl"),
                        rs.getString("keseimbangan"), rs.getString("keterangan"),
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Infus.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 8).
                    toString());
            Tranfusi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 9).
                    toString());
            Minum.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                    toString());
            Urine.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).
                    toString());
            Drain.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 12).
                    toString());
            NGT.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 13).
                    toString());
            IWL.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 14).
                    toString());
            Keseimbangan.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 15).
                    toString());
            Keterangan.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 16).
                    toString());
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_keseimbangan_cairan",
                "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,infus=?,"
                + "tranfusi=?,minum=?,urine=?,drain=?,ngt=?,iwl=?,keseimbangan=?,keterangan=?,nip=?",
                16, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + ""), Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                    getSelectedItem(),
                    Infus.getText(), Tranfusi.getText(), Minum.getText(), Urine.
                    getText(), Drain.getText(), NGT.getText(), IWL.getText(),
                    Keseimbangan.getText(),
                    Keterangan.getText(), NIP.getText(), tbObat.getValueAt(
                    tbObat.getSelectedRow(), 6).toString(), tbObat.getValueAt(
            ...
            tbObat.setValueAt(Keseimbangan.getText(), tbObat.getSelectedRow(),
                    15);
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from catatan_keseimbangan_cairan where tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                3, new String[]{
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_keseimbangan_cairan",
                "?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 13, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + ""), Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
    ...
    private static final Logger LOG = Logger.getLogger(
            RMDataCatatanKeseimbanganCairan.class.getName());
