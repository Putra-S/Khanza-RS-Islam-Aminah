 ...
 * @author perpustakaan
 */
public class ValidasiSBARRalan extends javax.swing.JDialog {
...
//    private SimpleDateFormat tanggalNow = new SimpleDateFormat("dd-MM-yyyy");
    ...
    public ValidasiSBARRalan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "Nama Pemeriksa", "Tanggal SBAR", "Jam SBAR", "S (SITUATION)",
            "B (BACKGROUND)",
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Validasi S.B.A.R ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Segoe UI", 0, 12), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        internalFrame1.getAccessibleContext().setAccessibleName("::[ Validasi S.B.A.R ]::");
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (KodeDokter1.getText().isEmpty() || NamaDokter.getText().
                isEmpty()) {
            Valid.textKosong(BtnDokter, "Nama Pemeriksa");
        } else if (TSituation.getText().isEmpty()) {
            Valid.textKosong(TSituation, "S (SITUATION");
        } else if (TBackground.getText().isEmpty()) {
            Valid.textKosong(TBackground, "B (BACKGROUND)");
        } else {
            if (Sequel.menyimpantf("validasi_pemeriksaan_sbar_ralan",
                    "?,?,?,?,?,?,?", "No.Rawat", 7, new String[]{
                        TNoRw.getText(), TanggalPemeriksaan.getText(),
                        JamPemeriksaan.getText(), KodeDokter.getText(),
    ...
    private void MnLaporanResumeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanResumeActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("norawat", tbObat.getValueAt(tbObat.getSelectedRow(), 2).
                    toString());
...
//        }
                ...
                if (!tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().
                    ...
                    try {
                        berkas.loadURL(
                                ...
                                PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/" + "berkasrawat/login2.php?act=login&usere=admin&passwordte=akusayangsamakamu&no_rawat=" + tbObat.
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            ValidasiSBARRalan dialog = new ValidasiSBARRalan(
        ...
        try {
            if (TCari.getText().isEmpty()) {
                ps = koneksi.prepareStatement(
                        ...
                        + "pemeriksaan_ralan_sbar.background,pemeriksaan_ralan_sbar.assesment,pemeriksaan_ralan_sbar.recommendation,pemeriksaan_ralan_sbar.nip,pegawai.nama,validasi_pemeriksaan_sbar_ralan.nik_validator,validasi_pemeriksaan_sbar_ralan.tgl_validasi,validasi_pemeriksaan_sbar_ralan.jam_validasi,validasi_pemeriksaan_sbar_ralan.status_validasi from pasien inner join reg_periksa inner join pemeriksaan_ralan_sbar "
                        + "on pemeriksaan_ralan_sbar.no_rawat=reg_periksa.no_rawat and reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join pegawai on pemeriksaan_ralan_sbar.nip=pegawai.nik left join validasi_pemeriksaan_sbar_ralan on pemeriksaan_ralan_sbar.jam_rawat = validasi_pemeriksaan_sbar_ralan.jam_rawat AND pemeriksaan_ralan_sbar.no_rawat = validasi_pemeriksaan_sbar_ralan.no_rawat AND pemeriksaan_ralan_sbar.tgl_perawatan = validasi_pemeriksaan_sbar_ralan.tgl_perawatan where "
                        + "pemeriksaan_ralan_sbar.no_rawat like ? and pemeriksaan_ralan_sbar.kd_dokter like ? and ISNULL(validasi_pemeriksaan_sbar_ralan.status_validasi) order by pemeriksaan_ralan_sbar.no_rawat desc");
            } else {
                ps = koneksi.prepareStatement(
                        "select reg_periksa.tgl_registrasi,reg_periksa.status_lanjut,pemeriksaan_ralan_sbar.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien, "
                        + "pemeriksaan_ralan_sbar.tgl_perawatan,pemeriksaan_ralan_sbar.jam_rawat,pemeriksaan_ralan_sbar.situation, "
                        + "pemeriksaan_ralan_sbar.background,pemeriksaan_ralan_sbar.assesment,pemeriksaan_ralan_sbar.recommendation,pemeriksaan_ralan_sbar.nip,pegawai.nama,validasi_pemeriksaan_sbar_ralan.nik_validator,validasi_pemeriksaan_sbar_ralan.tgl_validasi,validasi_pemeriksaan_sbar_ralan.jam_validasi,validasi_pemeriksaan_sbar_ralan.status_validasi from pasien inner join reg_periksa inner join pemeriksaan_ralan_sbar "
                        + "on pemeriksaan_ralan_sbar.no_rawat=reg_periksa.no_rawat and reg_periksa.no_rkm_medis=pasien.no_rkm_medis inner join pegawai on pemeriksaan_ralan_sbar.nip=pegawai.nik left join validasi_pemeriksaan_sbar_ralan on pemeriksaan_ralan_sbar.jam_rawat = validasi_pemeriksaan_sbar_ralan.jam_rawat AND pemeriksaan_ralan_sbar.no_rawat = validasi_pemeriksaan_sbar_ralan.no_rawat AND pemeriksaan_ralan_sbar.tgl_perawatan = validasi_pemeriksaan_sbar_ralan.tgl_perawatan where "
                        + "pemeriksaan_ralan_sbar.tgl_perawatan between ? and ? and pemeriksaan_ralan_sbar.no_rawat like ? and pemeriksaan_ralan_sbar.kd_dokter like ? and ISNULL(validasi_pemeriksaan_sbar_ralan.status_validasi) order by pemeriksaan_ralan_sbar.no_rawat desc");
            }
            try {
                if (!TCari.getText().isEmpty()) {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("situation"), rs.getString("background"),
                        rs.getString("assesment"), rs.
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            TSituation.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 9).
                    toString());
            TBackground.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                    toString());
            TAssesment.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).
                    toString());
            TRecomendation.setText(tbObat.
...
//        BtnPrint.setEnabled(akses.getdata_resume_pasien()); 
    ...
    private static final Logger LOG = Logger.getLogger(ValidasiSBARRalan.class.
            getName());
