...
public final class RMDataCatatanObservasiVentilator extends javax.swing.JDialog {
    ...
    public RMDataCatatanObservasiVentilator(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tgl.Lahir", "Tgl.Obser",
                    "Jam Obser", "Mode", "VT", "Pakar", "RR", "REF/PS", "EE", "NIP", "Nama Petugas"}) {
            @Override
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanObservasiVentilator dialog = new RMDataCatatanObservasiVentilator(new javax.swing.JFrame(),
                    true);
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jam_rawat"), rs.getString("mode"), rs.getString("vt"), rs.getString("pakar"),
                        rs.getString("rr"), rs.getString("reefps"), rs.getString("ee"), rs.getString("nip"),
                        rs.getString("nama")});
                }
            } catch (Exception e) {
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_observasi_ventilator", "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,mode=?,vt=?,pakar=?,rr=?,reefps=?,ee=?,nip=?", 13,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    ...
                    Mode.getSelectedItem().toString(), VT.getText(), Pakar.getText(), RR.getText(),
                    REEFPS.getText(), EE.getText(), NIP.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from catatan_observasi_ventilator where tgl_perawatan=? and jam_rawat=? and no_rawat=?", 3,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_observasi_ventilator", "?,?,?,?,?,?,?,?,?,?", "Data", 10,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    Mode.getSelectedItem().toString(), VT.getText(), Pakar.getText(), RR.getText(),
                    REEFPS.getText(), EE.getText(), NIP.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), Umur.getText(),
                ...
                Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                Mode.getSelectedItem().toString(), VT.getText(), Pakar.getText(), RR.getText(), REEFPS.getText(),
                EE.getText(), NIP.getText(), NamaPetugas.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }
