...
public final class RMSkriningKesehatanGigiMulutRemaja extends javax.swing.JDialog {
    ...
    public RMSkriningKesehatanGigiMulutRemaja(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "J.K.",
            "Kode Petugas", "Nama Petugas", "Tanggal", "Pernah Pemeriksaan Gigi Mulut", "Jumlah Gigi Tumbuh",
            "Kondisi Kebersihan Gigi Mulut", "Punya Gigi Berlubang", "Pernah Gusi Berdarah", "Punya Karang Gigi",
            "Gigi Depan Tidak Teratur", "Menyikat Gigi Sebelum Tidur", "Punya Sariawan", "Pemeriksaan Fisik",
            "Pemeriksaan Penunjang", "Hasil Skrining", "Keterangan"}) {
            @Override
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMSkriningKesehatanGigiMulutRemaja dialog = new RMSkriningKesehatanGigiMulutRemaja(new javax.swing.JFrame(),
                    true);
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("pernah_pemeriksaan_gigimulut"), rs.getString("jumlah_gigi_tumbuh"),
                        rs.getString("kondisi_kebersihan_gigimulut"), rs.getString("punya_gigi_berlubang"),
                        rs.getString("pernah_gusi_berdarah"), rs.getString("punya_karang_gigi"),
                        rs.getString("gigi_depan_tidak_teratur"), rs.getString("menyikat_gigi_sebelum_tidur"),
                        rs.getString("punya_sariawan"), rs.getString("pemeriksaan_fisik"),
                        rs.getString("pemeriksaan_penunjang"), rs.getString("hasil_skrining"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("skrining_kesehatan_gigi_mulut_remaja", "no_rawat=?",
                "no_rawat=?,tanggal=?,pernah_pemeriksaan_gigimulut=?,jumlah_gigi_tumbuh=?,kondisi_kebersihan_gigimulut=?,"
                ...
                + "pemeriksaan_penunjang=?,hasil_skrining=?,keterangan=?,nip=?",
                17,
                new String[]{TNoRw.getText(),
                    ...
                    PernahPemeriksaanGigi.getSelectedItem().toString(),
                    JumlahGigiTumbuh.getSelectedItem().toString(), KebersihanGigiMulut.getSelectedItem().toString(),
                    GigiBerlubang.getSelectedItem().toString(), GusiBerdarah.getSelectedItem().toString(),
                    KarangGigi.getSelectedItem().toString(), GigiTidakteratur.getSelectedItem().toString(),
                    KebiasaanMenyikatGigi.getSelectedItem().toString(), PunyaSariawan.getSelectedItem().toString(),
                    PemeriksaanFisik.getText(), PemeriksaanPenunjang.getText(), HasilSkrining.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from skrining_kesehatan_gigi_mulut_remaja where no_rawat=?", 1,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("skrining_kesehatan_gigi_mulut_remaja", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 16,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    PernahPemeriksaanGigi.getSelectedItem().toString(),
            ...
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), TglLahir.getText(),
                ...
                Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                PernahPemeriksaanGigi.getSelectedItem().toString(), JumlahGigiTumbuh.getSelectedItem().toString(),
                KebersihanGigiMulut.getSelectedItem().toString(), GigiBerlubang.getSelectedItem().toString(),
                GusiBerdarah.getSelectedItem().toString(), KarangGigi.getSelectedItem().toString(),
                GigiTidakteratur.getSelectedItem().toString(), KebiasaanMenyikatGigi.getSelectedItem().toString(),
                PunyaSariawan.getSelectedItem().toString(), PemeriksaanFisik.getText(),
                PemeriksaanPenunjang.getText(), HasilSkrining.getText(), Keterangan.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }
