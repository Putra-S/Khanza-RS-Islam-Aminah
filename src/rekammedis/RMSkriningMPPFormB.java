...
public class RMSkriningMPPFormB extends javax.swing.JDialog {
    ...
    public RMSkriningMPPFormB(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "J.K.", "Tgl.Lahir",
            "Alamat", "Tanggal Catatan", "Masalah", "Tindak Lanjut", "Evaluasi", "NIP", "Nama Petugas"}) {
            @Override
    ...
    public void tampil() {
        ...
        try {
            tgl = " mpp_evaluasi_catatan.tgl_implementasi between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "")
                    + " 00:00:00' and '" + Valid.SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' ";
            if (TCari.getText().trim().isEmpty()) {
                ...
                ps = koneksi.prepareStatement(
                        ...
                        + "inner join kabupaten on pasien.kd_kab=kabupaten.kd_kab "
                        + "inner join propinsi on pasien.kd_prop=propinsi.kd_prop where " + tgl
                        + "and (mpp_evaluasi_catatan.no_rawat like '%" + TCari.getText().trim()
                        + "%' or reg_periksa.no_rkm_medis like '%" + TCari.getText().trim() + "%' or "
                        + "pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or petugas.nama like '%"
                        + TCari.getText().trim() + "%') " + "order by mpp_evaluasi_catatan.no_rawat");
            }
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from mpp_evaluasi_catatan where no_rawat=? and tgl_implementasi=?", 2,
    ...
    private void ganti() {
        if (Sequel.mengedittf("mpp_evaluasi_catatan", "no_rawat=? and tgl_implementasi=?",
                "no_rawat=?,tgl_implementasi=?,masalah=?,tinjut=?,evaluasi=?,nip=?", 8,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(TglImplementasi.getSelectedItem() + "") + " "
                    + TglImplementasi.getSelectedItem().toString().substring(11, 19),
    ...
    private static final Logger LOG = Logger.getLogger(RMSkriningMPPFormB.class.getName());
