 ...
 * @author perpustakaan
 */
public class RMDataCatatanObservasiIGD extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Terapi = new javax.swing.JTextArea();
        ...
        FormInput.setPreferredSize(new java.awt.Dimension(100, 325));
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "23-05-2023" }));
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptDataCatatanObservasiIGD.jasper", "report",
                        "::[ Data Catatan Observasi IGD ]::",
                        ...
                        + "catatan_observasi_igd.tgl_perawatan between '" + Valid.
                                SetTgl(DTPCari1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + "' order by catatan_observasi_igd.tgl_perawatan,catatan_observasi_igd.jam_rawat",
                        param);
            } else {
                Valid.MyReportqry("rptDataCatatanObservasiIGD.jasper", "report",
                        "::[ Data Catatan Observasi IGD ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "pasien.nm_pasien like '%" + TCari.getText().trim() + "%' or catatan_observasi_igd.nip like '%" + TCari.
    ...
    private void MnCatatanObservasiIGDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanObservasiIGDActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("logo", Sequel.cariGambar(
                    "select setting.logo from setting"));
            Valid.MyReportqry("rptFormulirCatatanObservasiIGD.jasper", "report",
                    "::[ Formulir Catatan Observasi IGD ]::",
                    ...
                    + "inner join petugas on catatan_observasi_igd.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
        ...
        try {
            if (TCari.getText().trim().isEmpty()) {
                ps = koneksi.prepareStatement(
                        ...
                        + "catatan_observasi_igd.terapi,catatan_observasi_igd.nip,petugas.nama from catatan_observasi_igd inner join reg_periksa on catatan_observasi_igd.no_rawat=reg_periksa.no_rawat "
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join petugas on catatan_observasi_igd.nip=petugas.nip where "
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("rr"), rs.getString("suhu"), rs.getString("spo2"), rs.getString("map"),
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            MAP.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 14).toString());
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_observasi_igd", "tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                "no_rawat=?,tgl_perawatan=?,jam_rawat=?,gcs=?,td=?,hr=?,rr=?,suhu=?,spo2=?,map=?,terapi=?,nip=?", 15,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    ...
                    MAP.getText(), Terapi.getText(), NIP.getText(),
            ...
            tbObat.setValueAt(MAP.getText(), tbObat.getSelectedRow(), 14);
            tbObat.setValueAt(Terapi.getText(), tbObat.getSelectedRow(), 15);
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from catatan_observasi_igd where tgl_perawatan=? and jam_rawat=? and no_rawat=?",
                3,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_observasi_igd", "?,?,?,?,?,?,?,?,?,?,?,?", "Data", 12,
                new String[]{TNoRw.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""),
                    Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    GCS.getText(), TD.getText(), HR.getText(), RR.getText(), Suhu.getText(), SPO.getText(),
                    MAP.getText(), Terapi.getText(), NIP.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), Umur.getText(),
                ...
                Jam.getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                GCS.getText(), TD.getText(), HR.getText(), RR.getText(), Suhu.getText(), SPO.getText(),
                MAP.getText(), Terapi.getText(), NIP.getText(), NamaPetugas.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }

    private static final Logger LOG = Logger.getLogger(RMDataCatatanObservasiIGD.class.getName());
