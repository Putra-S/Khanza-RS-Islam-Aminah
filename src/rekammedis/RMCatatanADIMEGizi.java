...
public class RMCatatanADIMEGizi extends javax.swing.JDialog {
    ...
    public RMCatatanADIMEGizi(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tanggal", "Asesmen", "Diagnosis",
                    "Intervensi", "Monitoring", "Evaluasi", "Instruksi", "NIP", "Petugas"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jk"), rs.getString("tanggal"), rs.getString("asesmen"),
                        rs.getString("diagnosis"), rs.getString("intervensi"), rs.getString("monitoring"),
                        rs.getString("evaluasi"), rs.getString("instruksi"), rs.getString("nip"),
                        rs.getString("nama")});
                }
            } catch (Exception e) {
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_adime_gizi", "tanggal=? and no_rawat=?",
                "no_rawat=?,tanggal=?,asesmen=?,diagnosis=?,intervensi=?,monitoring=?,evaluasi=?,instruksi=?,nip=?", 11,
                new String[]{TNoRw.getText(),
                    ...
                    Asesmen.getText(), Diagnosis.getText(), Intervensi.getText(), Monitoring.getText(),
                    Evaluasi.getText(), Instruksi.getText(), KdPetugas.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from catatan_adime_gizi where tanggal=? and no_rawat=?", 2,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("catatan_adime_gizi", "?,?,?,?,?,?,?,?,?", "Data", 9,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    Asesmen.getText(), Diagnosis.getText(), Intervensi.getText(), Monitoring.getText(),
                    Evaluasi.getText(), Instruksi.getText(), KdPetugas.getText()}) == true) {
            tabMode.addRow(
                    new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), Umur.getText(), JK.getText(),
                        Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                        + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                        Asesmen.getText(), Diagnosis.getText(), Intervensi.getText(), Monitoring.getText(),
                        Evaluasi.getText(), Instruksi.getText(), KdPetugas.getText(), NmPetugas.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }

    private static final Logger LOG = Logger.getLogger(RMCatatanADIMEGizi.class.getName());
