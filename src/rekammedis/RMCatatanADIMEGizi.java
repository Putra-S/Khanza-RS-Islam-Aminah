 ...
 * @author perpustakaan
 */
public class RMCatatanADIMEGizi extends javax.swing.JDialog {
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().isEmpty()) {
                Valid.MyReportqry("rptDataCatatanADIMEGizi.jasper", "report",
                        "::[ Data Catatan ADIME Gizi Pasien ]::",
                        ...
                        + "catatan_adime_gizi.tanggal between '" + Valid.SetTgl(
                                DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.
                        SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' order by catatan_adime_gizi.tanggal ",
                        param);
            } else {
                Valid.MyReportqry("rptDataCatatanADIMEGizi.jasper", "report",
                        "::[ Data Catatan ADIME Gizi Pasien ]::",
                        ...
                        + "(reg_periksa.no_rawat like '%" + TCari.getText().
                                ...
                                getText().trim() + "%' or "
                        + "catatan_adime_gizi.asesmen like '%" + TCari.getText().
                                trim() + "%' or catatan_adime_gizi.diagnosis like '%" + TCari.
                                getText().trim() + "%' or catatan_adime_gizi.intervensi like '%" + TCari.
                                getText().trim() + "%' or "
                        + "catatan_adime_gizi.monitoring like '%" + TCari.
                                getText().trim() + "%' or catatan_adime_gizi.evaluasi like '%" + TCari.
                                getText().trim() + "%' or catatan_adime_gizi.instruksi like '%" + TCari.
                                getText().trim() + "%' or "
                        + "catatan_adime_gizi.nip like '%" + TCari.getText().
                                trim() + "%' or petugas.nama like '%" + TCari.
                                getText().trim() + "%') order by catatan_adime_gizi.tanggal ",
                        param);
            }
        }
    ...
    private void MnCatatanADIMEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanADIMEActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            param.put("diagnosa", Sequel.cariIsi(
                    ...
                    TNoRw.getText()));
            Valid.MyReportqry("rptFormulirCatatanADIMEGizi.jasper", "report",
                    "::[ Formulir Catatan ADIME Gizi Pasien ]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"
                    + "pasien.jk,catatan_adime_gizi.tanggal,catatan_adime_gizi.asesmen,catatan_adime_gizi.diagnosis,"
                    + "catatan_adime_gizi.intervensi,catatan_adime_gizi.monitoring,catatan_adime_gizi.evaluasi,catatan_adime_gizi.instruksi,"
                    + "catatan_adime_gizi.nip,petugas.nama "
                    + "from catatan_adime_gizi inner join reg_periksa on catatan_adime_gizi.no_rawat=reg_periksa.no_rawat "
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "inner join petugas on catatan_adime_gizi.nip=petugas.nip where reg_periksa.no_rawat='" + tbObat.
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMCatatanADIMEGizi dialog = new RMCatatanADIMEGizi(
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("tanggal"), rs.getString("asesmen"), rs.
                        getString("diagnosis"),
                        rs.getString("intervensi"), rs.getString("monitoring"),
                        rs.getString("evaluasi"),
                        rs.getString("instruksi"), rs.getString("nip"), rs.
                        getString("nama")
                    });
                }
            } catch (Exception e) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            ...
            Asesmen.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 6).
                    toString());
            Diagnosis.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 7).
                    toString());
            Intervensi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 8).
                    toString());
            Monitoring.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 9).
                    toString());
            Evaluasi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                    toString());
            Instruksi.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 11).
                    toString());
        }
    }
    ...
    private void ganti() {
        if (Sequel.mengedittf("catatan_adime_gizi", "tanggal=? and no_rawat=?",
                "no_rawat=?,tanggal=?,asesmen=?,diagnosis=?,intervensi=?,monitoring=?,evaluasi=?,instruksi=?,nip=?",
                11, new String[]{
                    TNoRw.getText(), Valid.
                    SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.
                    getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                    getSelectedItem(), Asesmen.getText(), Diagnosis.getText(),
                    Intervensi.getText(), Monitoring.getText(), Evaluasi.
                    getText(), Instruksi.getText(), KdPetugas.getText(), tbObat.
    ...
    private void hapus() {
        if (Sequel.queryu2tf(
                "delete from catatan_adime_gizi where tanggal=? and no_rawat=?",
                2, new String[]{
    ...
    private void simpan() {
        if (Sequel.
                menyimpantf("catatan_adime_gizi", "?,?,?,?,?,?,?,?,?", "Data", 9,
                        new String[]{
                            TNoRw.getText(), Valid.SetTgl(Tanggal.
                            getSelectedItem() + "") + " " + Jam.
                                    getSelectedItem() + ":" + Menit.
                                    getSelectedItem() + ":" + Detik.
                                    getSelectedItem(), Asesmen.getText(),
                            Diagnosis.getText(), Intervensi.getText(),
                            Monitoring.getText(), Evaluasi.getText(), Instruksi.
            ...
            tabMode.addRow(new String[]{
                ...
                Asesmen.getText(), Diagnosis.getText(), Intervensi.getText(),
                Monitoring.getText(), Evaluasi.getText(), Instruksi.getText(),
    ...
    private static final Logger LOG = Logger.getLogger(RMCatatanADIMEGizi.class.
            getName());
