...
public class RMHasilPemeriksaanEKG extends javax.swing.JDialog {
    ...
    public RMHasilPemeriksaanEKG(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "Kode Dokter", "Nama Dokter", "Tanggal",
                    "Kiriman Dari", "Diagnosa Klinis", "Irama", "Laju Jantung", "Gelombang P", "Interval PR",
                    "Axis", "Kompleks QRS", "Segmen ST", "Gelombang T", "Kesimpulan"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("diagnosa_klinis"), rs.getString("irama"), rs.getString("laju_jantung"),
                        rs.getString("gelombangp"), rs.getString("intervalpr"), rs.getString("axis"),
                        rs.getString("kompleksqrs"), rs.getString("segmenst"), rs.getString("gelombangt"),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from hasil_pemeriksaan_ekg where no_rawat=?", 1,
    ...
    private void ganti() {
        if (Sequel.mengedittf("hasil_pemeriksaan_ekg", "no_rawat=?",
                ...
                new String[]{TNoRw.getText(),
                    ...
                    KdDokter.getText(), DiagnosaKlinis.getText(), KirimanDari.getText(), Irama.getText(),
                    LajuJantung.getText(), GelombangP.getText(), IntervalPR.getText(), Axis.getText(),
                    ...
                    if (rs.next()) {
                        if (rs.getString("photo").isEmpty() || rs.getString("photo").equals("-")) {
                            ...
                            LoadHTML2.setText("<html><body><center><a href='http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                    + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/hasilpemeriksaanekg/"
    ...
    private void simpan() {
        if (Sequel.menyimpantf("hasil_pemeriksaan_ekg", "?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Rawat", 14,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " "
                    + Tanggal.getSelectedItem().toString().substring(11, 19),
                    KdDokter.getText(), DiagnosaKlinis.getText(), KirimanDari.getText(), Irama.getText(),
                    LajuJantung.getText(), GelombangP.getText(), IntervalPR.getText(), Axis.getText(),
                    KompleksQRS.getText(), SegmenST.getSelectedItem().toString(),
                    GelombangT.getSelectedItem().toString(), Kesimpulan.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), TglLahir.getText(),
                KdDokter.getText(), NmDokter.getText(),
                Valid.SetTgl(Tanggal.getSelectedItem() + "") + " "
                + Tanggal.getSelectedItem().toString().substring(11, 19),
                KirimanDari.getText(), DiagnosaKlinis.getText(), Irama.getText(), LajuJantung.getText(),
                GelombangP.getText(), IntervalPR.getText(), Axis.getText(), KompleksQRS.getText(),
                SegmenST.getSelectedItem().toString(), GelombangT.getSelectedItem().toString(),
                Kesimpulan.getText()});
            emptTeks();
            LCount.setText("" + tabMode.getRowCount());
        }
    ...
    private static final Logger LOG = Logger.getLogger(RMHasilPemeriksaanEKG.class.getName());
