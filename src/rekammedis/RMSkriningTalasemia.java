...
public final class RMSkriningTalasemia extends javax.swing.JDialog {
    ...
    public RMSkriningTalasemia(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "J.K.", "Kode Petugas", "Nama Petugas",
                    "Tanggal", "Rutin Menerima Transfusi", "Keluarga Rutin Transfusi", "Ada Saudara Thalassemia",
                    "Tumbuh Kembang Terlambat", "Anemia", "Ikterus", "Perut Buncit", "Gizi Kurang/Buruk",
                    "Facies Cooley", "Perawakan Pendek", "Hiperpigmentasi Kulit", "HB", "MVC", "MCHC", "Darah Tepi",
                    "Tindak Lanjut"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("transfusi_darah"), rs.getString("rutin_transfusi"),
                        rs.getString("saudara_thalassemia"), rs.getString("tumbuh_kembang_terlambat"),
                        rs.getString("anemia"), rs.getString("ikterus"), rs.getString("perut_buncit"),
                        rs.getString("gizi_kurang"), rs.getString("facies_cooley"),
                        rs.getString("perawakan_pendek"), rs.getString("hiperpigmentasi_kulit"),
                        rs.getString("hemoglobin"), rs.getString("mvc"), rs.getString("mchc"),
                        rs.getString("darah_tepi"), rs.getString("tindak_lanjut")});
                }
            } catch (Exception e) {
    ...
    private void ganti() {
        if (Sequel.mengedittf("skrining_thalassemia", "no_rawat=?",
                "no_rawat=?,tanggal=?,nip=?,transfusi_darah=?,rutin_transfusi=?,saudara_thalassemia=?,tumbuh_kembang_terlambat=?,anemia=?,"
                + "ikterus=?,perut_buncit=?,gizi_kurang=?,facies_cooley=?,perawakan_pendek=?,hiperpigmentasi_kulit=?,hemoglobin=?,mvc=?,mchc=?,darah_tepi=?,tindak_lanjut=?",
                20,
                new String[]{TNoRw.getText(),
                    ...
                    KdPetugas.getText(), TransfusiDarah.getSelectedItem().toString(),
                    RutinTransfusi.getSelectedItem().toString(), SaudaraTalasemia.getSelectedItem().toString(),
                    TumbangTerlambat.getSelectedItem().toString(), Anemia.getSelectedItem().toString(),
                    Icterus.getSelectedItem().toString(), PerutBuncit.getSelectedItem().toString(),
                    GiziKurang.getSelectedItem().toString(), FaciesCooley.getSelectedItem().toString(),
                    Pendek.getSelectedItem().toString(), Hiperpigmentasi.getSelectedItem().toString(),
                    Hb.getSelectedItem().toString(), MVC.getSelectedItem().toString(),
                    MCHC.getSelectedItem().toString(), DarahTepi.getSelectedItem().toString(),
                    TindakLanjut.getText(), tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()}) == true) {
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from skrining_thalassemia where no_rawat=?", 1,
    ...
    private void simpan() {
        if (Sequel.menyimpantf("skrining_thalassemia", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 19,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    KdPetugas.getText(), TransfusiDarah.getSelectedItem().toString(),
            ...
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), TglLahir.getText(),
                ...
                Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                TransfusiDarah.getSelectedItem().toString(), RutinTransfusi.getSelectedItem().toString(),
                SaudaraTalasemia.getSelectedItem().toString(), TumbangTerlambat.getSelectedItem().toString(),
                Anemia.getSelectedItem().toString(), Icterus.getSelectedItem().toString(),
                PerutBuncit.getSelectedItem().toString(), GiziKurang.getSelectedItem().toString(),
                FaciesCooley.getSelectedItem().toString(), Pendek.getSelectedItem().toString(),
                Hiperpigmentasi.getSelectedItem().toString(), Hb.getSelectedItem().toString(),
                MVC.getSelectedItem().toString(), MCHC.getSelectedItem().toString(),
                DarahTepi.getSelectedItem().toString(), TindakLanjut.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }
