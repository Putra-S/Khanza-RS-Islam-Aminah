...
public final class RMSkriningPUMA extends javax.swing.JDialog {
    ...
    public RMSkriningPUMA(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.RM", "Nama Pasien", "Tgl.Lahir", "Umur", "Kode Petugas", "Nama Petugas",
                    "Tanggal", "Jenis Kelamin", "N.JK", "Umur", "N.U", "Pernah Merokok", "N.M", "Jml.Rokok",
                    "Lama Merokok", "Nafas Pendek", "N.N.P", "Sulit Keluar Dahak", "N.K.D", "Biasa Batuk", "N.B.B",
                    "Spirometri", "N.S", "Total Skor", "Tindakan"}) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (PernahMerokok.getSelectedItem().toString().equals("Pernah") && JumlahRokok.getText().trim().isEmpty()) {
            Valid.textKosong(JumlahRokok, "Jumlah Merokok");
        } else if (PernahMerokok.getSelectedItem().toString().equals("Pernah") && LamaMerokok.getText().trim().isEmpty()) {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("nama"), rs.getString("tanggal"), rs.getString("jk"), rs.getString("nilai_jk"),
                        rs.getString("usia"), rs.getString("nilai_usia"), rs.getString("pernah_merokok"),
                        rs.getString("nilai_pernah_merokok"), rs.getString("jumlah_rokok_perhari"),
                        rs.getString("lama_merokok"), rs.getString("napas_pendek"),
                        rs.getString("nilai_napas_pendek"), rs.getString("punya_dahak"),
                        rs.getString("nilai_punya_dahak"), rs.getString("biasa_batuk"),
                        rs.getString("nilai_biasa_batuk"), rs.getString("spirometri"),
                        rs.getString("nilai_spirometri"), rs.getString("nilai_total"),
    ...
    private void ganti() {
        if (Sequel.mengedittf("skrining_puma", "no_rawat=?",
                "no_rawat=?,tanggal=?,jk=?,nilai_jk=?,usia=?,nilai_usia=?,pernah_merokok=?,nilai_pernah_merokok=?,jumlah_rokok_perhari=?,lama_merokok=?,napas_pendek=?,nilai_napas_pendek=?,punya_dahak=?,nilai_punya_dahak=?,biasa_batuk=?,nilai_biasa_batuk=?,"
                + "spirometri=?,nilai_spirometri=?,nilai_total=?,keterangan_hasil_skrining=?,nip=?",
                22,
                new String[]{TNoRw.getText(),
                    ...
                    JK.getSelectedItem().toString(), skorJk.getText(), Usia.getSelectedItem().toString(),
                    skorUsia.getText(), PernahMerokok.getSelectedItem().toString(), skorPernahMerokok.getText(),
                    JumlahRokok.getText(), LamaMerokok.getText(), NafasPendek.getSelectedItem().toString(),
                    skorNafas.getText(), Dahak.getSelectedItem().toString(), skordahak.getText(),
                    Batuk.getSelectedItem().toString(), skorBatuk.getText(),
                    Spirometri.getSelectedItem().toString(), skorSpirometri.getText(), TotalHasil.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from skrining_puma where no_rawat=?", 1,
    ...
    private void isTotal() {
        try {
            TotalHasil.setText("" + (Integer.parseInt(skorJk.getText()) + Integer.parseInt(skorUsia.getText())
                    + Integer.parseInt(skorPernahMerokok.getText()) + Integer.parseInt(skorNafas.getText())
            ...
            if (Integer.parseInt(TotalHasil.getText()) >= 5) {
                Tindakan.setText(
                        "Edukasi & berikan konseling upaya berhenti merokok, lakukan pemeriksaan spirometri (bila tersedia), rujuk ke FKRTL");
            } else {
    ...
    private void simpan() {
        if (Sequel.menyimpantf("skrining_puma", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 21,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    JK.getSelectedItem().toString(), skorJk.getText(), Usia.getSelectedItem().toString(),
                    ...
                    Batuk.getSelectedItem().toString(), skorBatuk.getText(),
                    Spirometri.getSelectedItem().toString(), skorSpirometri.getText(), TotalHasil.getText(),
                    Tindakan.getText(), KdPetugas.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), TglLahir.getText(),
                ...
                JumlahRokok.getText(), LamaMerokok.getText(), NafasPendek.getSelectedItem().toString(),
                skorNafas.getText(), Dahak.getSelectedItem().toString(), skordahak.getText(),
                Batuk.getSelectedItem().toString(), skorBatuk.getText(), Spirometri.getSelectedItem().toString(),
                skorSpirometri.getText(), TotalHasil.getText(), Tindakan.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    }
