...
public class RMPemantauanEWSNeonatus extends javax.swing.JDialog {
    ...
    public RMPemantauanEWSNeonatus(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"No.Rawat", "No.R.M.", "Nama Pasien", "Umur", "JK", "Tanggal", "Pernapasan", "Skor 1",
                    "Saturasi Oksigen", "Skor 2", "Penggunaan O2", "Skor 3", "Denyut Jantung", "Skor 4",
                    "Retraksi Dada", "Skor 5", "CRT", "Skor 6", "Suhu", "Skor 7", "Warna Kulit", "Skor 8",
                    "Ttl.Skor", "Kesimpulan", "Code Blue", "NIP", "Petugas", "Tgl.Lahir"}) {
            @Override
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        ...
                        rs.getString("jk"), rs.getString("tanggal"), rs.getString("parameter1"),
                        rs.getString("skor1"), rs.getString("parameter2"), rs.getString("skor2"),
                        rs.getString("parameter3"), rs.getString("skor3"), rs.getString("parameter4"),
                        rs.getString("skor4"), rs.getString("parameter5"), rs.getString("skor5"),
                        rs.getString("parameter6"), rs.getString("skor6"), rs.getString("parameter7"),
                        rs.getString("skor7"), rs.getString("parameter8"), rs.getString("skor8"),
                        rs.getString("skor_total"), rs.getString("parameter_total"), rs.getString("code_blue"),
    ...
    private void isjml() {
        if ((!Skor1.getText().isEmpty()) && (!Skor2.getText().isEmpty()) && (!Skor3.getText().isEmpty())
            ...
            TotalSkor.setText(Valid.SetAngka2(Double.parseDouble(Skor1.getText().trim())
                    ...
                    + Double.parseDouble(Skor4.getText().trim()) + Double.parseDouble(Skor5.getText().trim())
                    + Double.parseDouble(Skor6.getText().trim()) + Double.parseDouble(Skor7.getText().trim())));
        }
    }
    ...
    private void ganti() {
        ...
        isjml();
        isHitung();
        if (Sequel.mengedittf("pemantauan_ews_neonatus", "tanggal=? and no_rawat=?",
                ...
                new String[]{TNoRw.getText(),
                    ...
                    Skor8.getText(), TotalSkor.getText(), ParameterSkor.getText(),
                    cmbCodeBlue.getSelectedItem().toString(), KdPetugas.getText(),
    ...
    private void hapus() {
        if (Sequel.queryu2tf("delete from pemantauan_ews_neonatus where tanggal=? and no_rawat=?", 2,
    ...
    private void simpan() {
        ...
        isjml();
        isHitung();
        if (Sequel.menyimpantf("pemantauan_ews_neonatus", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "Data", 22,
                new String[]{TNoRw.getText(),
                    Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":"
                    + Menit.getSelectedItem() + ":" + Detik.getSelectedItem(),
                    cmbSkor1.getSelectedItem().toString(), Skor1.getText(), cmbSkor2.getSelectedItem().toString(),
                    ...
                    cmbSkor7.getSelectedItem().toString(), Skor7.getText(), cmbSkor8.getSelectedItem().toString(),
                    Skor8.getText(), TotalSkor.getText(), ParameterSkor.getText(),
                    cmbCodeBlue.getSelectedItem().toString(), KdPetugas.getText()}) == true) {
            tabMode.addRow(new String[]{TNoRw.getText(), TNoRM.getText(), TPasien.getText(), Umur.getText(),
                ...
                cmbSkor7.getSelectedItem().toString(), Skor7.getText(), cmbSkor8.getSelectedItem().toString(),
                Skor8.getText(), TotalSkor.getText(), ParameterSkor.getText(),
                cmbCodeBlue.getSelectedItem().toString(), KdPetugas.getText(), NmPetugas.getText(),
                TglLahir.getText()});
            LCount.setText("" + tabMode.getRowCount());
            emptTeks();
        }
    ...
    private static final Logger LOG = Logger.getLogger(RMPemantauanEWSNeonatus.class.getName());
