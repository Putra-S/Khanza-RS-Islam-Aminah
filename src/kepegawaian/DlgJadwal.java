...
private void BtnPoliActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPoliActionPerformed
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("select jadwal.kd_dokter,dokter.nm_dokter,jadwal.hari_kerja, "
                    + "jadwal.jam_mulai,jadwal.jam_selesai,poliklinik.nm_poli,jadwal.kuota "
                    + "from jadwal inner join poliklinik inner join dokter " + "on jadwal.kd_dokter=dokter.kd_dokter "
                    + "and jadwal.kd_poli=poliklinik.kd_poli "
                    + "where jadwal.kd_dokter like ? or dokter.nm_dokter like ? or "
                    + "jadwal.hari_kerja like ? or poliklinik.nm_poli like ? " + "order by jadwal.kd_dokter");
            try {
    ...
    private void getData() {
        ...
        if (row != -1) {
            ...
            Sequel.cariIsi("select poliklinik.kd_poli from poliklinik where poliklinik.nm_poli='"
                    + tabMode.getValueAt(row, 6).toString() + "'", KdPoli);
        }
    }

    private static final Logger LOG = Logger.getLogger(DlgJadwal.class.getName());
