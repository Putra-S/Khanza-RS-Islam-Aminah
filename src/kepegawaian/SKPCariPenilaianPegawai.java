...
public class SKPCariPenilaianPegawai extends javax.swing.JDialog {
    ...
    public SKPCariPenilaianPegawai(java.awt.Frame parent, boolean modal) {
        ...
        NoPenilaian.setDocument(new batasInput(20).getKata(NoPenilaian));
...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (NoPenilaian.getText().trim().isEmpty()) {
            Valid.textKosong(NoPenilaian, "No.Penilaian");
        } else {
            if (Valid.SetInteger(LTotal.getText()) > 0) {
                if (Sequel.meghapustf("skp_penilaian", "nomor_penilaian",
                        NoPenilaian.getText()) == true) {
                    tampil();
                }
            } else {
                JOptionPane.showMessageDialog(null,
                        "Maaf, Pastikan nomor penilaian yang mau dihapus sudah benar...!!!!");
            }
        }
    ...
    private void tampil() {
        try {
            ...
            htmlContent.append("<tr class='head'>"
                    ...
                    + "<td valign='top' bgcolor='#FFFAFA' align='center' width='105px'>Tanggal</td>"
                    + "<td valign='top' bgcolor='#FFFAFA' align='center' width='300px'>Keterangan</td>"
                    + "<td valign='top' bgcolor='#FFFAFA' align='center' width='100px'>Status</td>" + "</tr>");
            ps = koneksi.prepareStatement(
                    ...
                    + "where skp_penilaian.tanggal between '" + Valid.SetTgl(Tanggal1.getSelectedItem() + "")
                    + " 00:00:00' and '" + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + " 23:59:59' "
                    + (!Status.getSelectedItem().toString().equals("Semua")
                    ? "and skp_penilaian.status='" + Status.getSelectedItem().toString() + "' " : "")
                    + (!KdPenilai.getText().isEmpty()
                    ? "and skp_penilaian.nik_penilai='" + KdPenilai.getText() + "' " : "")
                    + (!KdDInilai.getText().isEmpty()
                    ? "and skp_penilaian.nik_dinilai='" + KdDInilai.getText() + "' " : "")
                    + (!NoPenilaian.getText().isEmpty()
                    ? "and skp_penilaian.nomor_penilaian='" + NoPenilaian.getText() + "' " : "")
                    + " order by skp_penilaian.tanggal");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    htmlContent.append("<tr class='isi'>" + "<td valign='top' align='center' rowspan='2'>"
                            + rs.getString("nomor_penilaian") + "</td>" + "<td valign='top' align='left'>"
                            + rs.getString("nik_dinilai") + " " + rs.getString("dinilai") + "</td>"
                            + "<td valign='top' align='left'>" + rs.getString("nik_penilai") + " "
                            + rs.getString("penilai") + "</td>" + "<td valign='top' align='center'>"
                            + rs.getString("tanggal") + "</td>" + "<td valign='top' align='left'>"
                            + rs.getString("keterangan") + "</td>" + "<td valign='top' align='center'>"
                            + rs.getString("status") + "</td>" + "</tr>" + "<tr class='isi'>"
                            ...
                            + "<td valign='top' bgcolor='#FFFAFA' align='center' width='25%'>Sasaran</td>" + "</tr>");
                    ps2 = koneksi.prepareStatement(
                            ...
                            + "inner join skp_kategori_penilaian on skp_kategori_penilaian.kode_kategori=skp_kriteria_penilaian.kode_kategori where skp_detail_penilaian.nomor_penilaian='"
                            + rs.getString("nomor_penilaian") + "' "
                            + (!Sasaran.getSelectedItem().toString().equals("Semua")
                            ? "and skp_kategori_penilaian.sasaran='"
                            + Sasaran.getSelectedItem().toString().substring(0, 1) + "' "
                            : "")
                            + (!KdKategori.getText().isEmpty()
                            ? "and skp_kategori_penilaian.kode_kategori='" + KdKategori.getText() + "' "
                            : "")
                            + (TCari.getText().trim().isEmpty() ? ""
                            : " and " + "(skp_detail_penilaian.kode_kriteria like '%" + TCari.getText()
                            + "%' or skp_kriteria_penilaian.nama_kriteria like '%"
                            + TCari.getText() + "%')")
                    ...
                    try {
                        ...
                        while (rs2.next()) {
                            htmlContent.append("<tr class='isi'>" + "<td valign='top' align='center'>"
                                    + rs2.getString("kode_kriteria") + "</td>" + "<td valign='top'>"
                                    + rs2.getString("nama_kriteria") + "</td>" + "<td valign='top' align='center'>"
                                    + rs2.getString("skala_penilaian") + "</td>" + "<td valign='top'>"
                                    + rs2.getString("nama_kategori") + "</td>" + "<td valign='top'>"
                                    + rs2.getString("sasaran")
    ...
    private static final Logger LOG = Logger.getLogger(SKPCariPenilaianPegawai.class.getName());
