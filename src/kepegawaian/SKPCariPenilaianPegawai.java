...
*/
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SKPCariPenilaianPegawai dialog = new SKPCariPenilaianPegawai(
    ...
    private void tampil() {
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + "where skp_penilaian.tanggal between '" + Valid.SetTgl(
                            Tanggal1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.
                    SetTgl(Tanggal2.getSelectedItem() + "") + " 23:59:59' "
                    + (!Status.getSelectedItem().toString().equals("Semua") ? "and skp_penilaian.status='" + Status.
                    getSelectedItem().toString() + "' " : "")
                    + (!KdPenilai.getText().isEmpty() ? "and skp_penilaian.nik_penilai='" + KdPenilai.
                    getText() + "' " : "")
                    + (!KdDInilai.getText().isEmpty() ? "and skp_penilaian.nik_dinilai='" + KdDInilai.
                    getText() + "' " : "")
                    + (!NoPenilaian.getText().isEmpty() ? "and skp_penilaian.nomor_penilaian='" + NoPenilaian.
                    getText() + "' " : "") + " order by skp_penilaian.tanggal");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    htmlContent.append(
                            "<tr class='isi'>"
                            + "<td valign='top' align='center' rowspan='2'>" + rs.
                                    getString("nomor_penilaian") + "</td>"
                            + "<td valign='top' align='left'>" + rs.getString(
                                    "nik_dinilai") + " " + rs.getString(
                                    "dinilai") + "</td>"
                            + "<td valign='top' align='left'>" + rs.getString(
                                    "nik_penilai") + " " + rs.getString(
                                    "penilai") + "</td>"
                    ...
                    ps2 = koneksi.prepareStatement(
                            ...
                            + "inner join skp_kategori_penilaian on skp_kategori_penilaian.kode_kategori=skp_kriteria_penilaian.kode_kategori where skp_detail_penilaian.nomor_penilaian='" + rs.
                                    getString("nomor_penilaian") + "' "
                            + (!Sasaran.getSelectedItem().toString().equals(
                                    "Semua") ? "and skp_kategori_penilaian.sasaran='" + Sasaran.
                                            getSelectedItem().toString().
                                            substring(0, 1) + "' " : "")
                            + (!KdKategori.getText().isEmpty() ? "and skp_kategori_penilaian.kode_kategori='" + KdKategori.
                            getText() + "' " : "") + (TCari.getText().trim().
                                    isEmpty() ? "" : " and "
                                    + "(skp_detail_penilaian.kode_kriteria like '%" + TCari.
                                            getText() + "%' or skp_kriteria_penilaian.nama_kriteria like '%" + TCari.
                                            getText() + "%')") + " order by skp_kategori_penilaian.sasaran");
                    try {
                        ...
                        while (rs2.next()) {
                            htmlContent.append(
                                    ...
                                    + "<td valign='top' align='center'>" + rs2.
                                            getString("kode_kriteria") + "</td>"
                                    + "<td valign='top'>" + rs2.getString(
                                            "nama_kriteria") + "</td>"
                                    + "<td valign='top' align='center'>" + rs2.
                                            getString("skala_penilaian") + "</td>"
                                    + "<td valign='top'>" + rs2.getString(
                                            "nama_kategori") + "</td>"
                                    + "<td valign='top'>"
                                    + rs2.getString("sasaran").replaceAll("1",
    ...
    private static final Logger LOG = Logger.getLogger(
            SKPCariPenilaianPegawai.class.getName());
}
