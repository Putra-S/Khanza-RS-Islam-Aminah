...
*/
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        for (i = 0; i < row; i++) {
            if (tbDokter.getValueAt(i, 2).toString().equals("true") || tbDokter.
        ...
        if (NoPenilaian.getText().trim().isEmpty()) {
        ...
        } else if (KdPenilai.getText().trim().isEmpty() || NmPenilai.getText().
                trim().isEmpty()) {
            Valid.textKosong(btnPenilai, "Yang Menilai");
        } else if (KdDInilai.getText().trim().isEmpty() || NmDinilai.getText().
                trim().isEmpty()) {
            ...
            index = JOptionPane.showConfirmDialog(rootPane,
                    "Eeiiiiiits, udah bener belum data yang mau disimpan..??",
                    "Konfirmasi", JOptionPane.YES_NO_OPTION);
            if (index == JOptionPane.YES_OPTION) {
                Sequel.AutoComitFalse();
                sukses = true;
                if (Sequel.menyimpantf2("skp_penilaian", "?,?,?,?,?,?",
                        "No.Permintaan", 6, new String[]{
                            NoPenilaian.getText(), KdPenilai.getText(),
                            KdDInilai.getText(), Valid.SetTgl(Tanggal.
                    ...
                    for (i = 0; i < row; i++) {
                        if (tbDokter.getValueAt(i, 2).toString().equals("true") || tbDokter.
                                getValueAt(i, 3).toString().equals("true")) {
                            if (Sequel.menyimpantf2("skp_detail_penilaian",
                                    "?,?,?", "Data", 3, new String[]{
                                        NoPenilaian.getText(), tbDokter.
                                        getValueAt(i, 0).toString(), (tbDokter.
                                        getValueAt(i, 2).toString().equals(
                                        "true") ? "Ya" : "Tidak")
                                    }) == false) {
                                sukses = false;
                            }
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
            ...
            try {
                if (tbDokter.getSelectedRow() != -1) {
                    if (tbDokter.getSelectedColumn() == 2) {
                        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 2).
                                toString().equals("true")) {
                            tbDokter.
                                    setValueAt(false, tbDokter.getSelectedRow(),
                                            3);
                        }
                    }
                    if (tbDokter.getSelectedColumn() == 3) {
                        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 3).
                                toString().equals("true")) {
                            tbDokter.
                                    setValueAt(false, tbDokter.getSelectedRow(),
                                            2);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SKPPenilaianPegawai dialog = new SKPPenilaianPegawai(
    ...
    private void tampil2() {
        try {
            ...
            for (i = 0; i < row; i++) {
                if (tbDokter.getValueAt(i, 2).toString().equals("true") || tbDokter.
                    ...
                    if (tbDokter.getValueAt(i, 2).toString().equals("true")) {
                        Ya[index] = true;
                    } else if (tbDokter.getValueAt(i, 2).toString().equals(
                            "false")) {
                        Ya[index] = false;
                    }
                    if (tbDokter.getValueAt(i, 3).toString().equals("true")) {
                        Tidak[index] = true;
                    } else if (tbDokter.getValueAt(i, 3).toString().equals(
                            "false")) {
                        Tidak[index] = false;
                    }
                    index++;
            ...
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{KodeKriteria[i], Kriteria[i], Ya[i],
                    Tidak[i]});
            }
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty() && KdKategori.getText().
                        trim().isEmpty() && Sasaran.getSelectedItem().toString().
                                isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeKriteria").asText(), list.path(
                            "Kriteria").asText(), false, false
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeKriteria").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase()) || list.
                                path("Kriteria").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase())) {
                            if (list.path("Kategori").asText().toLowerCase().
                                    contains(NmKategori.getText().toLowerCase())) {
                                if (list.path("Sasaran").asText().toLowerCase().
                                        contains(Sasaran.getSelectedItem().
                                                toString().substring(0, 1).
                                                replaceAll("S", ""))) {
                                    tabMode.addRow(new Object[]{
                                        list.path("KodeKriteria").asText(),
                                        list.path("Kriteria").asText(), false,
                                        false
                                    });
                                }
                            }
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(skp_penilaian.nomor_penilaian,3),signed)),0) from skp_penilaian where left(skp_penilaian.tanggal,10)='" + Valid.
    ...
    private static final Logger LOG = Logger.getLogger(
            SKPPenilaianPegawai.class.getName());
