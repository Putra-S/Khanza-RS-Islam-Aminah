...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
        ...
        try {
            ...
            for (i = 0; i < row; i++) {
                if (tbDokter.getValueAt(i, 2).toString().equals("true")
                        || tbDokter.getValueAt(i, 3).toString().equals("true")) {
                    jml++;
                }
            }
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty() && KdKategori.getText().trim().isEmpty()
                        && Sasaran.getSelectedItem().toString().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("KodeKriteria").asText(),
                            list.path("Kriteria").asText(), false, false});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeKriteria").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                                || list.path("Kriteria")
                                        .asText()
                                        .toLowerCase()
                                        .contains(TCari.getText().toLowerCase())) {
                            if (list.path("Kategori")
                                    .asText()
                                    .toLowerCase()
                                    .contains(NmKategori.getText().toLowerCase())) {
                                if (list.path("Sasaran")
                                        .asText()
                                        .toLowerCase()
                                        .contains(
                                                Sasaran.getSelectedItem().toString().substring(0, 1).replaceAll("S", ""))) {
                                    tabMode.addRow(new Object[]{list.path("KodeKriteria").asText(),
                                        list.path("Kriteria").asText(), false, false});
                                }
                            }
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(skp_penilaian.nomor_penilaian,3),signed)),0) from skp_penilaian where left(skp_penilaian.tanggal,10)='"
    ...
    private static final Logger LOG = Logger.getLogger(SKPPenilaianPegawai.class.getName());
