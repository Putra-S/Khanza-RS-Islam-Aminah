...
public class DlgCariPetugas extends javax.swing.JDialog {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    iyem = iyem + "{\"NIP\":\"" + rs.getString(1) + "\",\"NamaPetugas\":\""
                            ...
                            + "\",\"SttsNikah\":\"" + rs.getString(8) + "\",\"Alamat\":\""
                            + rs.getString(9).replaceAll("\"", "") + "\",\"Jabatan\":\"" + rs.getString(10)
                            + "\",\"NoTelp\":\"" + rs.getString(11) + "\"},";
                }
            } catch (Exception e) {
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("NIP").asText(), list.path("NamaPetugas").asText(),
                            list.path("JK").asText(), list.path("TmpLahir").asText(),
                            list.path("TglLahir").asText(), list.path("GD").asText(), list.path("Agama").asText(),
                            list.path("SttsNikah").asText(), list.path("Alamat").asText(),
                            list.path("Jabatan").asText(), list.path("NoTelp").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("NIP").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                                || list.path("NamaPetugas")
                                        .asText()
                                        .toLowerCase()
                            ...
                            tabMode.addRow(new Object[]{list.path("NIP").asText(), list.path("NamaPetugas").asText(),
                                ...
                                list.path("Agama").asText(), list.path("SttsNikah").asText(),
                                list.path("Alamat").asText(), list.path("Jabatan").asText(),
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariPetugas.class.getName());
