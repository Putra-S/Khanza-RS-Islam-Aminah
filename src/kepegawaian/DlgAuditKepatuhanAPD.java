 ...
 * @author perpustakaan
 */
public class DlgAuditKepatuhanAPD extends javax.swing.JDialog {
    ...
    public DlgAuditKepatuhanAPD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            "Tanggal Audit", "Tindakan", "NIP/Kode", "Dokter/Paramedis",
            "Profesi/Jabatan", "Topi", "Masker", "Kacamata", "Sarung Tangan",
            "Apron", "Sepatu", "Ttl.Nilai(%)"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (Nip.getText().trim().isEmpty() || NamaPetugas.getText().trim().
            ...
            if (Sequel.menyimpantf("audit_kepatuhan_apd", "?,?,?,?,?,?,?,?,?",
                    "Data", 9, new String[]{
                        Nip.getText(), Tindakan.getText(), Valid.SetTgl(Tanggal.
                        getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":" + Menit.
                        getSelectedItem() + ":" + Detik.getSelectedItem(), Topi.
                        getSelectedItem().toString(),
                        Masker.getSelectedItem().toString(), KacaMata.
                        getSelectedItem().toString(), SarungTangan.
                                getSelectedItem().toString(), Apron.
                                getSelectedItem().toString(), Sepatu.
                                getSelectedItem().toString()
                    }) == true) {
                tampil();
                emptTeks();
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (tbObat.getSelectedRow() != -1) {
            if (Sequel.queryu2tf(
                    "delete from audit_kepatuhan_apd where nik=? and tindakan=? and tanggal=?",
                    3, new String[]{
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (Nip.getText().trim().isEmpty() || NamaPetugas.getText().trim().
            ...
            Sequel.mengedit("audit_kepatuhan_apd",
                    "nik=? and tindakan=? and tanggal=?",
                    "nik=?,tindakan=?,tanggal=?,topi=?,masker=?,kacamata=?,sarungtangan=?,apron=?,sepatu=?",
                    12, new String[]{
                        Nip.getText(), Tindakan.getText(), Valid.SetTgl(Tanggal.
                        getSelectedItem() + "") + " " + Jam.getSelectedItem() + ":" + Menit.
                        getSelectedItem() + ":" + Detik.getSelectedItem(), Topi.
                        getSelectedItem().toString(), Masker.getSelectedItem().
                                toString(), KacaMata.getSelectedItem().
                                toString(),
                        SarungTangan.getSelectedItem().toString(), Apron.
                        getSelectedItem().toString(), Sepatu.getSelectedItem().
                                toString(), tbObat.getValueAt(tbObat.
                                getSelectedRow(), 2).toString(), tbObat.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptAuditKepatuhanAPD.jasper", "report",
                        "::[ Data Audit Kepatuhan APD ]::",
                        ...
                        SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' order by audit_kepatuhan_apd.tanggal",
                        param);
            } else {
                Valid.MyReportqry("rptAuditKepatuhanAPD.jasper", "report",
                        "::[ Data Audit Kepatuhan APD ]::",
                        ...
                        + "and (audit_kepatuhan_apd.nik like '%" + TCari.
                                getText().trim() + "%' or pegawai.nama like '%" + TCari.
                                getText().trim() + "%' or "
                        + "pegawai.jbtn like '%" + TCari.getText().trim() + "%' audit_kepatuhan_apd.tindakan like '%" + TCari.
                        getText().trim() + "%') order by audit_kepatuhan_apd.tanggal",
                        param);
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgAuditKepatuhanAPD dialog = new DlgAuditKepatuhanAPD(
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                i = 1;
                while (rs.next()) {
                    topi = Double.parseDouble(rs.getString("topi").replaceAll(
                            "Ya", "1").replaceAll("Tidak", "0"));
                    ttltopi += topi;
                    masker = Double.parseDouble(rs.getString("masker").
                            replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlmasker += masker;
                    kacamata = Double.parseDouble(rs.getString("kacamata").
                            replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlkacamata += kacamata;
                    sarungtangan = Double.parseDouble(rs.getString(
                            "sarungtangan").replaceAll("Ya", "1").replaceAll(
                            "Tidak", "0"));
                    ttlsarungtangan += sarungtangan;
                    apron = Double.parseDouble(rs.getString("apron").replaceAll(
                            "Ya", "1").replaceAll("Tidak", "0"));
                    ttlapron += apron;
                    sepatu = Double.parseDouble(rs.getString("sepatu").
                            replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlsepatu += sepatu;
                    ttlpenilaian += (((topi + masker + kacamata + sarungtangan + apron + sepatu) / 6) * 100);
                    tabMode.addRow(new String[]{
                        rs.getString("tanggal"), rs.getString("tindakan"), rs.
                        getString("nik"), rs.getString("nama"), rs.getString(
                        "jbtn"), rs.getString("topi"),
                        rs.getString("masker"), rs.getString("kacamata"), rs.
                        getString("sarungtangan"), rs.getString("apron"), rs.
                        getString("sepatu"),
                        Math.round(
                        ((topi + masker + kacamata + sarungtangan + apron + sepatu) / 6) * 100) + " %"
                    });
                    i++;
                }
                i -= 1;
                if (i > 0) {
                    tabMode.addRow(new String[]{
                        "", "Ya", ":", "", "", "" + ttltopi, "" + ttlmasker,
                        "" + ttlkacamata, "" + ttlsarungtangan, "" + ttlapron,
                        "" + ttlsepatu,
                        "" + (ttltopi + ttlmasker + ttlkacamata + ttlsarungtangan + ttlapron + ttlsepatu)
                    });
                    tabMode.addRow(new String[]{
                        "", "Tidak", ":", "", "", "" + (i - ttltopi),
                        "" + (i - ttlmasker), "" + (i - ttlkacamata),
                        "" + (i - ttlsarungtangan), "" + (i - ttlapron),
                        "" + (i - ttlsepatu),
                        "" + ((i - ttltopi) + (i - ttlmasker) + (i - ttlkacamata) + (i - ttlsarungtangan) + (i - ttlapron) + (i - ttlsepatu))
                    });
                    tabMode.addRow(new String[]{
                        "", "Rata-rata", ":", "", "", Math.round(
                        (ttltopi / i) * 100) + " %", Math.round(
                        (ttlmasker / i) * 100) + " %", Math.round(
                        (ttlkacamata / i) * 100) + " %",
                        Math.round((ttlsarungtangan / i) * 100) + " %", Math.
                        round((ttlapron / i) * 100) + " %", Math.round(
                        (ttlsepatu / i) * 100) + " %", Math.round(
                        ttlpenilaian / i) + " %"
                    });
                }
            } catch (Exception e) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            if (!tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString().
                    isEmpty()) {
                Tindakan.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 1).
                        toString());
                Nip.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 2).
                        toString());
                NamaPetugas.setText(tbObat.
                        getValueAt(tbObat.getSelectedRow(), 3).toString());
                Departemen.setText(
                        tbObat.getValueAt(tbObat.getSelectedRow(), 4).toString());
                Topi.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),
                        5).toString());
                Masker.setSelectedItem(tbObat.
                        getValueAt(tbObat.getSelectedRow(), 6).toString());
                KacaMata.setSelectedItem(tbObat.getValueAt(tbObat.
                        getSelectedRow(), 7).toString());
                SarungTangan.setSelectedItem(tbObat.getValueAt(tbObat.
                        getSelectedRow(), 8).toString());
                Apron.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),
                        9).toString());
                Sepatu.setSelectedItem(tbObat.
    ...
    private static final Logger LOG = Logger.getLogger(
            DlgAuditKepatuhanAPD.class.getName());
}
