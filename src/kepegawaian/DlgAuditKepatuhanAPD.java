...
public class DlgAuditKepatuhanAPD extends javax.swing.JDialog {
    ...
    private double topi = 0, masker = 0, kacamata = 0, sarungtangan = 0, apron = 0, sepatu = 0, ttltopi = 0,
            ttlmasker = 0, ttlkacamata = 0, ttlsarungtangan = 0, ttlapron = 0, ttlsepatu = 0, ttlpenilaian = 0;
    ...
    public DlgAuditKepatuhanAPD(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Tanggal Audit", "Tindakan", "NIP/Kode", "Dokter/Paramedis", "Profesi/Jabatan", "Topi",
                    "Masker", "Kacamata", "Sarung Tangan", "Apron", "Sepatu", "Ttl.Nilai(%)"}) {
            @Override
        ...
        petugas.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if (petugas.getTable().getSelectedRow() != -1) {
                    ...
                    Departemen
                            .setText(petugas.getTable().getValueAt(petugas.getTable().getSelectedRow(), 3).toString());
                }
                Nip.requestFocus();
            }
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    ttlmasker += masker;
                    kacamata = Double
                            .parseDouble(rs.getString("kacamata").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlkacamata += kacamata;
                    sarungtangan = Double
                            .parseDouble(rs.getString("sarungtangan").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ...
                    tabMode.addRow(new String[]{rs.getString("tanggal"), rs.getString("tindakan"),
                        rs.getString("nik"), rs.getString("nama"), rs.getString("jbtn"), rs.getString("topi"),
                        rs.getString("masker"), rs.getString("kacamata"), rs.getString("sarungtangan"),
                        rs.getString("apron"), rs.getString("sepatu"),
                ...
                i -= 1;
                if (i > 0) {
                    tabMode.addRow(new String[]{"", "Ya", ":", "", "", "" + ttltopi, "" + ttlmasker, "" + ttlkacamata,
                        "" + ttlsarungtangan, "" + ttlapron, "" + ttlsepatu,
                        "" + (ttltopi + ttlmasker + ttlkacamata + ttlsarungtangan + ttlapron + ttlsepatu)});
                    tabMode.addRow(new String[]{"", "Tidak", ":", "", "", "" + (i - ttltopi), "" + (i - ttlmasker),
                        "" + (i - ttlkacamata), "" + (i - ttlsarungtangan), "" + (i - ttlapron),
                        "" + (i - ttlsepatu), "" + ((i - ttltopi) + (i - ttlmasker) + (i - ttlkacamata)
                        + (i - ttlsarungtangan) + (i - ttlapron) + (i - ttlsepatu))});
                    tabMode.addRow(new String[]{"", "Rata-rata", ":", "", "", Math.round((ttltopi / i) * 100) + " %",
                        Math.round((ttlmasker / i) * 100) + " %", Math.round((ttlkacamata / i) * 100) + " %",
                        Math.round((ttlsarungtangan / i) * 100) + " %", Math.round((ttlapron / i) * 100) + " %",
                        Math.round((ttlsepatu / i) * 100) + " %", Math.round(ttlpenilaian / i) + " %"});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgAuditKepatuhanAPD.class.getName());
