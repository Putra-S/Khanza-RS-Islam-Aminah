...
public class DlgTemporaryPresensi extends javax.swing.JDialog {
    ...
    public DlgTemporaryPresensi(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        Object[] row = {"P", "Id", "NIK", "Nama", "Shift", "Jam Datang", "Saat Ini", "Status", "Keterlambatan",
            "Durasi", "Photo"};
    ...
    public void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("SELECT pegawai.id, pegawai.nik, pegawai.nama, temporary_presensi.shift, "
                    + "temporary_presensi.jam_datang, now() as jam_pulang, temporary_presensi.status,  "
                    + "temporary_presensi.keterlambatan, ((unix_timestamp(now()) - unix_timestamp(jam_datang))/3600) as durasi,temporary_presensi.photo  from pegawai  "
                    + "inner join temporary_presensi on pegawai.id=temporary_presensi.id "
                    + "where  pegawai.nik like ? or " + "pegawai.nama like ? or "
                    + "temporary_presensi.shift like ? or " + "temporary_presensi.jam_datang like ? or "
                    + "temporary_presensi.status like ? or " + "temporary_presensi.keterlambatan like ?  ");
            try {
    ...
    private static final Logger LOG = Logger.getLogger(DlgTemporaryPresensi.class.getName());
