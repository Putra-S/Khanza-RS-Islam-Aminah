...
public class DlgAuditBundleIDO extends javax.swing.JDialog {
    ...
    public DlgAuditBundleIDO(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Tanggal Audit", "ID Ruang", "Ruang/Unit",
                    "1.Pencukuran Rambut Yang Mengganggu Jalannya Operasi", "2.Antibiotik",
                    "3.Temperatur (Suhu Pasien)", "4.Sugar (Gula Darah Pasien)", "Ttl.Nilai(%)"}) {
            @Override
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                i = 1;
                while (rs.next()) {
                    pencukuran_rambut = Double
                            .parseDouble(rs.getString("pencukuran_rambut").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlpencukuran_rambut += pencukuran_rambut;
                    antibiotik = Double
                            .parseDouble(rs.getString("antibiotik").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlantibiotik += antibiotik;
                    temperature = Double
                            .parseDouble(rs.getString("temperature").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ...
                    tabMode.addRow(new String[]{rs.getString("tanggal"), rs.getString("id_ruang"),
                        rs.getString("nama_ruang"), rs.getString("pencukuran_rambut"), rs.getString("antibiotik"),
                        rs.getString("temperature"), rs.getString("sugar"),
                        Math.round(((pencukuran_rambut + antibiotik + temperature + sugar) / 4) * 100) + " %"});
                    i++;
                }
                i -= 1;
                if (i > 0) {
                    tabMode.addRow(new String[]{"", "Ya", ":", "" + ttlpencukuran_rambut, "" + ttlantibiotik,
                        "" + ttltemperature, "" + ttlsugar,
                        "" + (ttlpencukuran_rambut + ttlantibiotik + ttltemperature + ttlsugar)});
                    tabMode.addRow(
                            new String[]{"", "Tidak", ":", "" + (i - ttlpencukuran_rambut), "" + (i - ttlantibiotik),
                                "" + (i - ttltemperature), "" + (i - ttlsugar), "" + ((i - ttlpencukuran_rambut)
                                + (i - ttlantibiotik) + (i - ttltemperature) + (i - ttlsugar))});
                    tabMode.addRow(new String[]{"", "Rata-rata", ":",
                        Math.round((ttlpencukuran_rambut / i) * 100) + " %",
                        Math.round((ttlantibiotik / i) * 100) + " %", Math.round((ttltemperature / i) * 100) + " %",
    ...
    private static final Logger LOG = Logger.getLogger(DlgAuditBundleIDO.class.getName());
