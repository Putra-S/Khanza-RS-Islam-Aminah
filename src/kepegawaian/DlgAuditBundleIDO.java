 ...
 * @author perpustakaan
 */
public class DlgAuditBundleIDO extends javax.swing.JDialog {
    ...
    public DlgAuditBundleIDO(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            ...
            "1.Pencukuran Rambut Yang Mengganggu Jalannya Operasi",
            "2.Antibiotik", "3.Temperatur (Suhu Pasien)",
            "4.Sugar (Gula Darah Pasien)", "Ttl.Nilai(%)"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (KdRuang.getText().trim().isEmpty() || NmRuang.getText().trim().
            ...
            if (Sequel.menyimpantf("audit_bundle_ido", "?,?,?,?,?,?", "Data", 6,
                    new String[]{
                        Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.
                        getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                        getSelectedItem(), KdRuang.getText(), PencukuranRambut.
                        getSelectedItem().toString(),
                        Antibiotik.getSelectedItem().toString(), SuhuPasien.
                        getSelectedItem().toString(), GulaDarah.
                                getSelectedItem().toString()
                    }) == true) {
                tampil();
                emptTeks();
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (tbObat.getSelectedRow() != -1) {
            if (Sequel.queryu2tf(
                    "delete from audit_bundle_ido where id_ruang=? and tanggal=?",
                    2, new String[]{
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (KdRuang.getText().trim().isEmpty() || NmRuang.getText().trim().
            ...
            Sequel.mengedit("audit_bundle_ido", "id_ruang=? and tanggal=?",
                    "tanggal=?,id_ruang=?,pencukuran_rambut=?,antibiotik=?,temperature=?,sugar=?",
                    8, new String[]{
                        Valid.SetTgl(Tanggal.getSelectedItem() + "") + " " + Jam.
                        getSelectedItem() + ":" + Menit.getSelectedItem() + ":" + Detik.
                        getSelectedItem(), KdRuang.getText(), PencukuranRambut.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (TCari.getText().trim().isEmpty()) {
                Valid.MyReportqry("rptAuditBundleIDO.jasper", "report",
                        "::[ Data Audit Bundle IDO ]::",
                        ...
                        SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' order by audit_bundle_ido.tanggal",
                        param);
            } else {
                Valid.MyReportqry("rptAuditBundleIDO.jasper", "report",
                        "::[ Data Audit Bundle IDO ]::",
                        ...
                        + "and (audit_bundle_ido.id_ruang like '%" + TCari.
                                getText().trim() + "%' or ruang_audit_kepatuhan.nama_ruang like '%" + TCari.
                                getText().trim() + "%') order by audit_bundle_ido.tanggal",
                        param);
            }
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgAuditBundleIDO dialog = new DlgAuditBundleIDO(
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                i = 1;
                while (rs.next()) {
                    pencukuran_rambut = Double.parseDouble(rs.getString(
                            "pencukuran_rambut").replaceAll("Ya", "1").
                            replaceAll("Tidak", "0"));
                    ttlpencukuran_rambut += pencukuran_rambut;
                    antibiotik = Double.parseDouble(rs.getString("antibiotik").
                            replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlantibiotik += antibiotik;
                    temperature = Double.parseDouble(
                            rs.getString("temperature").replaceAll("Ya", "1").
                                    replaceAll("Tidak", "0"));
                    ttltemperature += temperature;
                    sugar = Double.parseDouble(rs.getString("sugar").replaceAll(
                    ...
                    tabMode.addRow(new String[]{
                        ...
                        "pencukuran_rambut"), rs.getString("antibiotik"),
                        rs.getString("temperature"), rs.getString("sugar"),
                        Math.round(
                        ((pencukuran_rambut + antibiotik + temperature + sugar) / 4) * 100) + " %"
                    });
                    i++;
                }
                i -= 1;
                if (i > 0) {
                    tabMode.addRow(new String[]{
                        "", "Ya", ":", "" + ttlpencukuran_rambut,
                        "" + ttlantibiotik, "" + ttltemperature, "" + ttlsugar,
                        "" + (ttlpencukuran_rambut + ttlantibiotik + ttltemperature + ttlsugar)
                    });
                    tabMode.addRow(new String[]{
                        "", "Tidak", ":", "" + (i - ttlpencukuran_rambut),
                        "" + (i - ttlantibiotik), "" + (i - ttltemperature),
                        "" + (i - ttlsugar), "" + ((i - ttlpencukuran_rambut)
                        + (i - ttlantibiotik) + (i - ttltemperature) + (i - ttlsugar))
                    });
                    tabMode.addRow(new String[]{
                        "", "Rata-rata", ":", Math.round(
                        (ttlpencukuran_rambut / i) * 100) + " %", Math.round(
                        (ttlantibiotik / i) * 100) + " %", Math.round(
                        (ttltemperature / i) * 100) + " %",
                        Math.round((ttlsugar / i) * 100) + " %", Math.round(
                        ttlpenilaian / i) + " %"
                    });
                }
            } catch (Exception e) {
        ...
        if (tbObat.getSelectedRow() != -1) {
            if (!tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString().
                ...
                NmRuang.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 2).
                        toString());
                PencukuranRambut.setSelectedItem(tbObat.getValueAt(tbObat.
                        getSelectedRow(), 3).toString());
                Antibiotik.setSelectedItem(tbObat.getValueAt(tbObat.
                        getSelectedRow(), 4).toString());
                SuhuPasien.setSelectedItem(tbObat.getValueAt(tbObat.
                        getSelectedRow(), 5).toString());
                GulaDarah.setSelectedItem(tbObat.getValueAt(tbObat.
    ...
    private static final Logger LOG = Logger.getLogger(DlgAuditBundleIDO.class.
            getName());
}
