...
public class DlgAuditBundleIADP extends javax.swing.JDialog {
    ...
    private double handhygiene = 0, apd = 0, skin_antiseptik = 0, lokasi_iv = 0, perawatan_rutin = 0,
            ttlhandhygiene = 0, ttlapd = 0, ttlskin_antiseptik = 0, ttllokasi_iv = 0, ttlperawatan_rutin = 0,
            ttlpenilaian = 0;
    ...
    public DlgAuditBundleIADP(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Tanggal Audit", "NIP/Kode", "Dokter/Paramedis",
            "1.Handhygiene", "2.APD", "3.Skin Antiseptik", "4.Lokasi IV", "5.Perawatan Rutin", "Ttl.Nilai(%)"}) {
            @Override
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    handhygiene = Double
                            .parseDouble(rs.getString("handhygiene").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlhandhygiene += handhygiene;
                    apd = Double.parseDouble(rs.getString("apd").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlapd += apd;
                    skin_antiseptik = Double
                            .parseDouble(rs.getString("skin_antiseptik").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttlskin_antiseptik += skin_antiseptik;
                    lokasi_iv = Double
                            .parseDouble(rs.getString("lokasi_iv").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ttllokasi_iv += lokasi_iv;
                    perawatan_rutin = Double
                            .parseDouble(rs.getString("perawatan_rutin").replaceAll("Ya", "1").replaceAll("Tidak", "0"));
                    ...
                    tabMode.addRow(new String[]{rs.getString("tanggal"), rs.getString("nik"), rs.getString("nama"),
                        rs.getString("handhygiene"), rs.getString("apd"), rs.getString("skin_antiseptik"),
                        rs.getString("lokasi_iv"), rs.getString("perawatan_rutin"),
                ...
                i -= 1;
                if (i > 0) {
                    tabMode.addRow(new String[]{"", "Ya", ":", "" + ttlhandhygiene, "" + ttlapd,
                        "" + ttlskin_antiseptik, "" + ttllokasi_iv, "" + ttlperawatan_rutin,
                        "" + (ttlhandhygiene + ttlapd + ttlskin_antiseptik + ttllokasi_iv + ttlperawatan_rutin)});
                    tabMode.addRow(new String[]{"", "Tidak", ":", "" + (i - ttlhandhygiene), "" + (i - ttlapd),
                        "" + (i - ttlskin_antiseptik), "" + (i - ttllokasi_iv), "" + (i - ttlperawatan_rutin),
                        "" + ((i - ttlhandhygiene) + (i - ttlapd) + (i - ttlskin_antiseptik) + (i - ttllokasi_iv)
                        + (i - ttlperawatan_rutin))});
                    tabMode.addRow(new String[]{"", "Rata-rata", ":", Math.round((ttlhandhygiene / i) * 100) + " %",
                        Math.round((ttlapd / i) * 100) + " %", Math.round((ttlskin_antiseptik / i) * 100) + " %",
                        Math.round((ttllokasi_iv / i) * 100) + " %",
                        Math.round((ttlperawatan_rutin / i) * 100) + " %", Math.round(ttlpenilaian / i) + " %"});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgAuditBundleIADP.class.getName());
