 ...
 * @author Via
 */
public class grafikpiutangtersedikit extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        try {
            ...
            ResultSet rs = stat.executeQuery(
                    "SELECT databarang.nama_brng,sum(detailpiutang.jumlah),sum(detailpiutang.total) from piutang inner join detailpiutang inner join databarang "
                    + " on piutang.nota_piutang=detailpiutang.nota_piutang and detailpiutang.kode_brng=databarang.kode_brng where "
                    + symbol
                    + " group by databarang.nama_brng order by sum(detailpiutang.jumlah) asc limit 10");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Grafik 10 Barang Piutang Tersedikit");
    ...
    private static final Logger LOG = Logger.getLogger(grafikpiutangtersedikit.class.getName());
