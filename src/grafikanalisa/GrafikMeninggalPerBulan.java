...
public class GrafikMeninggalPerBulan extends javax.swing.JDialog {
    ...
    public GrafikMeninggalPerBulan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Bulan", "Jumlah Meninggal Per Bulan", "Persentase Per Bulan(%)"}) {
            @Override
    ...
    private void BtnPrint3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint3ActionPerformed
        ...
        try {
            rs = koneksi.prepareStatement(
                    "select DATE_FORMAT(pasien_mati.tanggal, '%Y-%m'),count(DATE_FORMAT(pasien_mati.tanggal, '%Y-%m')) as jumlah "
                    + "from pasien_mati where tanggal between '" + Valid.SetTgl(
                            Tanggal1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(Tanggal2.getSelectedItem() + "") + "' group by DATE_FORMAT(pasien_mati.tanggal, '%Y-%m')").
                    executeQuery();
            while (rs.next()) {
        ...
        JFreeChart freeChart = ChartFactory.createBarChart(
                "Grafik Pasien Meninggal Per Bulan Periode " + Valid.SetTgl(
                        Tanggal1.getSelectedItem() + "") + " s.d. " + Valid.
                SetTgl(Tanggal2.getSelectedItem() + ""), "Bulan", "Jumlah", dcd,
                PlotOrientation.VERTICAL, true, true, true);
        ChartFrame cf = new ChartFrame("Grafik Pasien Meninggal Per Bulan",
                freeChart);
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    "select DATE_FORMAT(pasien_mati.tanggal, '%Y-%m'),count(DATE_FORMAT(pasien_mati.tanggal, '%Y-%m')) as jumlah "
                    + "from pasien_mati where tanggal between '" + Valid.SetTgl(Tanggal1.getSelectedItem() + "")
                    + "' and '" + Valid.SetTgl(Tanggal2.getSelectedItem() + "")
                    + "' group by DATE_FORMAT(pasien_mati.tanggal, '%Y-%m')");
            try {
                rs = ps.executeQuery();
                total = 0;
                while (rs.next()) {
    ...
    private static final Logger LOG = Logger.getLogger(GrafikMeninggalPerBulan.class.getName());
