 ...
 * @author Via
 */
public class grafikperiksaperjk extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Jenis Kelamin";
        String series2 = "Jenis Kelamin";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("select pasien.jk,count(pasien.jk) as jumlah "
                    + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis " + symbol
                    + " group by pasien.jk");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Jenis Kelamin");
    ...
    public grafikperiksaperjk(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikperiksaperjk.class.getName());
