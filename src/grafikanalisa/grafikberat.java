 ...
 * @author Via
 */
public class grafikberat extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Berat Badan Bayi";
        String series2 = "Berat Badan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("SELECT berat_badan,count(berat_badan) as jumlah  FROM pasien_bayi " + " "
                    + symbol + " group by berat_badan");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Berat Badan Bayi");
    ...
    private static final Logger LOG = Logger.getLogger(grafikberat.class.getName());
