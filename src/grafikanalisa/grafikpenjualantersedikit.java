...
public class grafikpenjualantersedikit extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        try {
            ps = koneksi.prepareStatement(
                    "SELECT  databarang.nama_brng,sum(detailjual.jumlah),sum(detailjual.total) from penjualan inner join detailjual on penjualan.nota_jual=detailjual.nota_jual "
                    + "inner join databarang on detailjual.kode_brng=databarang.kode_brng where penjualan.status='Sudah Dibayar' and "
                    + symbol + " group by databarang.nama_brng order by sum(detailjual.jumlah) asc limit 10");
            rs = ps.executeQuery();
            try {
                while (rs.next()) {
    ...
    private static final Logger LOG = Logger.getLogger(grafikpenjualantersedikit.class.getName());
