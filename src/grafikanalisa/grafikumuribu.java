 ...
 * @author Via
 */
public class grafikumuribu extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Umur Ibu";
        String series2 = "Umur";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat
                    .executeQuery("SELECT umur,count(umur) as jumlah  FROM pasien_ibu " + " " + symbol + " group by umur");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Umur Ibu");
    ...
    public grafikumuribu(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikumuribu.class.getName());
