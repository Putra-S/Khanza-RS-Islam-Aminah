 ...
 * @author Via
 */
public class grafikperiksaperhari extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Tanggal";
        String series2 = "Tanggal";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery(
                    "select DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m-%d'),count(DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m-%d')) as jumlah "
                    + "from reg_periksa  " + symbol
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Tanggal Periksa");
     ...
     * @param symbol
     */
    public grafikperiksaperhari(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikperiksaperhari.class.getName());
