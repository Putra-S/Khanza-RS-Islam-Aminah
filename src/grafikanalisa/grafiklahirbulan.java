...
public class grafiklahirbulan extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Kelahiran Perbulan Bayi";
        String series2 = "Kelahiran Perbulan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery(
                    "SELECT DATE_FORMAT(tgl_lahir, '%Y-%m'),count(DATE_FORMAT(tgl_lahir, '%Y-%m')) as jumlah  FROM pasien_bayi "
                    + "inner join pasien on pasien.no_rkm_medis=pasien_bayi.no_rkm_medis" + " " + symbol
                    + " group by DATE_FORMAT(tgl_lahir, '%Y-%m')");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Kelahiran Perbulan Bayi");
    ...
    private static final Logger LOG = Logger.getLogger(grafiklahirbulan.class.getName());
