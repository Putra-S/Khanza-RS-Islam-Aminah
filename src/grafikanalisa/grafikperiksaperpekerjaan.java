 ...
 * @author Via
 */
public class grafikperiksaperpekerjaan extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Pekerjaan";
        String series2 = "Pekerjaan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("select pasien.pekerjaan,count(pasien.pekerjaan) as jumlah "
                    + "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis " + symbol
                    + " group by pasien.pekerjaan");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Pekerjaan Pasien");
    ...
    private static final Logger LOG = Logger.getLogger(grafikperiksaperpekerjaan.class.getName());
