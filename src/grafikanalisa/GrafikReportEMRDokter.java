...
public class GrafikReportEMRDokter extends javax.swing.JDialog {
    ...
    private void BtnPrint3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint3ActionPerformed
        ...
        try {
            rs = koneksi.prepareStatement("SELECT dokter.nm_dokter,\n"
                    ...
                    + "	FROM resep_obat \n"
                    + "	WHERE resep_obat.tgl_peresepan BETWEEN '" + Valid.
                    ...
                    + "	AND dokter.STATUS = '1' \n"
                    + "GROUP BY\n"
                    + "	reg_periksa.kd_dokter order by RESEPNONRACIK DESC ").
                    executeQuery();
            while (rs.next()) {
        ...
        JFreeChart freeChart = ChartFactory.createBarChart(
                "Input Resep Dokter Periode " + Valid.SetTgl(Tanggal1.
                        getSelectedItem() + "") + " s.d. " + Valid.SetTgl(
                        Tanggal2.getSelectedItem() + ""), "Bulan", "Jumlah", dcd,
    ...
    private void BtnPrint4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint4ActionPerformed
        grafiksql2 kas = new grafiksql2("Report EMR Dokter " + Valid.SetTgl(
                Tanggal1.getSelectedItem() + "") + " s.d. " + Valid.SetTgl(
                ...
                + "BETWEEN '" + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "'and '" + Valid.
                SetTgl(Tanggal2.getSelectedItem() + "") + "' AND pemeriksaan_ralan.nip = reg_periksa.kd_dokter ) AS CPPTRALANDOKTER \n"
                + "FROM reg_periksa INNER JOIN dokter ON reg_periksa.kd_dokter = dokter.kd_dokter "
                + "where reg_periksa.status_bayar = 'Sudah Bayar'  AND dokter.STATUS = '1'  AND reg_periksa.tgl_registrasi between '" + Valid.
                        SetTgl(Tanggal1.getSelectedItem() + "") + "' and '" + Valid.
                SetTgl(Tanggal2.getSelectedItem() + "") + "' "
                + "group by reg_periksa.kd_dokter order by CPPTRALANDOKTER DESC ",
                "NIP");
    ...
    private void BtnPrint5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint5ActionPerformed
        ...
        try {
            rs = koneksi.prepareStatement(
                    "SELECT dokter.nm_dokter, count( reg_periksa.no_rawat ) AS jumlahpasien "
                    + "from reg_periksa INNER JOIN dokter ON reg_periksa.kd_dokter = dokter.kd_dokter \n"
                    + "WHERE reg_periksa.tgl_registrasi between '" + Valid.
                            SetTgl(Tanggal1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(Tanggal2.getSelectedItem() + "") + "' group by reg_periksa.kd_dokter").
                    executeQuery();
            while (rs.next()) {
        ...
        JFreeChart freeChart = ChartFactory.createPieChart(
                "Jumlah Pasien Periode " + Valid.SetTgl(Tanggal1.
                        getSelectedItem() + "") + " s.d. " + Valid.SetTgl(
                        Tanggal2.getSelectedItem() + ""), dpd, true, true, false); //String title,PieDatasheet datasheet,boolean legends,boolean tooltips,boolean url
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("periode",
                    "Periode " + Tanggal1.getSelectedItem() + " s.d. " + Tanggal2.
                    getSelectedItem());
            ...
            Valid.MyReport("rptMeninggalPerBulan.jasper", "report",
                    "::[ Laporan Jumlah Pasien Meninggal Per Bulan ]::", param);
        }
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("SELECT " + "reg_periksa.kd_dokter," + "dokter.nm_dokter,"
                    + "count( reg_periksa.no_rawat ) AS jumlahpasien,"
                    + "(SELECT count( penilaian_medis_igd.no_rawat ) FROM penilaian_medis_igd WHERE penilaian_medis_igd.tanggal BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "AND penilaian_medis_igd.kd_dokter = reg_periksa.kd_dokter ) AS awalmedisigd,"
                    + "((SELECT	count( penilaian_medis_ranap.no_rawat )	FROM	penilaian_medis_ranap WHERE	penilaian_medis_ranap.tanggal BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "AND penilaian_medis_ranap.kd_dokter = reg_periksa.kd_dokter) +"
                    + "(SELECT count( penilaian_medis_ranap_kandungan.no_rawat ) FROM penilaian_medis_ranap_kandungan WHERE penilaian_medis_ranap_kandungan.tanggal BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "AND penilaian_medis_ranap_kandungan.kd_dokter = reg_periksa.kd_dokter )) AS AWRANAP,"
                    + "(SELECT count( pemeriksaan_ralan.no_rawat ) FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.tgl_perawatan BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "AND pemeriksaan_ralan.nip = reg_periksa.kd_dokter ) AS CPPTRALANDOKTER,"
                    + "(SELECT count( pemeriksaan_ranap.no_rawat ) FROM pemeriksaan_ranap WHERE pemeriksaan_ranap.tgl_perawatan BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "AND pemeriksaan_ranap.nip = reg_periksa.kd_dokter ) AS CPPTRANAP,"
                    + "(SELECT	count( resep_obat.no_resep ) FROM	resep_obat WHERE resep_obat.tgl_peresepan BETWEEN '"
                    + Valid.SetTgl(Tanggal1.getSelectedItem() + "") + "' AND '"
                    + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    ...
                    + "        reg_periksa.tgl_registrasi BETWEEN '" + Valid.SetTgl(Tanggal1.getSelectedItem() + "")
                    + "' AND '" + Valid.SetTgl(Tanggal2.getSelectedItem() + "") + "'"
                    + "        AND reg_periksa.status_bayar = 'Sudah Bayar' " + "        AND dokter.STATUS = '1' "
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode.addRow(new String[]{rs.getString("kd_dokter"), rs.getString("nm_dokter"),
                        rs.getString("jumlahpasien"), rs.getString("awalmedisigd"), rs.getString("CPPTRALANDOKTER"),
                        rs.getString("AWRANAP"), rs.getString("CPPTRANAP"), rs.getString("RESEPNONRACIK")
    ...
    private static final Logger LOG = Logger.getLogger(GrafikReportEMRDokter.class.getName());
