 ...
 * @author Via
 */
public class grafikpembeliantersedikit extends JDialog {
     ...
     * @return A dataset.
     */
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        // dan D

        DefaultCategoryDataset result = new DefaultCategoryDataset();
        String series1 = "Barang Terbeli";
        ...
        try {
            ...
            ResultSet rs = stat.executeQuery(
                    "SELECT databarang.nama_brng,sum(detailbeli.jumlah),sum(detailbeli.subtotal) from pembelian inner join detailbeli inner join databarang"
                    + " on pembelian.no_faktur=detailbeli.no_faktur and detailbeli.kode_brng=databarang.kode_brng where "
                    + symbol + " group by databarang.nama_brng order by sum(detailbeli.jumlah) asc limit 10");
            while (rs.next()) {
                String tksbr = rs.getString(1) + "(" + df2.format(rs.getDouble(2)) + "; Rp"
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        NumberAxis rangeAxis2 = new NumberAxis("Barang Terbeli");
        ...
        CategoryAxis domainAxis = new CategoryAxis("Grafik 10 Barang Pembelian Tersedikit");
    ...
    private static final Logger LOG = Logger.getLogger(grafikpembeliantersedikit.class.getName());
