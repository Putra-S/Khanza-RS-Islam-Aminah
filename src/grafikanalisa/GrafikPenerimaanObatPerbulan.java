...
public class GrafikPenerimaanObatPerbulan extends javax.swing.JDialog {
    ...
    public GrafikPenerimaanObatPerbulan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Bulan Penerimaan", "Nilai Penerimaan Per Bulan",
            "Persentase Penerimaan Per Bulan(%)", "Nilai Penerimaan"}) {
            @Override
    ...
    private void BtnPrint3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint3ActionPerformed
        ...
        try {
            rs = koneksi.prepareStatement(
                    "select DATE_FORMAT(pemesanan.tgl_pesan, '%y-%m'),sum(pemesanan.tagihan) as jumlah "
                    + "from pemesanan where tgl_pesan between '" + Valid.SetTgl(
                            Tanggal1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(Tanggal2.getSelectedItem() + "") + "' group by DATE_FORMAT(pemesanan.tgl_pesan, '%y-%m')").
                    executeQuery();
            while (rs.next()) {
        ...
        JFreeChart freeChart = ChartFactory.createBarChart(
                "Grafik Penerimaan Obat, Alkes & BHP Per Bulan Tanggal " + Valid.
                        SetTgl(Tanggal1.getSelectedItem() + "") + " S.D. " + Valid.
                SetTgl(Tanggal2.getSelectedItem() + ""), "Bulan",
                "Nilai Penerimaan", dcd, PlotOrientation.VERTICAL, true, true,
                true);
        ChartFrame cf = new ChartFrame(
                "Grafik Penerimaan Obat, Alkes & BHP Per Bulan", freeChart);
    ...
    private void BtnPrint4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint4ActionPerformed
        grafiksql2 kas = new grafiksql2(
                ...
                + "where tgl_pesan between '" + Valid.SetTgl(Tanggal1.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReport("rptPenerimaanObatPerBulan.jasper", "report",
                    "::[ Laporan Penerimaan Obat, Alkes & BHP Per Bulan ]::",
                    param);
        }
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi
                    .prepareStatement("select DATE_FORMAT(pemesanan.tgl_pesan, '%y-%m'),sum(pemesanan.tagihan) as jumlah "
                            + "from pemesanan where tgl_pesan between '" + Valid.SetTgl(Tanggal1.getSelectedItem() + "")
                            + "' and '" + Valid.SetTgl(Tanggal2.getSelectedItem() + "")
                            + "' group by DATE_FORMAT(pemesanan.tgl_pesan, '%y-%m')");
            try {
                rs = ps.executeQuery();
                total = 0;
                while (rs.next()) {
                    ...
                    tabMode
                            .addRow(new String[]{rs.getString(1), Valid.SetAngka(rs.getDouble(2)), "", rs.getString(2)});
                }
                tabMode.addRow(new String[]{"Total : ", Valid.SetAngka(total) + "", "100 %", total + ""});
                for (i = 0; i < tbBangsal.getRowCount(); i++) {
                    tbBangsal.setValueAt(
                            Math.round((Double.parseDouble(tbBangsal.getValueAt(i, 3).toString()) / total) * 100)
                            + " %",
                            i, 2);
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(GrafikPenerimaanObatPerbulan.class.getName());
