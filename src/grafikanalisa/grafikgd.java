 ...
 * @author Via
 */
public class grafikgd extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Golongan Darah Ibu";
        String series2 = "Golongan Darah";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("SELECT gol_darah,count(gol_darah) as jumlah  FROM pasien_ibu " + " "
                    + symbol + " group by gol_darah");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Golongan Darah Ibu");
     ...
     * @param symbol
     */
    public grafikgd(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikgd.class.getName());
