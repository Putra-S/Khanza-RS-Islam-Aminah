...
public class grafikjkelbayi extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        try {
            ...
            ResultSet rs = stat.executeQuery("SELECT jk,count(jk) as jumlah  FROM pasien inner join pasien_bayi "
                    + "on pasien.no_rkm_medis=pasien_bayi.no_rkm_medis " + symbol + " group by jk");
            while (rs.next()) {
    ...
    private static final Logger LOG = Logger.getLogger(grafikjkelbayi.class.getName());
