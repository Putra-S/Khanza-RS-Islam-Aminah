 ...
 * @author Via
 */
public class grafikibu extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Status Keinginan";
        String series2 = "Pendidikan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("SELECT stts_ingin,count(stts_ingin) as ingin  FROM pasien_ibu " + " "
                    + symbol + " group by stts_ingin");
            while (rs.next()) {
                String tksbr = rs.getString(1) + "(" + rs.getString(2) + ")";
                double luas = rs.getDouble(2);
                double jml = rs.getDouble(2);

                // result.addValue(luas, series1,tksbr);
                result.addValue(jml, series1, tksbr);
            }
        } catch (Exception e) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Status Keinginan");
    ...
    public grafikibu(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikibu.class.getName());
