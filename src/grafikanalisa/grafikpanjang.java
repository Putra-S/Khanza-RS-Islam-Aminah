 ...
 * @author Via
 */
public class grafikpanjang extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Panjang Badan Bayi";
        String series2 = "Panjang Badan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery("SELECT panjang_badan,count(panjang_badan) as jumlah  FROM pasien_bayi "
                    + " " + symbol + " group by panjang_badan");
            while (rs.next()) {
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Panjang Badan Bayi");
    ...
    private static final Logger LOG = Logger.getLogger(grafikpanjang.class.getName());
