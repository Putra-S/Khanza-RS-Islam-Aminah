 ...
 * @author Via
 */
public class grafikperiksaperbulan extends JDialog {
    ...
    public static CategoryDataset createDataset1(String symbol) { // data grafik nilai K
        ...
        String series1 = "Bulan";
        String series2 = "Bulan";

        try {
            Statement stat = koneksiDB.condb().createStatement();
            ResultSet rs = stat.executeQuery(
                    "select DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m'),count(DATE_FORMAT(reg_periksa.tgl_registrasi, '%y-%m')) as jumlah "
    ...
    private static JFreeChart createChart(String symbol) {
        ...
        CategoryAxis domainAxis = new CategoryAxis("Bulan Periksa");
    ...
    public grafikperiksaperbulan(String title, String symbol) {
        // super(title);
        setTitle(title);
    ...
    private static final Logger LOG = Logger.getLogger(grafikperiksaperbulan.class.getName());
