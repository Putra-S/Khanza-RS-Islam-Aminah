...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPenjualanDarah.jasper", "report",
    ...
    private void ppCetakNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppCetakNotaActionPerformed
        ...
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            Valid.panggilUrl("billing/NotaDarah2.php?nopenyerahan=" + tbDokter.
                    getValueAt(tbDokter.getSelectedRow(), 0).toString().trim() + "&usere=" + koneksiDB.
            ...
            if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                if (reply == JOptionPane.YES_OPTION) {
                    ...
                    try {
                        ...
                        try {
                            psdarah.setString(1, tbDokter.getValueAt(tbDokter.
                                    getSelectedRow(), 0).toString());
                            rs2 = psdarah.executeQuery();
                            while (rs2.next()) {
                                Sequel.
                                        mengedit("utd_stok_darah",
                                                "no_kantong=?", "status='Ada'",
                                                1,
                                                new String[]{rs2.getString(1)});
                            }
                        ...
                        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 5).
                                toString().equals("Sudah Dibayar")) {
                            pscekmedis = koneksi.prepareStatement(sqlpscekmedis);
                            try {
                                pscekmedis.setString(1, tbDokter.getValueAt(
                            ...
                            psceknonmedis = koneksi.prepareStatement(
                                    sqlpsceknonmedis);
                            try {
                                psceknonmedis.setString(1, tbDokter.getValueAt(
                                        tbDokter.getSelectedRow(), 0).toString());
                            ...
                            if (sukses == true) {
                                subtotalpendapatan = Sequel.cariIsiAngka(
                                        "select sum(total) from utd_penyerahan_darah_detail where no_penyerahan=?",
                                        tbDokter.getValueAt(tbDokter.
                                                getSelectedRow(), 0).toString());
                                Sequel.queryu("delete from tampjurnal");
                                Sequel.menyimpan("tampjurnal", "'" + Sequel.
                                        cariIsi("select Penyerahan_Darah from set_akun") + "','PENJUALAN DARAH UTD','" + subtotalpendapatan + "','0'",
                                        "Rekening");
                                Sequel.menyimpan("tampjurnal", "'" + Sequel.
                                        cariIsi("select kd_rek from utd_penyerahan_darah where no_penyerahan=?",
                                                tbDokter.getValueAt(tbDokter.
                                                        getSelectedRow(), 0).
                                                        toString()) + "','CARA BAYAR','0','" + subtotalpendapatan + "'",
                                        "Rekening");
                                sukses = jur.
                                        simpanJurnal(nopenyerahan.getText(), "U",
                                                "PEMBATALAN PENJUALAN DARAH DI UTD" + ", OLEH " + akses.
                                                        getkode());
                            }
                        }
                    } catch (SQLException e) {
                    ...
                    if (sukses == true) {
                        Sequel.meghapus("utd_penyerahan_darah", "no_penyerahan",
                                tbDokter.
                    ...
                    if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 5).
                        ...
                        if (sukses == true) {
                            ...
                            Sequel.menyimpan("tampjurnal", "'" + Sequel.cariIsi(
                                    "select Penyerahan_Darah from set_akun") + "','PENJUALAN DARAH UTD','0','" + subtotalpendapatan + "'",
                                    "Rekening");
                            Sequel.menyimpan("tampjurnal", "'" + Sequel.cariIsi(
                                    "select kd_rek from utd_penyerahan_darah where no_penyerahan=?",
                                    tbDokter.getValueAt(tbDokter.
                                            getSelectedRow(), 0).toString()) + "','CARA BAYAR','" + subtotalpendapatan + "','0'",
                                    "Rekening");
                        ...
                        if (sukses == true) {
                            Sequel.menyimpan("tagihan_sadewa", "'" + tbDokter.
                                    getValueAt(tbDokter.getSelectedRow(), 0).
                                    toString() + "','-','" + tbDokter.
                                            getValueAt(tbDokter.getSelectedRow(),
                                                    6).toString().
                                            replaceAll("'", "") + "','-',concat('" + tbDokter.
                                    getValueAt(tbDokter.getSelectedRow(), 1).
                                    toString()
                                    + "',' ',CURTIME()),'Pelunasan','" + subtotalpendapatan + "','" + subtotalpendapatan + "','Sudah','" + akses.
                                            getkode() + "'", "No.Nota");
                            Sequel.mengedit("utd_penyerahan_darah",
                                    "no_penyerahan=?", "status='Sudah Dibayar'",
                        ...
                        Sequel.AutoComitTrue();
                    } else if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 5).
                            toString().equals("Sudah Dibayar")) {
                        JOptionPane.showMessageDialog(rootPane,
                                "Maaf transaksi sudah diverifikasi..!!");
                    }
            ...
            if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                ...
                if (reply == JOptionPane.YES_OPTION) {
                    try {
                        ...
                        Sequel.meghapus("utd_penggunaan_medis_penyerahan_darah",
                                "no_penyerahan", tbDokter.getValueAt(tbDokter.
                                        getSelectedRow(), 0).toString());
