 ...
 * @author dosen
 */
public class InformasiTelusurKunjunganPasien extends javax.swing.JDialog {
    ...
    public InformasiTelusurKunjunganPasien(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No.R.M", "Nama Pasien", "No.SIM/KTP", "Umur", "J.K.",
            "Tmp.Lahir", "Tgl.Lahir", "Nama Ibu", "Alamat", "No.Telp"}) {
            @Override
        ...
        tabModeRegistrasi = new DefaultTableModel(null, new Object[]{"No.", "No.Rawat", "Tanggal", "Jam", "Kd.Dokter",
            "Dokter Dituju/DPJP", "Umur", "Poliklinik/Kamar", "Jenis Bayar", "Diagnosa Utama"}) {
            @Override
            public boolean isCellEditable(int rowIndex, int colIndex) {
                return false;
            }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            InformasiTelusurKunjunganPasien dialog = new InformasiTelusurKunjunganPasien(new javax.swing.JFrame(),
                    true);
        ...
        if (!TCari.getText().isEmpty()) {
            ...
            try {
                ...
                try {
                    ...
                    rs = ps.executeQuery();
                    while (rs.next()) {
                        tabMode.addRow(new String[]{rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                            rs.getString("no_ktp"), rs.getString("umur"), rs.getString("jk"),
                            rs.getString("tmp_lahir"), rs.getString("tgl_lahir"), rs.getString("nm_ibu"),
        ...
        if (tbKamar.getSelectedRow() != -1) {
            ...
            try {
                ...
                try {
                    ...
                    while (rs.next()) {
                        i++;
                        kdpenyakit = Sequel.cariIsi(
                                "select kd_penyakit from diagnosa_pasien where no_rawat=? and status='Ralan' and prioritas='1'",
                                rs.getString("no_rawat"));
                        namapenyakit = Sequel.cariIsi("select nm_penyakit from penyakit where kd_penyakit=?",
                                kdpenyakit);

                        tabModeRegistrasi.addRow(new String[]{i + "", rs.getString("no_rawat"),
                            rs.getString("tgl_registrasi"), rs.getString("jam_reg"), rs.getString("kd_dokter"),
                            rs.getString("nm_dokter"), rs.getString("umurdaftar") + " " + rs.getString("sttsumur"),
                            rs.getString("kd_poli") + " " + rs.getString("nm_poli"), rs.getString("png_jawab"),
                            kdpenyakit + " " + namapenyakit});
                        ps2 = koneksi.prepareStatement("select rujukan_internal_poli.kd_dokter,dokter.nm_dokter,"
                        ...
                        try {
                            ...
                            while (rs2.next()) {
                                i++;
                                tabModeRegistrasi.addRow(
                                        new String[]{i + "", rs.getString("no_rawat"), rs.getString("tgl_registrasi"),
                                            "", rs2.getString("kd_dokter"), rs2.getString("nm_dokter"),
                                            rs.getString("umurdaftar") + " " + rs.getString("sttsumur"),
                                            rs2.getString("kd_poli") + " " + rs2.getString("nm_poli"),
                                            rs.getString("png_jawab"), kdpenyakit + " " + namapenyakit});
                            }
                        } catch (Exception e) {
                            System.out.println("Notifikasi : " + e);
                        } finally {
                        ...
                        if (rs.getString("status_lanjut").equals("Ranap")) {
                            kddpjp = Sequel.cariIsi(
                                    "select dpjp_ranap.kd_dokter from dpjp_ranap where dpjp_ranap.no_rawat=?",
                                    rs.getString("no_rawat"));
                            if (!kddpjp.isEmpty()) {
                                dpjp = Sequel.cariIsi("select dokter.nm_dokter from dokter where dokter.kd_dokter=?",
                                        kddpjp);
                            } else {
                        ...
                        try {
                            ...
                            while (rs2.next()) {
                                i++;
                                tabModeRegistrasi.addRow(new String[]{i + "", rs.getString("no_rawat"),
                                    rs2.getString("tgl_masuk"), rs2.getString("jam_masuk"), kddpjp, dpjp,
                                    rs.getString("umurdaftar") + " " + rs.getString("sttsumur"),
                                    rs2.getString("kd_kamar") + " " + rs2.getString("nm_bangsal"),
                                    rs.getString("png_jawab"), kdpenyakit + " " + namapenyakit});
                            }
                        } catch (Exception e) {
                            System.out.println("Notifikasi : " + e);
    ...
    private static final Logger LOG = Logger.getLogger(InformasiTelusurKunjunganPasien.class.getName());
