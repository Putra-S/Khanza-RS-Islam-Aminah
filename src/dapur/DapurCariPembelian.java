    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPembelian.jasper", "report",
                    "::[ Transaksi Pembelian Barang Dapur Kering & Basah ]::",
        ...
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().trim().
            ...
            try {
                pscaribeli = koneksi.prepareStatement(
                        "select dapurpembelian.no_faktur,dapurpembelian.tagihan,dapurpembelian.tgl_beli,dapurpembelian.kd_rek,dapurpembelian.ppn,(dapurpembelian.total+dapurpembelian.meterai) as total from dapurpembelian where dapurpembelian.no_faktur=?");
                try {
                    ...
                    if (rs.next()) {
                        ...
                        sukses = true;
                        psdapurdetailbeli = koneksi.prepareStatement(
                                "select dapurdetailbeli.kode_brng,dapurdetailbeli.jumlah from dapurdetailbeli where dapurdetailbeli.no_faktur=? ");
                        try {
                        ...
                        if (rs.getDouble("ppn") > 0) {
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                        ...
                                        PPN_Masukan, "PPN Masukan Dapur", "0",
                                        rs.getString("ppn")
                                    });
                        }
                        Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                        ...
                        sukses = jur.
                                simpanJurnal(rs.getString("no_faktur"), "U",
                                        "PEMBATALAN PEMBELIAN BARANG DAPUR KERING & BASAH" + ", OLEH " + akses.
                                                getkode());
                        if (sukses == true) {
                            Sequel.queryu2(
                                    "delete from dapurpembelian where no_faktur=?",
