...
public class DapurPermintaan extends javax.swing.JDialog {
    ...
    public DapurPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Jml", "Kode Barang", "Nama Barang", "Satuan", "Jenis Barang", "Keterangan"};
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    private void tampil2() {
        try {
            ...
            for (i = 0; i < jml; i++) {
                tabMode.addRow(
                        new Object[]{jumlah[i], kodebarang[i], namabarang[i], satuan[i], jenis[i], keterangan[i]});
            }
                    ...
                    if (list.path("KodeBarang").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                        ...
                        tabMode.addRow(
                                new Object[]{"", list.path("KodeBarang").asText(), list.path("NamaBarang").asText(),
                                    list.path("Satuan").asText(), list.path("Jenis").asText(), ""});
                    }
                }
            }
            myObj.close();
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(permintaan_dapur.no_permintaan,3),signed)),0) from permintaan_dapur where permintaan_dapur.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PD" + Tanggal.getSelectedItem().toString().substring(8, 10)
                + Tanggal.getSelectedItem().toString().substring(3, 5)
                + Tanggal.getSelectedItem().toString().substring(0, 2),
                3, NoPermintaan);
    }

    private static final Logger LOG = Logger.getLogger(DapurPermintaan.class.getName());
