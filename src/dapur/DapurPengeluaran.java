...
public class DapurPengeluaran extends javax.swing.JDialog {
    ...
    public DapurPengeluaran(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Jml", "Kode Barang", "Nama Barang", "Satuan", "Stok",
            "Harga", "Total"};
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoKeluar.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("dapurpengeluaran", "?,?,?,?",
                        "No.Keluar", 4, new String[]{NoKeluar.getText(), Valid.
                            SetTgl(TglKeluar.getSelectedItem() + ""), kdptg.
                            getText(), Keterangan.getText()}) == true) {
                    jml = tbDokter.getRowCount();
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("dapurdetailpengeluaran",
                                    "?,?,?,?,?,?", "Transaksi Pengeluaran", 6,
                                    new String[]{
                                        NoKeluar.getText(), tbDokter.getValueAt(
                ...
                if (sukses == true) {
                    ...
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi("select Stok_Keluar_Dapur from set_akun"),
                        "PERSEDIAAN BARANG", "" + (ttl), "0"});
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi(
                        "select Kontra_Stok_Keluar_Dapur from set_akun"),
                        "KAS DI TANGAN", "0", "" + (ttl)});
                    sukses = jur.simpanJurnal(NoKeluar.getText(), "U",
                            "PENGGUNAAN BARANG DAPUR KERING DAN BASAH" + ", OLEH " + akses.
                                    getkode());
                }
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurPengeluaran dialog = new DapurPengeluaran(
    ...
    private void tampil() {
        ...
        for (i = 0; i < row; i++) {
            if (!tbDokter.getValueAt(i, 0).toString().isEmpty()) {
                ...
                harga[index] = Double.parseDouble(tbDokter.getValueAt(i, 5).
                        toString());
                total[index] = Double.parseDouble(tbDokter.getValueAt(i, 6).
                        toString());
                index++;
            }
        }
        ...
        for (i = 0; i < jml; i++) {
            tabMode.addRow(new Object[]{jumlah[i], kodebarang[i], namabarang[i],
                satuan[i], stok[i], harga[i], total[i]});
        }
        try {
    ...
    private void getData() {
        ...
        if (row != -1) {
            if (!tbDokter.getValueAt(row, 0).toString().isEmpty()) {
                try {
                    tbDokter.setValueAt(Double.parseDouble(tbDokter.getValueAt(
                            row, 5).toString()) * Double.parseDouble(tbDokter.
                                    getValueAt(row, 0).toString()), row, 6);
                } catch (Exception e) {
                    tbDokter.setValueAt("", row, 0);
                    tbDokter.setValueAt(0, row, 6);
                }
                if (Double.parseDouble(tbDokter.getValueAt(row, 0).toString()) > Double.
                        parseDouble(tbDokter.getValueAt(row, 4).toString())) {
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(dapurpengeluaran.no_keluar,3),signed)),0) from dapurpengeluaran where dapurpengeluaran.tanggal='" + Valid.
                        SetTgl(TglKeluar.getSelectedItem() + "") + "' ",
                "SKD" + TglKeluar.getSelectedItem().toString().substring(8, 10) + TglKeluar.
                getSelectedItem().toString().substring(3, 5) + TglKeluar.
    ...
    public void tampil(String nopermintaan, String keterangan) {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select dapurbarang.kode_brng, concat(dapurbarang.nama_brng,' (',dapurbarang.jenis,')'),"
                ...
                while (rs.next()) {
                    if (rs.getDouble("jumlah") > rs.getDouble("stok")) {
                        ...
                        tabMode.addRow(new Object[]{
                            ...
                            getString(3),
                            rs.getString(4), rs.getDouble(5), (rs.getDouble(
                            "jumlah") * rs.getDouble("harga"))
                        });
                    } else {
                        tabMode.addRow(new Object[]{
                            rs.getString("jumlah"), rs.getString(1), rs.
    ...
    private void isHitung() {
        ...
        for (i = 0; i < jml; i++) {
            ...
            try {
                keluar = Double.
                        parseDouble(tbDokter.getValueAt(i, 6).toString());
            } catch (Exception e) {
                keluar = 0;
            }
            ttl += keluar;
    ...
    private static final Logger LOG = Logger.getLogger(DapurPengeluaran.class.
            getName());
}
