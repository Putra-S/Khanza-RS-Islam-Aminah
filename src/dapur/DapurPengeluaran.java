...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if (NoKeluar.getText().trim().isEmpty()) {//GEN-FIRST:event_BtnSimpanActionPerformed
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    private void tampil() {
        ...
        for (i = 0; i < jml; i++) {
            tabMode.addRow(
                    new Object[]{jumlah[i], kodebarang[i], namabarang[i], satuan[i], stok[i], harga[i], total[i]});
        }
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{"", rs.getString(1), rs.getString(2), rs.getString(3),
                        rs.getString(4), rs.getDouble(5), 0});
                }
            } catch (Exception e) {
            ...
            if (!tbDokter.getValueAt(row, 0).toString().isEmpty()) {
                try {
                    tbDokter.setValueAt(Double.parseDouble(tbDokter.getValueAt(row, 5).toString())
                            * Double.parseDouble(tbDokter.getValueAt(row, 0).toString()), row, 6);
                } catch (Exception e) {
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(dapurpengeluaran.no_keluar,3),signed)),0) from dapurpengeluaran where dapurpengeluaran.tanggal='"
                + Valid.SetTgl(TglKeluar.getSelectedItem() + "") + "' ",
                "SKD" + TglKeluar.getSelectedItem().toString().substring(8, 10)
                + TglKeluar.getSelectedItem().toString().substring(3, 5)
                + TglKeluar.getSelectedItem().toString().substring(0, 2),
                3, NoKeluar);
    }
            ...
            try {
                ...
                while (rs.next()) {
                    if (rs.getDouble("jumlah") > rs.getDouble("stok")) {
                        ...
                        tabMode.addRow(new Object[]{rs.getString("jumlah"), rs.getString(1), rs.getString(2),
                            rs.getString(3), rs.getString(4), rs.getDouble(5),
                            (rs.getDouble("jumlah") * rs.getDouble("harga"))});
                    }
    ...
    private static final Logger LOG = Logger.getLogger(DapurPengeluaran.class.getName());
