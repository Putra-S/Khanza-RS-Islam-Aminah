    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoFaktur.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if (Sequel.menyimpantf2("dapurpembelian",
                        "?,?,?,?,?,?,?,?,?,?,?", "data", 11, new String[]{
                            NoFaktur.getText(), kdsup.getText(), kdptg.getText(),
                            Valid.SetTgl(TglBeli.getSelectedItem() + ""),
                            "" + sbttl, "" + ttldisk, "" + ttl, "" + ppn,
                            "" + meterai, "" + (ttl + ppn + meterai), akunbayar
                        }) == true) {
                    jml = tbDokter.getRowCount();
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("dapurdetailbeli",
                                    "?,?,?,?,?,?,?,?,?", "Transaksi Pembelian",
                                    9, new String[]{
                                ...
                                Trackbarang.catatRiwayat(tbDokter.getValueAt(i,
                                        ...
                                        "Pengadaan", akses.getkode(), "Simpan");
                                Sequel.mengedit("dapurbarang", "kode_brng=?",
                                        "stok=stok+?", 2, new String[]{
                                            tbDokter.getValueAt(i, 0).toString(),
                                            tbDokter.getValueAt(i, 1).toString()
                                        });
                                if (tbDokter.getValueAt(i, 4).toString().equals(
                                        "true") && (akses.getdapur_barang() == true)) {
                                    Sequel.
                                            mengedit("dapurbarang",
                                                    "kode_brng=?", "harga=?", 2,
                                                    new String[]{
                                                        (Double.parseDouble(
                                                                tbDokter.
                ...
                if (sukses == true) {
                    ...
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        akunpembelian, "PEMBELIAN", "" + (ttl + meterai), "0"});
                    if (ppn > 0) {
                        Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                new String[]{PPN_Masukan, "PPN Masukan Dapur",
                                    "" + ppn, "0"});
                    }
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        akunbayar, "KAS KELUAR", "0", "" + (ttl + ppn + meterai)});
                    sukses = jur.simpanJurnal(NoFaktur.getText(), "U",
                            "PEMBELIAN BARANG DAPUR KERING & BASAH " + ", OLEH " + akses.
                                    getkode());
                }
                if (sukses == true) {
                    Sequel.Commit();
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurPembelian dialog = new DapurPembelian(new javax.swing.JFrame(),
                    true);
    ...
    private void tampil() {
        try {
            ...
            fileWriter.write("{\"penerimaandapur\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(dapurpembelian.no_faktur,3),signed)),0) from dapurpembelian where dapurpembelian.tgl_beli='" + Valid.
                        SetTgl(TglBeli.getSelectedItem() + "") + "' ",
                "PD" + TglBeli.getSelectedItem().toString().substring(6, 10) + TglBeli.
    ...
    private static final Logger LOG = Logger.getLogger(DapurPembelian.class.
            getName());
