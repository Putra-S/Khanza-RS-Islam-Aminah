    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPembelian.jasper", "report",
                    "::[ Transaksi Pembelian Barang Non Medis dan Penunjang ( Lab & RO ) ]::",
        ...
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().trim().
            ...
            try {
                pscaribeli = koneksi.prepareStatement(
                        "select ipsrspembelian.no_faktur,ipsrspembelian.tagihan,ipsrspembelian.tgl_beli,ipsrspembelian.kd_rek,ipsrspembelian.ppn,(ipsrspembelian.total+ipsrspembelian.meterai) as total from ipsrspembelian where ipsrspembelian.no_faktur=?");
                try {
                    ...
                    if (rs.next()) {
                        ...
                        sukses = true;
                        psipsrsdetailbeli = koneksi.prepareStatement(
                                "select ipsrsdetailbeli.kode_brng,ipsrsdetailbeli.jumlah from ipsrsdetailbeli where ipsrsdetailbeli.no_faktur=? ");
                        try {
                        ...
                        if (rs.getDouble("ppn") > 0) {
                            Sequel.
                                    menyimpan("tampjurnal", "?,?,?,?",
                                        ...
                                        PPN_Masukan, "PPN Masukan IPSRS", "0",
                                        rs.getString("ppn")
                                    });
                        }
                        Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                        ...
                        sukses = jur.
                                simpanJurnal(rs.getString("no_faktur"), "U",
                                        "PEMBATALAN PEMBELIAN BARANG NON MEDIS DAN PENUNJANG (LAB & RAD)" + ", OLEH " + akses.
                                                getkode());
                        if (sukses == true) {
                            Sequel.queryu2(
                                    "delete from ipsrspembelian where no_faktur=?",
