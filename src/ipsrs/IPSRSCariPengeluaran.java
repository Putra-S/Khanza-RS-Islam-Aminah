    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if (TabRawat.getSelectedIndex() == 0) {
            ...
            if (tabMode.getRowCount() == 0) {
                ...
                for (i = 0; i < row; i++) {
                    Sequel.menyimpan("temporary", "'" + i + "','"
                            ...
                            getalamatip() + "'", "Transaksi Pengeluaran");
                }
                i++;
                ...
                Valid.MyReportqry("rptPengeluaranIpsrs.jasper", "report",
            ...
            if (tabMode2.getRowCount() == 0) {
                ...
                param.put("logo", Sequel.cariGambar(
                        "select setting.logo from setting"));
                Valid.MyReportqry("rptPengeluaranIpsrs2.jasper", "report",
                        "::[ Transaksi Pengeluaran Barang ]::",
                        ...
                        + "where ipsrspengeluaran.tanggal between '" + Valid.
                                SetTgl(TglBeli1.getSelectedItem() + "") + "' and '" + Valid.
                        SetTgl(TglBeli2.getSelectedItem() + "") + "' and ipsrspengeluaran.no_keluar like '%" + NoKeluar.
                        getText() + "%' and petugas.nama like '%" + nmptg.
                                getText() + "%'  and ipsrsbarang.jenis like '%" + kdjenis.
                                getText() + "%' and ipsrsbarang.nama_brng like '%" + nmbar.
                                getText() + "%' and "
                        + "(ipsrspengeluaran.no_keluar like '%" + TCari.
                                getText() + "%' or ipsrspengeluaran.keterangan like '%" + TCari.
                                getText() + "%' or ipsrspengeluaran.nip like '%" + TCari.
                                getText() + "%' or petugas.nama like '%" + TCari.
                                getText() + "%' or ipsrsbarang.jenis like '%" + TCari.
                                getText() + "%' or ipsrsdetailpengeluaran.kode_brng like '%" + TCari.
                                getText() + "%' or "
                        + "ipsrsbarang.nama_brng like '%" + TCari.getText() + "%' or ipsrsdetailpengeluaran.kode_sat like '%" + TCari.
                        getText() + "%' or kodesatuan.satuan like '%" + TCari.
                                getText() + "%') order by ipsrspengeluaran.tanggal,ipsrspengeluaran.no_keluar ",
                        param);
            }
            this.setCursor(Cursor.getDefaultCursor());
        }
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().trim().
            isEmpty()) {
        Valid.textKosong(TCari, tbDokter.
                getValueAt(tbDokter.getSelectedRow(), 1).toString());
    } else {
        try {
            psdetailpengeluaran = koneksi.prepareStatement(
                    "select kode_brng,jumlah,total from ipsrsdetailpengeluaran where no_keluar=? ");
            try {
                Sequel.AutoComitFalse();
                sukses = true;
                psdetailpengeluaran.setString(1, tbDokter.getValueAt(tbDokter.
                        getSelectedRow(), 1).toString());
                rs2 = psdetailpengeluaran.executeQuery();
                total = 0;
                while (rs2.next()) {
                    Trackbarang.catatRiwayat(rs2.getString("kode_brng"), rs2.
                            getDouble("jumlah"), 0, "Stok Keluar", akses.
                            getkode(), "Hapus");
                    Sequel.mengedit("ipsrsbarang", "kode_brng=?", "stok=stok+?",
                            2, new String[]{
                ...
                Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                    Sequel.cariIsi(
                    "select Kontra_Stok_Keluar_Ipsrs from set_akun"),
                    "KAS DI TANGAN", "" + total, "0"});
                sukses = jur.simpanJurnal(tbDokter.getValueAt(tbDokter.
                        getSelectedRow(), 1).toString(), "U",
                        "PEMBATALAN PENGGUNAAN BARANG NON MEDIS" + ", OLEH " + akses.
                                getkode());

                if (sukses == true) {
                    Sequel.queryu2(
                            "delete from ipsrspengeluaran where no_keluar=?", 1,
