    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoFaktur.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if (Sequel.menyimpantf2("ipsrspembelian",
                        "?,?,?,?,?,?,?,?,?,?,?", "data", 11, new String[]{
                    ...
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("ipsrsdetailbeli",
                                    "?,?,?,?,?,?,?,?,?", "Transaksi Pembelian",
                                    9, new String[]{
                ...
                if (sukses == true) {
                    ...
                    if (ppn > 0) {
                        Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                new String[]{PPN_Masukan, "PPN Masukan IPSRS",
                                    "" + ppn, "0"});
                    }
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        akunbayar, "KAS KELUAR", "0", "" + (ttl + ppn + meterai)});
                    sukses = jur.simpanJurnal(NoFaktur.getText(), "U",
                            "PEMBELIAN BARANG NON MEDIS DAN PENUNJANG(LAB & RAD) " + ", OLEH " + akses.
                                    getkode());
                }
                if (sukses == true) {
                    Sequel.Commit();
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            IPSRSPembelian dialog = new IPSRSPembelian(new javax.swing.JFrame(),
                    true);
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(ipsrspembelian.no_faktur,3),signed)),0) from ipsrspembelian where ipsrspembelian.tgl_beli='" + Valid.
                        SetTgl(TglBeli.getSelectedItem() + "") + "' ",
                "PI" + TglBeli.getSelectedItem().toString().substring(6, 10) + TglBeli.
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSPembelian.class.
            getName());
