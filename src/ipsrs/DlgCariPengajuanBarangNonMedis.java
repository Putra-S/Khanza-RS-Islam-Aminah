...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPengajuanBarangNonMedis.jasper", "report",
                    "::[ Data Pengajuan Permintaan/Pengadaan Barang Non Medis dan Penunjang ( Lab & RO ) ]::",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (Sequel.cariInteger(
            "select count(no_pengajuan) from pengajuan_barang_nonmedis where no_pengajuan=?",
            tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().trim()) == 0) {
        Valid.textKosong(TCari, "pilihan data");
    } else {
        Sequel.queryu(
                "delete from pengajuan_barang_nonmedis where no_pengajuan=?",
    ...
    private void ppProsesPengajuanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppProsesPengajuanActionPerformed
        if (Sequel.cariInteger(
            ...
            Sequel.queryu(
                    "update pengajuan_barang_nonmedis set status='Proses Pengajuan' where no_pengajuan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim());
            tampil();
        }
    ...
    private void ppDisetujuiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDisetujuiActionPerformed
        if (Sequel.cariInteger(
            ...
            if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 2).toString().
                ...
                Sequel.queryu(
                        "update pengajuan_barang_nonmedis set status='Disetujui' where no_pengajuan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).
                                toString().trim());
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                IPSRSSuratPemesanan aplikasi = new IPSRSSuratPemesanan(null,
                        false);
    ...
    private void ppDitolakActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDitolakActionPerformed
        if (Sequel.cariInteger(
            ...
            Sequel.queryu(
                    "update pengajuan_barang_nonmedis set status='Ditolak' where no_pengajuan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim());
            tampil();
        }
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi
                    .prepareStatement("select pengajuan_barang_nonmedis.tanggal,pengajuan_barang_nonmedis.no_pengajuan, "
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    ps2 = koneksi
                            .prepareStatement("select detail_pengajuan_barang_nonmedis.kode_brng,ipsrsbarang.nama_brng, "
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariPengajuanBarangNonMedis.class.getName());
