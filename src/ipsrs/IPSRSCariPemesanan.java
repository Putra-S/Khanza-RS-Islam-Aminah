...
public class IPSRSCariPemesanan extends javax.swing.JDialog {
    ...
    public IPSRSCariPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No.Faktur", "Suplier", "Petugas", "Jml", "Harga Beli(Rp)", "SubTotal(Rp)", "Disk(%)",
...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void kdbarKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdbarKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi(
                    "select ipsrsbarang.nama_brng from ipsrsbarang where ipsrsbarang.kode_brng=?",
                    nmbar, kdbar.getText());
        } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            Sequel.cariIsi(
                    "select ipsrsbarang.nama_brng from ipsrsbarang where ipsrsbarang.kode_brng=?",
    ...
    private void kdjenisKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdjenisKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi(
                    "select nm_jenis from ipsrsjenisbarang  where kd_jenis=?",
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPemesananIPSRS.jasper", "report",
                    "::[ Transaksi Penerimaan Barang Non Medis ]::",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
        ...
        try {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    sukses = true;
                    psipsrsdetailpesan = koneksi.prepareStatement(
                            "select ipsrsdetailpesan.kode_brng,ipsrsdetailpesan.jumlah from ipsrsdetailpesan where ipsrsdetailpesan.no_faktur=? ");
                    try {
                        psipsrsdetailpesan.setString(1, rs.
                                getString("no_faktur"));
                        rs2 = psipsrsdetailpesan.executeQuery();
                        while (rs2.next()) {
                            Trackbarang.catatRiwayat(rs2.getString("kode_brng"),
                                    0, rs2.getDouble("jumlah"), "Penerimaan",
                                    akses.getkode(), "Hapus");
                    ...
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                            new String[]{
                                Sequel.cariIsi(
                                        "select set_akun.Penerimaan_NonMedis from set_akun"),
                                "PERSEDIAAN BARANG NON MEDIS", "0", rs.
                                        getString("total")
                            });
                    if (rs.getDouble("ppn") > 0) {
                        Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                                new String[]{
                                    ...
                                    "PPN Masukan Barang Non Medis", "0", rs.
                                            getString("ppn")
                                });
                    }
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                            new String[]{
                                Sequel.cariIsi(
                                        "select set_akun.Kontra_Penerimaan_NonMedis from set_akun"),
                                "HUTANG BARANG NON MEDIS", rs.getString(
                                        "tagihan"), "0"
                            });
                    sukses = jur.simpanJurnal(rs.getString("no_faktur"), "U",
                            "BATAL TRANSAKSI PENERIMAAN BARANG NON MEDIS" + ", OLEH " + akses.
                                    getkode());

                    if (sukses == true) {
                        Sequel.queryu2(
                                "delete from ipsrspemesanan where no_faktur=?",
                                1, new String[]{tbDokter.getValueAt(tbDokter.
                                            getSelectedRow(), 0).toString()});
                        Sequel.Commit();
                        tampil();
                    } else {
    ...
    private void ppBayarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBayarActionPerformed
        if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 8).toString().trim().
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            KeuanganBayarPemesananNonMedis bayarpesan = new KeuanganBayarPemesananNonMedis(
                    null, false);
            bayarpesan.setData(
    ...
    private void btnAmbilPhotoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilPhotoActionPerformed
        if (tbDokter.getSelectedRow() != -1) {
            if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 8).toString().
                ...
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.meghapus("bukti_pemesanan_logistik", "no_faktur",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString());
                Sequel.menyimpan2("bukti_pemesanan_logistik", "?,''", 1,
                        new String[]{tbDokter.getValueAt(tbDokter.
                                    getSelectedRow(), 0).toString()});
                Valid.panggilUrl(
                        "penerimaanlogistik/login.php?act=login&usere=" + koneksiDB.
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("select ipsrspemesanan.tgl_pesan,ipsrspemesanan.no_faktur, "
                    ...
                    + " inner join ipsrsjenisbarang " + " on ipsrsdetailpesan.kode_brng=ipsrsbarang.kode_brng "
        ...
        if (FormPhoto.isVisible() == true) {
            try {
                ps = koneksi.prepareStatement(
                        "select bukti_pemesanan_logistik.photo from bukti_pemesanan_logistik where bukti_pemesanan_logistik.no_faktur=?");
                try {
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSCariPemesanan.class.getName());
