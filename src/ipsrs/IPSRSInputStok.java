...
public class IPSRSInputStok extends javax.swing.JDialog {
    ...
    public IPSRSInputStok(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Real", "Kode Barang", "Nama Barang", "Kategori", "Satuan",
            "Harga", "Stok", "Selisih", "Nominal Hilang(Rp)", "Lebih", "Nominal Lebih(Rp)"}) {
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if (catatan.getText().trim().isEmpty()) {//GEN-FIRST:event_BtnSimpanActionPerformed
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    public void tampil() {
        try {
            ...
            Valid.tabelKosong(tabMode);
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{real[i], kodebarang[i], namabarang[i], kategori[i], satuan[i],
            ...
            pstampil = koneksi.prepareStatement(
                    ...
                    + " ipsrsbarang.nama_brng like ? or ipsrsbarang.kode_sat like ? or ipsrsjenisbarang.nm_jenis like ?) "
                    + order);
            try {
                ...
                while (rstampil.next()) {
                    tabMode.addRow(new Object[]{"", rstampil.getString("kode_brng"), rstampil.getString("nama_brng"),
                        rstampil.getString("nm_jenis"), rstampil.getString("kode_sat"), rstampil.getDouble("harga"),
                        rstampil.getDouble("stok"), 0, 0, 0, 0});
                }
            } catch (Exception e) {
                System.out.println("Ntif : " + e);
                ...
                if (!tbDokter.getValueAt(i, 0).toString().isEmpty()) {
                    try {
                        if (Double.parseDouble(tabMode.getValueAt(i, 0).toString()) > 0) {
                            ...
                            if (kurang > 0) {
                                tbDokter.setValueAt(kurang * Double.parseDouble(tbDokter.getValueAt(i, 5).toString()),
                                        i, 8);
                                tbDokter.setValueAt(0, i, 10);
                            } else {
                                tbDokter.setValueAt(0, i, 8);
                                tbDokter.setValueAt(
                                        -1 * kurang * Double.parseDouble(tbDokter.getValueAt(i, 5).toString()), i, 10);
                            }
                        }
                    } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSInputStok.class.getName());
