...
public class IPSRSPemesanan extends javax.swing.JDialog {
    ...
    public IPSRSPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Jml", "Kode Barang", "Nama Barang", "Satuan", "G",
            "Hrg.Beli(Rp)", "Subtotal Beli(Rp)", "Disk(%)", "Diskon(Rp)",
            "Ttl.Beli"};
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoFaktur.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("ipsrspemesanan",
                        "?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Faktur", 14,
                        new String[]{
                    ...
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("ipsrsdetailpesan",
                                    "?,?,?,?,?,?,?,?,?", "Transaksi Penerimaan",
                                    9, new String[]{
                                        NoFaktur.getText(), tbDokter.getValueAt(
                                        i, 1).toString(), tbDokter.getValueAt(i,
                                                3).toString(),
                                ...
                                Trackbarang.catatRiwayat(tbDokter.getValueAt(i,
                                        1).toString(), Valid.SetAngka(tbDokter.
                                                getValueAt(i, 0).toString()), 0,
                                        "Penerimaan", akses.getkode(), "Simpan");
                                Sequel.mengedit("ipsrsbarang", "kode_brng=?",
                                        "stok=stok+?", 2, new String[]{
                                            tbDokter.getValueAt(i, 0).toString(),
                                            tbDokter.getValueAt(i, 1).toString()
                                        });
                                if (tbDokter.getValueAt(i, 4).toString().equals(
                                        "true") && (akses.getipsrs_barang() == true)) {
                                    Sequel.
                                            mengedit("ipsrsbarang",
                                                    "kode_brng=?", "harga=?", 2,
                                                    new String[]{
                                                        (Double.parseDouble(
                                                                tbDokter.
                ...
                if (sukses == true) {
                    ...
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi(
                        "select Penerimaan_NonMedis from set_akun"),
                        "PERSEDIAAN BARANG NON MEDIS", "" + (ttl + meterai), "0"});
                    if (ppn > 0) {
                        Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                new String[]{Sequel.cariIsi(
                                            "select set_akun.PPN_Masukan from set_akun"),
                                    "PPN Masukan Barang Non Medis", "" + ppn,
                                    "0"});
                    }
                    Sequel.menyimpan("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi(
                        "select Kontra_Penerimaan_NonMedis from set_akun"),
                        "HUTANG BARANG NON MEDIS", "0",
                        "" + (ttl + ppn + meterai)});
                    sukses = jur.simpanJurnal(NoFaktur.getText(), "U",
                            "PENERIMAAN BARANG NON MEDIS/PENUNJANG" + ", OLEH " + akses.
                                    getkode());
                }
...
private void tbDokterMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDokterMouseClicked
    if (tbDokter.getRowCount() != 0) {
        try {
            if ((tbDokter.getSelectedColumn() == 1) || (tbDokter.
                    getSelectedColumn() == 4) || (tbDokter.getSelectedColumn() == 5) || (tbDokter.
                    getSelectedColumn() == 6) || (tbDokter.getSelectedColumn() == 8)) {
                getData();
            } else if (tbDokter.getSelectedColumn() == 7) {
                tbDokter.setValueAt(Math.round(Double.parseDouble(tbDokter.
                        getValueAt(tbDokter.getSelectedRow(), 6).toString())
                        * (Double.parseDouble(tbDokter.getValueAt(tbDokter.
                                getSelectedRow(), 7).toString()) / 100)),
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
    if (tbDokter.getRowCount() != 0) {
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if ((tbDokter.getSelectedColumn() == 1) || (tbDokter.
                    getSelectedColumn() == 5) || (tbDokter.getSelectedColumn() == 6)) {
                getData();
            } else if ((tbDokter.getSelectedColumn() == 7) || (tbDokter.
                    getSelectedColumn() == 8)) {
                if (Double.parseDouble(tbDokter.getValueAt(tbDokter.
                        getSelectedRow(), 7).toString()) > 0) {
                    tbDokter.setValueAt(Math.round(Double.parseDouble(tbDokter.
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            IPSRSPemesanan dialog = new IPSRSPemesanan(new javax.swing.JFrame(),
                    true);
    ...
    private void tampil() {
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{"", rs.getString(1), rs.
                        getString(2), rs.getString(3), false, rs.getDouble(4), 0,
                        0, 0, 0});
                    iyem = iyem + "{\"KodeBarang\":\"" + rs.getString(1) + "\",\"NamaBarang\":\"" + rs.
                            getString(2).replaceAll("\"", "") + "\",\"Satuan\":\"" + rs.
                            getString(3) + "\",\"HrgBeli\":\"" + rs.getString(4) + "\"},";
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"penerimaanipsrs\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
        ...
        try {
            ...
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{jumlah[i], kodebarang[i],
                    namabarang[i], satuan[i], ganti[i], harga[i], subtotal[i],
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            ...
                            false, list.path("HrgBeli").asDouble(), 0, 0, 0, 0
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeBarang").asText().toLowerCase().
                            ...
                            tabMode.addRow(new Object[]{
                                ...
                                asText(), false, list.path("HrgBeli").asDouble(),
                                0, 0, 0, 0
                            });
                        }
                    }
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(ipsrspemesanan.no_faktur,3),signed)),0) from ipsrspemesanan where ipsrspemesanan.tgl_pesan='" + Valid.
                        SetTgl(TglPesan.getSelectedItem() + "") + "'",
                "PNM" + TglPesan.getSelectedItem().toString().substring(6, 10) + TglPesan.
    ...
    public void tampil(String noorder) {
        ...
        kdsup.setText(Sequel.cariIsi(
                "select surat_pemesanan_non_medis.kode_suplier from surat_pemesanan_non_medis where surat_pemesanan_non_medis.no_pemesanan=?",
                noorder));
        nmsup.setText(Sequel.cariIsi(
                "select ipsrssuplier.nama_suplier from ipsrssuplier where ipsrssuplier.kode_suplier=?",
                kdsup.getText()));
        meterai = Sequel.cariIsiAngka(
                "select surat_pemesanan_non_medis.meterai from surat_pemesanan_non_medis where surat_pemesanan_non_medis.no_pemesanan=?",
                noorder);
        ppn = Sequel.cariIsiAngka(
                "select surat_pemesanan_non_medis.ppn from surat_pemesanan_non_medis where surat_pemesanan_non_medis.no_pemesanan=?",
                noorder);
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{
                        rs.getString("jumlah"), rs.getString("kode_brng"), rs.
                        getString("nama_brng"),
                        rs.getString("kode_sat"), true, rs.getDouble("h_pesan"),
                        rs.getDouble("subtotal"),
                        rs.getDouble("dis"), rs.getDouble("besardis"), rs.
                        getDouble("total"),});
                }
                getData();
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSPemesanan.class.
            getName());
