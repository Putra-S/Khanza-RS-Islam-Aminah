    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoFaktur.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if (Sequel.
                        menyimpantf2("ipsrs_hibah", "?,?,?,?,?,?", "No.Hibah", 6,
                                new String[]{
                                    NoFaktur.getText(), kdsup.getText(), kdptg.
                                    getText(), Valid.SetTgl(TglBeli.
                                            getSelectedItem() + ""), sbttl + "",
                                    Keterangan.getText()
                                }) == true) {
                    jml = tbDokter.getRowCount();
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("ipsrs_detail_hibah",
                                    "?,?,?,?,?,?", "Transaksi Hibah", 6,
                                    new String[]{
                ...
                if (sukses == true) {
                    ...
                    Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi("select Hibah_Non_Medis from set_akun"),
                        "PERSEDIAAN BARANG NON MEDIS", "" + (sbttl), "0"});
                    Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4, new String[]{
                        Sequel.cariIsi(
                        "select Kontra_Hibah_Non_Medis from set_akun"),
                        "PENDAPATAN HIBAH", "0", "" + (sbttl)});
                    sukses = jur.simpanJurnal(NoFaktur.getText(), "U",
                            "HIBAH BARANG NON MEDIS, OLEH " + akses.getkode());
                }
