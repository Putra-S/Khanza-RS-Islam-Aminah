...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptHibahNonMedis.jasper", "report",
                    "::[ Transaksi Hibah Barang Non Medis ]::",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (tbDokter.getSelectedRow() > -1) {
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            try {
                pscaribeli = koneksi.prepareStatement(
                        "select no_hibah,totalhibah,tgl_hibah from ipsrs_hibah where no_hibah=?");
                try {
                    ...
                    if (rs.next()) {
                        ...
                        ps2 = koneksi.prepareStatement(
                                "select kode_brng,jumlah from ipsrs_detail_hibah where no_hibah=? ");
                        try {
                        ...
                        Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                new String[]{Sequel.cariIsi(
                                            "select Hibah_Non_Medis from set_akun"),
                                    "PERSEDIAAN BARANG NON MEDIS", "0", rs.
                                            getString("totalhibah")});
                        Sequel.menyimpan2("tampjurnal", "?,?,?,?", 4,
                                new String[]{Sequel.cariIsi(
                                            "select Kontra_Hibah_Non_Medis from set_akun"),
                                    "PENDAPATAN HIBAH", rs.getString(
                                            "totalhibah"), "0"});
                        sukses = jur.simpanJurnal(NoFaktur.getText(), "U",
                                "PEMBATALAN HIBAH BARANG NON MEDIS, OLEH " + akses.
                                        getkode());

                        if (sukses == true) {
                            Sequel.queryu2(
                                    "delete from ipsrs_hibah where no_hibah=?",
                                    1, new String[]{rs.getString("no_hibah")});
                            Sequel.Commit();
                            tampil();
                        } else {
                            JOptionPane.showMessageDialog(null,
        ...
        JOptionPane.showMessageDialog(null,
                "Silahkan pilih hibah yang mau dihapus..!");
    }
