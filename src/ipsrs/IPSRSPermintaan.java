...
public class IPSRSPermintaan extends javax.swing.JDialog {
    ...
    public IPSRSPermintaan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] judul = {"Jml", "Kode Barang", "Nama Barang", "Satuan", "Jenis Barang", "Keterangan", "Stok"};
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    iyem = iyem + "{\"KodeBarang\":\"" + rs.getString(1) + "\",\"NamaBarang\":\""
                            ...
                            + "\",\"Jenis\":\"" + rs.getString(4) + "\",\"Stok\":\"" + rs.getString(5) + "\"},";
                }
            } catch (Exception e) {
                System.out.println("Notifikasi : " + e);
            } finally {
    ...
    private void tampil2() {
        try {
            ...
            Valid.tabelKosong(tabMode);
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{jumlah[i], kodebarang[i], namabarang[i], satuan[i], jenis[i],
                    keterangan[i], stok[i]});
            }
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{"", list.path("KodeBarang").asText(),
                            ...
                            list.path("Jenis").asText(), "",
                            Sequel.cariInteger(
                            "select ipsrsbarang.stok " + " from ipsrsbarang where ipsrsbarang.kode_brng=?",
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(permintaan_non_medis.no_permintaan,3),signed)),0) from permintaan_non_medis where permintaan_non_medis.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PN" + Tanggal.getSelectedItem().toString().substring(8, 10)
                + Tanggal.getSelectedItem().toString().substring(3, 5)
                + Tanggal.getSelectedItem().toString().substring(0, 2),
                3, NoPermintaan);
    }

    private static final Logger LOG = Logger.getLogger(IPSRSPermintaan.class.getName());
