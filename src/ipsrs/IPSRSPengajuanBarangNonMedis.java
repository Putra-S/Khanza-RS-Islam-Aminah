...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    iyem = iyem + "{\"KodeBarang\":\"" + rs.getString(1) + "\",\"NamaBarang\":\""
                            ...
                            + "\",\"Jenis\":\"" + rs.getString(4) + "\",\"Harga\":\"" + rs.getString(5) + "\"},";
                }
            } catch (Exception e) {
                System.out.println("Notifikasi : " + e);
            } finally {
        ...
        try {
            ...
            for (i = 0; i < row; i++) {
                if (!tbDokter.getValueAt(i, 0).toString().isEmpty()) {
                    ...
                    harga[index] = Double.valueOf(tbDokter.getValueAt(i, 5).toString());
                    subtotal[index] = Double.valueOf(tbDokter.getValueAt(i, 6).toString());
                    index++;
                }
            }
            Valid.tabelKosong(tabMode);
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{jumlah[i], kodebarang[i], namabarang[i], satuan[i], ipsrsjenisbarang[i],
                    harga[i], subtotal[i]});
            }
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{"", list.path("KodeBarang").asText(),
                            ...
                            list.path("Jenis").asText(), list.path("Harga").asDouble(), 0});
                    }
                } else {
                    for (JsonNode list : response) {
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(pengajuan_barang_nonmedis.no_pengajuan,3),signed)),0) from pengajuan_barang_nonmedis where pengajuan_barang_nonmedis.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "PBNM" + Tanggal.getSelectedItem().toString().substring(6, 10)
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSPengajuanBarangNonMedis.class.getName());
