...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
                ...
                if ((tbDokter.getSelectedColumn() == 2) || (tbDokter.
                    ...
                    try {
                        tbDokter.setValueAt(Math.round(Double.parseDouble(
                                tbDokter.
                                        getValueAt(tbDokter.getSelectedRow(), 5).
                                        toString())
                                * (Double.parseDouble(tbDokter.getValueAt(
                                        tbDokter.getSelectedRow(), 6).toString()) / 100)),
            ...
            if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                try {
                    if (tbDokter.getSelectedColumn() == 0) {
                        ...
                        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 4).
                                toString().equals("0") || tbDokter.getValueAt(
                        ...
                        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 6).
                                toString().equals("0") || tbDokter.getValueAt(
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoPemesanan.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("surat_pemesanan_non_medis",
                        "?,?,?,?,?,?,?,?,?,?,?", "No.Pemesanan", 11,
                        new String[]{
    ...
    private void BtnPrint5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint5ActionPerformed
        ...
        if (NoPemesanan.getText().trim().isEmpty()) {
            ...
            Valid.MyReportqry("rptSuratPemesananNonMedis.jasper", "report",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            IPSRSSuratPemesanan dialog = new IPSRSSuratPemesanan(
    ...
    private void tampil() {
        try {
            ...
            fileWriter.write("{\"suratpemesananipsrs\":[" + iyem.substring(0,
                    iyem.length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
        ...
        try {
            ...
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{jumlah[i], satuanbeli[i],
                    kodebarang[i], namabarang[i], harga[i], subtotal[i],
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            "", list.path("Satuan").asText(), list.path(
                            "KodeBarang").asText(), list.path("NamaBarang").
                            asText(), list.path("Harga").asDouble(), 0, 0, 0, 0
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeBarang").asText().toLowerCase().
                            ...
                            tabMode.addRow(new Object[]{
                                "", list.path("Satuan").asText(), list.path(
                                "KodeBarang").asText(), list.path("NamaBarang").
                                asText(), list.path("Harga").asDouble(), 0, 0, 0,
                                0
                            });
                        }
                    }
                    ...
                    if (Double.parseDouble(tbDokter.getValueAt(row, 0).
                        ...
                        tbDokter.setValueAt(Double.parseDouble(tbDokter.
                                ...
                                parseDouble(tbDokter.getValueAt(row, 4).
                                        toString()), row, 5);
                        tbDokter.setValueAt(Double.parseDouble(tbDokter.
                                getValueAt(row, 5).toString()) - Double.
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_pemesanan_non_medis.no_pemesanan,3),signed)),0) from surat_pemesanan_non_medis where surat_pemesanan_non_medis.tanggal='" + Valid.
                        SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "SPM" + Tanggal.getSelectedItem().toString().substring(6, 10) + Tanggal.
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{
                        ...
                        getString("kode_brng"), rs.getString("nama_brng"),
                        rs.getDouble("h_pengajuan"), rs.getDouble("total"), 0, 0,
                        rs.getDouble("total")
                    });
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(
            IPSRSSuratPemesanan.class.getName());
}
