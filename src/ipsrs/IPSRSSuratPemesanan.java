...
public class IPSRSSuratPemesanan extends javax.swing.JDialog {
    ...
    public IPSRSSuratPemesanan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Jml", "Satuan", "Kode Barang", "Nama Barang", "Harga(Rp)",
            "Subtotal(Rp)", "Disk(%)", "Diskon(Rp)", "Total"}) {
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
    ...
    private void tampil2() {
        try {
            ...
            Valid.tabelKosong(tabMode);
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{jumlah[i], satuanbeli[i], kodebarang[i], namabarang[i], harga[i],
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode
                                .addRow(new Object[]{"", list.path("Satuan").asText(), list.path("KodeBarang").asText(),
                            list.path("NamaBarang").asText(), list.path("Harga").asDouble(), 0, 0, 0, 0});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeBarang").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                            ...
                            tabMode.addRow(new Object[]{"", list.path("Satuan").asText(),
                                list.path("KodeBarang").asText(), list.path("NamaBarang").asText(),
                                list.path("Harga").asDouble(), 0, 0, 0, 0});
                        }
                    }
                }
            }
            ...
            if (!tbDokter.getValueAt(row, 0).toString().isEmpty()) {
                try {
                    if (Double.parseDouble(tbDokter.getValueAt(row, 0).toString()) > 0) {
                        ...
                        tbDokter.setValueAt(Double.parseDouble(tbDokter.getValueAt(row, 5).toString())
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(surat_pemesanan_non_medis.no_pemesanan,3),signed)),0) from surat_pemesanan_non_medis where surat_pemesanan_non_medis.tanggal='"
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                "SPM" + Tanggal.getSelectedItem().toString().substring(6, 10)
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{rs.getString("jumlah"), rs.getString("kode_sat"),
                        rs.getString("kode_brng"), rs.getString("nama_brng"), rs.getDouble("h_pengajuan"),
                        rs.getDouble("total"), 0, 0, rs.getDouble("total")});
                }
            } catch (Exception e) {
                System.out.println("Notifikasi : " + e);
            } finally {
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSSuratPemesanan.class.getName());
