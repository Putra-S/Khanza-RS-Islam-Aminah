...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptReturBeliNonMedis.jasper", "report",
                    "::[ Transaksi Retur Ke Suplier Barang Non Medis dan Penunjang ( Lab & RO ) ]::",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
        ...
        if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 6).toString().trim().
            ...
            try {
                pscaripesan = koneksi.prepareStatement(
                        "select no_retur_beli, total from ipsrsreturbeli where no_retur_beli=?");
                try {
                    ...
                    if (rs.next()) {
                        ...
                        sukses = true;
                        psipsrs_detail_returbeli = koneksi.prepareStatement(
                                "select kode_brng,jml_retur from ipsrs_detail_returbeli where no_retur_beli=? ");
                        try {
                            psipsrs_detail_returbeli.setString(1, rs.
                                    getString(1));
                        ...
                        Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                                new String[]{
                                    Sequel.cariIsi(
                                            "select Retur_Beli_Non_Medis from set_akun"),
                                    "RETUR BELI NON MEDIS", rs.
                                            getString("total"), "0"
                                });
                        Sequel.menyimpan("tampjurnal", "?,?,?,?", "Rekening", 4,
                                new String[]{
                                    Sequel.cariIsi(
                                            "select Kontra_Retur_Beli_Non_Medis from set_akun"),
                                    "KONTRA RETUR BELI NON MEDIS", "0", rs.
                                            getString("total")
                                });
                        sukses = jur.simpanJurnal(rs.getString("no_retur_beli"),
                                "U",
                                "BATAL TRANSAKSI RETUR BELI BARANG PENUNJANG/NON MEDIS" + ", OLEH " + akses.
                                        getkode());

                        if (sukses == true) {
                            Sequel.queryu2(
                                    "delete from ipsrsreturbeli where no_retur_beli=?",
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            IPSRSCariReturBeli dialog = new IPSRSCariReturBeli(
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select ipsrs_detail_returbeli.kode_brng,ipsrsbarang.nama_brng, "
                            ...
                            + " ipsrs_detail_returbeli.no_retur_beli='" + rs.
                                    getString("no_retur_beli") + "'" + carijenis + caribarang
                            + " and (ipsrs_detail_returbeli.kode_brng like '%" + TCari.
                                    getText().trim() + "%' or ipsrsbarang.nama_brng like '%" + TCari.
                                    getText().trim() + "%' or "
                            + " ipsrs_detail_returbeli.kode_sat like '%" + TCari.
                                    getText().trim() + "%' or ipsrsjenisbarang.nm_jenis like '%" + TCari.
                                    getText().trim() + "%' or "
                            + " ipsrs_detail_returbeli.no_faktur like '%" + TCari.
                                    getText().trim() + "%') "
    ...
    private static final Logger LOG = Logger.getLogger(IPSRSCariReturBeli.class.
            getName());
