    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if (NoFaktur.getText().trim().isEmpty()) {//GEN-FIRST:event_BtnSimpanActionPerformed
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if (Sequel.menyimpantf2("inventaris_pemesanan",
                        "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", "No.Faktur", 15,
                        new String[]{
                            ...
                            "Belum Dibayar", akunaset
                        }) == true) {
                    jml = tbDokter.getRowCount();
                    for (i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("inventaris_detail_pesan",
                                    "?,?,?,?,?,?,?,?", "Transaksi Penerimaan", 8,
                                    new String[]{
...
private void btnPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPetugasActionPerformed
            ...
            try {
                if ((tbDokter.getSelectedColumn() == 1) || (tbDokter.
                    ...
                    tbDokter.setValueAt(Math.round(Double.parseDouble(tbDokter.
                            getValueAt(tbDokter.getSelectedRow(), 8).toString())
                            * (Double.parseDouble(tbDokter.getValueAt(tbDokter.
                                    getSelectedRow(), 9).toString()) / 100)),
    ...
    private void autoNomor() {
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(inventaris_pemesanan.no_faktur,3),signed)),0) from inventaris_pemesanan where inventaris_pemesanan.tgl_pesan='"
                + Valid.SetTgl(TglPesan.getSelectedItem() + "") + "'",
                "PAI" + TglPesan.getSelectedItem().toString().substring(6, 10)
    ...
    public void tampil(String noorder) {
        /*
		 * NoOrder.setText(noorder); kdsup.setText(Sequel.
		 * cariIsi("select kode_suplier from surat_pemesanan_non_medis where no_pemesanan=?"
		 * ,noorder)); nmsup.setText(Sequel.
		 * cariIsi("select ipsrssuplier.nama_suplier from ipsrssuplier where ipsrssuplier.kode_suplier=?"
		 * ,kdsup.getText())); meterai=Sequel.
		 * cariIsiAngka("select meterai from surat_pemesanan_non_medis where no_pemesanan=?"
		 * ,noorder); ppn=Sequel.
		 * cariIsiAngka("select ppn from surat_pemesanan_non_medis where no_pemesanan=?"
		 * ,noorder); Meterai.setText(Valid.SetAngka2(meterai)); try{
		 * Valid.tabelKosong(tabMode); ps=koneksi.prepareStatement(
		 ...
		 * ); try { ps.setString(1,noorder); rs=ps.executeQuery(); while(rs.next()){
		 ...
		 * getData(); } catch (Exception e) { System.out.println("Notifikasi : "+e); }
    ...
    private void tampilAkun() {
        try {
            ...
            iyem = "";
            ps = koneksi.prepareStatement(
                    "select rekening.nm_rek,akun_aset_inventaris.kd_rek from akun_aset_inventaris inner join rekening on akun_aset_inventaris.kd_rek=rekening.kd_rek group by rekening.nm_rek order by rekening.nm_rek");
            try {
                rs = ps.executeQuery();
    ...
    private static final Logger LOG = Logger.getLogger(InventarisPemesanan.class.getName());
