...
public class KeslingMutuAirLimbah extends javax.swing.JDialog {
    ...
    public KeslingMutuAirLimbah(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"No.", "NIP", "Petugas", "Tanggal", "Meteran",
            "Jml.Harian(M³)", "pH", "Suhu(°C)", "TDS (PPM)", "EC (US/Cm)", "Salt (PPM)"}) {
            @Override
...
private void NmPetugasKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_NmPetugasKeyPressed
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi
                    .prepareStatement("select kesling_mutu_air_limbah.nip,petugas.nama,kesling_mutu_air_limbah.tanggal,"
                            ...
                            : " and (kesling_mutu_air_limbah.nip like ? or petugas.nama like ?) ")
            ...
            if (total > 0) {
                tabMode.addRow(new String[]{"", "", "JUMLAH :", "", "", "" + total, "" + (ph / (i - 1)),
                    "" + (suhu / (i - 1)), "" + (tds / (i - 1)), "" + (ec / (i - 1)), "" + (salt / (i - 1))});
            }
        } catch (Exception e) {
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            if (!tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().isEmpty()) {
    ...
    private static final Logger LOG = Logger.getLogger(KeslingMutuAirLimbah.class.getName());
