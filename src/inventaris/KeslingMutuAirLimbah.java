 ...
 * @author perpustakaan
 */
public class KeslingMutuAirLimbah extends javax.swing.JDialog {
    ...
    public KeslingMutuAirLimbah(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            "No.", "NIP", "Petugas", "Tanggal", "Meteran", "Jml.Harian(M³)",
            "pH", "Suhu(°C)", "TDS (PPM)", "EC (US/Cm)", "Salt (PPM)"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (Meteran.getText().trim().isEmpty()) {
            Valid.textKosong(Meteran, "Meteran");
        } else if (JmlHarian.getText().trim().isEmpty()) {
        ...
        } else if (TDS.getText().trim().isEmpty()) {
            Valid.textKosong(TDS, "TDS");
        } else if (EC.getText().trim().isEmpty()) {
            Valid.textKosong(EC, "EC");
        } else if (Salt.getText().trim().isEmpty()) {
            Valid.textKosong(Salt, "Salt");
        } else if (NmPetugas.getText().trim().isEmpty()) {
            Valid.textKosong(KdPetugas, "Petugas yang bertugas");
        } else {
            if (Sequel.menyimpantf("kesling_mutu_air_limbah",
                    "?,?,?,?,?,?,?,?,?", "Data", 9, new String[]{
                        KdPetugas.getText(), Valid.SetTgl(Tanggal.
                        getSelectedItem() + "") + " " + Tanggal.
                                getSelectedItem().toString().substring(11, 19),
                        Meteran.getText(), JmlHarian.getText(), PH.getText(),
                        Suhu.getText(), TDS.getText(), EC.getText(), Salt.
                        getText()
                    }) == true) {
                tampil();
                emptTeks();
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            Sequel.meghapus("kesling_mutu_air_limbah", "nip", "tanggal", tbObat.
                    getValueAt(tbObat.getSelectedRow(), 1).toString(), tbObat.
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (Meteran.getText().trim().isEmpty()) {
            ...
            if (tbObat.getSelectedRow() > -1) {
                if (Sequel.mengedittf("kesling_mutu_air_limbah",
                        "nip=? and tanggal=?",
                        "nip=?,tanggal=?,meteran=?,jumlahharian=?,ph=?,suhu=?,tds=?,ec=?,salt=?",
                        11, new String[]{
                            KdPetugas.getText(), Valid.SetTgl(Tanggal.
                            getSelectedItem() + "") + " " + Tanggal.
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptMutuAirLimbah.jasper", "report",
                    "::[ Data Mutu Air Limbah ]::",
                    ...
                    + "where kesling_mutu_air_limbah.tanggal between '" + Valid.
                            SetTgl(DTPCari1.getSelectedItem() + "") + " 00:00:00' and '" + Valid.
                    SetTgl(DTPCari2.getSelectedItem() + "") + " 23:59:59' "
                    + (TCari.getText().trim().isEmpty() ? "" : " and (kesling_mutu_air_limbah.nip like '%" + TCari.
...
private void NmPetugasKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_NmPetugasKeyPressed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeslingMutuAirLimbah dialog = new KeslingMutuAirLimbah(
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + (TCari.getText().trim().isEmpty() ? "" : " and (kesling_mutu_air_limbah.nip like ? or petugas.nama like ?) ")
    ...
    private void getData() {
        if (tbObat.getSelectedRow() != -1) {
            if (!tbObat.getValueAt(tbObat.getSelectedRow(), 1).toString().
                    isEmpty()) {
                KdPetugas.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 1).
                        toString());
                NmPetugas.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 2).
                        toString());
                Valid.SetTgl2(Tanggal, tbObat.
                        getValueAt(tbObat.getSelectedRow(), 3).toString());
                Meteran.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 4).
                        toString());
                JmlHarian.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 5).
                        toString());
                PH.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 6).
                        toString());
                Suhu.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 7).
                        toString());
                TDS.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 8).
                        toString());
                EC.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 9).
                        toString());
                Salt.setText(tbObat.getValueAt(tbObat.getSelectedRow(), 10).
                        toString());
            }
        }
    }
    ...
    private static final Logger LOG = Logger.getLogger(
            KeslingMutuAirLimbah.class.getName());
}
