...
public class DlgLamaPelayananPasien extends javax.swing.JDialog {
    ...
    private double ttlJRsdSB = 0, ttlJRsdBP = 0, JRsdDP = 0, ttlJRsdDP = 0, JRsdRDk = 0, ttlJRsdRDk = 0, JRsdRDv = 0,
            ttlJRsdRDv = 0, ttlJRsdTB = 0, ttlSBsdBP = 0, ttlSBsdDP = 0, SBsdDP = 0, ttlSBsdRDk = 0, SBsdRDk = 0,
            ttlSBsdRDv = 0, SBsdRDv = 0, ttlSBsdTB = 0, BPsdDP = 0, ttlBPsdDP = 0, BPsdRDk = 0, ttlBPsdRDk = 0,
            BPsdRDv = 0, ttlBPsdRDv = 0, ttlBPsdTB = 0, DPsdRDk = 0, ttlDPsdRDk = 0, DPsdRDv = 0, ttlDPsdRDv = 0,
            DPsdTB = 0, ttlDPsdTB = 0, RDksdRDv = 0, ttlRDksdRDv = 0, RDksdTB = 0, ttlRDksdTB = 0, RDvsdTB = 0,
            ttlRDvsdTB = 0;
        ...
        try {
            ...
            try (BufferedWriter bw = new BufferedWriter(new FileWriter(f))) {
                bw.write(LoadHTML.getText().replaceAll("<head>",
                        ...
                        getemailrs() + "<br><br>"
                        + "<font size='2' face='Tahoma'>LAMA PELAYANAN PASIEN RAWAT JALAN<br>PERIODE " + Tgl1.
...
private void btnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btnCariKeyPressed
    ...
    private void prosesCari() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,poliklinik.nm_poli,dokter.nm_dokter,date_format(reg_periksa.tgl_registrasi,'%d/%m/%Y') as tgl_registrasi,reg_periksa.jam_reg,"
                    ...
                    + "pasien.nm_pasien like ? or poliklinik.nm_poli like ? or dokter.nm_dokter like ? ) ")
            ...
            try {
                ...
                ttlRDvsdTB = 0;
                while (rs.next()) {
                    dilayanipoli = Sequel.cariIsi(
                            "select date_format(pemeriksaan_ralan.jam_rawat,'%H:%i:%s') from pemeriksaan_ralan where pemeriksaan_ralan.no_rawat=? order by pemeriksaan_ralan.tgl_perawatan,pemeriksaan_ralan.jam_rawat asc limit 1",
                            rs.getString("no_rawat"));
                    resepdikirim = Sequel.cariIsi(
                            "select date_format(resep_obat.jam_peresepan,'%H:%i:%s') from resep_obat where resep_obat.no_rawat=? order by resep_obat.tgl_peresepan,resep_obat.jam_peresepan asc limit 1",
                            rs.getString("no_rawat"));
                    resepdivalidasi = Sequel.cariIsi(
                            "select date_format(resep_obat.jam,'%H:%i:%s') from resep_obat where resep_obat.no_rawat=? order by resep_obat.tgl_peresepan,resep_obat.jam_peresepan asc limit 1",
                            rs.getString("no_rawat"));
                    ttlJRsdSB += rs.getDouble("JRsdSB");
                    ttlJRsdBP += rs.getDouble("JRsdBP");
                    JRsdDP = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + dilayanipoli
                            + "')-TIME_TO_SEC('" + rs.getString("jam_reg") + "'))/60,2),'')");
                    ttlJRsdDP += JRsdDP;
                    JRsdRDk = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdikirim
                            + "')-TIME_TO_SEC('" + rs.getString("jam_reg") + "'))/60,2),'')");
                    ttlJRsdRDk += JRsdRDk;
                    JRsdRDv = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdivalidasi
                            + "')-TIME_TO_SEC('" + rs.getString("jam_reg") + "'))/60,2),'')");
                    ttlJRsdRDv += JRsdRDv;
                    ttlJRsdTB += rs.getDouble("JRsdTB");
                    ttlSBsdBP += rs.getDouble("SBsdBP");
                    SBsdDP = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + dilayanipoli
                            + "')-TIME_TO_SEC('" + rs.getString("dikirim") + "'))/60,2),'')");
                    ttlSBsdDP += SBsdDP;
                    SBsdRDk = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdikirim
                            + "')-TIME_TO_SEC('" + rs.getString("dikirim") + "'))/60,2),'')");
                    ttlSBsdRDk += SBsdRDk;
                    SBsdRDv = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdivalidasi
                            + "')-TIME_TO_SEC('" + rs.getString("dikirim") + "'))/60,2),'')");
                    ttlSBsdRDv += SBsdRDv;
                    ttlSBsdTB += rs.getDouble("SBsdTB");
                    BPsdDP = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + dilayanipoli
                            + "')-TIME_TO_SEC('" + rs.getString("diterima") + "'))/60,2),'')");
                    ttlBPsdDP += BPsdDP;
                    BPsdRDk = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdikirim
                            + "')-TIME_TO_SEC('" + rs.getString("diterima") + "'))/60,2),'')");
                    ttlBPsdRDk += BPsdRDk;
                    BPsdRDv = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdivalidasi
                            + "')-TIME_TO_SEC('" + rs.getString("diterima") + "'))/60,2),'')");
                    ttlBPsdRDv += BPsdRDv;
                    ttlBPsdTB += rs.getDouble("BPsdTB");
                    DPsdRDk = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdikirim
                            + "')-TIME_TO_SEC('" + dilayanipoli + "'))/60,2),'')");
                    ttlDPsdRDk += DPsdRDk;
                    DPsdRDv = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdivalidasi
                            + "')-TIME_TO_SEC('" + dilayanipoli + "'))/60,2),'')");
                    ttlDPsdRDv += DPsdRDv;
                    DPsdTB = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + rs.getString("jambilling")
                            + "')-TIME_TO_SEC('" + dilayanipoli + "'))/60,2),'')");
                    ttlDPsdTB += DPsdTB;
                    RDksdRDv = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + resepdivalidasi
                            + "')-TIME_TO_SEC('" + resepdikirim + "'))/60,2),'')");
                    ttlRDksdRDv += RDksdRDv;
                    RDksdTB = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + rs.getString("jambilling")
                            + "')-TIME_TO_SEC('" + resepdikirim + "'))/60,2),'')");
                    ttlRDksdTB += RDksdTB;
                    RDvsdTB = Sequel.cariIsiAngka("select ifnull(round((TIME_TO_SEC('" + rs.getString("jambilling")
                            + "')-TIME_TO_SEC('" + resepdivalidasi + "'))/60,2),'')");
                    ttlRDvsdTB += RDvsdTB;

                    htmlContent.append("<tr class='isi'>" + "<td valign='middle' align='center'>"
                            + rs.getString("no_rawat") + "</td>" + "<td valign='middle' align='center'>"
                            + rs.getString("no_rkm_medis") + "</td>" + "<td valign='middle' align='left'>"
                            + rs.getString("nm_pasien") + "</td>" + "<td valign='middle' align='left'>"
                            + rs.getString("nm_poli") + "</td>" + "<td valign='middle' align='left'>"
                            + rs.getString("nm_dokter") + "</td>" + "<td valign='middle' align='center'>"
                            + rs.getString("tgl_registrasi") + "</td>" + "<td valign='middle' align='center'>"
                            + rs.getString("jam_reg") + "</td>" + "<td valign='middle' align='center'>"
                            + rs.getString("dikirim") + "</td>" + "<td valign='middle' align='center'>"
                            + rs.getString("diterima") + "</td>" + "<td valign='middle' align='center'>" + dilayanipoli
                            + "</td>" + "<td valign='middle' align='center'>" + resepdikirim + "</td>"
                            ...
                            + "<td valign='middle' align='center'>" + RDvsdTB + "</td>" + "</tr>");
                    i++;
                }
                if (i > 1) {
                    htmlContent.append("<tr class='isi'>" + "<td valign='middle'></td>"
    ...
    private static final Logger LOG = Logger.getLogger(DlgLamaPelayananPasien.class.getName());
