 ...
 * @author perpustakaan
 */
public class DlgPembatalanPeriksaPerDokter extends javax.swing.JDialog {
            ...
            for (int r = 0; r < tabMode.getRowCount(); r++) {
                if (!tbBangsal.getValueAt(r, 0).toString().contains(">>")) {
                    Sequel.menyimpan("temporary", "'" + r + "','"
                            ...
                            + tabMode.getValueAt(r, 5).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                            getalamatip() + "'", "Rekap Nota Pembayaran");
                }
            }

            Valid.MyReportqry("rptPembatalanPeriksaPerDokter.jasper", "report",
                    "::[ Laporan Pembatalan Periksa Rawat Jalan Per Dokter ]::",
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    public void tampil() {
        try {
            ...
            try {
                ...
                ttltotal = 0;
                while (rs.next()) {
                    batal = Sequel.cariInteger("select count(kd_dokter) from reg_periksa where tgl_registrasi between '"
                            + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                            + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' and kd_dokter='"
                            + rs.getString("kd_dokter") + "' and stts='Batal'");
                    total = Sequel.cariInteger("select count(kd_dokter) from reg_periksa where tgl_registrasi between '"
                            + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                            + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' and kd_dokter='"
                    ...
                    ttlperiksa += periksa;
                    ttltotal += total;
                    tabMode.addRow(new Object[]{i + ".", rs.getString("kd_dokter"), rs.getString("nm_dokter"), total,
                        batal, periksa});

                    i++;
                }
                if (i >= 2) {
                    tabMode.addRow(new Object[]{"", "Total", ":", ttltotal, ttlbatal, ttlperiksa});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgPembatalanPeriksaPerDokter.class.getName());
