...
public class DlgOperasiPerBulan extends javax.swing.JDialog {
    ...
    public DlgOperasiPerBulan(java.awt.Frame parent, boolean modal) {
        ...
        Object[] rowRwJlDr = {"No.", "Nama Operasi", "Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Agu", "Sep",
            "Okt", "Nov", "Des", "Total"};
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
            ...
            for (int r = 0; r < tabMode.getRowCount(); r++) {
                if (!tbBangsal.getValueAt(r, 0).toString().contains(">>")) {
                    Sequel.menyimpan("temporary", "'" + r + "','"
                            ...
                            getalamatip() + "'", "Operasi Per Bulan");
                }
            }

            Valid.MyReportqry("rptOperasiPerBulan.jasper", "report",
                    "::[ Operasi Per Bulan ]::",
    ...
    public void tampil() {
        try {
            ...
            ps = koneksi
                    .prepareStatement("select paket_operasi.nm_perawatan,count(paket_operasi.nm_perawatan) from operasi "
            ...
            try {
                ...
                i = 1;
                while (rs.next()) {
                    tabMode.addRow(new String[]{i + "", rs.getString(1), Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-01' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-02' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-03' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-04' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-05' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-06' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-07' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-08' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-09' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-10' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-11' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        Sequel.cariIsi(
                        "select count(paket_operasi.nm_perawatan) from operasi inner join paket_operasi on paket_operasi.kode_paket=operasi.kode_paket where left(operasi.tgl_operasi,7)='"
                        + ThnCari.getSelectedItem() + "-12' and paket_operasi.nm_perawatan=? ",
                        rs.getString(1)),
                        rs.getString(2)});
                    i++;
                }
    ...
    private static final Logger LOG = Logger.getLogger(DlgOperasiPerBulan.class.getName());
