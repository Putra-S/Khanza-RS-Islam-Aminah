 ...
 * @author perpustakaan
 */
public class DlgHitungBOR extends javax.swing.JDialog {
    ...
    public DlgHitungBOR(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new String[]{"No", "No.Rawat", "Nomer RM", "Nama Pasien", "Kamar",
            "Tgl.Masuk", "Tgl.Keluar", "Lama", "Status"}) {
            @Override
        ...
        tabMode2 = new DefaultTableModel(null, new String[]{"No", "No.Rawat", "Nomer RM", "Nama Pasien", "Kamar",
            "Tgl.Masuk", "Tgl.Keluar", "Lama", "Status"}) {
            @Override
            public boolean isCellEditable(int rowIndex, int colIndex) {
                return false;
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (TabRawat.getSelectedIndex() == 0) {
            if (tabMode.getRowCount() == 0) {
                ...
                Valid.MyReportqry("rptHitungBor.jasper", "report",
                        "::[ Data Hitung BOR ]::",
                ...
                for (int r = 0; r < tabMode2.getRowCount(); r++) {
                    if (!Tabel2.getValueAt(r, 0).toString().contains(">>")) {
                        Sequel.menyimpan("temporary", "'" + r + "','"
                                ...
                                + tabMode2.getValueAt(r, 8).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                                getalamatip() + "'", "Transaksi");
                    }
                }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    public void tampil() {
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + (Kamar.getText().isEmpty() ? "" : "and bangsal.nm_bangsal=?")
                    + " order by kamar_inap.tgl_masuk");

            try {
                ps.setString(1, Valid.SetTgl(Tgl1.getSelectedItem() + ""));
                ps.setString(2, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                if (!Kamar.getText().isEmpty()) {
                ...
                while (rs.next()) {
                    ...
                    tanggalAwal = LocalDate.parse(rs.getString("tgl_masuk"), formatter);
                    tanggalAkhir = LocalDate.parse(rs.getString("tgl_keluar"), formatter);
                    jumlahHari = ChronoUnit.DAYS.between(tanggalAwal, tanggalAkhir) + 1;

                    tabMode.addRow(new Object[]{i, rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        rs.getString("nm_pasien"), rs.getString("kamar"), rs.getString("tgl_masuk"),
                        rs.getString("tgl_keluar"), jumlahHari, rs.getString("stts_pulang")});
                    hari += jumlahHari;
                    i++;
                }
                if (hari > 0) {
                    // kamar = Sequel.cariInteger(
                    // "select count(*) from kamar inner join bangsal on kamar.kd_bangsal
                    // = bangsal.kd_bangsal where kamar.statusdata='1' and
                    // bangsal.nm_bangsal not like '%(B)' and bangsal.nm_bangsal not like
                    // 'Neonatus 2' and bangsal.nm_bangsal not like '%ODC' and
                    // bangsal.nm_bangsal not like '%coba'");
                    kamar = 105;
                    jumlahhari = Sequel.cariInteger("select (to_days('" + Valid.SetTgl(Tgl2.getSelectedItem() + "")
                            + "')-to_days('" + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "'))") + 1;
                    tabMode.addRow(new Object[]{"", "", "", "Jumlah Hari Perawatan", ":", "", "", hari, ""});
                    tabMode.addRow(new Object[]{"", "", "", "Jumlah Kamar", ":", "", "", kamar, ""});
                    tabMode
                            .addRow(new Object[]{"", "", "", "Jumlah Hari Dalam Periode", ":", "", "", jumlahhari, ""});
                    tabMode.addRow(new Object[]{"", "", "", "Perhitungan BOR ",
                        ": (" + hari + "/(" + kamar + " X " + jumlahhari + ")) X 100%", "", "",
                        Valid.SetAngka4((hari / (kamar * jumlahhari)) * 100) + " %", ""});
                }
            } catch (Exception e) {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode2.addRow(new Object[]{i, rs.getString("no_rawat"), rs.getString("no_rkm_medis"),
                        rs.getString("nm_pasien"), rs.getString("kamar"), rs.getString("tgl_masuk"),
                        rs.getString("tgl_keluar"), jumlahHari, rs.getString("stts_pulang")});
                    hari += jumlahHari;
                    i++;
                ...
                if (hari > 0) {
                    // kamar = Sequel.cariInteger(
                    // "select count(*) from kamar where statusdata='1'");
                    kamar = 105;
                    jumlahhari = Sequel.cariInteger("select (to_days('" + Valid.SetTgl(Tgl2.getSelectedItem() + "")
                            + "')-to_days('" + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "'))") + 1;
                    ...
                    tabMode2
                            .addRow(new Object[]{"", "", "", "Jumlah Hari Dalam Periode", ":", "", "", jumlahhari, ""});
                    tabMode2.addRow(new Object[]{"", "", "", "Perhitungan BOR ",
                        ": (" + hari + "/(" + kamar + " X " + jumlahhari + ")) X 100%", "", "",
                        Valid.SetAngka4((hari / (kamar * jumlahhari)) * 100) + " %", ""});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgHitungBOR.class.getName());
