...
public class DlgBookingMCUPerusahaan extends javax.swing.JDialog {
    ...
    public DlgBookingMCUPerusahaan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"P", "No.RM", "Nama Pasien", "JK", "No.KTP", "No.Pegawai",
            "No.Booking", "Tgl.Booking", "Jam Booking", "Status", "Tahun", "Bulan", "Hari"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if(NmPerusahaan.getText().trim().equals("")||KdPerusahaan.getText().trim().equals("")){//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        } else if (KdPoli.getText().trim().isEmpty() || NmPoli.getText().trim().isEmpty()) {
            Valid.textKosong(BtnPoli, "Unit/Poli MCU");
        } else if (KdCaraBayar.getText().trim().isEmpty() || NmCaraBayar.getText().trim().isEmpty()) {
...
private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
    if (NmPerusahaan.getText().trim().isEmpty()) {
        Valid.textKosong(BtnPerusahaan, "Perusahaan/Instansi MCU");
    } else {
        tampil();
    }
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    private void ppPengajuanPasienBaruActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppPengajuanPasienBaruActionPerformed
        if (NmPerusahaan.getText().trim().isEmpty() || KdPerusahaan.getText().trim().isEmpty()) {
            Valid.textKosong(BtnPerusahaan, "Perusahaan/Instansi MCU");
        } else if (KdCaraBayar.getText().trim().isEmpty() || NmCaraBayar.getText().trim().isEmpty()) {
            Valid.textKosong(BtnCaraBayar, "Jenis/Cara Bayar MCU");
        } else {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                        rs.getString("jk"), rs.getString("no_ktp"), rs.getString("nip"), rs.getString("no_mcu"),
                        rs.getString("tanggal_booking"), rs.getString("jam_booking"), rs.getString("daftar"),
                        rs.getString("tahun"), rs.getString("bulan"), rs.getString("hari")});
                }
            } catch (Exception e) {
    ...
    private void isNumber() {
            ...
            switch (URUTNOREG) {
                case "poli":
                    if (Sequel.cariInteger(
                            ...
                            + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "'") >= Sequel.cariInteger(
                ...
                case "dokter + poli":
                    if (Sequel.cariInteger(
                            ...
                            + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "'") >= Sequel.cariInteger(
                            "select ifnull(MAX(CONVERT(reg_periksa.no_reg,signed)),0) from reg_periksa where reg_periksa.kd_dokter='"
                            + KdDokter.getText() + "' and reg_periksa.kd_poli='"
                            + KdPoli.getText() + "' and reg_periksa.tgl_registrasi='"
                            + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "'")) {
                        Valid.autoNomer3(
                                "select ifnull(MAX(CONVERT(booking_registrasi.no_reg,signed)),0) from booking_registrasi where booking_registrasi.kd_dokter='"
                        ...
                        Valid.autoNomer3(
                                "select ifnull(MAX(CONVERT(reg_periksa.no_reg,signed)),0) from reg_periksa where kd_dokter='"
                                + KdDokter.getText() + "' and reg_periksa.kd_poli='" + KdPoli.getText()
        ...
        Valid.autoNomer3(
                ...
                + Valid.SetTgl(Tanggal.getSelectedItem() + "") + "' ",
                dateformat.format(Tanggal.getDate()) + "/", 6, TNoRw);
    }
