...
public class DlgCariBookingMCUPerusahaan extends javax.swing.JDialog {
    ...
    public DlgCariBookingMCUPerusahaan(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"P", "No.RM", "Nama Pasien", "JK", "Umur", "No.HP", "No.KTP", "No.Pegawai", "No.Booking",
                    "Tgl.Booking", "Jam Booking", "Status MCU", "Kode Instansi", "Nama Instansi", "No.Pelayanan",
                    "No.Urut", "Kode Dokter", "Nama Dokter MCU", "Kode Unit", "Nama Unit/Poli MCU", "Kode Bayar",
                    "Cara/Jenis Bayar", "Stts Daftar", "Stts Poli", "Tgl.MCU"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnDokterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        dokter.isCek();//GEN-FIRST:event_BtnDokterActionPerformed
    ...
    private void tampil() {
        ...
        try {
            if (NmPoli.getText().trim().isEmpty() && NmDokter.getText().trim().isEmpty()
                    && NmPerusahaan.getText().trim().isEmpty() && TCari.getText().trim().isEmpty()) {
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new Object[]{false, rs.getString("no_rkm_medis"), rs.getString("nm_pasien"),
                        rs.getString("jk"), rs.getString("umurdaftar") + " " + rs.getString("sttsumur"),
                        rs.getString("no_tlp"), rs.getString("no_ktp"), rs.getString("nip"), rs.getString("no_mcu"),
                        rs.getString("tanggal_booking"), rs.getString("jam_booking"), rs.getString("statusmcu"),
                        rs.getString("kode_perusahaan"), rs.getString("nama_perusahaan"), rs.getString("no_rawat"),
                        rs.getString("no_reg"), rs.getString("kd_dokter"), rs.getString("nm_dokter"),
                        rs.getString("kd_poli"), rs.getString("nm_poli"), rs.getString("kd_pj"),
                        rs.getString("png_jawab"), rs.getString("stts_daftar"), rs.getString("status_poli"),
