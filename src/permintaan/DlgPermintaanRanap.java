...
public class DlgPermintaanRanap extends javax.swing.JDialog {
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoRw.getText().trim().isEmpty() || NoRM.getText().trim().isEmpty() || NmPasien.
        ...
        } else if (KdBangsal.getText().trim().isEmpty() || KdKamar.getText().
                trim().isEmpty() || NmBangsal.getText().trim().isEmpty()) {
            ...
            if (Sequel.menyimpantf("permintaan_ranap", "?,?,?,?,?", "Pasien", 5,
                    new String[]{
                        NoRw.getText(), Valid.SetTgl(
                        DTPTgl.getSelectedItem() + ""), KdKamar.getText(),
                        Diagnosa.getText(), Catatan.getText()
                    }) == true) {
                tampil();
                Sequel.mengedit("kamar", "kd_kamar=?", "status='DIBOOKING'", 1,
                        new String[]{KdKamar.getText()});
                emptTeks();
            }
        }
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (tbObat.getSelectedRow() != -1) {
                if (Sequel.cariRegistrasi(NoRw.getText()) > 0) {
                    ...
                    kamarinap.emptTeks();
                    kamarinap.isCek();
                    kamarinap.setNoRm(NoRw.getText(), NoRM.getText(), NmPasien.
                            getText());
                    ...
                    try {
                        ...
                        if (rs.next()) {
                            if (rs.getString("no_peserta").length() < 13) {
                                ...
                                form.setNoRm(NoRw.getText(), rs.getString(
                                        "no_peserta"), NoRM.getText(), NmPasien.
    ...
    private void BtnPersetujuanRanapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPersetujuanRanapActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            if (tbObat.getSelectedRow() != -1) {
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                SuratPersetujuanRawatInap resume = new SuratPersetujuanRawatInap(
                        null, false);
                resume.isCek();
                resume.setSize(internalFrame1.getWidth() - 20, internalFrame1.
                        getHeight() - 20);
