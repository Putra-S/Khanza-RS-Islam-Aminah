...
public class DlgRujuk extends javax.swing.JDialog {
    ...
    public DlgRujuk(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No.Rujuk", "No.Rawat", "No.R.M.", "Nama Pasien", "Tempat Rujuk", "Tgl.Rujuk", "Jam Rujuk",
        ...
        penyakit.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if (penyakit.getTable().getSelectedRow() != -1) {
                    TDiagnosa.setText(penyakit.getTable().getValueAt(penyakit.getTable().getSelectedRow(), 0).toString()
                            + " - "
...
private void TDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TDokterKeyPressed
    ...
    public void tampil() {
        ...
        try {
            tgl = " rujuk.tgl_rujuk between '" + Valid.SetTgl(DTPCari1.getSelectedItem() + "") + "' and '"
                    + Valid.SetTgl(DTPCari2.getSelectedItem() + "") + "' ";
            sql = "select rujuk.no_rujuk,rujuk.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,"
                    + "rujuk.rujuk_ke,rujuk.tgl_rujuk,rujuk.jam,rujuk.keterangan_diagnosa,rujuk.kd_dokter,dokter.nm_dokter,rujuk.kat_rujuk,rujuk.ambulance,rujuk.keterangan "
                    + "from rujuk inner join reg_periksa inner join pasien inner join dokter "
                    + "on rujuk.no_rawat=reg_periksa.no_rawat " + "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                    + "and rujuk.kd_dokter=dokter.kd_dokter " + "where " + tgl + " and (no_rujuk like '%"
                    + TCari.getText().trim() + "%' or rujuk.no_rawat like '%" + TCari.getText().trim() + "%' or "
                    + "reg_periksa.no_rkm_medis like '%" + TCari.getText().trim() + "%' or pasien.nm_pasien like '%"
                    + TCari.getText().trim() + "%' or " + "rujuk.rujuk_ke like '%" + TCari.getText().trim()
                    + "%' or rujuk.keterangan_diagnosa like '%" + TCari.getText().trim() + "%' or "
                    + "rujuk.kd_dokter like '%" + TCari.getText().trim() + "%' or dokter.nm_dokter like '%"
                    + TCari.getText().trim() + "%') " + " order by rujuk.no_rujuk";
