...
public class DlgPeriksaLaboratoriumMB extends javax.swing.JDialog {
    ...
    private PreparedStatement pstindakan, pstindakan2, pstindakan3, pstindakan4, pstampil, pstampil2, pstampil3,
            pstampil4, psset_tarif, pssetpj, psrekening;
    ...
    private String aktifkanparsial = "no", noorder = "", kelas = "", kamar, namakamar, cara_bayar_lab = "Yes",
            kelas_lab = "Yes", pilihan = "", status = "", diagnosa = "";
...
private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
    ...
    private void simpanlab() {
        ...
        try {
            ...
            for (i = 0; i < tbTarif.getRowCount(); i++) {
                if (tbTarif.getValueAt(i, 0).toString().equals("true")) {
                    if (Sequel.menyimpantf2("periksa_lab", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'MB'", "Kode Pemeriksaan",
                            16,
                        ...
                        if (!noorder.isEmpty()) {
                            if (Sequel.cariIsi(
                                    "select permintaan_pemeriksaan_labmb.stts_bayar from permintaan_pemeriksaan_labmb where permintaan_pemeriksaan_labmb.noorder='"
                                    + noorder + "' and permintaan_pemeriksaan_labmb.kd_jenis_prw='"
                    ...
                    if ((!tbPemeriksaan.getValueAt(i, 6).toString().isEmpty())
                        ...
                        try {
                            if (Sequel.menyimpantf2("detail_periksa_lab", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",
                                ...
                                if (!noorder.isEmpty()) {
                                    if (Sequel.cariIsi(
                                            "select permintaan_detail_permintaan_labmb.stts_bayar from permintaan_detail_permintaan_labmb where permintaan_detail_permintaan_labmb.noorder='"
                                            + noorder
                                            + "' and permintaan_detail_permintaan_labmb.kd_jenis_prw='"
                                            + tbPemeriksaan.getValueAt(i, 15).toString()
                                            + "' and permintaan_detail_permintaan_labmb.id_template='"
                                            + tbPemeriksaan.getValueAt(i, 6).toString() + "'")
                                            .equals("Belum")) {
            ...
            if (sukses == true) {
                if (!noorder.isEmpty()) {
                    Sequel
                            .mengedit("permintaan_labmb", "noorder=?", "tgl_hasil=?,jam_hasil=?", 3,
                                    new String[]{
    ...
    private static final Logger LOG = Logger.getLogger(DlgPeriksaLaboratoriumMB.class.getName());
