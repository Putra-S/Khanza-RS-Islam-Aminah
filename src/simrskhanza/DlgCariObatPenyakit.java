 ...
 * @author dosen
 */
public class DlgCariObatPenyakit extends javax.swing.JDialog {
    ...
    public DlgCariObatPenyakit(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Kode Obat", "Nama Obat", "Jenis Obat", "Harga Obat", "Jml", "Embalase", "Tuslah",
                    "Total Biaya", "Kode Penyakit", "Nama Penyakit", "Ciri-ciri Penyakit", "Keterangan",
                    "Kategori Penyakit", "Ciri-ciri Umum", "Referensi", "H.Beli", "Stok", "No.Batch",
                    "No.Faktur"}) {
            @Override
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        ...
        obatpenyakit.isCek();
        obatpenyakit.setSize(internalFrame1.getWidth() - 20, internalFrame1.
                getHeight() - 20);
...
private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
            ...
            for (int r = 0; r < row; r++) {
                if (Valid.SetAngka(tabMode.getValueAt(r, 4).toString()) > 0) {
                    if (Sequel.menyimpantf("detail_pemberian_obat",
                            ...
                            + Valid.SetAngka(tabMode.getValueAt(r, 15).
                                    toString()) + "','"
                            + Valid.
                                    SetAngka(tabMode.getValueAt(r, 3).toString()) + "','"
                            + Valid.
                                    SetAngka(tabMode.getValueAt(r, 4).toString()) + "','"
                            + Valid.
                                    SetAngka(tabMode.getValueAt(r, 5).toString()) + "','"
                            + Valid.
                                    SetAngka(tabMode.getValueAt(r, 6).toString()) + "','"
                            + Valid.
                                    SetAngka(tabMode.getValueAt(r, 7).toString()) + "','Ralan','" + bangsal + "','"
                        ...
                        if (aktifkanbatch.equals("yes")) {
                            Sequel.mengedit("data_batch",
                                    ...
                                    "sisa=sisa-?", 4, new String[]{
                                        "" + (tabMode.getValueAt(r, 4).
                                                ...
                                                r, 17).toString(), tabMode.
                                                getValueAt(r, 0).toString(),
                                        tabMode.getValueAt(r, 18).toString()
                                    });
                            Trackobat.catatRiwayat(tabMode.getValueAt(r, 0).
                                    toString(), 0, Valid.SetAngka(tabMode.
                                            getValueAt(r, 4).toString()),
                                    "Pemberian Obat", akses.getkode(), bangsal,
                                    "Simpan", tabMode.getValueAt(r, 17).
                                            toString(), tabMode.
                                            getValueAt(r, 18).toString(), TNoRw.
                                    getText() + " " + TPasien.getText());
                            Sequel.menyimpan("gudangbarang", "'" + tabMode.
                                    getValueAt(r, 0).toString() + "','" + bangsal + "','-" + tabMode.
                                    getValueAt(r, 4).toString() + "','" + tabMode.
                                    getValueAt(r, 17).toString() + "','" + tabMode.
                                    getValueAt(r, 18).toString() + "'",
                                    "stok=stok-'" + tabMode.getValueAt(r, 4).
                                            toString() + "'",
                                    "kode_brng='" + tabMode.getValueAt(r, 0).
                                            toString() + "' and kd_bangsal='" + bangsal + "' and no_batch='" + tabMode.
                                            getValueAt(r, 17).toString() + "' and no_faktur='" + tabMode.
                                    getValueAt(r, 18).toString() + "'");
                        } else {
                            Trackobat.catatRiwayat(tabMode.getValueAt(r, 0).
                                    toString(), 0, Valid.SetAngka(tabMode.
                            ...
                            Sequel.menyimpan("gudangbarang", "'" + tabMode.
                                    ...
                                    "stok=stok-'" + tabMode.getValueAt(r, 4).
                                            toString() + "'",
                                    "kode_brng='" + tabMode.getValueAt(r, 0).
                                            toString() + "' and kd_bangsal='" + bangsal + "' and no_batch='' and no_faktur=''");
                        }
                    } else {
                        sukses = false;
                    }
            ...
            if (aktifkanbatch.equals("yes")) {
                if (kenaikan > 0) {
                    ps = koneksi.prepareStatement(
                            ...
                            + "jenis.nama,(data_batch.h_beli+(data_batch.h_beli*?)) as harga,obat_penyakit.referensi,databarang."
                            + hppfarmasi
                            + " as dasar,gudangbarang.stok,gudangbarang.no_batch,gudangbarang.no_faktur "
                            ...
                            + "where gudangbarang.stok>0 and gudangbarang.kd_bangsal=? and penyakit.nm_penyakit like ? order by databarang.nama_brng LIMIT "
                            + awal + ",500");
                } else {
                    ps = koneksi.prepareStatement(
                            "select obat_penyakit.kd_penyakit,penyakit.nm_penyakit,penyakit.ciri_ciri,penyakit.keterangan, "
                            + "kategori_penyakit.nm_kategori,kategori_penyakit.ciri_umum,obat_penyakit.kode_brng,databarang.nama_brng,"
                            + "jenis.nama,databarang.ralan,obat_penyakit.referensi,databarang." + hppfarmasi
                            + " as dasar,gudangbarang.stok,gudangbarang.no_batch,gudangbarang.no_faktur "
                            + "from obat_penyakit inner join penyakit on obat_penyakit.kd_penyakit=penyakit.kd_penyakit "
                            + "inner join kategori_penyakit on penyakit.kd_ktg=kategori_penyakit.kd_ktg "
                            + "inner join databarang on obat_penyakit.kode_brng=databarang.kode_brng "
                ...
                try {
                    ...
                    if (kenaikan > 0) {
                        ...
                        rs = ps.executeQuery();
                        while (rs.next()) {
                            tabMode.addRow(new String[]{rs.getString("kode_brng"), rs.getString("nama_brng"),
                                rs.getString("nama"), rs.getString("harga"), "", "0", "0", "0",
                                rs.getString("kd_penyakit"), rs.getString("nm_penyakit"), rs.getString("ciri_ciri"),
                                rs.getString("keterangan"), rs.getString("nm_kategori"), rs.getString("ciri_umum"),
                                rs.getString("referensi"), rs.getString("dasar"), rs.getString("stok"),
                        ...
                        while (rs.next()) {
                            tabMode.addRow(new String[]{rs.getString("kode_brng"), rs.getString("nama_brng"),
                                rs.getString("nama"), rs.getString("ralan"), "", "0", "0", "0",
                                rs.getString("kd_penyakit"), rs.getString("nm_penyakit"), rs.getString("ciri_ciri"),
                                rs.getString("keterangan"), rs.getString("nm_kategori"), rs.getString("ciri_umum"),
                                rs.getString("referensi"), rs.getString("dasar"), rs.getString("stok"),
                                rs.getString("no_batch"), rs.getString("no_faktur")});
                ...
                if (kenaikan > 0) {
                    ps = koneksi.prepareStatement(
                            "select obat_penyakit.kd_penyakit,penyakit.nm_penyakit,penyakit.ciri_ciri,penyakit.keterangan, "
                            + "kategori_penyakit.nm_kategori,kategori_penyakit.ciri_umum,obat_penyakit.kode_brng,databarang.nama_brng,"
                            + "jenis.nama,(databarang.h_beli+(databarang.h_beli*?)) as harga,obat_penyakit.referensi,databarang."
                            ...
                            + "where gudangbarang.no_batch='' and gudangbarang.no_faktur='' and gudangbarang.stok>0 and gudangbarang.kd_bangsal=? and penyakit.nm_penyakit like ? order by databarang.nama_brng LIMIT "
                            + awal + ",500");
                } else {
                    ps = koneksi.prepareStatement(
                            "select obat_penyakit.kd_penyakit,penyakit.nm_penyakit,penyakit.ciri_ciri,penyakit.keterangan, "
    ...
    private void getData() {
        ...
        if (bangsal.trim().isEmpty()) {
            Valid.textKosong(PenyakitCari, "Asal Stok");
        } else if (row != -1) {
            if (!tabMode.getValueAt(row, 4).toString().isEmpty()) {
                try {
                    if (Double.parseDouble(tabMode.getValueAt(row, 4).toString()) > 0) {
                        ...
                        if (jumlah < x) {
                            ...
                            tabMode.setValueAt(Double.toString(Valid.SetAngka(tabMode.getValueAt(row, 3).toString())
                                    * Valid.SetAngka(tabMode.getValueAt(row, 4).toString())
                                    + Valid.SetAngka(tabMode.getValueAt(row, 5).toString())
                                    + Valid.SetAngka(tabMode.getValueAt(row, 6).toString())), row, 7);
                        }
                    }
    ...
    public void setNoRm(String norwt, String penyakit) {
        ...
        tampil();
        TNoRw.setText(norwt);
        Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=? ", Kd2,
                TNoRw.getText());
        Sequel.cariIsi("select pasien.nm_pasien from pasien where pasien.no_rkm_medis=? ", TPasien, Kd2.getText());

        kenaikan = Sequel.cariIsiAngka("select (hargajual/100) from set_harga_obat_ralan where kd_pj='"
                + Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", norwt)
                + "'");

        bangsal = Sequel.cariIsi("select kd_bangsal from set_depo_ralan where kd_poli=?", Sequel
                .cariIsi("select reg_periksa.kd_poli from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText()));
        if (bangsal.isEmpty()) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariObatPenyakit.class.getName());
