...
public class DlgCariCaraBayar extends javax.swing.JDialog {
    ...
    public DlgCariCaraBayar(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"No.", "Kode Asuransi", "Nama Asuransi", "Perusahaan Asuransi", "Alamat Asuransi", "No.Telp",
            "Attn"};
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                ...
                i = 1;
                while (rs.next()) {
                    tabMode.addRow(new Object[]{i, rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4),
                        rs.getString(5), rs.getString(6)});
                    iyem = iyem + "{\"KodeAsuransi\":\"" + rs.getString(1) + "\",\"NamaAsuransi\":\"" + rs.getString(2)
                            + "\",\"PerusahaanAsuransi\":\"" + rs.getString(3) + "\",\"AlamatAsuransi\":\""
                            + rs.getString(4) + "\",\"NoTelp\":\"" + rs.getString(5) + "\",\"Attn\":\""
                            + rs.getString(6) + "\"},";
                    i++;
                }
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    ...
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{i, list.path("KodeAsuransi").asText(),
                            list.path("NamaAsuransi").asText(), list.path("PerusahaanAsuransi").asText(),
                            list.path("AlamatAsuransi").asText(), list.path("NoTelp").asText(),
                            list.path("Attn").asText()});
                        i++;
                    }
                } else {
                    i = 1;
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariCaraBayar.class.getName());
