...
public class DlgCariCaraBayar2 extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariCaraBayar2 dialog = new DlgCariCaraBayar2(
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    iyem = iyem + "{\"KodeAsuransi\":\"" + rs.getString(1) + "\",\"NamaAsuransi\":\"" + rs.
                            getString(2) + "\",\"PerusahaanAsuransi\":\"" + rs.
                            getString(3) + "\",\"AlamatAsuransi\":\"" + rs.
                            getString(4) + "\",\"NoTelp\":\"" + rs.getString(5) + "\",\"Attn\":\"" + rs.
                            getString(6) + "\"},";
                }
            ...
            fileWriter.write("{\"penjab\":[" + iyem.substring(0,
                    iyem.length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
        ...
        try {
            ...
            for (i = 0; i < tbKamar.getRowCount(); i++) {
                if (tbKamar.getValueAt(i, 0).toString().equals("true")) {
                    ...
                    PerusahaanAsuransi[index] = tbKamar.getValueAt(i, 3).
                            toString();
            ...
            Valid.tabelKosong(tabMode);
            for (i = 0; i < jml; i++) {
                tabMode.addRow(new Object[]{pilih[i], KodeAsuransi[i],
                    NamaAsuransi[i], PerusahaanAsuransi[i], AlamatAsuransi[i],
                    NoTelp[i], Attn[i]});
            }
                ...
                if (TCari.getText().trim().isEmpty()) {
                    ...
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            false, list.path("KodeAsuransi").asText(), list.
                            path("NamaAsuransi").asText(), list.path(
                            "PerusahaanAsuransi").asText(), list.path(
                            "AlamatAsuransi").asText(), list.path("NoTelp").
                            asText(), list.path("Attn").asText()
                        });
                        i++;
                    }
                } else {
                    i = 1;
                    for (JsonNode list : response) {
                        if (list.path("KodeAsuransi").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase()) || list.
                                path("NamaAsuransi").asText().toLowerCase().
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariCaraBayar2.class.
            getName());
}
