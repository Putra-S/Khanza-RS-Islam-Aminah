...
public class DlgCariBangsal extends javax.swing.JDialog {
    ...
    public DlgCariBangsal(java.awt.Frame parent, boolean modal) {
        ...
        setSize(656, 250);
        tabMode = new DefaultTableModel(null, new String[]{"Kode Kamar",
            "Nama Kamar"}) {
            @Override
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariBangsal dialog = new DlgCariBangsal(new javax.swing.JFrame(),
                    true);
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select * from bangsal where bangsal.status='1' order by bangsal.nm_bangsal");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(
                            new Object[]{rs.getString(1), rs.getString(2)});
                    iyem = iyem + "{\"KodeKamar\":\"" + rs.getString(1) + "\",\"NamaKamar\":\"" + rs.
            ...
            fileWriter.write("{\"bangsal\":[" + iyem.substring(0,
                    iyem.length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeKamar").asText(), list.path(
                            "NamaKamar").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("NamaKamar").asText().toLowerCase().
    ...
    public String tampil3(String kode) {
        ...
        try {
            ...
            if (response.isArray()) {
                for (JsonNode list : response) {
                    if (list.path("KodeKamar").asText().toLowerCase().equals(
                            kode)) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariBangsal.class.
            getName());
}
