...
//        Valid.pindah(evt,TCari,BtnSimpan,TCari);
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("NamaProp").asText(), list.path("KodeProp").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("NamaProp").asText().toLowerCase().contains(TCari.getText().toLowerCase())) {
                            tabMode.addRow(
                                    new Object[]{list.path("NamaProp").asText(), list.path("KodeProp").asText()});
                        }
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(DlgPropinsi.class.getName());
