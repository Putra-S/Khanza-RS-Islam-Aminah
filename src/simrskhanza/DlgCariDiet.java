 ...
 * @author dosen
 */
public class DlgCariDiet extends javax.swing.JDialog {
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        ...
        diet.emptTeks();
        diet.isCek();
        diet.setSize(internalFrame1.getWidth() - 20,
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select diet.kd_diet, diet.nama_diet from diet order by diet.nama_diet");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(
                            new Object[]{rs.getString(1), rs.getString(2)});
                    iyem = iyem + "{\"KodeDiet\":\"" + rs.getString(1) + "\",\"NamaDiet\":\"" + rs.
                            getString(2) + "\"},";
                }
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeDiet").asText(), list.
                            path("NamaDiet").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeDiet").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase()) || list.
                                path("NamaDiet").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase())) {
                            tabMode.addRow(new Object[]{
                                list.path("KodeDiet").asText(), list.path(
                                "NamaDiet").asText()
                            });
                        }
                    }
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariDiet.class.
            getName());
}
