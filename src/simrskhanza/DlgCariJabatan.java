 ...
 * @author dosen
 */
public class DlgCariJabatan extends javax.swing.JDialog {
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        ...
        jabatan.emptTeks();
        jabatan.setSize(internalFrame1.getWidth() - 20, internalFrame1.
                getHeight() - 20);
    ...
    private void tampil() {
        ...
        try {
            ...
            iyem = "";
            ps = koneksi
                    .prepareStatement("select jabatan.kd_jbtn, jabatan.nm_jbtn from jabatan order by jabatan.nm_jbtn");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(
                                new Object[]{list.path("KodeJabatan").asText(), list.path("NamaJabatan").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeJabatan").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                                || list.path("NamaJabatan")
                                        .asText()
                                        .toLowerCase()
                                        .contains(TCari.getText().toLowerCase())) {
                            tabMode.addRow(new Object[]{list.path("KodeJabatan").asText(),
                                list.path("NamaJabatan").asText()});
                        }
                    }
                }
            }
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariJabatan.class.getName());
