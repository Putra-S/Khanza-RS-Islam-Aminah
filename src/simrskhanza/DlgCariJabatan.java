...
public class DlgCariJabatan extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariJabatan dialog = new DlgCariJabatan(new javax.swing.JFrame(),
                    true);
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select jabatan.kd_jbtn, jabatan.nm_jbtn from jabatan order by jabatan.nm_jbtn");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(
                            new String[]{rs.getString(1), rs.getString(2)});
                    iyem = iyem + "{\"KodeJabatan\":\"" + rs.getString(1) + "\",\"NamaJabatan\":\"" + rs.
                            getString(2) + "\"},";
                }
            ...
            fileWriter.write("{\"jabatan\":[" + iyem.substring(0,
                    iyem.length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeJabatan").asText(), list.path(
                            "NamaJabatan").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeJabatan").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase()) || list.
                                path("NamaJabatan").asText().toLowerCase().
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariJabatan.class.
            getName());
}
