...
public class InventoryRingkasanPemesananBarangMedis extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public InventoryRingkasanPemesananBarangMedis(java.awt.Frame parent,
            boolean modal) {
        super(parent, modal);
        initComponents();
...
*/
    ...
    private void kdptgKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdptgKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi("select nama from pegawai where nip=?", nmptg, kdptg.
                    getText());
        } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            Sequel.cariIsi("select nama from pegawai where nip=?", nmptg, kdptg.
                    getText());
    ...
    private void kdsatKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdsatKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi("select nama from jenis where kdjns=?", nmsat, kdsat.
                    getText());
        } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            Sequel.cariIsi("select nama from jenis where kdjns=?", nmsat, kdsat.
                    getText());
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptRingkasanPemesananObat.jasper", "report",
                    "::[ Laporan Ringkasan Pemesanan Obat/Alkes/BHP Medis ]::",
                    ...
                    + " where surat_pemesanan_medis.tanggal between '" + Valid.
                            SetTgl(TglBeli1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(TglBeli2.getSelectedItem() + "") + "' "
                    + " and surat_pemesanan_medis.no_pemesanan like '%" + NoFaktur.
                            getText() + "%' and datasuplier.nama_suplier like '%" + nmsup.
                            getText() + "%' and pegawai.nama like '%" + nmptg.
                            getText() + "%' "
                    + " and jenis.nama like '%" + nmsat.getText() + "%' and databarang.nama_brng like '%" + nmbar.
                    getText() + "%' and industrifarmasi.nama_industri like '%" + NmIF.
                            getText() + "%' and "
                    + " (surat_pemesanan_medis.no_pemesanan like '%" + TCari.
                            getText() + "%' or surat_pemesanan_medis.kode_suplier like '%" + TCari.
                            getText() + "%' or datasuplier.nama_suplier like '%" + TCari.
                            getText() + "%' "
                    + " or surat_pemesanan_medis.nip like '%" + TCari.getText() + "%' or pegawai.nama like '%" + TCari.
                    getText() + "%' or detail_surat_pemesanan_medis.kode_brng like '%" + TCari.
                            getText() + "%' "
                    + " or databarang.nama_brng like '%" + TCari.getText() + "%' or databarang.kode_sat like '%" + TCari.
                    getText() + "%' or industrifarmasi.nama_industri like '%" + TCari.
                            getText() + "%' "
                    + " or jenis.nama like '%" + TCari.getText() + "%' ) group by detail_surat_pemesanan_medis.kode_brng " + order,
                    param);
        }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            InventoryRingkasanPemesananBarangMedis dialog = new InventoryRingkasanPemesananBarangMedis(
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + " and databarang.kode_industri=industrifarmasi.kode_industri "
                    + " and surat_pemesanan_medis.nip=pegawai.nik and databarang.kdjns=jenis.kdjns"
                    + " where surat_pemesanan_medis.tanggal between ? and ? and surat_pemesanan_medis.no_pemesanan like ? and datasuplier.nama_suplier like ? and pegawai.nama like ?  and jenis.nama like ? and databarang.nama_brng like ? and industrifarmasi.nama_industri like ? and "
                    + " (surat_pemesanan_medis.no_pemesanan like ? or surat_pemesanan_medis.kode_suplier like ? or datasuplier.nama_suplier like ? or surat_pemesanan_medis.nip like ? or pegawai.nama like ? or detail_surat_pemesanan_medis.kode_brng like ? or databarang.nama_brng like ? "
                    + " or databarang.kode_sat like ? or industrifarmasi.nama_industri like ? or jenis.nama like ? )"
                    + " group by detail_surat_pemesanan_medis.kode_brng " + order);
            try {
    ...
    private static final Logger LOG = Logger.getLogger(
            InventoryRingkasanPemesananBarangMedis.class.getName());
