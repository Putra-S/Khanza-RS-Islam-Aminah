...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptPermintaan.jasper", "report",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
        ...
        Sequel.queryu("delete from permintaan_medis where no_permintaan=?",
    ...
    private void ppDisetujuiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDisetujuiActionPerformed
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            if (Sequel.cariIsi(
                    "select permintaan_medis.status from permintaan_medis where permintaan_medis.no_permintaan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim()).equals("Baru")) {
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    ...
    private void ppTidakDisetujuiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppTidakDisetujuiActionPerformed
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            Sequel.queryu(
                    "update permintaan_medis set status='Tidak Disetujui' where no_permintaan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim());
            tampil();
        }
        ...
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            if (Sequel.cariIsi(
                    ...
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().
                            trim()).equals("Baru")) {
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                DlgPengeluaranApotek aplikasi2 = new DlgPengeluaranApotek(null,
                        false);
                aplikasi2.tampilkanpermintaan = false;
                aplikasi2.tampil(tbDokter.getValueAt(tbDokter.getSelectedRow(),
                        1).toString().trim());
                aplikasi2.isCek();
                aplikasi2.setSize(internalFrame1.getWidth() - 20,
