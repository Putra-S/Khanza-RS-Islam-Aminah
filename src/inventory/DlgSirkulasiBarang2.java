...
public class DlgSirkulasiBarang2 extends javax.swing.JDialog {
    ...
    private double jumlahjual = 0, jumlahbeli = 0, jumlahpesan = 0, jumlahpiutang = 0, jumlahutd = 0, jumlahkeluar = 0,
            ...
            jumlahpasin = 0, jumlahrespulang = 0, jumlahhibah = 0, stok = 0, stokawal = 0, stokakhir = 0,
            rowstokawal = 0;
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    // pemesanan
                    ps2 = koneksi
                            .prepareStatement("select sum(detailpesan.jumlah2) " + " from pemesanan inner join detailpesan "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(detreturbeli.jml_retur2) " + " from returbeli inner join detreturbeli "
                    ...
                    // returjual
                    ps2 = koneksi.prepareStatement(
                            "select sum(detreturjual.jml_retur) " + " from returjual inner join detreturjual "
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detreturpiutang.jml_retur) "
                            ...
                            + " where detreturpiutang.kode_brng=? and " + " returpiutang.tgl_retur between ? and ?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(mutasibarang.jml) " + " from mutasibarang where mutasibarang.kode_brng=? and "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(opname.real) from opname where opname.kode_brng=? and opname.tanggal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        rs2 = ps2.executeQuery();
                    ...
                    if ((jumlahbeli > 0) || (jumlahpesan > 0) || (jumlahjual > 0) || (jumlahpasin > 0)
                        ...
                        tabMode.addRow(new Object[]{rs.getString(1), rs.getString(2), rs.getString(3), tglopname,
                            Valid.SetAngka(stokawal), Valid.SetAngka(jumlahbeli), Valid.SetAngka(jumlahpesan),
                            Valid.SetAngka(jumlahjual), Valid.SetAngka(jumlahpasin), Valid.SetAngka(jumlahpiutang),
                            Valid.SetAngka(jumlahretbeli), Valid.SetAngka(jumlahretjual),
                            Valid.SetAngka(jumlahretpiut), Valid.SetAngka(jumlahutd), Valid.SetAngka(jumlahkeluar),
                            Valid.SetAngka(jumlahrespulang), Valid.SetAngka(jumlahmutasimasuk),
                            Valid.SetAngka(jumlahmutasikeluar), Valid.SetAngka(jumlahhibah),
                            Valid.SetAngka(stokakhir)});
                    }
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    // piutang
                    ps2 = koneksi.prepareStatement(
                            "select sum(detailpiutang.jumlah) " + " from piutang inner join detailpiutang "
                            + " on piutang.nota_piutang=detailpiutang.nota_piutang "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(detreturpiutang.jml_retur) " + " from returpiutang inner join detreturpiutang "
                            + " on returpiutang.no_retur_piutang=detreturpiutang.no_retur_piutang "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(opname.real) from opname where opname.kode_brng=? and opname.tanggal=? and opname.kd_bangsal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
