 ...
 * @author dosen
 */
public class DlgCariSatuan extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariSatuan dialog = new DlgCariSatuan(new javax.swing.JFrame(),
                    true);
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select * from kodesatuan order by satuan ");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(
                            new String[]{rs.getString(1), rs.getString(2)});
                    iyem = iyem + "{\"KodeSatuan\":\"" + rs.getString(1) + "\",\"NamaSatuan\":\"" + rs.
                            getString(2) + "\"},";
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"satuanbarang\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeSatuan").asText(), list.path(
                            "NamaSatuan").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("NamaSatuan").asText().toLowerCase().
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariSatuan.class.
            getName());
}
