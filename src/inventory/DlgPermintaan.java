...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        ...
        if (NoPermintaan.getText().trim().isEmpty()) {
        ...
        } else if (nmgudangTujuan.getText().trim().isEmpty()) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantf2("permintaan_medis", "?,?,?,?,?,?",
                        "No.Permintaan", 6, new String[]{
                            NoPermintaan.getText(), kdgudangasal.getText(),
                            kdptg.getText(), Valid.SetTgl(Tanggal.
                            getSelectedItem() + ""), "Baru", kdgudangTujuan.
                                    getText()
                        }) == true) {
                    ...
                    JOptionPane.showMessageDialog(rootPane,
                            "Gagal Menyimpan, kemungkinan No.Permintaan sudah ada sebelumnya...!!");
                }
                if (sukses == true) {
                    Sequel.Commit();
                    jml = tbDokter.getRowCount();
    ...
    private void kdgudangTujuanKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdgudangTujuanKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi(
                    ...
                    nmgudangTujuan, kdgudangTujuan.getText());
        } else if (evt.getKeyCode() == KeyEvent.VK_PAGE_UP) {
            Sequel.cariIsi(
                    "select bangsal.nm_bangsal from bangsal where bangsal.kd_bangsal=?",
                    nmgudangTujuan, kdgudangTujuan.getText());
