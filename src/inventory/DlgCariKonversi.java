 ...
 * @author dosen
 */
public class DlgCariKonversi extends javax.swing.JDialog {
...
// TODO add your handling code here:
    ...
    private void isHitung() {
        if ((!n1.getText().isEmpty()) && (!n2.getText().isEmpty())) {
            if (!nmsat1.getText().equals(nmsat2.getText())) {
                try {
                    ResultSet rs = koneksi.createStatement()
                            .executeQuery("select nilai,nilai_konversi from konver_sat where kode_sat='"
                                    + kdsat1.getSelectedItem() + "' and sat_konversi='" + kdsat2.getSelectedItem() + "'");
                    if (rs.next()) {
                        n2.setText(Double
                                .toString(Double.parseDouble(n1.getText()) * (rs.getDouble(2) / rs.getDouble(1))));
                    } else if (!rs.next()) {
                        int reply = JOptionPane.showConfirmDialog(rootPane,
                                "Maaf, konversi satuan tidak ditemukan. Apa anda ingin menambahkan..??", "Konfirmasi",
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariKonversi.class.getName());
