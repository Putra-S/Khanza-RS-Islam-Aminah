...
public class DlgSirkulasiBarang5 extends javax.swing.JDialog {
    ...
    private double jumlahjual = 0, jumlahbeli = 0, jumlahpesan = 0, jumlahpiutang = 0, jumlahutd = 0, jumlahkeluar = 0,
            jumlahmutasikeluar = 0, jumlahmutasimasuk = 0, jumlahretbeli = 0, jumlahretjual = 0, jumlahretpiut = 0,
            jumlahpasin = 0, jumlahrespulang = 0, jumlahhibah = 0, stok = 0, stokawal = 0, stokakhir = 0, totaljual = 0,
            totalbeli = 0, totalpesan = 0, totalpiutang = 0, totalutd = 0, totalkeluar = 0, totalmutasikeluar = 0,
            totalmutasimasuk = 0, totalretbeli = 0, totalretjual = 0, totalretpiut = 0, totalpasin = 0,
            totalrespulang = 0, totalhibah = 0, totalstok = 0, totalstokawal = 0, totalstokakhir = 0, ttltotaljual = 0,
            ttltotalbeli = 0, ttltotalpesan = 0, ttltotalpiutang = 0, ttltotalutd = 0, ttltotalkeluar = 0,
            ttltotalmutasikeluar = 0, ttltotalmutasimasuk = 0, ttltotalretbeli = 0, ttltotalretjual = 0,
            ttltotalretpiut = 0, ttltotalpasin = 0, ttltotalrespulang = 0, ttltotalhibah = 0, ttltotalstok = 0,
            ttltotalstokawal = 0, ttltotalstokakhir = 0, rowstokawal = 0, harga = 0;
    ...
    public DlgSirkulasiBarang5(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null,
                new Object[]{"Kode Barang", "Nama Barang", "Satuan", "Tgl.Opname", "Stok Awal", "Stok Awal(Rp)",
                    "Pengadaan", "Pengadaan(Rp)", "Penerimaan", "Penerimaan(Rp)", "Penjualan", "Penjualan(Rp)",
                    "Ke Pasien", "Ke Pasien(Rp)", "Piutang Jual", "Piutang Jual(Rp)", "Retur Beli",
                    "Retur Beli(Rp)", "Retur Jual", "Retur Jual(Rp)", "Retur Piutang", "Retur Piutang(Rp)",
                    "Pengambilan UTD", "Pengambilan UTD(Rp)", "Stok Keluar Medis", "Stok Keluar Medis(Rp)",
                    "Resep Pulang", "Resep Pulang(Rp)", "Mutasi Masuk", "Mutasi Masuk(Rp)", "Mutasi Keluar",
                    "Mutasi Keluar(Rp)", "Hibah", "Hibah(Rp)", "Stok Akhir", "Stok Akhir(Rp)"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void prosesCari() {
        ...
        try {
            ...
            ttltotalstokawal = 0;
            ttltotalstokakhir = 0;
            ps = koneksi.prepareStatement("select databarang.kode_brng,databarang.nama_brng,kodesatuan.satuan "
            ...
            try {
                ...
                if (aktifkanbatch.equals("yes")) {
                    qrystok = "select sum(gudangbarang.stok),(sum(gudangbarang.stok)*databarang." + hppfarmasi
                            + ") as aset,databarang." + hppfarmasi + " as harga "
                ...
                while (rs.next()) {
                    ...
                    totalstokakhir = 0;
                    harga = 0;
                    tglopname = Sequel.cariIsi("select opname.tanggal from opname where opname.kode_brng='"
                            + rs.getString(1) + "' and opname.tanggal between '"
                            + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                            + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' order by opname.tanggal asc limit 1");

                    if (tglopname.isEmpty()) {
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detailbeli.jumlah2), sum(detailbeli.total) "
                            + " from pembelian inner join detailbeli " + " on pembelian.no_faktur=detailbeli.no_faktur "
                            + " where detailbeli.kode_brng=? and pembelian.tgl_beli " + " between ? and ? ");
                    try {
                        ps2.setString(1, rs.getString(1));
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detailpesan.jumlah2), sum(detailpesan.total) "
                            ...
                            + " where detailpesan.kode_brng=? and pemesanan.tgl_pesan " + " between ? and ? ");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detailpiutang.jumlah), sum(detailpiutang.total) "
                            ...
                            + " where detailpiutang.kode_brng=? and " + " piutang.tgl_piutang between ? and ? ");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detreturbeli.jml_retur2), sum(detreturbeli.total) "
                            ...
                            + " where detreturbeli.kode_brng=? and " + " returbeli.tgl_retur between ? and ? ");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi.prepareStatement("select sum(detreturjual.jml_retur), sum(detreturjual.subtotal) "
                            ...
                            + " where detreturjual.kode_brng=? and " + " returjual.tgl_retur between ? and ? ");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, Valid.SetTgl(Tgl2.getSelectedItem() + ""));
                    ...
                    ps2 = koneksi
                            .prepareStatement("select sum(detreturpiutang.jml_retur), sum(detreturpiutang.subtotal) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(detail_pengeluaran_obat_bhp.jumlah), sum(detail_pengeluaran_obat_bhp.total) "
                    ...
                    // hibah
                    ps2 = koneksi.prepareStatement(
                            "select sum(detailhibah_obat_bhp.jumlah2), sum(detailhibah_obat_bhp.subtotaldiakui) "
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(opname.real),(opname.real*opname.h_beli) from opname where opname.kode_brng=? and opname.tanggal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        rs2 = ps2.executeQuery();
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select count(opname.real) from opname where opname.kode_brng=? and opname.tanggal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        rs2 = ps2.executeQuery();
                    ...
                    ps2 = koneksi
                            .prepareStatement("select sum(mutasibarang.jml), sum(mutasibarang.jml*mutasibarang.harga) "
                    ...
                    boolean adaPerubahan = jumlahbeli > 0 || jumlahpesan > 0 || jumlahjual > 0 || jumlahpasin > 0
                            || jumlahpiutang > 0 || jumlahhibah > 0 || jumlahmutasimasuk > 0 || jumlahmutasikeluar > 0
                            || jumlahutd > 0 || jumlahkeluar > 0 || jumlahretbeli > 0 || jumlahretjual > 0
                            || jumlahretpiut > 0 || stok > 0 || jumlahrespulang > 0;
                    if (adaPerubahan) {
//					if ((jumlahbeli > 0) || (jumlahpesan > 0) || (jumlahjual > 0) || (jumlahpasin > 0)
//							|| (jumlahpiutang > 0) || (jumlahhibah > 0) || (jumlahutd > 0) || (jumlahkeluar > 0)
//							|| (jumlahretbeli > 0) || (jumlahretjual > 0) || (jumlahretpiut > 0) || (stok > 0)
//							|| (jumlahrespulang > 0)) {
                        if (rowstokawal <= 0) {
                            stokawal = stok - jumlahbeli - jumlahpesan - jumlahmutasimasuk + jumlahmutasikeluar
                                    + jumlahjual + jumlahpasin + jumlahpiutang - jumlahhibah + jumlahretbeli
                                    - jumlahretjual - jumlahretpiut + jumlahutd + jumlahkeluar + jumlahrespulang;
                            stokakhir = stok;
                            totalstokawal = totalstok - totalbeli - totalpesan - totalmutasimasuk + totalmutasikeluar
                                    + totaljual + totalpasin + totalpiutang - totalhibah + totalretbeli - totalretjual
                                    - totalretpiut + totalutd + totalkeluar + totalrespulang;
                            totalstokakhir = totalstok;
                        } else {
                            stokakhir = stokawal + jumlahbeli + jumlahpesan + jumlahmutasimasuk - jumlahmutasikeluar
                                    - jumlahjual - jumlahpasin - jumlahpiutang + jumlahhibah - jumlahretbeli
                                    + jumlahretjual + jumlahretpiut - jumlahutd - jumlahkeluar - jumlahrespulang;
                            totalstokakhir = stokakhir * harga;
                        }

                        // Menyimpan data untuk ditambahkan ke tabMode setelah selesai menghitung
                        List<Object[]> rowDataList = new ArrayList<>();

                        rowDataList.add(new Object[]{
                            rs.getString(1), rs.getString(2), rs.getString(3), tglopname,
                            Valid.SetAngka(stokawal), Valid.SetAngka(totalstokawal), Valid.SetAngka(jumlahbeli),
                            Valid.SetAngka(totalbeli), Valid.SetAngka(jumlahpesan), Valid.SetAngka(totalpesan),
                            Valid.SetAngka(jumlahjual), Valid.SetAngka(totaljual), Valid.SetAngka(jumlahpasin),
                            Valid.SetAngka(totalpasin), Valid.SetAngka(jumlahpiutang), Valid.SetAngka(totalpiutang),
                            Valid.SetAngka(jumlahretbeli), Valid.SetAngka(totalretbeli), Valid.SetAngka(jumlahretjual),
                            Valid.SetAngka(totalretjual), Valid.SetAngka(jumlahretpiut), Valid.SetAngka(totalretpiut),
                            Valid.SetAngka(jumlahutd), Valid.SetAngka(totalutd), Valid.SetAngka(jumlahkeluar),
                            Valid.SetAngka(totalkeluar), Valid.SetAngka(jumlahrespulang), Valid.SetAngka(totalrespulang),
                            ...
                            Valid.SetAngka(jumlahhibah), Valid.SetAngka(totalhibah), Valid.SetAngka(stokakhir),
                            Valid.SetAngka(totalstokakhir)
                        });

                        // Setelah selesai menghitung semua data, masukkan ke tabMode
                        for (Object[] rowData : rowDataList) {
                            tabMode.addRow(rowData);
                        }
//						tabMode.addRow(new Object[] { rs.getString(1), rs.getString(2), rs.getString(3), tglopname,
//								Valid.SetAngka(stokawal), Valid.SetAngka(totalstokawal), Valid.SetAngka(jumlahbeli),
//								Valid.SetAngka(totalbeli), Valid.SetAngka(jumlahpesan), Valid.SetAngka(totalpesan),
//								Valid.SetAngka(jumlahjual), Valid.SetAngka(totaljual), Valid.SetAngka(jumlahpasin),
//								Valid.SetAngka(totalpasin), Valid.SetAngka(jumlahpiutang), Valid.SetAngka(totalpiutang),
...
//								Valid.SetAngka(jumlahretpiut), Valid.SetAngka(totalretpiut), Valid.SetAngka(jumlahutd),
//								Valid.SetAngka(totalutd), Valid.SetAngka(jumlahkeluar), Valid.SetAngka(totalkeluar),
...
//								Valid.SetAngka(totalstokakhir) });
            ...
            tabMode.addRow(new Object[]{"<>>", "Total :", "", "", "", Valid.SetAngka(ttltotalstokawal), "",
                Valid.SetAngka(ttltotalbeli), "", Valid.SetAngka(ttltotalpesan), "", Valid.SetAngka(ttltotaljual),
                "", Valid.SetAngka(ttltotalpasin), "", Valid.SetAngka(ttltotalpiutang), "",
                Valid.SetAngka(ttltotalretbeli), "", Valid.SetAngka(ttltotalretjual), "",
                Valid.SetAngka(ttltotalretpiut), "", Valid.SetAngka(ttltotalutd), "",
                Valid.SetAngka(ttltotalkeluar), "", Valid.SetAngka(ttltotalrespulang), "",
                Valid.SetAngka(ttltotalmutasimasuk), "", Valid.SetAngka(ttltotalmutasikeluar), "",
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    totalstokakhir = 0;
                    harga = 0;
                    tglopname = Sequel.cariIsi("select opname.tanggal from opname where opname.tanggal between '"
                            + Valid.SetTgl(Tgl1.getSelectedItem() + "") + "' and '"
                            + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' and opname.kode_brng='" + rs.getString(1)
                            + "' and opname.kd_bangsal='" + lokasi + "'  order by opname.tanggal asc limit 1");
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select sum(opname.real),(opname.real*opname.h_beli) from opname where opname.kode_brng=? and opname.tanggal=? and opname.kd_bangsal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                    ...
                    ps2 = koneksi.prepareStatement(
                            "select count(opname.real) from opname where opname.kode_brng=? and opname.tanggal=? and opname.kd_bangsal=?");
                    try {
                        ps2.setString(1, rs.getString(1));
                        ps2.setString(2, tglopname);
                        ps2.setString(3, lokasi);
                    ...
                    if (adaPerubahan) {
//					if ((jumlahbeli > 0) || (jumlahpesan > 0) || (jumlahjual > 0) || (jumlahpasin > 0)
//							|| (jumlahpiutang > 0) || (jumlahhibah > 0) || (jumlahmutasimasuk > 0)
//							|| (jumlahmutasikeluar > 0) || (jumlahutd > 0) || (jumlahkeluar > 0) || (jumlahretbeli > 0)
//							|| (jumlahretjual > 0) || (jumlahretpiut > 0) || (stok > 0) || (jumlahrespulang > 0)) {
                        if (rowstokawal <= 0) {
                            stokawal = stok - jumlahbeli - jumlahpesan - jumlahmutasimasuk + jumlahmutasikeluar
                                    + jumlahjual + jumlahpasin + jumlahpiutang - jumlahhibah + jumlahretbeli
                                    - jumlahretjual - jumlahretpiut + jumlahutd + jumlahkeluar + jumlahrespulang;
