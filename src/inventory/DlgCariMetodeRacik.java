...
public class DlgCariMetodeRacik extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariMetodeRacik dialog = new DlgCariMetodeRacik(
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select * from metode_racik order by metode_racik.nm_racik ");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new String[]{i + "", rs.getString(1), rs.
                        getString(2)});
                    iyem = iyem + "{\"KodeRacik\":\"" + rs.getString(1) + "\",\"NamaRacik\":\"" + rs.
                            getString(2) + "\"},";
                    i++;
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"metoderacik\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            i + "", list.path("KodeRacik").asText(), list.path(
                            "NamaRacik").asText()
                        });
                        i++;
                    }
                    ...
                    for (JsonNode list : response) {
                        if (list.path("NamaRacik").asText().toLowerCase().
                                contains(TCari.getText().toLowerCase())) {
                            tabMode.addRow(new Object[]{
                                i + "", list.path("KodeRacik").asText(), list.
                                path("NamaRacik").asText()
                            });
                            i++;
                        }
                    }
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariMetodeRacik.class.
            getName());
}
