...
public class DlgReturJual extends javax.swing.JDialog {
    ...
    public DlgReturJual(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            "No.Nota", "Kode Barang", "Nama Barang", "Satuan", "Hrg.Retur(Rp)",
...
private void BtnGudangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnGudangActionPerformed
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgReturJual dialog = new DlgReturJual(new javax.swing.JFrame(),
                    true);
    ...
    public void setPasien(String norm, String norawat) {
        ...
        Valid.autoNomer3(
                "select ifnull(MAX(CONVERT(RIGHT(no_retur_jual,2),signed)),0) from returjual where no_retur_jual like '%" + norawat + "%' ",
                norawat, 2, NoRetur);
        formvalid = "No";
        Sequel.cariIsi(
                "select pasien.nm_pasien from pasien where pasien.no_rkm_medis=?",
    ...
    private void autonomor() {
        if (!formvalid.equals("No")) {
            Valid.autoNomer3(
                    "select ifnull(MAX(CONVERT(RIGHT(no_retur_jual,3),signed)),0) from returjual where tgl_retur='" + Valid.
                            SetTgl(TglRetur.getSelectedItem() + "") + "' ",
                    "RJ" + TglRetur.getSelectedItem().toString().
                            substring(6, 10) + TglRetur.getSelectedItem().
                    toString().substring(3, 5) + TglRetur.getSelectedItem().
                    toString().substring(0, 2), 3, NoRetur);
        } else {
            Valid.autoNomer3(
                    "select ifnull(MAX(CONVERT(RIGHT(no_retur_jual,2),signed)),0) from returjual where no_retur_jual like '%" + norawat + "%' ",
                    norawat, 2, NoRetur);
    ...
    private void simpan() {
        try {
            ...
            try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    if (Sequel.menyimpantf2("detreturjual",
                            "?,?,?,?,?,?,?,?,?,?,?", "data barang sama", 11,
                            new String[]{
                                NoRetur.getText(), rs.getString(1), rs.
                        ...
                        if (aktifkanbatch.equals("yes")) {
                            Sequel.mengedit("data_batch",
                                    ...
                                    "sisa=sisa+?", 4, new String[]{
                                        rs.getString(6), rs.getString(9), rs.
                                        getString(2), rs.getString(10)
                                    });
                            Trackobat.catatRiwayat(rs.getString(2), rs.
                                    getDouble(6), 0, "Retur Jual", akses.
                                    getkode(), kdgudang.getText(), "Simpan", rs.
                                    getString(9), rs.getString(10), NoRetur.
                                    getText() + " " + kdmem.getText() + " " + nmmem.
                                    getText());
                            Sequel.menyimpan("gudangbarang", "'" + rs.getString(
                                    2) + "','" + kdgudang.getText() + "','" + rs.
                                    getString(6) + "','" + rs.getString(9) + "','" + rs.
                                    getString(10) + "'",
                                    "stok=stok+'" + rs.getString(6) + "'",
                                    "kode_brng='" + rs.getString(2) + "' and kd_bangsal='" + kdgudang.
                                    getText() + "' and no_batch='" + rs.
                                            getString(9) + "' and no_faktur='" + rs.
                                    getString(10) + "'");
                        } else {
                            Trackobat.catatRiwayat(rs.getString(2), rs.
                                    getDouble(6), 0, "Retur Jual", akses.
                                    getkode(), kdgudang.getText(), "Simpan", "",
                                    "", NoRetur.getText() + " " + kdmem.
    ...
    private static final Logger LOG = Logger.getLogger(DlgReturJual.class.
            getName());
