...
public class DlgProyeksiBeriObat extends javax.swing.JDialog {
    ...
    public DlgProyeksiBeriObat(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Tgl.Beri", "No.Rawat", "Barang", "Satuan", "Biaya Obat",
            "Jml.Obat", "Subtotal Biaya", "Emb+Tsl", "Total Biaya", "Harga Beli", "Total Beli", "Keuntungan"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            say = " detail_pemberian_obat.tgl_perawatan between '" + Valid.
                    SetTgl(Tgl1.getSelectedItem() + "") + "' and '" + Valid.
                    SetTgl(Tgl2.getSelectedItem() + "") + "' "
                    + " and detail_pemberian_obat.status like '%" + Status.
            ...
            Valid.MyReportqry("rptProyeksiBeriObat.jasper", "report",
                    "::[ Proyeksi Keuntungan Pemberian Obat ]::",
                    ...
                    + (TCari.getText().trim().isEmpty() ? "" : "and (detail_pemberian_obat.tgl_perawatan like '%" + TCari.
                    getText().trim() + "%' or "
                    + "detail_pemberian_obat.no_rawat like '%" + TCari.getText().
                            trim() + "%' or detail_pemberian_obat.kode_brng like '%" + TCari.
                            getText().trim() + "%' or "
                    + "databarang.nama_brng like '%" + TCari.getText().trim() + "%' or kodesatuan.satuan like '%" + TCari.
                    getText().trim() + "%') ")
    ...
    private void prosesCari() {
        ...
        try {
            ...
            say = " detail_pemberian_obat.tgl_perawatan between '" + Valid.SetTgl(Tgl1.getSelectedItem() + "")
                    + "' and '" + Valid.SetTgl(Tgl2.getSelectedItem() + "") + "' "
                    + " and detail_pemberian_obat.status like '%"
                    + Status.getSelectedItem().toString().replaceAll("Semua", "") + "%' ";
            ps = koneksi.prepareStatement(
                    ...
                    : "and (detail_pemberian_obat.tgl_perawatan like '%" + TCari.getText().trim()
                    + "%' or " + "detail_pemberian_obat.no_rawat like '%"
                    + TCari.getText().trim() + "%' or detail_pemberian_obat.kode_brng like '%"
                    + TCari.getText().trim() + "%' or " + "databarang.nama_brng like '%"
                    + TCari.getText().trim() + "%' or kodesatuan.satuan like '%"
                    + TCari.getText().trim() + "%') ")
                    + "order by detail_pemberian_obat.tgl_perawatan,detail_pemberian_obat.no_rawat");
            try {
                rs = ps.executeQuery();
                total = 0;
                while (rs.next()) {
                    total += rs.getDouble(13);
                    tabMode.addRow(new String[]{rs.getString(1), rs.getString(2),
                        rs.getString(3) + ", " + rs.getString(4), rs.getString(5), df2.format(rs.getDouble(6)),
                        df2.format(rs.getDouble(7)), df2.format(rs.getDouble(8)), df2.format(rs.getDouble(9)),
                        df2.format(rs.getDouble(10)), df2.format(rs.getDouble(11)), df2.format(rs.getDouble(12)),
    ...
    private static final Logger LOG = Logger.getLogger(DlgProyeksiBeriObat.class.getName());
