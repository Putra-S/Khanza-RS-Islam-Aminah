...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void prosesCari() {
        ...
        try {
            ...
            kolom = 0;
            ps = koneksi
                    .prepareStatement("select kd_bangsal,nm_bangsal from bangsal where status='1' and kd_bangsal<>'-' ");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    posisigudang[kolom] = rs.getString("kd_bangsal");
                    kolom++;
                    htmlContent.append("<td valign='middle' bgcolor='#FFFAFA' align='center' width='75px'>"
                            + rs.getString("nm_bangsal") + "</td>");
                }
            } catch (Exception e) {
            ...
            htmlContent.append("<td valign='middle' bgcolor='#FFFAFA' align='center' width='75px'>Total</td>"
                    + "<td valign='middle' bgcolor='#FFFAFA' align='center' width='85px'>Nilai Aset</td>" + "</tr>");
            ...
            ps = koneksi.prepareStatement("select kode_brng,nama_brng,kode_sat," + hppfarmasi
            ...
            try {
                ...
                while (rs.next()) {
                    total = 0;
                    htmlContent.append("<tr class='isi'>" + "<td valign='middle' align='center'>" + no + "</td>"
                    ...
                    htmlContent.append("<td valign='middle' align='right'>" + Valid.SetAngka(total) + "</td>"
                            + "<td valign='middle' align='right'>" + Valid.SetAngka(rs.getDouble("dasar") * total)
                            + "</td>" + "</tr>");
                    no++;
                }
                htmlContent.append("<tr class='isi'>" + "<td valign='middle' align='center'></td>"
                        + "<td valign='middle' align='left' colspan='" + (kolom + 5)
                        + "'>Total Nilai Aset Obat, Alkes & BHP Medis</td>" + "<td valign='middle' align='right'>"
                        + Valid.SetAngka(totalaset) + "</td>" + "</tr>");
            } catch (Exception e) {
            ...
            if (kolom == 0) {
                ...
                LoadHTML.setText("<html>" + "<table width='" + Integer.toString(480 + (kolom * 75))
    ...
    private static final Logger LOG = Logger.getLogger(DlgSisaStok.class.getName());
