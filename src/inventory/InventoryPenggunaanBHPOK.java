...
public class InventoryPenggunaanBHPOK extends javax.swing.JDialog {
    ...
    public InventoryPenggunaanBHPOK(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{"Tgl.Operasi", "No.Rawat", "No.RM", "Pasien", "Kode BHP",
            "Nama BHP", "Satuan", "Harga Satuan", "Jml", "Total"}) {
            @Override
...
//GEN-FIRST:event_TKdKeyPressed
    private void Tgl1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-LAST:event_TKdKeyPressed
        Valid.pindah(evt, TCari, Tgl2);//GEN-FIRST:event_Tgl1KeyPressed
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + "beri_obat_operasi.hargasatuan,beri_obat_operasi.jumlah,(beri_obat_operasi.hargasatuan*beri_obat_operasi.jumlah) as total from beri_obat_operasi inner join obatbhp_ok "
                    + "on beri_obat_operasi.kd_obat=obatbhp_ok.kd_obat inner join kodesatuan on obatbhp_ok.kode_sat=kodesatuan.kode_sat inner join reg_periksa on beri_obat_operasi.no_rawat=reg_periksa.no_rawat "
                    + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis where beri_obat_operasi.tanggal between ? and ? "
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode.addRow(new String[]{rs.getString("tanggal"), rs.getString("no_rawat"),
                        rs.getString("no_rkm_medis"), rs.getString("nm_pasien"), rs.getString("kd_obat"),
                        rs.getString("nm_obat"), rs.getString("satuan"),
                        Valid.SetAngka(rs.getDouble("hargasatuan")), rs.getString("jumlah"),
                        Valid.SetAngka(rs.getDouble("total"))});
                }
                if (total > 0) {
                    tabMode
                            .addRow(new String[]{">> Total BHP", "", "", "", "", "", "", "", "", Valid.SetAngka(total)});
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(InventoryPenggunaanBHPOK.class.getName());
