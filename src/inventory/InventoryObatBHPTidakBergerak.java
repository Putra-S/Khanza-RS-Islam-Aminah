...
//GEN-FIRST:event_TKdKeyPressed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            for (int i = 0; i < row; i++) {
                Sequel.menyimpan("temporary", "'" + i + "','"
                        ...
                        getalamatip() + "'", "Sirkulasi Barang Keluar Masuk");
            }
            ...
            Valid.MyReportqry("rptObatBHPTidakBergerak.jasper", "report",
                    "::[ Obat/Alkes/BHP Tidak Bergerak ]::",
    ...
    private void prosesCari() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select databarang.kode_brng,databarang.nama_brng,kodesatuan.satuan,jenis.nama,databarang."
                    + hppfarmasi + " as harga "
                    ...
                    + "databarang.kode_brng not in (select riwayat_barang_medis.kode_brng from riwayat_barang_medis "
                    + pilihan + ") "
                ...
                while (rs.next()) {
                    if (aktifkanbatch.equals("yes")) {
                        psstok = koneksi.prepareStatement(
                                "select sum(gudangbarang.stok) from gudangbarang inner join bangsal on gudangbarang.kd_bangsal=bangsal.kd_bangsal where bangsal.status='1' and gudangbarang.no_batch<>'' and gudangbarang.no_faktur<>'' and gudangbarang.kode_brng=?");
                    } else {
                        psstok = koneksi.prepareStatement(
                                "select sum(gudangbarang.stok) from gudangbarang inner join bangsal on gudangbarang.kd_bangsal=bangsal.kd_bangsal where bangsal.status='1' and gudangbarang.no_batch='' and gudangbarang.no_faktur='' and gudangbarang.kode_brng=?");
                    }
                    ...
                    if (stok > 0) {
                        tabMode.addRow(new Object[]{rs.getString("kode_brng"), rs.getString("nama_brng"),
                            rs.getString("satuan"), rs.getString("nama"), stok,
                            Valid.SetAngka(rs.getDouble("harga")), Valid.SetAngka(stok * rs.getDouble("harga"))});
                    }
                }
            } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(InventoryObatBHPTidakBergerak.class.getName());
