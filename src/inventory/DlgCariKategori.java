...
public class DlgCariKategori extends javax.swing.JDialog {
    ...
    private void tampil() {
        ...
        try {
            ...
            fileWriter.write("{\"kategoriobat\":[" + iyem.substring(0, iyem.length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
        } catch (Exception e) {
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("KodeKategori").asText(),
                            list.path("NamaKategori").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariKategori.class.getName());
