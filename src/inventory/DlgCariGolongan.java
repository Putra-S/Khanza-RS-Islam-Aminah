 ...
 * @author dosen
 */
public class DlgCariGolongan extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariGolongan dialog = new DlgCariGolongan(
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select * from golongan_barang order by golongan_barang.nama ");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(
                            new String[]{rs.getString(1), rs.getString(2)});
                    iyem = iyem + "{\"KodeGolongan\":\"" + rs.getString(1) + "\",\"NamaGolongan\":\"" + rs.
                            getString(2) + "\"},";
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"golonganobat\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeGolongan").asText(), list.path(
                            "NamaGolongan").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("NamaGolongan").asText().toLowerCase().
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariGolongan.class.
            getName());
}
