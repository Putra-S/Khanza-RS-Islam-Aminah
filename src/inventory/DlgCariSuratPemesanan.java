...
//GEN-FIRST:event_TKdKeyPressed
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            Valid.MyReportqry("rptDataPemesanan.jasper", "report",
                    "::[ Data Surat Pemesanan Barang ]::",
...
private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
    if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
        ...
        Sequel.queryu2("delete from surat_pemesanan_medis where no_pemesanan=?",
                1, new String[]{tbDokter.
    ...
    private void ppDatangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDatangActionPerformed
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            if (Sequel.cariIsi(
                    "select status from surat_pemesanan_medis where no_pemesanan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()).
                    equals("Sudah Datang")) {
                JOptionPane.showMessageDialog(null,
                        "Data pemesanan sudah tervalidasi..!!");
            } else {
                Sequel.queryu(
                        "update surat_pemesanan_medis set status='Sudah Datang' where no_pemesanan=?",
                        tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).
                                toString().trim());
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                aplikasi.tampikan = false;
                aplikasi.isCek();
                aplikasi.tampil(tbDokter.
                        getValueAt(tbDokter.getSelectedRow(), 0).toString().
                        trim());
    ...
    private void ppProsesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppProsesActionPerformed
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().
            ...
            Sequel.queryu(
                    "update surat_pemesanan_medis set status='Proses Pesan' where no_pemesanan=?",
                    tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().
                            trim());
            tampil();
        }
    ...
    private void BtnPrint5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint5ActionPerformed
        if (Apoteker.getText().trim().isEmpty()) {
            ...
            try {
                ...
                try {
                    ...
                    if (rs.next()) {
                        ...
                        param.put("total", Valid.
                                SetAngka(rs.getDouble("total2")));
                        param.put("ppn", Valid.SetAngka(rs.getDouble("ppn")));
                        param.put("meterai", Valid.SetAngka(rs.getDouble(
                                "meterai")));
                        param.put("tagihan", Valid.SetAngka(rs.getDouble(
                                "tagihan")));
                        param.put("tanggal", akses.getkabupatenrs() + ", " + rs.
                                getString("tanggal"));
                        ...
                        ps2 = koneksi.prepareStatement(
                                "select detail_surat_pemesanan_medis.kode_brng,databarang.nama_brng,databarang.kode_sat as satuan2, "
                        ...
                        try {
                            ...
                            while (rs2.next()) {
                                Sequel.menyimpan(
                                        "temporary", "'" + i + "','" + rs2.
                                                getDouble("jumlah") + "','" + rs2.
                                        getString("satuan") + "','" + rs2.
                                        getString("kode_brng") + "','"
                                        + rs2.getString("nama_brng") + "','" + rs2.
                                        getString("satuan2") + "','" + Valid.
                                        SetAngka(rs2.getDouble("h_pesan")) + "','"
                                        + Valid.SetAngka(rs2.getDouble(
                                                "subtotal")) + "','" + rs2.
                                                getDouble("dis") + "','" + Valid.
                                        SetAngka(rs2.getDouble("besardis")) + "','"
                                        + Valid.SetAngka(rs2.getDouble("total")) + "','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                                        getalamatip() + "'",
                                        "Transaksi Pemesanan"
                                );
                                i++;
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement("select surat_pemesanan_medis.tanggal,surat_pemesanan_medis.no_pemesanan, "
                    + "surat_pemesanan_medis.kode_suplier,datasuplier.nama_suplier, "
                    + "surat_pemesanan_medis.nip,pegawai.nama,"
                    + "surat_pemesanan_medis.status,surat_pemesanan_medis.total2,"
                    + "surat_pemesanan_medis.ppn,surat_pemesanan_medis.meterai,"
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    ps2 = koneksi
                            .prepareStatement("select detail_surat_pemesanan_medis.kode_brng,databarang.nama_brng, "
                                    + "detail_surat_pemesanan_medis.kode_sat,kodesatuan.satuan,detail_surat_pemesanan_medis.jumlah,detail_surat_pemesanan_medis.h_pesan, "
                                    + "detail_surat_pemesanan_medis.subtotal,detail_surat_pemesanan_medis.dis,detail_surat_pemesanan_medis.besardis,detail_surat_pemesanan_medis.total,"
                    ...
                    tabMode.addRow(new Object[]{"Tgl.Pemesanan", ": " + rs.getString("tanggal"), "", "", "", "",
                        "Total", ":", Valid.SetAngka(rs.getDouble("total2"))});
                    tabMode.addRow(new Object[]{"Status Pemesanan", ": " + rs.getString("status"), "", "", "", "",
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariSuratPemesanan.class.getName());
