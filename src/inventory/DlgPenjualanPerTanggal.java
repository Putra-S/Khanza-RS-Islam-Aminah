...
public class DlgPenjualanPerTanggal extends javax.swing.JDialog {
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (lokasi.isEmpty()) {
                Valid.MyReportqry("rptPenjualanPerTanggal.jasper", "report",
                        "::[ Penjualan Bebas Obat/Alkes/BHP Per Tanggal ]::",
                ...
                Valid.MyReportqry("rptPenjualanPerTanggal2.jasper", "report",
                        "::[ Penjualan Bebas Obat/Alkes/BHP Per Tanggal ]::",
                        "select * from temporary where temporary.temp37='" + akses.
                                getalamatip() + "' order by temporary.no", param);
            }
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    tabMode.addRow(new Object[]{rs.getString("kode_brng") + " " + rs.getString("nama_brng"),
                        rs.getString("satuan"), rs.getString("jenis"), rs.getString("kategori"),
                        rs.getString("golongan"), h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, h15,
    ...
    private double JmlObat(String tanggal, String kodebarang) {
        return Sequel.cariIsiAngka("select sum(detailjual.jumlah)" + " from penjualan inner join detailjual "
                + " on penjualan.nota_jual=detailjual.nota_jual "
                + " where penjualan.status='Sudah Dibayar' and detailjual.kode_brng='" + kodebarang
                + "' and penjualan.tgl_jual=?", tanggal);
    }
    ...
    private double JmlObat(String tanggal, String kodebarang, String lokasi) {
        return Sequel.cariIsiAngka("select sum(detailjual.jumlah)" + " from penjualan inner join detailjual "
                + " on penjualan.nota_jual=detailjual.nota_jual "
                + " where penjualan.status='Sudah Dibayar' and detailjual.kode_brng='" + kodebarang
                + "' and kd_bangsal='" + lokasi + "' and penjualan.tgl_jual=?", tanggal);
    }

    private static final Logger LOG = Logger.getLogger(DlgPenjualanPerTanggal.class.getName());
