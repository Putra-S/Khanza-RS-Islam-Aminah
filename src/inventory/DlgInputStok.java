...
public class DlgInputStok extends javax.swing.JDialog {
    ...
    public DlgInputStok(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row = {"Real", "Kode Barang", "Nama Barang", "Jenis", "Satuan", "Harga", "Stok", "Selisih", "Lebih",
            "Nominal Hilang(Rp)", "Nominal Lebih(Rp)", "No.Batch", "No.Faktur"};
                    ...
                    try {
                        if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 11).
                            ...
                            psstok = koneksi.prepareStatement(
                                    "select data_batch.no_faktur,data_batch." + hppfarmasi + " as dasar from data_batch where data_batch.no_batch=? and data_batch.kode_brng=? and data_batch.sisa>0 order by data_batch.tgl_kadaluarsa limit 1");
                            try {
                                psstok.setString(1, tbDokter.getValueAt(
                                        tbDokter.getSelectedRow(), 11).
                                        toString());
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        ...
        opname.isCek();
        opname.emptTeks();
        opname.setSize(internalFrame1.getWidth() - 20, internalFrame1.
                getHeight() - 20);
...
//GEN-FIRST:event_TKdKeyPressed
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-LAST:event_TKdKeyPressed
        if (nmgudang.getText().trim().isEmpty() || kdgudang.getText().trim().//GEN-FIRST:event_BtnSimpanActionPerformed
                isEmpty()) {
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                for (i = 0; i < tbDokter.getRowCount(); i++) {
                    if (!tbDokter.getValueAt(i, 0).toString().isEmpty()) {
                        try {
                            if (Double.parseDouble(tbDokter.getValueAt(i, 0).
                                    toString()) >= 0) {
                                if (Sequel.menyimpantf2("opname",
                                        "?,?,?,?,?,?,?,?,?,?,?,?,?",
                                        "Stok Opname", 13, new String[]{
                                    ...
                                    JOptionPane.showMessageDialog(null,
                                            tbDokter.getValueAt(i, 1).toString() + " " + tbDokter.
                                            getValueAt(i, 2).toString() + " gagal disimpan...!");
                                }
                            }
                        } catch (Exception e) {
                            sukses = false;
...
private void BtnGudangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnGudangActionPerformed
    ...
    private void ppStokActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppStokActionPerformed
        if (nmgudang.getText().trim().isEmpty() || kdgudang.getText().trim().
                isEmpty()) {
            Valid.textKosong(kdgudang, "Lokasi");
        } else {
            for (i = 0; i < tbDokter.getRowCount(); i++) {
                try {
                    ...
                    if (aktifkanbatch.equals("yes")) {
                        ...
                        try {
                            ...
                            psstok.setString(2, tbDokter.getValueAt(i, 1).
                                    toString());
                            psstok.setString(3, tbDokter.getValueAt(i, 11).
                                    toString());
                            psstok.setString(4, tbDokter.getValueAt(i, 12).
                                    toString());
                            rsstok = psstok.executeQuery();
                            if (rsstok.next()) {
            ...
            if (tabMode.getRowCount() == 0) {
                ...
                for (int i = 0; i < row; i++) {
                    Sequel.menyimpan("temporary", "'" + i + "','"
                            ...
                            + tabMode.getValueAt(i, 6).toString() + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.
                ...
                Valid.MyReportqry("rptInputStokOpname.jasper", "report",
                        "::[ Data Persediaan Stok Ruangan ]::",
        ...
        if (kdgudang.getText().isEmpty()) {
            try {
                ...
                pstampil = koneksi.prepareStatement(
                        ...
                        + (TCari.getText().trim().isEmpty() ? "" : "and (data_batch.kode_brng like ? or databarang.nama_brng like ? or jenis.nama like ?) ") + order);
                try {
                    if (!TCari.getText().trim().isEmpty()) {
                        pstampil.
                                setString(1, "%" + TCari.getText().trim() + "%");
                        pstampil.
                    ...
                    while (rstampil.next()) {
                        tabMode.addRow(new Object[]{
                            "", rstampil.getString("kode_brng"), rstampil.
                            getString("nama_brng"), rstampil.getString("nama"),
                            rstampil.getString("kode_sat"),
                            rstampil.getDouble("dasar"), rstampil.getDouble(
                            "sisa"), 0, 0, 0, 0, rstampil.getString("no_batch"),
                            rstampil.getString("no_faktur")
                        });
                        iyem = iyem + "{\"KodeBarang\":\"" + rstampil.getString(
                                "kode_brng") + "\",\"NamaBarang\":\"" + rstampil.
                                        getString("nama_brng").replaceAll("\"",
                                        "") + "\",\"Kategori\":\"" + rstampil.
                                        getString("nama") + "\",\"Satuan\":\"" + rstampil.
                                getString("kode_sat") + "\",\"Harga\":\"" + rstampil.
                                getString("dasar") + "\",\"NoBatch\":\"" + rstampil.
                                getString("no_batch") + "\",\"NoFaktur\":\"" + rstampil.
                                getString("no_faktur") + "\",\"Stok\":\"" + rstampil.
                                getDouble("sisa") + "\"},";
                    }
                } catch (Exception e) {
                    System.out.println("Notif : " + e);
                ...
                fileWriter.write("{\"stokopname\":[" + iyem.substring(0, iyem.
                        length() - 1) + "]}");
                fileWriter.flush();
                fileWriter.close();
                iyem = null;
    ...
    private void tampil() {
        try {
            ...
            try {
                rstampil = pstampil.executeQuery();
                while (rstampil.next()) {
                    tabMode.addRow(new Object[]{"", rstampil.getString("kode_brng"), rstampil.getString("nama_brng"),
                        rstampil.getString("nama"), rstampil.getString("kode_sat"), rstampil.getDouble("dasar"), 0,
                        0, 0, 0, 0, "", ""});
                    iyem = iyem + "{\"KodeBarang\":\"" + rstampil.getString("kode_brng") + "\",\"NamaBarang\":\""
                            + rstampil.getString("nama_brng").replaceAll("\"", "") + "\",\"Kategori\":\""
                            + rstampil.getString("nama") + "\",\"Satuan\":\"" + rstampil.getString("kode_sat")
                            + "\",\"Harga\":\"" + rstampil.getString("dasar")
                            + "\",\"NoBatch\":\"\",\"NoFaktur\":\"\",\"Stok\":\"0\"},";
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
            } finally {
    ...
    private void tampil2() {
        try {
            ...
            for (i = 0; i < jml; i++) {
                tabMode
                        .addRow(new Object[]{real[i], kodebarang[i], namabarang[i], kategori[i], satuan[i], hargabeli[i],
                    stok[i], selisih[i], lebih[i], nomihilang[i], nomilebih[i], nobatch[i], nofaktur[i]});
            }
                    ...
                    for (JsonNode list : response) {
                        tabMode.addRow(
                                new Object[]{"", list.path("KodeBarang").asText(), list.path("NamaBarang").asText(),
                                    list.path("Kategori").asText(), list.path("Satuan").asText(),
                                    list.path("Harga").asDouble(), list.path("Stok").asDouble(), 0, 0, 0, 0,
                                    list.path("NoBatch").asText(), list.path("NoFaktur").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeBarang").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                            ...
                            tabMode.addRow(new Object[]{"", list.path("KodeBarang").asText(),
                                list.path("NamaBarang").asText(), list.path("Kategori").asText(),
                                list.path("Satuan").asText(), list.path("Harga").asDouble(),
                                list.path("Stok").asDouble(), 0, 0, 0, 0, list.path("NoBatch").asText(),
                                list.path("NoFaktur").asText()});
                        }
                    }
                }
            }
            ...
            if (!tbDokter.getValueAt(i, 0).toString().isEmpty()) {
                try {
                    if (Double.parseDouble(tabMode.getValueAt(i, 0).toString()) >= 0) {
                        ...
                        harga = Sequel.cariIsiAngka("select data_batch." + hppfarmasi
                                + " as dasar from data_batch where data_batch.kode_brng='"
                                + tbDokter.getValueAt(i, 1).toString() + "' and no_batch='"
                                + tbDokter.getValueAt(i, 11).toString() + "' and no_faktur='"
                        ...
                        if (kurang > 0) {
                            tbDokter.setValueAt(kurang * Double.parseDouble(tbDokter.getValueAt(i, 5).toString()), i,
                                    9);
                            tbDokter.setValueAt(0, i, 10);
                        } else {
                            tbDokter.setValueAt(0, i, 9);
                            tbDokter.setValueAt(-1 * kurang * Double.parseDouble(tbDokter.getValueAt(i, 5).toString()),
                                    i, 10);
                        }
                    }
                } catch (Exception e) {
    ...
    private static final Logger LOG = Logger.getLogger(DlgInputStok.class.getName());
