...
public class DlgCariIndustriFarmasi extends javax.swing.JDialog {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgCariIndustriFarmasi dialog = new DlgCariIndustriFarmasi(
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    "select * from industrifarmasi order by industrifarmasi.nama_industri ");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    //"Kode I.F.","Industri Farmasi","Alamat Industri Farmasi","Kota","No.Telp"
                    tabMode.addRow(new String[]{
                        rs.getString("kode_industri"), rs.getString(
                        "nama_industri"), rs.getString("alamat"), rs.getString(
                        "kota"), rs.getString("no_telp")
                    });
                    iyem = iyem + "{\"KodeIF\":\"" + rs.getString(
                            "kode_industri") + "\",\"NamaIF\":\"" + rs.
                                    getString("nama_industri") + "\",\"Alamat\":\"" + rs.
                            getString("alamat") + "\",\"Kota\":\"" + rs.
                            getString("kota") + "\",\"NoTelp\":\"" + rs.
                            getString("no_telp") + "\"},";
                }
            } catch (Exception e) {
            ...
            fileWriter.write("{\"industrifarmasi\":[" + iyem.substring(0, iyem.
                    length() - 1) + "]}");
            fileWriter.flush();
            fileWriter.close();
            iyem = null;
                ...
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{
                            list.path("KodeIF").asText(), list.path("NamaIF").
                            asText(), list.path("Alamat").asText(), list.path(
                            "Kota").asText(), list.path("NoTelp").asText()
                        });
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeIF").asText().toLowerCase().contains(
                                TCari.getText().toLowerCase()) || list.path(
                                "NamaIF").asText().toLowerCase().contains(TCari.
                                        getText().toLowerCase()) || list.path(
                                        "Kota").asText().toLowerCase().contains(
                                                TCari.getText().toLowerCase())) {
                            tabMode.addRow(new Object[]{
                                list.path("KodeIF").asText(), list.
                                path("NamaIF").asText(), list.path("Alamat").
    ...
    private static final Logger LOG = Logger.getLogger(
            DlgCariIndustriFarmasi.class.getName());
}
