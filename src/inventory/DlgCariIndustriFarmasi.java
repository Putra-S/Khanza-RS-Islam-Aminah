...
public class DlgCariIndustriFarmasi extends javax.swing.JDialog {
    ...
    private void BtnTambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTambahActionPerformed
        ...
        nama.emptTeks();
        nama.isCek();
        nama.setSize(internalFrame1.getWidth() - 20,
    ...
    private void tampil() {
        ...
        try {
            ...
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    // "Kode I.F.","Industri Farmasi","Alamat Industri
                    // Farmasi","Kota","No.Telp"
                    tabMode.addRow(new String[]{rs.getString("kode_industri"), rs.getString("nama_industri"),
                        rs.getString("alamat"), rs.getString("kota"), rs.getString("no_telp")});
                    iyem = iyem + "{\"KodeIF\":\"" + rs.getString("kode_industri") + "\",\"NamaIF\":\""
                            + rs.getString("nama_industri") + "\",\"Alamat\":\"" + rs.getString("alamat")
                            + "\",\"Kota\":\"" + rs.getString("kota") + "\",\"NoTelp\":\"" + rs.getString("no_telp")
                            + "\"},";
                }
            } catch (Exception e) {
            ...
            if (response.isArray()) {
                if (TCari.getText().trim().isEmpty()) {
                    for (JsonNode list : response) {
                        tabMode.addRow(new Object[]{list.path("KodeIF").asText(), list.path("NamaIF").asText(),
                            list.path("Alamat").asText(), list.path("Kota").asText(),
                            list.path("NoTelp").asText()});
                    }
                } else {
                    for (JsonNode list : response) {
                        if (list.path("KodeIF").asText().toLowerCase().contains(TCari.getText().toLowerCase())
                                || list.path("NamaIF").asText().toLowerCase().contains(TCari.getText().toLowerCase())
    ...
    private static final Logger LOG = Logger.getLogger(DlgCariIndustriFarmasi.class.getName());
