...
public class koneksiDB {
    ...
    public static Connection condb() {
        if (connection == null) {
            try {
                ...
                dataSource.setURL("jdbc:mysql://" + EnkripsiAES.decrypt(prop.getProperty("HOST")) + ":"
                        + EnkripsiAES.decrypt(prop.getProperty("PORT")) + "/"
                        + EnkripsiAES.decrypt(prop.getProperty("DATABASE"))
                        + "?zeroDateTimeBehavior=convertToNull&autoReconnect=true&useCompression=true&useSSL=false");
    ...
    public static String URLEKLAIMINACBG() {
        try {
            prop.loadFromXML(new FileInputStream("setting/database.xml"));
    ...
    public static String KEYEKLAIMINACBG(){
        try{
            prop.loadFromXML(new FileInputStream("setting/database.xml"));
            var=EnkripsiAES.decrypt(prop.getProperty("KEYEKLAIMINACBG"));
        }catch(Exception e){
            var=""; 
        }
        return var;
    ...
    public static String KELASRSEKLAIMINACBG(){
        try{
            prop.loadFromXML(new FileInputStream("setting/database.xml"));
            var=prop.getProperty("KELASRSEKLAIMINACBG");
        }catch(Exception e){
            var=""; 
        }
        return var;
    ...
    private static final Logger LOG = Logger.getLogger(koneksiDB.class.getName());
