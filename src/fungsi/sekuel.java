...
public class sekuel {
  ...
  private String folder, AKTIFKANTRACKSQL = koneksiDB.AKTIFKANTRACKSQL(), BOLEHMUNDUR = "No";
  private final Connection connect = koneksiDB.condb();
  private PreparedStatement ps, psbolehmundur;
  private ResultSet rs, rsbolehmundur;
  private int angka = 0;
    ...
    try {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            null,
            "Maaf, gagal menyimpan data. Kemungkinan ada "
                + sama
      ...
      try {
        ps =
            connect.prepareStatement(
                "update " + table + " set " + update + " where " + acuan_field);
  ...
  public void menyimpan(String table, String isisimpan, String isiedit, String acuan_field) {
    try {
      ...
      try {
        ps =
            connect.prepareStatement(
                "update " + table + " set " + isiedit + " where " + acuan_field);
        ps.executeUpdate();
        if (ps != null) {
          ps.close();
        }
  ...
  public void menyimpan(String table, String value, String sama, JTextField AlmGb) {
    try {
      ...
      try {
        ps.setBinaryStream(
            1, new FileInputStream(AlmGb.getText()), new File(AlmGb.getText()).length());
        ps.executeUpdate();
      } catch (Exception e) {
        System.out.println("Notifikasi : " + e);
        JOptionPane.showMessageDialog(
   ...
   */
  public void menyimpan(
      String table, String value, String sama, JTextField AlmGb, JTextField AlmPhoto) {
    try {
      ps = connect.prepareStatement("insert into " + table + " values(" + value + ",?,?)");
      try {
        ps.setBinaryStream(
            1, new FileInputStream(AlmGb.getText()), new File(AlmGb.getText()).length());
        ps.setBinaryStream(
            2, new FileInputStream(AlmPhoto.getText()), new File(AlmPhoto.getText()).length());
        ps.executeUpdate();
      } catch (Exception e) {
        System.out.println("Notifikasi : " + e);
        JOptionPane.showMessageDialog(
   ...
   * @param nilai_field2
   */
  public void meghapus(
      String table, String field, String field2, String nilai_field, String nilai_field2) {
    try {
      ps =
          connect.prepareStatement(
              "delete from " + table + " where " + field + "=? and " + field2 + "=?");
      try {
        ps.setString(1, nilai_field);
  ...
  public void mengedit(String table, String acuan_field, String update) {
    try {
      ...
      try {
        ...
        System.out.println("Notifikasi : " + e);
        JOptionPane.showMessageDialog(
            null, "Maaf, Gagal Mengedit. Mungkin kode sudah digunakan sebelumnya...!!!!");
      } finally {
        if (ps != null) {
          ps.close();
        }
  ...
  public int cariRegistrasi(String norawat) {
    angka = 0;
    angka =
        cariInteger("select count(billing.no_rawat) from billing where billing.no_rawat=?", norawat)
            + cariInteger(
                "select count(reg_periksa.no_rawat) from reg_periksa where reg_periksa.no_rawat=? and reg_periksa.stts='Batal'",
                norawat);
    return angka;
  }
  ...
  public boolean cekTanggalRegistrasi(String tanggalregistrasi, String tanggalinputdata) {
    try {
      psbolehmundur =
          connect.prepareStatement(
              "select set_validasi_boleh_mundur.boleh_mundur from set_validasi_boleh_mundur");
      try {
        rsbolehmundur = psbolehmundur.executeQuery();
        if (rsbolehmundur.next()) {
          BOLEHMUNDUR = rsbolehmundur.getString("boleh_mundur");
        } else {
          BOLEHMUNDUR = "No";
        }
      } catch (Exception e) {
        ...
        if (rsbolehmundur != null) {
          rsbolehmundur.close();
        }
        if (psbolehmundur != null) {
          psbolehmundur.close();
        }
      }
    } catch (Exception e) {
    ...
    if (BOLEHMUNDUR.equals("Yes")) {
      ...
      try {
        ...
        if (bedawaktu < 0) {
          ...
          JOptionPane.showMessageDialog(
              null,
              "Maaf, jam input data / perubahan data minimal di jam "
                  + tanggalregistrasi
                  + " ! atau hubungi pendaftaran untuk update jam registrasi");
        } else {
          bool = true;
        }
      } catch (Exception ex) {
    ...
    if (BOLEHMUNDUR.equals("Yes")) {
      ...
      try {
        ...
        if (bedawaktu > 172800) {
          bool = false;
          JOptionPane.showMessageDialog(
              null, "Maaf, perubahan data / penghapusan data tidak boleh lebih dari 2 x 24 jam !");
        } else {
          bool = true;
        }
      } catch (Exception ex) {
  ...
  public void cariGambar(String sql, JLabel txt) {
    try {
      ...
      try {
        ...
        if (rs.next()) {
          icon =
              new javax.swing.ImageIcon(rs.getBlob(1).getBytes(1L, (int) rs.getBlob(1).length()));
          createThumbnail();
          txt.setIcon(icon);
        } else {
          txt.setText(null);
    ...
    try {
      ...
      try {
        ...
        for (int I = 0; rs.next(); I++) {
          ...
          ((Painter) txt)
              .setImageIcon(new javax.swing.ImageIcon(blob.getBytes(1, (int) (blob.length()))));
        }
      } catch (Exception ex) {
        cetak(ex.toString());
      } finally {
  ...
  private static final Logger LOG = Logger.getLogger(sekuel.class.getName());
}
